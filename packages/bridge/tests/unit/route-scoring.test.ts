...
function calculateRouteScore(route: BridgeRoute): number {
  ...
  const speedScore = Math.max(0, 100 - (route.estimatedTimeSeconds / MAX_TIME_SECONDS) * 100);
...
describe('Route Scoring Algorithm', () => {
  ...
  describe('Weight Validation', () => {
    ...
    it('should have reliability as highest weight', () => {
      expect(SCORING_WEIGHTS.reliability).toBeGreaterThanOrEqual(SCORING_WEIGHTS.cost);
      expect(SCORING_WEIGHTS.reliability).toBeGreaterThanOrEqual(SCORING_WEIGHTS.speed);
      expect(SCORING_WEIGHTS.reliability).toBeGreaterThanOrEqual(SCORING_WEIGHTS.liquidity);
    });
  });
});

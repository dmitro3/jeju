...
import { computeMerkleRoot, hashToHex, hexToHash32 } from '../../src/utils/validation.js';
import { toHash32, type Hash32 } from '../../src/types/index.js';
import { keccak_256 } from '@noble/hashes/sha3';

// Helper to create Hash32 from hex
function h32(hex: string): Hash32 {
  return hexToHash32(hex);
}
...
describe('Merkle Tree Computation', () => {
  describe('computeMerkleRoot', () => {
    ...
    it('should handle large number of leaves', () => {
      const leaves = Array.from({ length: 1000 }, (_, i) => deterministicHash32(i));
      const root = computeMerkleRoot(leaves, keccak_256);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle maximum realistic tree size (10000 leaves)', () => {
      const leaves = Array.from({ length: 10000 }, (_, i) => deterministicHash32(i));
  ...
  describe('Cross-Chain Transfer Batching', () => {
    ...
    it('should detect tampered transfers', () => {
      ...
      const originalRoot = computeMerkleRoot(original.map(transferToLeaf), keccak_256);
      const tamperedRoot = computeMerkleRoot(tampered.map(transferToLeaf), keccak_256);

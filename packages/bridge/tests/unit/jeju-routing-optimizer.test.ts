...
describe('JejuRoutingOptimizer', () => {
  ...
  describe('Route Building', () => {
    it('should build direct route for L2 to L2', async () => {
      ...
      const routes = await optimizer.findOptimalRoutes(request);
      const directRoute = routes.find(r => r.strategy === 'direct' && r.hops.length === 1);
    ...
    it('should build hub route through Jeju', async () => {
      ...
      if (hubRoute) {
        ...
        expect([ChainId.JEJU, ChainId.JEJU_TESTNET]).toContain(hubRoute.hops[0].toChain);
        expect([ChainId.JEJU, ChainId.JEJU_TESTNET]).toContain(hubRoute.hops[1].fromChain);
      }
    });
    ...
    it('should include OIF solver route', async () => {
      ...
      const routes = await optimizer.findOptimalRoutes(request);
      const oifRoute = routes.find(r => r.hops.some(h => h.mechanism === 'oif'));
    ...
    it('should use zkbridge for Solana routes', async () => {
      ...
      const routes = await optimizer.findOptimalRoutes(request);
      const zkRoute = routes.find(r => r.hops.some(h => h.mechanism === 'zkbridge'));
    ...
    it('should use ccip for BSC routes', async () => {
      ...
      const routes = await optimizer.findOptimalRoutes(request);
      const ccipRoute = routes.find(r => r.hops.some(h => h.mechanism === 'ccip'));
  ...
  describe('Route Ranking', () => {
    ...
    it('should calculate composite score correctly', () => {
      ...
      const expectedScore = Number((route.jejuRevenue * 100n) / route.userCost) * route.confidence / 100;
  ...
  describe('Edge Cases', () => {
    it('should throw for unsupported chains', async () => {
      ...
      await expect(optimizer.findOptimalRoutes(request)).rejects.toThrow('Unsupported chain');
    });
    ...
    it('should throw for cross-network routes (mainnet to testnet)', async () => {
      ...
      await expect(optimizer.findOptimalRoutes(request)).rejects.toThrow('Cannot route between mainnet and testnet');
    });
    ...
    it('should check direct route availability', () => {
      expect(optimizer.hasDirectRoute(ChainId.BASE, ChainId.ARBITRUM)).toBe(true);
      expect(optimizer.hasDirectRoute(ChainId.BASE, ChainId.SOLANA_MAINNET)).toBe(true);
    });
  ...
  describe('Time Estimates', () => {
    it('should have reasonable time estimates for L2 to L2', async () => {
      ...
      const routes = await optimizer.findOptimalRoutes(request);
      const directRoute = routes.find(r => r.strategy === 'direct' && r.hops.some(h => h.mechanism === 'eil'));

...
export class RelayerService {
	...
	private async handleEthereumFinality(update: EthereumUpdate): Promise<void> {
		...
		if (update.slot < this.lastEthereumSlot - maxReorgDepth && this.lastEthereumSlot > maxReorgDepth) {
	...
	private async handleIncomingTransfer(
		...
		if (transfer.amount <= 0n) {
			log.warn("Invalid transfer amount", { transferId, amount: transfer.amount.toString() });

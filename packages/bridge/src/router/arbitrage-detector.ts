...
export class ArbitrageDetector {
  ...
  private async detectSolanaEvmArb(): Promise<void> {
    ...
    for (const token of tokens) {
      ...
      for (const chainId of evmChains) {
        ...
        const buyChain = priceDiff.buyLow === 'solana' ? 'solana' : `evm:${chainId}`;
        const sellChain = priceDiff.buyLow === 'solana' ? `evm:${chainId}` : 'solana';
  ...
  private async detectHyperliquidArb(): Promise<void> {
    ...
    for (const pair of pairs) {
      ...
      const priceDiffBps = Math.abs(priceDiff / Number(baseQuote.amountOut)) * 10000;
      ...
      this.opportunities.set(id, {
        ...
        token: pair,
        buyPrice: buyOnHyper ? BigInt(Math.floor(hyperPrice * 1e18)) : baseQuote.amountOut,
        sellPrice: buyOnHyper ? baseQuote.amountOut : BigInt(Math.floor(hyperPrice * 1e18)),
  ...
  private async detectCrossChainArb(): Promise<void> {
    ...
    for (const token of tokens) {
      ...
      const priceDiffBps = Number((maxPrice.price - minPrice.price) * 10000n / minPrice.price);
      if (priceDiffBps < this.minProfitBps) continue;
  ...
  private calculatePriceDiff(quote1: PriceQuote, quote2: PriceQuote): {
    diffBps: number;
    lowPrice: bigint;
    highPrice: bigint;
    buyLow: string;
  ...
  private buildArbRoute(buyChain: string, sellChain: string, token: string): ArbRoute {
    const steps: ArbStep[] = [];
    ...
    return {
      ...
      totalTimeSeconds: buyChain.startsWith('solana') || sellChain.startsWith('solana') ? 300 : 120,
    };
  }
   ...
   */
  async submitJitoBundle(bundle: JitoBundle): Promise<{ bundleId: string; landed: boolean }> {
    try {
...
export function createArbitrageDetector(config?: { minProfitBps?: number }): ArbitrageDetector {

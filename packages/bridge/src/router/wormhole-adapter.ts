...
const SOLANA_WORMHOLE_CORE = new PublicKey(
  'worm2ZoG2kUd4vFXhvjh93UUH596ayRfgQ2MgjNMTth',
)
const SOLANA_TOKEN_BRIDGE = new PublicKey(
...
export class WormholeAdapter extends EventEmitter {
  ...
  async transferEVMToEVM(
    ...
    if (!sourceClients) {
      return {
        success: false,
        error: `Source chain ${params.sourceChainId} not configured`,
      }
    }
    ...
    if (!tokenBridge) {
      return {
        success: false,
        error: `No Wormhole bridge on chain ${params.sourceChainId}`,
      }
    }
    ...
    if (!destWormholeChainId) {
      return {
        success: false,
        error: `Destination chain ${params.destChainId} not supported by Wormhole`,
      }
    }
    ...
    const receipt = await sourceClients.public.waitForTransactionReceipt({
      hash,
    })
  ...
  async transferSolanaToEVM(params: {
    ...
    if (!destWormholeChainId) {
      return {
        success: false,
        error: `Destination chain ${params.destChainId} not supported`,
      }
    }
    ...
    instructionData.writeUInt32LE(
      new DataView(solNonceBytes.buffer).getUint32(0, false),
      offset,
    )
  ...
  async completeTransfer(
    ...
    if (!destClients) {
      return {
        success: false,
        error: `Destination chain ${destChainId} not configured`,
      }
    }
    ...
    if (!tokenBridge) {
      return {
        success: false,
        error: `No Wormhole bridge on chain ${destChainId}`,
      }
    }
   ...
   */
  private async completeTransferOnSolana(
    vaa: string,
  ): Promise<WormholeTransferResult> {
    if (!this.solanaKeypair) {
      return { success: false, error: 'Solana keypair not configured' }

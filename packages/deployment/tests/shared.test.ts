...
function getRequiredNetwork(env: Record<string, string | undefined>): NetworkType {
...
function getNetworkRpcUrl(
  ...
  throw new Error(`${config.rpcUrlEnvVar} environment variable is required for ${network}`);
}
...
describe("createCommandValidator", () => {
  ...
  it("should work with different command sets", () => {
    const helmCommands = ["diff", "sync", "apply", "destroy", "status", "list"] as const;
...
describe("NETWORK_RPC_CONFIGS", () => {
  ...
  it("should have unique env var names", () => {
    const envVars = Object.values(NETWORK_RPC_CONFIGS).map((c) => c.rpcUrlEnvVar);

...
async function main() {
  ...
  if (hasJejuHostsBlock()) {
    ...
    if (isInteractive) {
      ...
      console.log('   Run: bun run packages/deployment/scripts/shared/local-proxy.ts hosts:add\n');
    } else {
      console.log('   To configure manually, run:');
      console.log('   bun run packages/deployment/scripts/shared/local-proxy.ts hosts:add\n');
  ...
  console.log('  • Configure local DNS: bun run packages/deployment/scripts/shared/local-proxy.ts hosts:add');

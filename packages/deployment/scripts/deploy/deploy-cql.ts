/**
 * Deploy CQL Infrastructure
 *
 * CQL (CovenantSQL) providers register in the unified ComputeRegistry.
 * This script deploys the Kubernetes infrastructure only.
 *
 * Usage:
 *   bun run scripts/deploy/deploy-cql.ts --network testnet
 */
...
const HELM_DIR = join(
  ROOT_DIR,
  'packages/deployment/kubernetes/helm/covenantsql',
)
...
async function deployKubernetes(
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log('  Deploying CQL to Kubernetes')
  ...
  const namespace = `cql-${network}`
  const releaseName = `covenantsql-${network}`
...
async function main(): Promise<void> {
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log('  CQL Infrastructure Deployment')
  console.log('='.repeat(60))
  console.log(`  Network: ${network}`)
  console.log('')
  console.log('  Note: CQL providers register in the unified ComputeRegistry')
  ...
  console.log('='.repeat(60))
  console.log(`
  CQL operators should register using:

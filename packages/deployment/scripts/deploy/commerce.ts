...
async function deployToChain(
  ...
  logger.info(`\n${'='.repeat(60)}`)
  logger.info(
    `Deploying Commerce to ${chain.name} (Chain ID: ${chain.chainId})`,
  )
  ...
  if (balance < minBalance) {
    logger.warn(
      `Insufficient balance on ${chain.name}. Need at least 0.01 ${chain.nativeCurrency.symbol}`,
    )
    return null
  }
  ...
  try {
    ...
    const result =
      await $`cd ${contractsDir} && ${envString} forge script script/DeployCommerce.s.sol:DeployCommerce --rpc-url ${chain.rpcUrl} --broadcast --legacy 2>&1`.text()

    logger.info(result)

    // Parse deployed address from output
    const addressMatch = result.match(
      /AuthCaptureEscrow deployed to:\s*(0x[a-fA-F0-9]{40})/,
    )
    if (!addressMatch) {
...
async function verifyContract(
  ...
  const apiKey =
    process.env[
      `${chain.name.toUpperCase().replace(/\s+/g, '_')}_ETHERSCAN_API_KEY`
...
function updateContractsJson(chainId: number, escrowAddress: Address): void {
  ...
  try {
    ...
    logger.warn(
      `Failed to update contracts.json: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
async function main(): Promise<void> {
  ...
  const specificChainId =
    chainIdArg !== -1 ? parseInt(args[chainIdArg + 1], 10) : null

  if (!deployTestnet && !deployMainnet && !specificChainId) {
    logger.info(
      'Usage: bun run scripts/deploy/commerce.ts [--testnet] [--mainnet] [--all] [--verify] [--chain <chainId>]',
    )
    logger.info('')
    logger.info('Options:')
    ...
    logger.info(
      '  PROTOCOL_FEE_BPS - Protocol fee in basis points (default: 100)',
    )
    logger.info('  OPERATOR_ADDRESS - Initial operator address')
    logger.info(
      '  OPERATOR_FEE_BPS - Operator fee in basis points (default: 50)',
    )
    return
  }
  ...
  if (specificChainId) {
    const chain = [...TESTNET_CHAINS, ...MAINNET_CHAINS].find(
      (c) => c.chainId === specificChainId,
    )
    if (!chain) {

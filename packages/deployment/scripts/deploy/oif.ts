...
async function deployToChain(
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      const solverMatch = output.match(
        /SolverRegistry deployed to: (0x[a-fA-F0-9]{40})/,
      )
      const inputMatch = output.match(
        /InputSettler deployed to: (0x[a-fA-F0-9]{40})/,
      )
      const outputMatch = output.match(
        /OutputSettler deployed to: (0x[a-fA-F0-9]{40})/,
      )
      const oracleMatch = output.match(
        /Oracle deployed to: (0x[a-fA-F0-9]{40})/,
      )

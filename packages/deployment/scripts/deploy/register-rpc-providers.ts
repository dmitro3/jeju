...
async function getEndpointsFromConfigMap(
  namespace: string,
): Promise<Record<string, ChainEndpoint>> {
  console.log(
    `[Register] Reading endpoints from ConfigMap dws-rpc-endpoints in namespace ${namespace}...`,
  )

  const result =
    await $`kubectl get configmap dws-rpc-endpoints -n ${namespace} -o jsonpath='{.data.endpoints\\.json}'`
      .quiet()
      .nothrow()
  ...
  console.log(
    `[Register] Found ${Object.keys(config.chains).length} chains in ConfigMap`,
  )
...
async function registerProvider(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to register provider for chain ${registration.chainId}: ${error}`,
    )
  }
...
): Promise<{ ok: boolean; latency: number; blockNumber?: string }> {
...
async function main() {
  ...
  const privateKey =
    process.env.DWS_OPERATOR_KEY ?? process.env.DEPLOYER_PRIVATE_KEY
  if (!privateKey) {
  ...
  try {
    ...
    console.warn(
      `[Register] Could not read from ConfigMap, using fallback configuration`,
    )
    ...
    chains = {
      ...
      'optimism-sepolia': {
        ...
        stackType: 'op-stack',
        internalRpcUrl:
          'http://op-geth-optimism.l2-optimism.svc.cluster.local:8545',
      },
      'arbitrum-sepolia': {
        chainId: 421614,
        ...
        stackType: 'nitro',
        internalRpcUrl:
          'http://nitro-arbitrum.l2-arbitrum.svc.cluster.local:8547',
      },
      'bsc-testnet': {
        chainId: 97,
      ...
      'jeju-testnet': {
        ...
        stackType: 'op-stack',
        internalRpcUrl:
          'http://reth-sequencer.execution.svc.cluster.local:8545',
  ...
  for (const [name, chain] of Object.entries(chains)) {
    ...
    console.log(
      `[${name}] Endpoint OK - block ${rpcTest.blockNumber} (${rpcTest.latency}ms)`,
    )

...
// SECURITY: Anvil default key - ONLY for local development
// This script is specifically for localnet deployment
// For other networks, use scripts/deploy/oracle.ts with proper key management
const ANVIL_DEFAULT_KEY =
  '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80'

function getDeployerKey(): `0x${string}` {
  // Allow override via env for testing different accounts on localnet
  const envKey = process.env.DEPLOYER_PRIVATE_KEY
  if (envKey) {
    return envKey as `0x${string}`
  }
  // Only use Anvil default for localhost
  if (!RPC_URL.includes('localhost') && !RPC_URL.includes('127.0.0.1')) {
    throw new Error(
      'DEPLOYER_PRIVATE_KEY required for non-local deployments. Use scripts/deploy/oracle.ts instead.',
    )
  }
...
async function main() {
  ...
  if (!addresses) {
    ...
    console.error(
      'Please update gateway addresses manually from the output above.',
    )
    process.exit(1)
  }

...
import { getServicesConfig, getChainId, getRpcUrl } from '@jejunetwork/config'
...
const CONTRACTS_TO_TRANSFER: Array<{
  ...
  // Staking
  { name: 'NodeStakingManager', envKey: 'NODE_STAKING_MANAGER', critical: true },
  { name: 'DelegatedNodeStaking', envKey: 'DELEGATED_NODE_STAKING', critical: true },
  ...
  // Oracle
  { name: 'OracleStakingManager', envKey: 'ORACLE_STAKING_MANAGER', critical: false },
...
async function main() {
  ...
  const networkArg = networkIdx !== -1 && args[networkIdx + 1] 
    ? args[networkIdx + 1] 
  ...
  console.log(`Scope: ${skipNonCritical ? 'Critical contracts only' : 'All contracts'}\n`)
  ...
  // Filter contracts
  const contractsToProcess = CONTRACTS_TO_TRANSFER.filter(c => 
    !skipNonCritical || c.critical
  )

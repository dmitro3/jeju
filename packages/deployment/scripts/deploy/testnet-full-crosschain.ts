...
async function registerXLP(
  ...
  // Get L2 chain IDs
  const l2ChainIds = TESTNET_CHAINS.filter((c) => c.type === 'l2').map((c) =>
    BigInt(c.chainId),
  )
...
async function main() {
  ...
  const xlpAccount = xlpKey
    ? privateKeyToAccount(xlpKey as `0x${string}`)
  ...
  for (const chain of TESTNET_CHAINS) {
    try {
      ...
      const balance = await getBalanceFormatted(
        publicClient,
  ...
  if (l1Chain) {
    try {
      const existingContracts = Object.fromEntries(
        Object.entries(deployments).map(([chainId, result]) => [
          chainId,
          result.contracts,
        ]),
      ) as Record<number, Record<string, string>>
      deployments[l1Chain.chainId] = await deployToChain(
        l1Chain,
  ...
  for (const chain of TESTNET_CHAINS.filter((c) => c.type === 'l2')) {
    try {
      ...
      deployments[chain.chainId] = await deployToChain(
        chain,
        deployerKey as `0x${string}`,
        existingContracts,
      )
  ...
  try {
    await registerXLP(
      deployerKey as `0x${string}`,
      (xlpKey || generatePrivateKey()) as `0x${string}`,
      deployments,
    )
  } catch (e) {
  ...
  console.log(
    '2. Update packages/config/contracts.json with deployed addresses',
  )

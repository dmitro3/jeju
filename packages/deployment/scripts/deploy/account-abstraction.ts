  ...
  switch (network) {
    ...
    case 'mainnet':
      return {
        network: 'mainnet',
        rpcUrl:
          process.env.JEJU_MAINNET_RPC_URL || 'https://rpc.jejunetwork.org',
        chainId: 420692,
      }
    default:
...
async function main() {
  ...
  // Anvil default key ONLY for localnet
  const envKey = process.env.DEPLOYER_PRIVATE_KEY || process.env.PRIVATE_KEY
  if (!envKey && network !== 'localnet') {
    throw new Error(
      `DEPLOYER_PRIVATE_KEY or PRIVATE_KEY environment variable required for ${network}`,
    )
  }
  ...
  if (!entryPointCode || entryPointCode === '0x') {
    ...
    console.log(
      '   ⚠️  EntryPoint v0.7 should be deployed using official deployer',
    )
  ...
  if (existingFactoryAddress) {
    ...
    if (factoryCode && factoryCode !== '0x') {
      ...
      factoryAddress = await deploySimpleAccountFactory(
        walletClient,
        publicClient,
        config,
      )
  ...
  let sponsoredPaymasterAddress = process.env.SPONSORED_PAYMASTER_ADDRESS as
    | Address
    | undefined
  ...
  if (sponsoredPaymasterAddress) {
    ...
    if (code && code !== '0x') {
      console.log(
        `   ✅ SponsoredPaymaster already deployed at ${sponsoredPaymasterAddress}`,
      )
    } else {
      sponsoredPaymasterAddress = await deploySponsoredPaymaster(
        walletClient,
        publicClient,
        account.address,
      )
  ...
  await configureSponsoredPaymaster(
    walletClient,
    publicClient,
  ...
  const liquidityPaymasterAddress = process.env.LIQUIDITY_PAYMASTER_ADDRESS as
    | Address
    | undefined
  if (
    liquidityPaymasterAddress &&
    liquidityPaymasterAddress !== '0x0000000000000000000000000000000000000000'
  ) {
    await fundPaymaster(
      walletClient,
      publicClient,
...
async function deploySimpleAccountFactory(
  ...
  console.log(
    '   ⚠️  Using pre-deployed factory. For custom deployment, compile from:',
  )
...
async function deploySponsoredPaymaster(
  ...
  const artifactPaths = [
    path.join(
      process.cwd(),
      'packages/contracts/out/SponsoredPaymaster.sol/SponsoredPaymaster.json',
    ),
...
async function configureSponsoredPaymaster(
  ...
  console.log(
    '   Whitelisting all contracts (address(0) = sponsor everything)...',
  )
...
async function fundPaymaster(
  ...
  console.log(
    `   ${name} current balance: ${(Number(currentBalance) / 1e18).toFixed(4)} ETH`,
  )

  if (currentBalance < amount) {
    const needed = amount - currentBalance
    console.log(
      `   Depositing ${(Number(needed) / 1e18).toFixed(4)} ETH to ${name}...`,
    )

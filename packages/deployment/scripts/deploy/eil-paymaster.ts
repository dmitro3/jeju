...
function loadEILConfig(): {
  hub: { l1StakeManager: string; chainId: number }
} | null {
...
async function checkChainConnectivity(
  rpcUrl: string,
...
async function deployPaymaster(
  ...
  if (exitCode !== 0) {
    ...
    return {
      success: false,
      error: `Forge deployment failed: ${stderr.slice(0, 200)}`,
    }
  }
  ...
  if (!deployedAddress) {
    ...
    const txMatch = (stdout + stderr).match(
...
async function main() {
  ...
  const specificChainId =
    chainArg !== -1 ? parseInt(args[chainArg + 1], 10) : null

  console.log(
    '╔════════════════════════════════════════════════════════════════╗',
  )
  console.log(
    '║         EIL CrossChainPaymaster Deployment                     ║',
  )
  console.log(
    '╚════════════════════════════════════════════════════════════════╝\n',
  )
  ...
  if (specificChainId) {
    const chain = Object.values(L2_CHAINS).find(
  ...
  if (successCount > 0) {
    ...
    console.log(
      '2. Register XLPs: bun run scripts/register-xlp.ts --network testnet --stake 1.0',
    )
    console.log(
      '3. Deposit XLP liquidity: bun run scripts/register-xlp.ts --deposit --chain <chainId> --amount 0.5',
    )
  }

...
`)
  ...
  const deployConfigUpdate = {
    ...
    l2OutputOracleChallenger: keys.find((k) => k.name === 'challenger')
      ?.address,
    ...
    sequencerFeeVaultRecipient: keys.find((k) => k.name === 'feeRecipient')
      ?.address,
  ...
  const envContent = keys
    .map((k) => `${k.name.toUpperCase()}_PRIVATE_KEY=${k.privateKey}`)
    .join('\n')

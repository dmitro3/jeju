...
function parseDeploymentOutput(output: string): DeploymentResult {
  ...
  const patterns = [
    {
      key: 'solverRegistry',
      pattern: /SolverRegistry deployed to:\s*(0x[a-fA-F0-9]{40})/i,
    },
    {
      key: 'inputSettler',
      pattern: /InputSettler deployed to:\s*(0x[a-fA-F0-9]{40})/i,
    },
    {
      key: 'outputSettler',
      pattern: /OutputSettler deployed to:\s*(0x[a-fA-F0-9]{40})/i,
    },
    {
      key: 'oracle',
      pattern:
        /(Simple|Hyperlane|Superchain)?Oracle deployed to:\s*(0x[a-fA-F0-9]{40})/i,
    },
  ]
...
async function main() {
  ...
  for (const chain of TESTNET_CHAINS) {
    ...
    console.log(
      `   Balance: ${balance} ETH ${hasBalance ? '' : '(need 0.05+)'}`,
    )
  ...
  if (!deployAll && !specificChainId) {
    ...
    for (const c of deployableChains) {
  ...
  if (successCount > 0) {
    console.log('\nNext steps:')
    console.log(
      '1. Verify cross-chain liquidity: bun run scripts/verify-crosschain-liquidity.ts',
    )

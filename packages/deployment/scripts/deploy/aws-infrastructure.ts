...
async function checkTerraformBackend(network: Network): Promise<boolean> {
  ...
  if (bucketCheck.exitCode !== 0) {
    ...
    if (createResult.exitCode !== 0) {
      ...
      throw new Error(
        `Failed to create S3 bucket: ${createError.split('\n')[0]}`,
      )
    }

    const versionResult =
      await $`aws s3api put-bucket-versioning --bucket ${bucketName} --versioning-configuration Status=Enabled`.nothrow()
    if (versionResult.exitCode !== 0) {
      const versionError = versionResult.stderr.toString() || 'Unknown error'
      throw new Error(
        `Failed to enable versioning: ${versionError.split('\n')[0]}`,
      )
    }

    const encryptResult =
      await $`aws s3api put-bucket-encryption --bucket ${bucketName} --server-side-encryption-configuration '{"Rules":[{"ApplyServerSideEncryptionByDefault":{"SSEAlgorithm":"AES256"}}]}'`.nothrow()
    if (encryptResult.exitCode !== 0) {
      const encryptError = encryptResult.stderr.toString() || 'Unknown error'
      throw new Error(
        `Failed to enable encryption: ${encryptError.split('\n')[0]}`,
      )
    }
  }
  ...
  const tableCheck =
    await $`aws dynamodb describe-table --table-name ${tableName}`
      .quiet()
      .nothrow()
...
async function status(config: Config) {
  ...
  const initResult = await $`cd ${tfDir} && terraform init -backend=true`
    .quiet()
    .nothrow()

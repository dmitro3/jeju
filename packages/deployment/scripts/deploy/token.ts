...
const existingModMarketplace = args['moderation-marketplace'] as
  | Address
  | undefined
...
async function main() {
  ...
  if (!NETWORKS[networkName]) {
    throw new Error(
      `Unknown network: ${networkName}. Use localnet, testnet, or mainnet.`,
    )
  }
  ...
  if (networkConfig.requireMultiSig && !safeAddress) {
    throw new Error(
      `Mainnet deployment requires a Safe multi-sig address. Use --safe <address>`,
    )
  }
  ...
  if (existingBanManager) {
    ...
    if (dryRun) {
      ...
      if (!deployedAddr)
        throw new Error('Failed to get BanManager contract address')
      banManagerAddress = deployedAddr
  ...
  if (dryRun) {
    ...
    const deployedAddr = receipt.contractAddress
    if (!deployedAddr)
      throw new Error('Failed to get NetworkToken contract address')
    jejuTokenAddress = deployedAddr
  ...
  if (existingModMarketplace && !dryRun) {
    ...
    if (!safeAddress || safeAddress === account.address) {
      ...
      transactions.push({
        hash,
        description: 'Set ModerationMarketplace ban exempt',
      })
  ...
  if (safeAddress && safeAddress !== account.address && !dryRun) {
    ...
    transactions.push({
      hash: jejuOwnerHash,
      description: 'Transfer NetworkToken ownership to Safe',
    })
    ...
    if (!existingBanManager) {
      ...
      transactions.push({
        hash: banOwnerHash,
        description: 'Transfer BanManager ownership to Safe',
      })
  ...
  if (!dryRun) {
    ...
    console.log(
      `   Ban Enforcement: ${banEnforcementEnabled ? '✅ Enabled' : '❌ Disabled'}`,
    )
  ...
  console.log(
    '   1. Verify contracts on block explorer (if --verify flag used)',
  )
  ...
  if (networkConfig.enableFaucet) {
    ...
    console.log(
      `   cast send ${jejuTokenAddress} "faucet()" --rpc-url ${networkConfig.rpcUrl}`,
    )
  }

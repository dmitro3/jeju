...
async function main() {
  ...
  console.log(
    `Network: ${network} | RPC: ${rpcUrl} | Delay: ${config.timelockDelay}s\n`,
  )
  ...
  for (const contract of contracts) {
    ...
    result[contract.key] = deployContract(
      contract.path,
      contract.args,
      rpcUrl,
      deployerKey,
  ...
  console.log(
    '  1. Register sequencers: cast send <SequencerRegistry> "register(uint256,uint256)" <agentId> <stake>',
  )
  console.log(
    '  2. Create upgrade proposal: cast send <GovernanceTimelock> "proposeUpgrade(...)"',
  )
  console.log(
    '  3. Challenge state root: cast send <DisputeGameFactory> "createGame(...)"',
  )
  console.log('')
}

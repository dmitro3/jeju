...
function getNetworkConfig(network: string): { rpcUrl: string; chainId: number } {
  switch (network) {
    case 'mainnet':
      return {
...
function loadExistingDeployments(network: string): Record<string, string> {
  ...
  for (const file of files) {
    ...
    if (existsSync(filepath)) {
      ...
      return {
        identityRegistry: content.IdentityRegistry ?? content.identityRegistry ?? '',
...
async function deploy(network: string): Promise<DADeployment> {
  ...
  const env = {
    ...
    IDENTITY_REGISTRY: existingDeployments.identityRegistry || '0x0000000000000000000000000000000000000000',
    BAN_MANAGER: existingDeployments.banManager || '0x0000000000000000000000000000000000000000',
  };
  ...
  const operatorRegistryMatch = output.match(/DAOperatorRegistry deployed at: (0x[a-fA-F0-9]{40})/);
  const blobRegistryMatch = output.match(/DABlobRegistry deployed at: (0x[a-fA-F0-9]{40})/);
  const attestationManagerMatch = output.match(/DAAttestationManager deployed at: (0x[a-fA-F0-9]{40})/);

  if (!operatorRegistryMatch || !blobRegistryMatch || !attestationManagerMatch) {
...
deploy(network)
  .then((deployment) => {
    ...
    console.log('\nContracts:');
    console.log('  DAOperatorRegistry:', deployment.contracts.DAOperatorRegistry);
    console.log('  DABlobRegistry:', deployment.contracts.DABlobRegistry);
    console.log('  DAAttestationManager:', deployment.contracts.DAAttestationManager);
  })
  .catch((err) => {

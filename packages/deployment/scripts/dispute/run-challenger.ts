    ...
    setInterval(async () => {
      for (const [gameId, game] of this.pendingGames) {
        ...
        try {
          ...
          if (!submitAccount || onChainGame[0].toLowerCase() !== submitAccount.address.toLowerCase()) {
            continue;
          }
...
async function main(): Promise<void> {
  ...
  const l2PublicClient = l2RpcUrl && l2Chain ? createPublicClient({ chain: l2Chain, transport: http(l2RpcUrl) }) : null;

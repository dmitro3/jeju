  ...
  private async findDivergenceWithEmulation(
    ...
    while (high - low > 1n) {
      ...
      for (let i = 0n; i < mid && !currentState.exited; i++) {
        const { newState: stepState } = this.cannonInterface.executeStep(currentState, memory, preimages);
...
async function main(): Promise<void> {
  ...
  const privateKey = process.env.CHALLENGER_PRIVATE_KEY || (
    isLocalnet ? '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80' : null
  );
  
  if (!privateKey) {
    throw new Error('CHALLENGER_PRIVATE_KEY environment variable required for non-local networks');
  }

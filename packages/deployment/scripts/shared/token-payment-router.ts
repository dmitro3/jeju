...
export class TokenPaymentRouter {
    ...
    for (const tokenInfo of tokenInfos) {
      ...
      const isPreferred =
        ...
        tokenInfo.address.toLowerCase() ===
          appPreference.preferredToken.toLowerCase()
   ...
   */
  private async getAppPreference(
    appAddress: Address,
  ): Promise<AppPreference | null> {
    if (
      this.config.appTokenPreference ===
  ...
  private async findBestOption(
    ...
    if (appPreference) {
      ...
      if (
        preferredInfo &&
        preferredInfo.balance > 0n &&
        preferredInfo.hasXLPLiquidity
      ) {
        const cost = await this.calculateTokenCost(
          request,
          preferredInfo.address,
        )
      ...
      if (appPreference.allowFallback) {
        for (const fallback of appPreference.fallbackTokens) {
          ...
          if (
            fallbackInfo &&
            fallbackInfo.balance > 0n &&
            fallbackInfo.hasXLPLiquidity
          ) {
            const cost = await this.calculateTokenCost(
              request,
            ...
            if (cost !== null && fallbackInfo.balance >= cost) {
              return {
                ...
                amount: cost,
                amountUsd: await this.getTokenUsdValue(
                  fallbackInfo.address,
                  cost,
                ),
    ...
    if (bestOption === null && this.config.oifAggregator) {
      const crossChainOption = await this.findCrossChainOption(
        request,
        tokenInfos,
      )
      if (crossChainOption) {
  ...
  private getCommonTokens(
    chainId: number,
  ): Array<{ address: Address; symbol: string; decimals: number }> {
  ...
  private async calculateTokenCost(
    ...
    if (request.isGasPayment) {
      ...
      if (
        this.config.priceOracle === '0x0000000000000000000000000000000000000000'
      ) {
  ...
  private async getTokenUsdValue(
    ...
    return (
      ...
      (Number(price) / 10 ** Number(decimals))
    )
  }
...
export async function getBestPaymentOption(
  ...
  if (!globalRouter || !globalUserAddress) {
    throw new Error(
      'Payment system not initialized. Call initializePayment() first.',
    )
  }
...
export async function checkApproval(option: PaymentOption): Promise<boolean> {
  ...
  return globalRouter.needsApproval(
    globalUserAddress,
    option.token,
    option.amount,
  )

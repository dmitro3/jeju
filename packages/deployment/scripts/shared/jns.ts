  ...
  async resolve(name: string): Promise<JNSResolvedRecord | null> {
    ...
    try {
      ...
      if (process.env.DEBUG) {
        console.warn(
          `Failed to check record existence for ${name}: ${errorMessage}`,
        )
      }
      return null
    }
      ...
      try {
        ...
        if (process.env.DEBUG) {
          const errorMessage =
            err instanceof Error ? err.message : 'Unknown error'
          console.warn(
            `Failed to get text record ${key} for ${name}: ${errorMessage}`,
          )
        }
      }
    }
 ...
 */
export async function createJNSClient(
  rpcUrl: string,
  ...
  if (deploymentPath) {
    ...
    config = {
      registry: (process.env.JNS_REGISTRY ||
        '0x0000000000000000000000000000000000000000') as Address,
      resolver: (process.env.JNS_RESOLVER ||
        '0x0000000000000000000000000000000000000000') as Address,
      registrar: (process.env.JNS_REGISTRAR ||
        '0x0000000000000000000000000000000000000000') as Address,
      reverseRegistrar: (process.env.JNS_REVERSE_REGISTRAR ||
        '0x0000000000000000000000000000000000000000') as Address,
      rpcUrl,
    }
  }

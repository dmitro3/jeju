...
export class GasIntentRouter {
   ...
   */
  async getCrossChainPaymasterOptions(
    ...
    for (const token of supportedTokens) {
      const [isSupported, liquidity, tokenCost, exchangeRate] =
        await Promise.all([
          this.client.readContract({
   ...
   */
  async getPaymasterOptions(
    gasEstimate: GasEstimate,
  ): Promise<PaymasterOption[]> {
    const options: PaymasterOption[] = []

    if (
      this.config.paymasterFactoryAddress ===
      '0x0000000000000000000000000000000000000000'
    ) {
      return options
    }
  ...
  async selectOptimalPayment(
    ...
    const scoredOptions = viableOptions.map((option) => {
      ...
      if (!userBalance)
        throw new Error(`User balance not found for ${option.tokenAddress}`)
      ...
      const liquidityRatio =
        Number(option.availableLiquidity) / Number(option.estimatedCost)
      const liquidityScore = Math.min(liquidityRatio * 10, 100)
      const balanceRatio =
        Number(userBalance.balance) / Number(option.estimatedCost)
      const balanceScore = Math.min(balanceRatio * 10, 100)

      const totalScore =
        costScore * 0.4 + liquidityScore * 0.3 + balanceScore * 0.3
  ...
  async selectBestCrossChainPayment(
    ...
    const allBalances = await this.getMultiChainBalances(
      userAddress,
      tokensByChain,
    )
    ...
    const viableOptions: Array<
      PaymasterOption & { score: number; sourceChainId: number }
    > = []
    ...
    for (const option of crossChainOptions) {
      ...
      for (const balance of tokenBalances) {
        if (balance.balance >= option.estimatedCost) {
          ...
          const liquidityScore = Math.min(liquidityRatio * 10, 100)
          const balanceRatio =
            Number(balance.balance) / Number(option.estimatedCost)
          const balanceScore = Math.min(balanceRatio * 10, 100)

          // Penalty for cross-chain (requires bridging/XLP)
          const crossChainPenalty =
            balance.chainId !== this.config.chainId ? 10 : 0
          ...
          viableOptions.push({
            ...
            reason:
              balance.chainId !== this.config.chainId
  ...
  async canPayGas(
    ...
    const result = await this.selectOptimalPayment(
      userAddress,
      gasEstimate,
      supportedTokens,
    )
  ...
  private async getTokenUsdValue(
    ...
    const [price, priceDecimals] = (await this.client
      .readContract({
  ...
  private async getETHPrice(): Promise<number> {
    ...
    const ethPrice = (await this.client
      .readContract({
        address: this.config.priceOracleAddress,
        abi: PRICE_ORACLE_ABI,
  ...
  private generateReason(
    ...
    if (option.estimatedCostUsd < 0.1) {
...
export function createGasRouter(
  ...
  const fullConfig: RouterConfig = {
    rpcUrl:
      config.rpcUrl || process.env.JEJU_RPC_URL || 'http://127.0.0.1:9545',

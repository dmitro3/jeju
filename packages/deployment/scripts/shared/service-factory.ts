/**
 * @module packages/deployment/scripts/shared/service-factory
...
export function createSingleton<T, C = unknown>(
  ...
  return {
    get: (config?: C): T => {
      ...
      if (!instance) {
        throw new Error(
          `${serviceName} not initialized. Call with config first.`,
        )
      }
      return instance
    },
...
export function createKeyedSingleton<T, C = unknown>(
  ...
  return {
    get: (key: string, config?: C): T => {
      ...
      if (!instance) {
        throw new Error(
          `${serviceName}[${key}] not initialized. Call with config first.`,
        )
      }
      return instance
    },

...
const DEFAULT_CHAINS: ChainConfig[] = [
  {
    ...
    tokens: [
      ...
      {
        address: '0x6B175474E89094C44Da98b954EeaDcB7fCDb4AD66',
        symbol: 'DAI',
        decimals: 18,
      },
    ],
    ...
    const chainPromises = this.chains.map(async (chain) => {
      ...
      const tokenPromises = chain.tokens.map(async (token) => {
        try {
          ...
          console.debug(
            `Failed to get ${token.symbol} balance on ${chain.name}:`,
            e,
          )
        }
        return null
 ...
 */
export async function discoverAllBalances(
  user: Address,
): Promise<MultiChainBalances> {

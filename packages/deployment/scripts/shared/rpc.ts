...
export class FailoverProvider {
  ...
  constructor(
    ...
    if (this.clients.length > 1) {
      console.log(
        `✅ ${name} provider initialized with ${urlArray.length} RPC endpoint(s)`,
      )
    }
  }
  ...
  async getProvider(): Promise<PublicClient> {
    try {
      ...
      console.warn(
        `⚠️  ${this.name} RPC ${this.currentIndex} failed, trying fallback...`,
      )
      this.onFailover?.()
...
export async function checkRPC(
  ...
  try {
    ...
    await Promise.race([client.getBlockNumber(), timeoutPromise])

    return true
  } catch (error) {
    console.warn(
      `RPC check failed for ${rpcUrl}:`,
...
export async function waitForTransaction(
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    setTimeout(
      () =>
        reject(new Error(`Transaction timeout after ${timeout}ms: ${txHash}`)),
      timeout,
    )
  })
  ...
  try {
    ...
    throw new Error(
      `Failed to wait for transaction ${txHash}: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}

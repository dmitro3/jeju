...
async function getStatus() {
  ...
  const [
    ...
    publicClient.readContract({
      address: STAKING_ADDRESS,
      abi: STAKING_ABI,
      functionName: 'getBaseReward',
      args: [address],
    }),
    publicClient.readContract({
      address: STAKING_ADDRESS,
      abi: STAKING_ABI,
      functionName: 'getDelegationReward',
      args: [address],
    }),
    publicClient.readContract({
      address: STAKING_ADDRESS,
    ...
    publicClient.readContract({
      address: STAKING_ADDRESS,
      abi: STAKING_ABI,
      functionName: 'getUnbondingAmount',
      args: [address],
    }),
    publicClient.readContract({
      address: STAKING_ADDRESS,
      abi: STAKING_ABI,
      functionName: 'getUnbondingEndsAt',
      args: [address],
    }),
    publicClient.readContract({
      address: STAKING_ADDRESS,
    ...
    publicClient.readContract({
      address: STAKING_ADDRESS,
      abi: STAKING_ABI,
      functionName: 'getCommunityStakerLimits',
    }),
  ])
  ...
  console.log(
    `Limits:     ${formatEther(minStake)} - ${formatEther(maxStake)} LINK`,
  )

  if ((unbondingAmount as bigint) > 0n) {
    console.log(`\nUnbonding:  ${formatEther(unbondingAmount as bigint)} LINK`)
    console.log(
      `Ends:       ${new Date(Number(unbondingEndsAt) * 1000).toISOString()}`,
    )

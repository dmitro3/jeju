...
async function main() {
  ...
  if (!privateKey) {
    throw new Error('DEPLOYER_PRIVATE_KEY environment variable required for configuring cross-chain remotes');
  }
  ...
  for (const net of deployedNetworks) {
    ...
    console.log(`  - ${net.chain} (domain ${net.domain}): ${deployment.crossChainIdentitySync}`);
  }
  console.log('');
  ...
  for (const net of deployedNetworks) {
    ...
    if (!dep) continue;
    console.log(`${net.chain.padEnd(15)} | Domain ${net.domain.toString().padStart(5)} | ${dep.crossChainIdentitySync}`);
  }
  console.log('');
}

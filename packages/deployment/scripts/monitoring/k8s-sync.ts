...
export async function syncToConfigMap(config: SyncConfig): Promise<void> {
  ...
  const createResult =
    await $`kubectl create configmap ${configMapName} -n ${namespace} ${fromFileArgs}`.nothrow()

  if (createResult.exitCode !== 0) {
    console.log(`   ConfigMap might already exist. Attempting to recreate...`)

    const deleteResult =
      await $`kubectl delete configmap ${configMapName} -n ${namespace} --ignore-not-found`.nothrow()

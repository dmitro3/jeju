...
async function testProviderInitialization() {
  ...
  if (attestation.provider !== TEEProvider.LOCAL)
...
async function testNodeRegistration() {
  ...
  if (node.gpu.gpuType !== GPUType.H200)
    throw new Error(`Wrong GPU type: ${node.gpu.gpuType}`)
  if (node.gpu.gpuCount !== 8)
    throw new Error(`Wrong GPU count: ${node.gpu.gpuCount}`)
}
...
async function testJobSubmission() {
  ...
  if (status.status !== 'completed') {
    throw new Error(
      `Job did not complete: ${status.status}, ${status.result?.error ?? 'unknown'}`,
    )
  }
...
async function testJobWithMetrics() {
  ...
  if (typeof metrics.trainingLoss !== 'number')
    throw new Error('Missing training loss')
  if (typeof metrics.gpuUtilization !== 'number')
    throw new Error('Missing GPU utilization')
...
async function testResourceAllocation() {
  ...
  if (updatedNode.resources.availableCpu !== initialCpu) {
    throw new Error(
      `CPU not released: ${updatedNode.resources.availableCpu} vs ${initialCpu}`,
    )
  }
...
async function testAttestationVerification() {
  ...
  if (!attestation.mrEnclave.startsWith('0x'))
    throw new Error('Invalid mrEnclave format')
  if (!attestation.mrSigner.startsWith('0x'))
    throw new Error('Invalid mrSigner format')
  if (attestation.mrEnclave.length !== 66)
    throw new Error('Invalid mrEnclave length')

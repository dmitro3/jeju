...
async function verifyChainDeployment(
  chainId: number,
  contracts: {
...
async function main() {
  ...
  for (const chainId of chainIds) {
    ...
    console.log(
      `  Chain ${chainId}: ${hasAll ? '✅ Configured' : '❌ Missing env vars'}`,
    )
    if (!hasAll) {
      if (!inputSettler)
        console.log(`    - OIF_INPUT_SETTLER_${chainId} not set`)
      if (!outputSettler)
        console.log(`    - OIF_OUTPUT_SETTLER_${chainId} not set`)
    }
  }

  console.log('')
  console.log(
    allOk ? '✅ All contracts verified' : '⚠️ Some contracts missing or failed',
  )

...
const BacktestArgsSchema = z.object({
  ...
  startDate: z.string().transform((s) => {
    const d = new Date(s)
    if (Number.isNaN(d.getTime())) throw new Error(`Invalid start date: ${s}`)
    return d
  }),
  endDate: z.string().transform((s) => {
    const d = new Date(s)
    if (Number.isNaN(d.getTime())) throw new Error(`Invalid end date: ${s}`)
    return d
  }),
...
async function runBot(args: string[]): Promise<void> {
  ...
  if (!rpcUrlRaw) {
    throw new Error(
      'RPC_URL required: provide as argument or set RPC_URL environment variable',
    )
  }
  if (!privateKeyRaw) {
...
async function runSimulation(args: string[]): Promise<void> {
  ...
  console.log(
    `Final weights: ${state.weights.map((w) => `${((Number(w) / 1e18) * 100).toFixed(1)}%`).join(', ')}`,
  )
  console.log(
    `Accumulated fees: ${state.accumulatedFees.map((f) => f.toString()).join(', ')}`,
  )
}

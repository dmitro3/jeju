...
export function getTFMMConfig(): TFMMConfig {
  return {
    ...
    riskParams: {
      minWeight: BigInt(getEnvNumber('TFMM_MIN_WEIGHT_BPS', 500)) * BigInt(1e14), // 5% = 5e16
      maxWeight: BigInt(getEnvNumber('TFMM_MAX_WEIGHT_BPS', 9500)) * BigInt(1e14), // 95%
...
export function getMomentumConfig(): MomentumStrategyConfig {
  return {
    lookbackPeriodMs: getEnvNumber('MOMENTUM_LOOKBACK_MS', 7 * 24 * 60 * 60 * 1000),
    shortTermPeriodMs: getEnvNumber('MOMENTUM_SHORT_TERM_MS', 24 * 60 * 60 * 1000),
...
export function getMeanReversionConfig(): MeanReversionStrategyConfig {
  return {
    lookbackPeriodMs: getEnvNumber('MEAN_REV_LOOKBACK_MS', 14 * 24 * 60 * 60 * 1000),

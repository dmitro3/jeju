...
async function main(): Promise<void> {
  ...
  console.log(
    '║           JEJU BOTS - COMPREHENSIVE ANALYSIS                 ║',
  )
  console.log(
    '╚══════════════════════════════════════════════════════════════╝\n',
  )
...
async function analyzeOracles(): Promise<void> {
  ...
  console.log(
    `  Fresh price (0s old): ${oracle.isStale(freshPrice, 60000) ? 'STALE' : 'FRESH'} ✓`,
  )
  console.log(
    `  Stale price (120s old, 60s threshold): ${oracle.isStale(stalePrice, 60000) ? 'STALE' : 'FRESH'} ✓\n`,
  )
  ...
  console.log(
    `  $3000 vs $3030 deviation: ${deviation} bps (${(deviation / 100).toFixed(2)}%)`,
  )
  console.log(
    `  Threshold: 50 bps - ${deviation > 50 ? 'WOULD TRIGGER ALERT' : 'Within bounds'}\n`,
  )
...
async function analyzeStrategies(): Promise<void> {
  ...
  for (const scenario of scenarios) {
    ...
    const priceData = dataFetcher.generateSyntheticData(
      ...
      86400000, // Daily
      {
        ...
        volatilities: {
          WETH: scenario.volatility,
          USDC: 0.01,
          WBTC: scenario.volatility * 0.9,
        },
    ...
    for (const strategy of STRATEGIES) {
      ...
      console.log(
        ` ${strategy.padEnd(17)} | ${returnStr} | ${sharpeStr} | ${ddStr} | ${volStr} | ${winStr}`,
      )
    }
  }
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      `  ${scenario.name.padEnd(20)} → ${bestStrategy.toUpperCase()} (Sharpe: ${bestSharpe.toFixed(2)})`,
    )
  }
  ...
  console.log(
    '  Reason: Best risk-adjusted returns across multiple market conditions\n',
  )
}
...
async function analyzeCrossChainArbitrage(): Promise<void> {
  ...
  for (const opp of simulatedOpportunities) {
    ...
    console.log(
      `  ${opp.pair.padEnd(12)} | ${opp.sourceChain.padEnd(10)} → ${opp.targetChain.padEnd(10)} | ${opp.spreadBps} bps | $${opp.estProfitUsd.toFixed(0).padStart(4)} | ${status}`,
    )
  }
...
async function analyzeRiskMetrics(): Promise<void> {
  ...
  console.log(
    `  Avg Recovery Time:     ${fmt(drawdowns.avgRecoveryDays, 1)} days`,
  )
  const longestRecoveryStr =
    drawdowns.longestRecoveryDays !== undefined
  ...
  for (const scenario of stressScenarios) {
    ...
    const loss =
      ((snapshots[snapshots.length - 1].valueUsd - stressedValue) /
        snapshots[snapshots.length - 1].valueUsd) *
      100
    console.log(
      `  ${scenario.name.padEnd(25)}: $${stressedValue.toFixed(0).padStart(8)} (-${loss.toFixed(1)}%)`,
    )
  }
...
async function generateOptimizationReport(): Promise<void> {
  ...
  console.log('   ─────────────────────')
  console.log(
    '   • Use COMPOSITE strategy as default (best risk-adjusted returns)',
  )
  ...
  console.log(
    '   • Correlation monitoring: Reduce exposure during correlation spikes\n',
  )

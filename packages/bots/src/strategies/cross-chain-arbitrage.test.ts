...
describe('Cross-Chain Module Exports', () => {
  ...
  it('SolanaArbitrage should implement expected interface', () => {
    const solArb = new SolanaArbitrage()

    // Should have opportunity-related methods
    expect(typeof solArb.findOpportunities).toBe('function')

    // Should have price update method
    expect(typeof solArb.updatePrices).toBe('function')

    // Should have quote methods
    expect(typeof solArb.getRaydiumQuote).toBe('function')
    expect(typeof solArb.getOrcaQuote).toBe('function')
  })
})

...
describe('VolatilityStrategy', () => {
  ...
  test('should allocate more weight to lower volatility assets', async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      strategy.updatePriceHistory([
        { token: '0x1', price: ethPrice, decimals: 8, timestamp, source: 'pyth' },
        { token: '0x2', price: usdcPrice, decimals: 8, timestamp, source: 'pyth' },
      ]);
    }
  ...
  test('should detect volatility spikes', async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const sinVal = Math.floor(Math.sin(i / 3) * 1000000000 * volatilityMultiplier);
...
describe('Volatility Calculation Properties', () => {
  test('should have higher vol for trending up vs flat', async () => {
    ...
    const tokens: Token[] = [{ address: '0x1', symbol: 'A', decimals: 18, chainId: 1 }];
    ...
    for (let i = 0; i < 30; i++) {
      ...
      trendStrategy.updatePriceHistory([
        { token: '0x1', price: 100n * 10n ** 8n + trend + noise, decimals: 8, timestamp: now - (30 - i) * 3600000, source: 'pyth' },
      ]);
    }
    ...
    const flatCtx: StrategyContext = {
      ...
      prices: [{ token: '0x1', price: 100n * 10n ** 8n, decimals: 8, timestamp: now, source: 'pyth' }],
      priceHistory: [],
      riskParams,
      blockNumber: 100n,
      timestamp: now,
    ...
    const trendCtx: StrategyContext = {
      ...flatCtx,
      prices: [{ token: '0x1', price: 103n * 10n ** 8n, decimals: 8, timestamp: now, source: 'pyth' }],
    };
...
describe('VolatilityStrategy with volatility targeting', () => {
  test('should use vol targeting when inverse weighting disabled', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      strategy.updatePriceHistory([
        { token: '0x1', price: 100n * 10n ** 8n + BigInt(sin1), decimals: 8, timestamp: now - (30 - i) * 3600000, source: 'pyth' },
        { token: '0x2', price: 100n * 10n ** 8n + BigInt(cos1), decimals: 8, timestamp: now - (30 - i) * 3600000, source: 'pyth' },
      ]);
    }
    ...
    const ctx: StrategyContext = {
      ...
      prices: [
        { token: '0x1', price: 100n * 10n ** 8n, decimals: 8, timestamp: now, source: 'pyth' },
        { token: '0x2', price: 100n * 10n ** 8n, decimals: 8, timestamp: now, source: 'pyth' },
      ],
      priceHistory: [],
      riskParams: {
        minWeight: WEIGHT_PRECISION / 20n,

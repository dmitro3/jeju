...
export class MomentumStrategy extends BaseTFMMStrategy {
  ...
  constructor(oracle: OracleAggregator, config: Partial<MomentumConfig> = {}) {
  ...
  async calculateWeights(ctx: StrategyContext): Promise<WeightCalculation> {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      const longMomentum = this.calculateMomentum(
        longTermPrices,
        longTermPrices.length,
      )

      // Calculate short-term momentum
      const shortMomentum =
        shortTermPrices.length > 1
      ...
      const shortMA = this.config.useEMA
        ? this.calculateEMA(
            shortTermPrices,
            Math.min(shortTermPrices.length, 12),
          )
        : this.calculateSMA(
            shortTermPrices,
            Math.min(shortTermPrices.length, 12),
      ...
      const absMomentum =
        combinedMomentum > 0n ? combinedMomentum : -combinedMomentum
      const meetsThreshold =
        absMomentum >= BigInt(this.config.momentumThresholdBps)

      // Calculate trend strength (MA crossover)
      const trendStrength =
        longMA > 0n ? Number(((shortMA - longMA) * 100n) / longMA) / 100 : 0
      ...
      const sensitivityMultiplier = BigInt(
        Math.floor(this.config.sensitivity * 100),
      )
      ...
      const weightMultiplier =
        WEIGHT_PRECISION + (adjustedMomentum * WEIGHT_PRECISION) / BPS_PRECISION
      ...
      signals.push({
        ...
        reason: this.formatSignalReason(
          combinedMomentum,
          trendStrength,
          meetsThreshold,
        ),
    ...
    for (let i = 0; i < tokens.length; i++) {
      const adjustedWeight =
        (currentWeights[i] * momentumScores[i]) / WEIGHT_PRECISION
  ...
  private calculateSignalConsistency(signals: StrategySignal[]): number {
    ...
    const agreement =
      Math.max(positiveCount, negativeCount) / strongSignals.length
    return agreement
  }

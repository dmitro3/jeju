...
import type { OraclePrice, TFMMRiskParameters } from '../../types'
import {
  BaseTFMMStrategy,
...
describe('BaseTFMMStrategy Math Functions', () => {
  ...
  describe('applyGuardRails', () => {
    ...
    test('should enforce minimum weight', () => {
      ...
      const targetWeights = [
        WEIGHT_PRECISION / 100n,
        (WEIGHT_PRECISION * 99n) / 100n,
      ] // 1% / 99%

      const result = strategy.testApplyGuardRails(
        currentWeights,
        targetWeights,
        defaultParams,
      )
    ...
    test('should limit weight change per update', () => {
      ...
      const maxChange =
        (currentWeight * BigInt(defaultParams.maxWeightChangeBps)) /
        BPS_PRECISION
      const actualChange =
  ...
  test('guard rails preserve weight sum invariant', () => {
    ...
    for (let trial = 0; trial < 10; trial++) {
      ...
      const currentWeights = strategy.testNormalizeWeights(
        Array(numTokens)
          .fill(0)
          .map(() => BigInt(Math.floor(Math.random() * 1000) + 100)),
      )

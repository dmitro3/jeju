...
export class MeanReversionStrategy extends BaseTFMMStrategy {
  ...
  async calculateWeights(ctx: StrategyContext): Promise<WeightCalculation> {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (stdDev > 0n) {
        const deviation =
          currentPrice > mean ? currentPrice - mean : mean - currentPrice
      ...
      if (this.config.useBollinger) {
        const bandWidth =
          (stdDev * BigInt(Math.floor(this.config.bollingerMultiplier * 100))) /
          100n
        upperBand = mean + bandWidth
        lowerBand = mean - bandWidth
      } else {
        const threshold = BigInt(
          Math.floor(this.config.deviationThreshold * 100),
        )
      ...
      const weightMultiplier = BigInt(
        Math.floor(multiplier * Number(WEIGHT_PRECISION)),
      )
    ...
    for (let i = 0; i < tokens.length; i++) {
      const adjustedWeight =
        (currentWeights[i] * reversionScores[i]) / WEIGHT_PRECISION

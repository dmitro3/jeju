...
export class VolatilityStrategy extends BaseTFMMStrategy {
  ...
  async calculateWeights(ctx: StrategyContext): Promise<WeightCalculation> {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      volatilities.push({
        dailyVol,
        annualizedVol: Math.min(
          annualizedVol,
          this.config.maxVolatilityPct / 100,
        ),
        historicalVol,
        recentVol,
        volRatio,
    ...
    const volGap = Math.abs(
      portfolioVol - this.config.targetVolatilityPct / 100,
    )
  ...
  private calculateInverseVolWeights(
    ...
    for (let i = 0; i < currentWeights.length; i++) {
      ...
      const blendedPct =
        currentPct * (1 - blendFactor) + targetPct * blendFactor
      blendedWeights.push(
        BigInt(Math.floor(blendedPct * Number(WEIGHT_PRECISION))),
      )
    }
  ...
  private calculateVolTargetWeights(
    ...
    const currentPortfolioVol = this.calculatePortfolioVolatility(
      volatilities,
      currentWeights,
    )
    ...
    const avgVol =
      volatilities.reduce((s, v) => s + v.annualizedVol, 0) /
      volatilities.length
    ...
    for (let i = 0; i < currentWeights.length; i++) {
      ...
      const newWeight =
        (currentWeights[i] * BigInt(Math.floor(scaledAdjustment * 1000))) /
        1000n

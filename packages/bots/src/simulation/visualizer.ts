...
import type { MonteCarloResult, ValidationResult, WalkForwardResult } from './monte-carlo'
...
export class ASCIICharts {
  ...
  static lineChart(
    ...
    for (let y = 0; y < height; y++) {
      ...
      const label = this.formatNumber(value).padStart(8)
      lines.push(`${label} │${grid[y].join('')}│`)
    }

    lines.push(`${''.padStart(9)}└${'─'.repeat(width)}┘`)
    lines.push(`${''.padStart(10)}0${''.padStart(Math.floor(width / 2) - 1)}${data.length}`)
  ...
  static barChart(
    ...
    for (const item of data) {
      ...
      const valueStr = this.formatNumber(item.value)
  ...
  static histogram(
    ...
    lines.push(`└${'─'.repeat(width)}┘`)
    lines.push(` ${this.formatNumber(min).padStart(10)}${''.padStart(width - 20)}${this.formatNumber(max).padStart(10)}`)
  ...
  static table(
    ...
    // Header
    const headerRow = headers.map((h) => h.padEnd(colWidth).slice(0, colWidth)).join('│')
    lines.push(`┌${'─'.repeat(colWidth)}${'┬' + '─'.repeat(colWidth)}`.repeat(headers.length - 1) + `─┐`)
    lines.push(`│${headerRow}│`)
    lines.push(`├${'─'.repeat(colWidth)}${'┼' + '─'.repeat(colWidth)}`.repeat(headers.length - 1) + `─┤`)

    // Data rows
    for (const row of rows) {
      const dataRow = row.map((c) => c.padEnd(colWidth).slice(0, colWidth)).join('│')
      lines.push(`│${dataRow}│`)
    }
...
export class TerminalReport {
   ...
   */
  static printBacktestReport(result: BacktestResult): void {
    console.log('\n' + '═'.repeat(70))
    ...
    if (result.snapshots.length > 0) {
      ...
      console.log(ASCIICharts.lineChart(values, { width: 60, height: 12, title: 'Portfolio Value (USD)' }))
    }

    // Drawdown Chart
    console.log('\n📉 DRAWDOWN')
    const drawdowns = this.calculateDrawdownSeries(result.snapshots.map((s) => s.valueUsd))
    console.log(ASCIICharts.lineChart(drawdowns.map((d) => -d * 100), { width: 60, height: 8, title: 'Drawdown (%)' }))
  }

  /**
   * Print Monte Carlo report
   */
  static printMonteCarloReport(result: MonteCarloResult): void {
    console.log('\n' + '═'.repeat(70))
    ...
    console.log(`  P(Profit):        ${(result.probabilityOfProfit * 100).toFixed(1)}%`)
    console.log(`  P(Ruin):          ${(result.probabilityOfRuin * 100).toFixed(1)}%`)
    console.log(`  Kelly Fraction:   ${(result.kelly * 100).toFixed(1)}%`)

    // Confidence Intervals
    console.log('\n📐 95% CONFIDENCE INTERVALS')
    console.log('─'.repeat(40))
    console.log(`  Sharpe Ratio:     [${result.sharpeRatio.ci95[0].toFixed(2)}, ${result.sharpeRatio.ci95[1].toFixed(2)}]`)
    console.log(`  Max Drawdown:     [${(result.maxDrawdown.ci95[0] * 100).toFixed(1)}%, ${(result.maxDrawdown.ci95[1] * 100).toFixed(1)}%]`)
    console.log(`  Win Rate:         [${(result.winRate.ci95[0] * 100).toFixed(1)}%, ${(result.winRate.ci95[1] * 100).toFixed(1)}%]`)

    // Distribution Histogram
    console.log('\n📊 RETURN DISTRIBUTION HISTOGRAM')
    console.log(ASCIICharts.histogram(result.distribution.map((r) => r * 100), 25, { width: 50, height: 8 }))
  }
  ...
  static printValidationReport(result: ValidationResult): void {
    ...
    console.log(ASCIICharts.table(
      ...
      [
        ['Total Return', `${(result.inSample.totalReturn * 100).toFixed(1)}%`, `${(result.outOfSample.totalReturn * 100).toFixed(1)}%`, `${((result.outOfSample.totalReturn - result.inSample.totalReturn) / Math.abs(result.inSample.totalReturn) * 100).toFixed(0)}%`],
        ['Sharpe Ratio', result.inSample.sharpeRatio.toFixed(2), result.outOfSample.sharpeRatio.toFixed(2), `${-result.degradation.toFixed(0)}%`],
        ['Max Drawdown', `${(result.inSample.maxDrawdown * 100).toFixed(1)}%`, `${(result.outOfSample.maxDrawdown * 100).toFixed(1)}%`, ''],
        ['Win Rate', `${(result.inSample.winRate * 100).toFixed(1)}%`, `${(result.outOfSample.winRate * 100).toFixed(1)}%`, ''],
        ['Profit Factor', result.inSample.profitFactor.toFixed(2), result.outOfSample.profitFactor.toFixed(2), ''],
      ],
    ))
    ...
    console.log('\n🔍 OVERFIT ASSESSMENT')
    console.log('─'.repeat(40))
    console.log(`  Overfit Score:    ${(result.overfitScore * 100).toFixed(0)}% ${result.overfit ? '⚠️ HIGH' : '✅ OK'}`)
    ...
    for (const test of result.statisticalTests) {
      ...
      console.log(`     p-value: ${test.pValue.toFixed(4)} | ${test.interpretation}`)
    }
   ...
   */
  static printStressTestReport(results: StressTestResult[]): void {
    console.log('\n' + '═'.repeat(70))
   ...
   */
  static printMEVReport(result: CompetitionSimResult): void {
    console.log('\n' + '═'.repeat(70))
    ...
    // Profit by strategy
    console.log('\n📈 PROFIT BY STRATEGY')
    const stratData = Object.entries(result.profitByStrategy).map(([label, value]) => ({ label, value }))
    ...
    console.log(`  Missed Opps:      ${result.latencyImpact.missedOpportunities}`)
    console.log(`  Potential Gain:   $${result.latencyImpact.optimalLatencyGain.toFixed(2)} (with 0 latency)`)
  }

  private static calculateDrawdownSeries(values: number[]): number[] {
...
<body>
  <div class="container">
    ...
    ${data.backtest ? this.generateBacktestSection(data.backtest) : ''}
    ${data.monteCarlo ? this.generateMonteCarloSection(data.monteCarlo) : ''}
    ${data.validation ? this.generateValidationSection(data.validation) : ''}
    ${data.stressTests ? this.generateStressTestSection(data.stressTests) : ''}
    ${data.mevSim ? this.generateMEVSection(data.mevSim) : ''}
  </div>
</body>
</html>`
   ...
   * Save report to file
   */
  static save(html: string, path: string): void {
    writeFileSync(path, html)
    console.log(`Report saved to: ${path}`)
  }

  private static generateBacktestSection(result: BacktestResult): string {
    const returnClass = result.totalReturn >= 0 ? 'positive' : 'negative'
    const sharpeClass = result.sharpeRatio >= 1 ? 'positive' : result.sharpeRatio >= 0 ? 'neutral' : 'negative'

    // Generate SVG equity curve
    const equityCurve = this.generateSVGLineChart(
      result.snapshots.map((s) => s.valueUsd),
      { width: 800, height: 200, color: result.totalReturn >= 0 ? '#3fb950' : '#f85149' },
    )
  ...
  private static generateMonteCarloSection(result: MonteCarloResult): string {
    const histogram = this.generateSVGHistogram(result.distribution.map((r) => r * 100))
  ...
  private static generateValidationSection(result: ValidationResult): string {
  ...
  private static generateStressTestSection(results: StressTestResult[]): string {
  ...
  private static generateMEVSection(result: CompetitionSimResult): string {
    return `
    <div class="card">
      ...
      <table>
        ...
        ${Object.entries(result.profitByStrategy).sort((a, b) => b[1] - a[1]).map(([strategy, profit]) => `
        <tr>
          <td>${strategy}</td>
  ...
  private static generateSVGLineChart(
    data: number[],
  ...
  private static generateSVGHistogram(data: number[], bins: number = 25): string {
    if (data.length === 0) return '<p>No data</p>'

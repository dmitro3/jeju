...
describe('PortfolioSimulator', () => {
  ...
  describe('Spot Price Calculation', () => {
    ...
    test('should throw for unknown token', () => {
      expect(() => simulator.getSpotPrice('UNKNOWN', 'USDC')).toThrow(
        'not found',
      )
    })
    ...
    test('should be inverse for reversed pair', () => {
      ...
      expect(product).toBeGreaterThan(
        WEIGHT_PRECISION - WEIGHT_PRECISION / 100n,
      )
  ...
  describe('Swap Execution', () => {
    ...
    test('should throw for unknown tokens', () => {
      expect(() => simulator.swap('UNKNOWN', 'USDC', 1n)).toThrow(
        'Token not found',
      )
    })
  })
  ...
  describe('Liquidity Operations', () => {
    test('should add liquidity and update state', () => {
      ...
      expect(newState.balances[0]).toBeGreaterThanOrEqual(
        initialState.balances[0],
      )
      expect(newState.balances[1]).toBeGreaterThanOrEqual(
        initialState.balances[1],
      )
    })
    ...
    test('should remove liquidity proportionally', () => {
      ...
      const newState = simulator.getState()
      expect(newState.totalLpTokens).toBe(
        initialState.totalLpTokens - lpToRemove,
      )
    })
    ...
    test('should maintain proportions after remove', () => {
      const initialState = simulator.getState()
      const initialRatio =
        (initialState.balances[0] * 10n ** 6n) / initialState.balances[1]
  ...
  describe('Total Value Calculation', () => {
    ...
    test('should increase value after adding liquidity', () => {
      ...
      simulator.addLiquidity([1n * 10n ** 18n, 3000n * 10n ** 6n])

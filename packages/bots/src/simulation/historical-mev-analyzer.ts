...
const CHAINS = [
  { chainId: 1, name: 'Ethereum', chain: mainnet, rpc: 'https://eth.llamarpc.com' },
  { chainId: 8453, name: 'Base', chain: base, rpc: 'https://mainnet.base.org' },
  { chainId: 42161, name: 'Arbitrum', chain: arbitrum, rpc: 'https://arb1.arbitrum.io/rpc' },
  { chainId: 10, name: 'Optimism', chain: optimism, rpc: 'https://mainnet.optimism.io' },
  { chainId: 56, name: 'BSC', chain: bsc, rpc: 'https://bsc-dataseed.binance.org' },
] as const
...
const UNISWAP_V3_SWAP = parseAbiItem('event Swap(address indexed sender, address indexed recipient, int256 amount0, int256 amount1, uint160 sqrtPriceX96, uint128 liquidity, int24 tick)')
...
export class HistoricalMEVAnalyzer {
   ...
   */
  private async analyzeChain(chainId: number, blockCount: number): Promise<ChainMEVStats> {
    ...
    for (let i = 0; i < sampleSize; i++) {
      ...
      try {
        ...
        const gasCostEth = Number(block.gasUsed) * Number(block.baseFeePerGas ?? 0n) / 1e18
    ...
    const avgGasPrice = sampledBlocks > 0 ? Number(totalGasPrice / BigInt(sampledBlocks)) / 1e9 : 30
    const avgPriorityFee = sampledBlocks > 0 ? Number(totalPriorityFee / BigInt(sampledBlocks)) / 1e9 : 2
  ...
  private async fetchPrices(): Promise<void> {
    try {
      const response = await fetch('https://coins.llama.fi/prices/current/ethereum:0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2,ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599')
      if (response.ok) {
        const data = await response.json() as { coins: Record<string, { price: number }> }
        for (const [key, value] of Object.entries(data.coins)) {
          if (key.includes('0xC02aaA39')) this.priceCache.set('ETH', value.price)
          if (key.includes('0x2260FAC5')) this.priceCache.set('BTC', value.price)
        }
      }
    } catch {
      // Use defaults
   ...
   */
  private calculateTopStrategies(chains: ChainMEVStats[]): Array<{ name: string; profit: number; count: number }> {
    ...
    for (const chain of chains) {
      ...
      strategies['Cross-Chain Arb'].count += Math.floor(chain.arbOpportunities * 0.1)
    }
  ...
  private generateTimeSeries(chains: ChainMEVStats[]): Array<{
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const totalMEV = chains.reduce((s, c) => s + c.totalMEVUsd / 30, 0) * variation
  ...
  private generateRecommendations(chains: ChainMEVStats[]): string[] {
    ...
    const sortedChains = [...chains].sort((a, b) => b.totalMEVUsd - a.totalMEVUsd)
    ...
    // Arb vs sandwich
    const arbVsSandwich = chains.reduce((s, c) => s + c.arbOpportunities, 0) /
      Math.max(1, chains.reduce((s, c) => s + c.sandwichAttacks, 0))
    if (arbVsSandwich > 2) {
      recommendations.push(`Strategy: Pure arb opportunities (${arbVsSandwich.toFixed(1)}x more) are safer than sandwich`)
    }
  ...
  private printSummary(
    ...
    console.log('│ Chain      │ MEV Est.   │ Arb Opps │ Gas (gwei) │ Competitors │')
    console.log('├' + '─'.repeat(68) + '┤')
    for (const chain of chains.sort((a, b) => b.totalMEVUsd - a.totalMEVUsd)) {
      console.log(`│ ${chain.chainName.padEnd(10)} │ $${chain.totalMEVUsd.toFixed(0).padStart(8)} │ ${chain.arbOpportunities.toString().padStart(8)} │ ${chain.avgGasPrice.toFixed(2).padStart(10)} │ ${chain.competitorCount.toString().padStart(11)} │`)
    }
    console.log('└' + '─'.repeat(68) + '┘')
    ...
    for (const strat of strategies) {
      ...
      console.log(`  ${strat.name.padEnd(15)} │${bar} $${strat.profit.toFixed(0)}`)
    }
    ...
    console.log(`  AVG MEV PER CHAIN:       $${(totalMEV / chains.length).toFixed(0)}`)
    console.log('─'.repeat(70))
...
export class RealOpportunityFetcher {
   ...
   */
  async fetchRecentSwaps(chainId: number, limit: number = 100): Promise<HistoricalSwap[]> {
    const swaps: HistoricalSwap[] = []

    // For mainnet, use Alchemy asset transfers API if available
    if (chainId === 1 && this.alchemyKey) {
      try {
        const response = await fetch(`https://eth-mainnet.g.alchemy.com/v2/${this.alchemyKey}`, {
          method: 'POST',
        ...
        if (response.ok) {
          const data = await response.json() as {
            result: { transfers: Array<{ blockNum: string; hash: string; from: string; to: string; value: number; asset: string }> }
          }
    ...
    if (swaps.length === 0) {
      for (let i = 0; i < limit; i++) {
        swaps.push({
          ...
          dex: Object.keys(DEX_ROUTERS[chainId] ?? DEX_ROUTERS[1])[Math.floor(Math.random() * 3)],
          tokenIn: 'WETH',
          tokenOut: 'USDC',
  ...
  async fetchSolanaSwaps(limit: number = 100): Promise<HistoricalSwap[]> {
    ...
    if (this.heliusKey) {
      try {
        ...
        const response = await fetch(`https://api.helius.xyz/v0/addresses/${jupiterProgram}/transactions?api-key=${this.heliusKey}&limit=${limit}`)
        if (response.ok) {
          const txs = await response.json() as Array<{ signature: string; slot: number; description: string }>
          for (const tx of txs) {
            swaps.push({
...
export { type HistoricalAnalysisResult, type ChainMEVStats }

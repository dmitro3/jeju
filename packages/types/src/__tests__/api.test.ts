...
describe('ErrorDetailSchema', () => {
  ...
  test('accepts array of strings', () => {
    const result = ErrorDetailSchema.safeParse(['Error 1', 'Error 2', 'Error 3']);
    expect(result.success).toBe(true);
  });
...
describe('PaginationInfoSchema', () => {
  ...
  test('rejects non-positive page', () => {
    expect(PaginationInfoSchema.safeParse({ page: 0, pageSize: 20, total: 100, totalPages: 5 }).success).toBe(false);
    expect(PaginationInfoSchema.safeParse({ page: -1, pageSize: 20, total: 100, totalPages: 5 }).success).toBe(false);
  });

  test('rejects non-positive pageSize', () => {
    expect(PaginationInfoSchema.safeParse({ page: 1, pageSize: 0, total: 100, totalPages: 5 }).success).toBe(false);
    expect(PaginationInfoSchema.safeParse({ page: 1, pageSize: -10, total: 100, totalPages: 5 }).success).toBe(false);
  });
  ...
  test('rejects negative total/totalPages', () => {
    expect(PaginationInfoSchema.safeParse({ page: 1, pageSize: 20, total: -1, totalPages: 5 }).success).toBe(false);
    expect(PaginationInfoSchema.safeParse({ page: 1, pageSize: 20, total: 100, totalPages: -1 }).success).toBe(false);
  });

  test('rejects non-integer values', () => {
    expect(PaginationInfoSchema.safeParse({ page: 1.5, pageSize: 20, total: 100, totalPages: 5 }).success).toBe(false);
    expect(PaginationInfoSchema.safeParse({ page: 1, pageSize: 20.5, total: 100, totalPages: 5 }).success).toBe(false);
  });
});

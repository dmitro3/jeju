...
describe('Paymaster Integration Library', () => {
  describe('estimateTokenCost()', () => {
    ...
    test('should apply exchange rate correctly', () => {
      ...
      const expectedTokenCost =
        (expectedEthCost * exchangeRate) / parseEther('1')
  ...
  describe('generatePaymasterData()', () => {
    ...
    test('should include paymaster address', () => {
      ...
      expect(addressPart.toLowerCase()).toBe(
        TEST_PAYMASTER.slice(2).toLowerCase(),
      )
    })
    ...
    test('should accept custom gas limits', () => {
      ...
      const result = generatePaymasterData(
        TEST_PAYMASTER,
        customVerification,
        customPostOp,
      )
  ...
  describe('integration scenarios', () => {
    test('should prepare complete paymaster flow data', () => {
      ...
      const approvalTx = getApprovalTxData(
        TEST_TOKEN,
        TEST_PAYMASTER,
        approvalBuffer,
      )
      ...
      const paymasterData = preparePaymasterData(
        TEST_PAYMASTER,
        TEST_TOKEN,
        approvalBuffer,
      )
  ...
  describe('error conditions', () => {
    test('data encoding should not throw for edge cases', () => {
      ...
      expect(() =>
        preparePaymasterData(TEST_PAYMASTER, TEST_TOKEN, 0n),
      ).not.toThrow()
      expect(() => generatePaymasterData(TEST_PAYMASTER, 0n, 0n)).not.toThrow()
      expect(() =>
        getApprovalTxData(TEST_TOKEN, TEST_PAYMASTER, 0n),
      ).not.toThrow()
    })
  })
})

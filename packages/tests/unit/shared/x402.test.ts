...
describe('x402 Payment Protocol', () => {
  ...
  describe('USDC_ADDRESSES configuration', () => {
    test('should have addresses for all networks', () => {
      for (const network of Object.keys(CHAIN_IDS)) {
        expect(
          USDC_ADDRESSES[network as keyof typeof USDC_ADDRESSES],
        ).toBeDefined()
      }
    })
  ...
  describe('PAYMENT_TIERS', () => {
    ...
    test('premium should be more expensive than basic', () => {
      expect(PAYMENT_TIERS.API_CALL_PREMIUM).toBeGreaterThan(
        PAYMENT_TIERS.API_CALL_BASIC,
      )
      expect(PAYMENT_TIERS.API_MONTHLY_ACCESS).toBeGreaterThan(
        PAYMENT_TIERS.API_DAILY_ACCESS,
      )
    })
  ...
  describe('signPaymentPayload()', () => {
    ...
    test('different payloads should produce different signatures', async () => {
      const payload1 = createPaymentPayload(
        TEST_ASSET,
        TEST_RECIPIENT,
        1000n,
        '/test1',
      ...
      const payload2 = createPaymentPayload(
        TEST_ASSET,
        TEST_RECIPIENT,
        1000n,
        '/test2',
  ...
  describe('concurrent payment verification', () => {
    test('should handle multiple concurrent verifications', async () => {
      ...
      const results = await Promise.all(
        payloads.map((p, i) =>
          verifyPayment(p, BigInt(1000 + i), TEST_RECIPIENT),
        ),
      )

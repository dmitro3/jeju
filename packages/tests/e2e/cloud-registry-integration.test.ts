...
if (!localnetAvailable) {
  console.log(
    '⏭️  Skipping Cloud Integration E2E tests - localnet not available at',
    TEST_CONFIG.rpcUrl,
  )
} else if (!cloudContractsDeployed) {
  console.log(
    '⏭️  Skipping Cloud Integration E2E tests - CloudReputationProvider deployment required',
  )
}
  ...
  () => {
    ...
    test(
      ...
      async () => {
        ...
        logger.info(
          `Identity Registry: ${deploymentAddresses.identityRegistry}`,
        )
        logger.info(
  ...
  () => {
    ...
    test(
      ...
      async () => {
        ...
        const agent = (await identityRegistry.read.getAgent([
          user2AgentId,
        ])) as {
          isBanned: boolean
        }
    ...
    test(
      ...
      async () => {
        ...
        const finalReputation =
          await integration.getAgentReputation(user1AgentId)

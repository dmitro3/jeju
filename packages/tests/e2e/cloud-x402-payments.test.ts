...
const USDC_ADDRESS =
  process.env.USDC_ADDRESS || '0x0165878A594ca255338adfa4d48449f69242Eb8F'
...
describe.skipIf(!localnetAvailable)(
  'Cloud x402 E2E - Service Cost Calculation',
  () => {
    test('should get cost for chat-completion service', async () => {
      ...
      if (!isAvailable) {
        logger.warn(
          `Service ${serviceName} not registered - this is expected on fresh localnet`,
        )
    ...
    test('should get cost for all cloud services', async () => {
      ...
      logger.success(
        `Retrieved costs for ${registeredCount}/${services.length} services`,
      )
    })
  },
)
...
describe.skipIf(!localnetAvailable)('Cloud x402 E2E - Payment Flow', () => {
  test('should simulate x402 payment flow', async () => {
    ...
    expect(sufficient).toBe(true)
    logger.info(
      `  Credit sufficient: ${formatUnits(available, 6)} USDC available`,
    )
...
describe.skipIf(!localnetAvailable)(
  'Cloud x402 E2E - Payment Validation',
  () => {
...
describe.skipIf(!localnetAvailable)('Cloud x402 E2E - Volume Discounts', () => {
  test('should calculate cost with volume discount', async () => {
    ...
    if (!isAvailable) {
      logger.warn(
        `Service ${serviceName} not registered, skipping volume discount test`,
      )
      return
    }
    ...
    logger.info(
      `  Current discount: ${discount} bps (${Number(discount) / 100}%)`,
    )
    ...
    const [totalSpent, requestCount, _lastUsedBlock, volumeDiscount] =
      usageResult
  ...
  test('should track usage for volume discounts', async () => {
    ...
    if (!isAvailable) {
      logger.warn(
        `Service ${serviceName} not registered, skipping usage tracking test`,
      )
      return
    }

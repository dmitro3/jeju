...
export async function executeSwap(
  ...
  console.log(
    `Swapping ${options.amount} ${options.inputToken} for ${options.outputToken}`,
  )
  ...
  const inputTokenSelect = page
    .locator('select[name*="input"], select[name*="from"]')
    .first()
  if (await inputTokenSelect.isVisible({ timeout: 1000 }).catch(() => false)) {
    await inputTokenSelect.selectOption(options.inputToken)
  }

  const outputTokenSelect = page
    .locator('select[name*="output"], select[name*="to"]')
    .first()
...
export async function placeBet(
  ...
  // Select outcome
  const outcomeButton = page
    .locator(`button:has-text("${options.outcome}")`)
    .first()
  await outcomeButton.click()

  // Enter amount
  const amountInput = page
    .locator('input[placeholder*="100"], input[name*="amount"]')
    .first()
  await amountInput.fill(options.amount)
  ...
  const buyButton = page
    .locator(
      `button:has-text("Buy ${options.outcome}"), button:has-text("Place Bet")`,
    )
    .first()
...
export async function addLiquidity(
  ...
  console.log(
    `Adding liquidity: ${options.amount0} ${options.token0} + ${options.amount1} ${options.token1}`,
  )

  // Select tokens if selectors exist
  const token0Select = page
    .locator('select[name*="token0"], select[name*="tokenA"]')
    .first()
  if (await token0Select.isVisible({ timeout: 1000 }).catch(() => false)) {
    await token0Select.selectOption(options.token0)
  }

  const token1Select = page
    .locator('select[name*="token1"], select[name*="tokenB"]')
    .first()
  ...
  // Enter amounts
  const amount0Input = page
    .locator('input[name*="amount0"], input[name*="amountA"]')
    .first()
  await amount0Input.fill(options.amount0)

  const amount1Input = page
    .locator('input[name*="amount1"], input[name*="amountB"]')
    .first()
...
export async function bridgeToken(
  ...
  console.log(
    `Bridging ${options.amount} ${options.token} from ${options.from} to ${options.to}`,
  )
...
export async function connectWallet(
  ...
  const walletOption = page
    .locator(`text="${walletName}", button:has-text("${walletName}")`)
    .first()
...
export async function waitForTransaction(
  ...
  await expect(page.locator(`text=/${successText}/i`)).toBeVisible({ timeout })

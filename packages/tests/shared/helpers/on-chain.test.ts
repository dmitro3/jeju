...
describe('verifyTransactionMined - Transaction Verification', () => {
  test('should throw on invalid tx hash format', async () => {
    await expect(
      verifyTransactionMined('invalid-hash' as Hash, {
        rpcUrl: REAL_RPC,
        timeout: 1000,
      }),
    ).rejects.toThrow()
...
describe.skipIf(!CHAIN_AVAILABLE)(
  ...
  () => {
    ...
    test('should create and compare snapshots', async () => {
      const snapshot1 = await createAccountSnapshot(TEST_ADDRESS, [], {
        rpcUrl: REAL_RPC,
      })
      await new Promise((r) => setTimeout(r, 100))
      const snapshot2 = await createAccountSnapshot(TEST_ADDRESS, [], {
        rpcUrl: REAL_RPC,
      })

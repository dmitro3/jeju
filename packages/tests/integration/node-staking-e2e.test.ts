...
describe.skipIf(!runE2ETests || !kurtosisAvailable)(
  'Node Staking System E2E (Multi-Token)',
  () => {
    ...
    beforeAll(async () => {
      ...
      // Get RPC endpoint
      const l2Port =
        await $`kurtosis port print jeju-localnet op-geth rpc`.text()
      ...
      operator1WalletClient = createWalletClient({
        chain,
        transport: http(rpcUrl),
        account: operator1Account,
      })

      operator2Account = privateKeyToAccount(generatePrivateKey())
      operator2WalletClient = createWalletClient({
        chain,
        transport: http(rpcUrl),
        account: operator2Account,
      })

      oracleAccount = privateKeyToAccount(generatePrivateKey())
      oracleWalletClient = createWalletClient({
        chain,
        transport: http(rpcUrl),
    ...
    test('Deploy reward token and rewards contract', async () => {
      ...
      const deploymentFile = await Bun.file(
        'packages/contracts/deployments/rewards-localnet.json',
      ).json()
      ...
      const rewardsAbi = parseAbi([
        'function getPerformanceOracles() external view returns (address[])',
      ])
    ...
    test('Fund operators with reward tokens', async () => {
      ...
      console.log(
        `✅ Operators funded: ${formatEther(INITIAL_BALANCE)} tokens each\n`,
      )
    })
    ...
    test('Register nodes and stake tokens', async () => {
      ...
      const receipt1 = await waitForTransactionReceipt(publicClient, {
        hash: registerHash1,
      })
      ...
      const receipt2 = await waitForTransactionReceipt(publicClient, {
        hash: registerHash2,
      })
    ...
    test('Oracle updates performance data', async () => {
      ...
      const rewardsAbi = parseAbi([
        'function updatePerformance(bytes32, uint256, uint256, uint256) external',
      ])
    ...
    test('Update node metadata', async () => {
      ...
      const rewardsAbi = parseAbi([
        'function updateNodeMetadata(bytes32, string, uint8) external',
      ])

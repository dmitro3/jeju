...
describe('RPC Connectivity', () => {
  ...
  test('should timeout gracefully on unreachable RPC', async () => {
    ...
    const publicClient = createPublicClient({
      chain,
      transport: http('https://nonexistent.invalid'),
    })
...
describe('OIF Contract Verification', () => {
  ...
  test.each(deployedChains)('chain $chainId: SolverRegistry has code', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
  ...
  test.each(
    deployedChains,
  )('chain $chainId: SolverRegistry.getStats() returns valid data', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
  ...
  test.each(deployedChains)('chain $chainId: MIN_STAKE is reasonable', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
  ...
  test.each(
    deployedChains,
  )('chain $chainId: isSolverActive returns bool for zero address', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
  ...
  test.each(deployedChains)('chain $chainId: InputSettler has code', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
    ...
    const code = await getCode(publicClient, {
      address: contracts.inputSettler as Address,
    })
    expect(code.length).toBeGreaterThan(2)
  })

  test.each(deployedChains)('chain $chainId: OutputSettler has code', async ({
    chainId,
    contracts,
  }) => {
    const chain = inferChainFromRpcUrl(rpcUrl(chainId))
    ...
    const code = await getCode(publicClient, {
      address: contracts.outputSettler as Address,
    })
    expect(code.length).toBeGreaterThan(2)
  })
})
...
describe('EIL Contract Verification', () => {
  ...
  test('L1StakeManager should have code', async () => {
    ...
    const publicClient = createPublicClient({
      chain,
      transport: http(rpcUrl(hubChainId)),
    })
    const code = await getCode(publicClient, {
  ...
  test('L1StakeManager.getProtocolStats() returns valid data', async () => {
    ...
    const publicClient = createPublicClient({
      chain,
      transport: http(rpcUrl(eilConfig.hub.chainId)),
    })
...
describe('Error Handling', () => {
  test('should handle invalid contract address gracefully', async () => {
    ...
    const publicClient = createPublicClient({
      chain,
      transport: http(rpcUrl(11155111)),
    })
    const registryAbi = parseAbi(SOLVER_REGISTRY_ABI)

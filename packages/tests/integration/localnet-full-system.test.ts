...
try {
  ...
  console.log(
    `Localnet not available at ${TEST_CONFIG.l2RpcUrl}, skipping full system tests`,
  )
}
...
describe.skipIf(!localnetAvailable)('Localnet Full System Integration', () => {
  ...
  beforeAll(async () => {
    ...
    user1Account = privateKeyToAccount(
      TEST_WALLETS.user1.privateKey as `0x${string}`,
    )
    user1WalletClient = createWalletClient({
      chain: l2Chain,
      transport: http(TEST_CONFIG.l2RpcUrl),
  ...
  describe('1. RPC Connectivity', () => {
    ...
    it('should have pre-funded test accounts', async () => {
      const balance = await getBalance(l2PublicClient, {
        address: TEST_WALLETS.deployer.address as Address,
      })
  ...
  describe('2. OP-Stack Predeploys', () => {
    ...
    it('should check for L2StandardBridge predeploy', async () => {
      const code = await getBytecode(l2PublicClient, {
        address: OP_PREDEPLOYS.L2StandardBridge as Address,
      })
      isOPStack = code !== '0x'
      if (isOPStack) {
    ...
    it('should check for WETH predeploy', async () => {
      const code = await getBytecode(l2PublicClient, {
        address: OP_PREDEPLOYS.WETH as Address,
      })
      if (code !== '0x') {
    ...
    it('should check for L2CrossDomainMessenger predeploy', async () => {
      const code = await getBytecode(l2PublicClient, {
        address: OP_PREDEPLOYS.L2CrossDomainMessenger as Address,
      })
      if (code !== '0x') {
  ...
  describe('3. Contract Deployments', () => {
    it('should deploy JEJU token and transfer tokens', async () => {
      ...
      const transferReceipt = await waitForTransactionReceipt(l2PublicClient, {
        hash: transferHash,
      })

      expect(transferReceipt.status).toBe('success')
      console.log(
        `   ✅ Transferred ${formatEther(transferAmount)} ELIZA to user1`,
      )
      ...
      console.log(
        `   💰 User1 token balance: ${formatEther(user1Balance)} ELIZA`,
      )
    })
  })
    ...
    it('should check indexer GraphQL endpoint is accessible', async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            '   ⚠️  GraphQL endpoint not yet running (expected if indexer not started)',
          )
        }
      } catch (_error) {
        console.log(
          '   ℹ️  Indexer not running (start with: cd apps/indexer && npm run dev)',
        )
      }
    })
    ...
    it('should query indexed blocks (if indexer running)', async () => {
      try {
        const response = await fetch(TEST_CONFIG.indexerGraphQL, {
          ...
          body: JSON.stringify({
            query:
              '{ blocks(limit: 5, orderBy: number_DESC) { number timestamp transactionCount } }',
          }),
        })
        ...
        if (response.ok) {
          ...
          if (data.data?.blocks) {
            ...
            console.log(
              `   📈 Latest block: ${data.data.blocks[0]?.number || 'N/A'}`,
            )
          }
        }
      } catch (_error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.data?.transactions) {
            console.log(
              `   📊 Indexed ${data.data.transactions.length} transactions`,
            )
          }
        }
      } catch (_error) {
  ...
  describe('6. Event Log Verification', () => {
    it('should capture and query Transfer events from token contract', async () => {
      ...
      console.log(
        `   💰 Total received by user1: ${formatEther(totalReceived)} ELIZA`,
      )
      ...
      const transferArgs = decodedEvent.args as {
        from: Address
        to: Address
        value: bigint
      }
  ...
  describe('7. Service Health Checks', () => {
    it('should verify block production by sending transactions', async () => {
      ...
      console.log(
        `   ✅ Block advanced to ${blockNum2} (triggered by transaction)`,
      )
    })
    ...
    it('should verify L2 gas price oracle', async () => {
      ...
      console.log(
        `   ⛽ Current gas price: ${formatUnits(gasPrice, 'gwei')} gwei`,
      )
    })
  })
  ...
  describe('9. System Integration Verification', () => {
    ...
    it('should print system summary', async () => {
      ...
      console.log(
        `Deployer Balance: ${formatEther(await getBalance(l2PublicClient, { address: deployerAccount.address }))} ETH`,
      )
      console.log('─────────────────────────────────────────\n')
    })
  })
...
describe.skipIf(!localnetAvailable)('End-to-End User Journey', () => {
  it('should simulate complete user transaction flow', async () => {
    ...
    const userBalance = await getBalance(publicClient, {
    ...
    const newBalance = await getBalance(freshClient, {
      address: userAccount.address,
    })
    ...
    console.log(
      `   4️⃣  User balance updated: ${formatEther(newBalance)} ETH (spent ${formatEther(userBalance - newBalance)} ETH)`,
    )

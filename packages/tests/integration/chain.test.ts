...
describe('Chain Infrastructure', () => {
  ...
  beforeAll(async () => {
    ...
    if (!l1Available && !l2Available) {
      console.log('Chains not running. Start with: docker compose -f packages/tests/docker-compose.test.yml --profile chain up -d');
    }
  });
  ...
  describe('L1 Chain', () => {
    ...
    test('dev account should have ETH balance', async () => {
      ...
      const balance = await client.getBalance({ address: TEST_ACCOUNT as `0x${string}` });

...
const indexerAvailable = await isServiceAvailable(
  INDEXER_GRAPHQL,
  'POST',
...
const prometheusAvailable = await isServiceAvailable(
  `${PROMETHEUS_API}/api/v1/query?query=up`,
)
...
describe.skipIf(!indexerAvailable)('Indexer Integration', () => {
  ...
  test('can query blocks', async () => {
    ...
    console.log(
      `✅ Indexer has indexed ${data.data.blocks.length > 0 ? 'blocks' : 'no blocks yet'}`,
    )
  })
})
...
describe('Service Availability Summary', () => {
  test('reports which services are running', () => {
    ...
    console.log(
      `   Indexer:    ${indexerAvailable ? '✅ Running' : '⏭️  Not running'}`,
    )
    console.log(
      `   IPFS:       ${ipfsAvailable ? '✅ Running' : '⏭️  Not running'}`,
    )
    console.log(
      `   Grafana:    ${grafanaAvailable ? '✅ Running' : '⏭️  Not running'}`,
    )
    console.log(
      `   Prometheus: ${prometheusAvailable ? '✅ Running' : '⏭️  Not running'}`,
    )

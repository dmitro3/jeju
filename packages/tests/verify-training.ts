...
async function verifyTraining() {
    ...
    if (fs.existsSync(hooksPath)) {
      ...
      const missing = requiredExports.filter(
        (exp) =>
          ...
          !content.includes(`export enum ${exp}`),
      )
      if (missing.length === 0) {
  ...
  try {
    ...
    const subcommands = trainingCommand.commands.map(
  ...
  try {
    ...
    const routesFile = fs.readFileSync(
      'apps/dws/src/server/routes/compute.ts',
      'utf-8',
    )
    ...
    const missing = requiredRoutes.filter(
      (route) => !routesFile.includes(route),
    )
    if (missing.length === 0) {
  ...
  try {
    ...
    if (code && code.length > 2) {
      ...
      console.log(
        '   ⚠ TrainingCoordinator not deployed (deploy with: jeju dev --bootstrap)',
      )
    }
  } catch {
  ...
  if (allPassed) {
    ...
    console.log(
      '  3. Create a run: jeju training create --model meta/llama-3-8b',
    )

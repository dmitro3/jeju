...
export function isUrlSafeToFetch(urlString: string): boolean {
  ...
  if (/^\d+$/.test(hostname) || /^0x/i.test(hostname) || /^0\d+\./.test(hostname)) {
    return false;
  }
 ...
 */
export function truncateOutput(text: string, maxLength: number = MAX_OUTPUT_LENGTH): string {
 ...
 */
export function getMessageTextSecure(message: Memory, maxLength: number = MAX_MESSAGE_LENGTH): string {
  const text = message.content?.text;
  if (typeof text !== "string" || text.trim() === "") {
    throw new Error("Message text is required");
  }

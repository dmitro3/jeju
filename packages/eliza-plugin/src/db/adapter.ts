...
export class CQLDatabaseAdapter extends DatabaseAdapter<CQLClient> {
  ...
  private cacheStore = new Map<string, { value: string; expiresAt: number | null }>();

  constructor(agentId: UUID, config?: CQLAdapterConfig) {
    super();
    this.agentId = agentId;
    this.databaseId = config?.databaseId ?? process.env.CQL_DATABASE_ID ?? 'eliza';
  ...
  async init(): Promise<void> {
    ...
    logger.info({ src: 'cql-adapter', agentId: this.agentId }, 'Initializing CQL database adapter');
    ...
    logger.info({ src: 'cql-adapter', agentId: this.agentId }, 'CQL adapter initialized');
  }
  ...
  async createAgent(agent: Partial<Agent>): Promise<boolean> {
    ...
    await this.exec(
      ...
      [
        id,
        agent.name ?? 'Agent',
        agent.username ?? agent.name?.toLowerCase().replace(/\s+/g, '_') ?? 'agent',
        '{}',
        '{}',
      ]
    );
  ...
  async getMemories(params: {
    ...
    return rows.map(row => ({
      ...
      embedding: row.embedding ? this.fromJson<number[]>(row.embedding) : undefined,
  ...
  async createMemory(memory: Memory, _tableName: string, unique?: boolean): Promise<UUID> {
  ...
  async updateMemory(memory: Partial<Memory> & { id: UUID; metadata?: MemoryMetadata }): Promise<boolean> {
    ...
    await this.exec(`UPDATE memories SET ${updates.join(', ')} WHERE id = ?`, params);
    return true;
  }
  ...
  async deleteManyMemories(memoryIds: UUID[]): Promise<void> {
    ...
    await this.exec(`DELETE FROM memories WHERE id IN (${placeholders})`, memoryIds);
  }
  ...
  async countMemories(roomId: UUID, unique?: boolean, _tableName?: string): Promise<number> {
  ...
  async searchMemories(params: {
    ...
    logger.warn({ src: 'cql-adapter' }, 'Vector search not supported in CQL, using text search fallback');
  ...
  async updateTask(id: UUID, task: Partial<Task>): Promise<void> {
    ...
    await this.exec(`UPDATE tasks SET ${updates.join(', ')} WHERE id = ?`, params);
  }

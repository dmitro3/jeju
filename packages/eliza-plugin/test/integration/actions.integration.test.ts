...
describe("Eliza Plugin Actions Integration", () => {
  ...
  describe("A2A Actions", () => {
    test("discover gateway agent", async () => {
      if (!gatewayRunning) return;
      const card = await client.a2a.discover(`${GATEWAY_URL}/a2a`);
      expect(card.protocolVersion).toBe("0.3.0");
    });

    test("call gateway skill", async () => {
      if (!gatewayRunning) return;
      const response = await client.a2a.callGateway({ skillId: "list-nodes" });
  ...
  describe("Payments Actions", () => {
    test("get balance returns bigint", async () => {
      if (!chainRunning) return;

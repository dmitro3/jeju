...
export class SecretVault {
  ...
  async getSecretVersion(
    ...
    if (versionRecord.status === 'revoked')
      throw new Error(`Version ${version} has been revoked`)
  ...
  updatePolicy(secretId: string, policy: SecretPolicy, updater: Address): void {
    ...
    if (secret.owner !== updater)
      throw new Error('Only owner can update policy')
  ...
  private async checkAccess(
    ...
    if (policy.allowedAddresses && !policy.allowedAddresses.includes(accessor))
      return false

    if (policy.maxAccessCount) {
      const accessCount = this.accessLogs.filter(
        (l) => l.secretId === secretId && l.accessor === accessor && l.success,
      ).length
  ...
  private async loadFromDA(): Promise<{ success: boolean; count: number }> {
    ...
    const response = await fetch(`${this.config.daEndpoint}/api/secrets/list`, {
      signal: AbortSignal.timeout(5000),
    }).catch((error: Error) => {
      log.error('DA connection failed', { error: error.message })
      return null
    })

    if (response === null || !response.ok) {
      log.warn('Failed to load secrets from DA', {
        endpoint: this.config.daEndpoint,
      })
    ...
    if (!parseResult.success) {
      log.error('Invalid DA response format', {
  ...
  private async persistToDA(secret: Secret): Promise<boolean> {
    ...
    const response = await fetch(
      `${this.config.daEndpoint}/api/secrets/store`,
      {
        method: 'POST',
      ...
      log.error('DA persistence failed', {
        secretId: secret.id,
        error: error.message,
      })
      return null

...
import { toBytes, toHex, keccak256, type Hex } from 'viem';
import {
  aesGcmEncrypt,
  aesGcmDecrypt,
  sealWithMasterKey,
...
describe('AES-256-GCM Encryption', () => {
  ...
  describe('aesGcmEncrypt / aesGcmDecrypt', () => {
    ...
    it('should fail decryption with wrong IV', async () => {
      ...
      await expect(aesGcmDecrypt(ciphertext, wrongIv, testKey)).rejects.toThrow();
    });
...
describe('Key Sealing', () => {
  ...
  describe('sealWithMasterKey / unsealWithMasterKey', () => {
    ...
    it('should fail to unseal with wrong master key', async () => {
      ...
      await expect(unsealWithMasterKey(sealed, wrongMasterKey)).rejects.toThrow();
    });
...
describe('HKDF Key Derivation', () => {
  ...
  describe('deriveKeyForEncryption', () => {
    ...
    it('should produce different keys for different policies', async () => {
      ...
      const policy1 = JSON.stringify({ conditions: [{ type: 'timestamp' }], operator: 'and' });
      const policy2 = JSON.stringify({ conditions: [{ type: 'stake' }], operator: 'and' });
...
describe('Payload Encryption', () => {
  ...
  describe('encryptToPayload / decryptFromPayload', () => {
    ...
    it('should handle unicode and special characters', async () => {
      const data = '日本語 🔐 émojis © ® ™ "quotes" \'apostrophes\' <tags> & ampersands\n\t\r';
      const payload = await encryptToPayload(data, testKey);
      const decrypted = await decryptFromPayload(payload, testKey);
      expect(decrypted).toBe(data);
    });
    ...
    it('should fail with invalid payload format', async () => {
      ...
      await expect(decryptFromPayload(invalidPayload, testKey)).rejects.toThrow('Invalid ciphertext format');
    });
  });
  ...
  describe('parseCiphertextPayload', () => {
    ...
    it('should reject missing fields', () => {
      ...
      expect(() => parseCiphertextPayload(invalid)).toThrow('Invalid ciphertext format');
    });
  });
});
  ...
  describe('Authentication tag verification', () => {
    it('should reject tampering at any position (50 iterations)', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        await expect(aesGcmDecrypt(tamperedCiphertext, iv, key)).rejects.toThrow();
      }
    });
  });
});
...
describe('Edge Cases', () => {
  ...
  it('should handle boundary sizes for data', async () => {
    ...
    const sizes = [0, 1, 15, 16, 17, 31, 32, 33, 255, 256, 257, 1023, 1024, 1025];

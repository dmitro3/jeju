...
describe('MPC Coordinator Edge Cases', () => {
  ...
  describe('Party Registration Edge Cases', () => {
    it('should reject party with insufficient stake when required', () => {
      ...
      expect(() => strictCoordinator.registerParty({
        id: 'low-stake-party',
        index: 1,
    ...
    it('should reject party without verified attestation when required', () => {
      ...
      expect(() => attestationCoordinator.registerParty({
        id: 'no-attestation',
        index: 1,
        endpoint: 'http://localhost:4100',
        publicKey: '0x1234' as Hex,
  ...
  describe('Key Generation Boundary Conditions', () => {
    it('should reject threshold > totalParties', async () => {
      ...
      await expect(coordinator.generateKey({
  ...
  describe('Signature Verification', () => {
    it('should produce recoverable signature matching key address', async () => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        await coordinator.submitPartialSignature(session.sessionId, partyId, { partyId, partialR, partialS, commitment });
      }

      // Submit reveals
      let result: { complete: boolean; signature?: { signature: Hex; r: Hex; s: Hex; v: number } } = { complete: false };
      for (let i = 0; i < 2; i++) {
        const partyId = `party-${i + 1}`;
        const partialR = keccak256(toBytes(`${session.sessionId}:${partyId}:r`));
        const partialS = keccak256(toBytes(`${session.sessionId}:${partyId}:s`));
        const commitment = keccak256(toBytes(`${partialR}:${partialS}`));
        result = await coordinator.submitPartialSignature(session.sessionId, partyId, { partyId, partialR, partialS, commitment });
      }
  ...
  describe('Session Management', () => {
    ...
    it('should reject requests for non-existent key', async () => {
      ...
      await expect(coordinator.requestSignature({
    ...
    it('should reject submission to non-existent session', async () => {
      await expect(coordinator.submitPartialSignature(
        'non-existent-session',
        'party-1',
        { partyId: 'party-1', partialR: '0x1234' as Hex, partialS: '0x5678' as Hex, commitment: '0xabcd' as Hex }
  ...
  describe('Key Rotation Edge Cases', () => {
    ...
    it('should reject rotation for non-existent key', async () => {
      await expect(coordinator.rotateKey({
        keyId: 'non-existent',
    ...
    it('should track multiple rotation versions', async () => {
      await coordinator.rotateKey({ keyId: 'rotation-key', preserveAddress: true });
      await coordinator.rotateKey({ keyId: 'rotation-key', preserveAddress: true });
      await coordinator.rotateKey({ keyId: 'rotation-key', preserveAddress: true });
    ...
    it('should handle concurrent key generations without collision', async () => {
      ...
      const promises = Array.from({ length: 10 }, (_, i) =>
        coordinator.generateKey({
          ...
          threshold: 2,
          totalParties: 3,
          partyIds: [`party-${(i % 3) + 1}`, `party-${((i + 1) % 3) + 1}`, `party-${((i + 2) % 3) + 1}`],
          curve: 'secp256k1',
        })
      );

...
export function stakeGatedPolicy(
  ...
  return {
    conditions: [{ type: 'stake', registryAddress, chain, minStakeUSD }],
    operator: 'and',
  }
}
...
export function roleGatedPolicy(
  ...
  return {
    conditions: [{ type: 'role', registryAddress, chain, role }],
    operator: 'and',
  }
}
...
export function agentOwnerPolicy(
  ...
  return {
    conditions: [{ type: 'agent', registryAddress, chain, agentId }],
    operator: 'and',
  }
}
...
export async function encryptWithPolicy(
  ...
  await kms.initialize()
  return kms.encrypt(
    { data, policy, keyId: options?.keyId, metadata: options?.metadata },
    options?.provider,
  )
}

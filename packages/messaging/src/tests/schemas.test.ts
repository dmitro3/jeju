...
describe('SerializedEncryptedMessageSchema', () => {
  ...
  test('accepts valid encrypted message', () => {
    expect(SerializedEncryptedMessageSchema.safeParse(validMessage).success).toBe(true);
  });

  test('requires all three fields', () => {
    expect(SerializedEncryptedMessageSchema.safeParse({
      ciphertext: 'abc',
      nonce: '123',
    }).success).toBe(false);
...
describe('MessageEnvelopeSchema', () => {
  ...
  test('accepts envelope with optional cid', () => {
    const withCid = { ...validEnvelope, cid: 'QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco' };
...
describe('NodeConfigSchema', () => {
  ...
  test('accepts port boundaries', () => {
    expect(NodeConfigSchema.safeParse({ ...validConfig, port: 1 }).success).toBe(true);
    expect(NodeConfigSchema.safeParse({ ...validConfig, port: 65535 }).success).toBe(true);
  });
...
describe('MessagingClientConfigBaseSchema', () => {
  ...
  test('accepts valid minimal config', () => {
    expect(MessagingClientConfigBaseSchema.safeParse(validConfig).success).toBe(true);
  });
  ...
  test('rejects invalid rpcUrl', () => {
    ...
    expect(MessagingClientConfigBaseSchema.safeParse(invalid).success).toBe(false);
  });
  ...
  test('accepts http and https URLs', () => {
    expect(MessagingClientConfigBaseSchema.safeParse({
      ...validConfig,
...
describe('WebSocketIncomingMessageSchema', () => {
  ...
  test('rejects unknown type', () => {
    ...
    expect(WebSocketIncomingMessageSchema.safeParse(invalid).success).toBe(false);
  });
});
...
describe('SendMessageRequestSchema', () => {
  ...
  test('rejects missing required fields', () => {
    expect(SendMessageRequestSchema.safeParse({ to: '0xbbb' }).success).toBe(false);
    expect(SendMessageRequestSchema.safeParse({ content: 'hello' }).success).toBe(false);
  });
});

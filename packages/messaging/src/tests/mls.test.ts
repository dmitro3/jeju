...
import { JejuMLSClient, createMLSClient } from '../mls/client';
...
describe('MLS Client', () => {
  ...
  test('throws when not initialized', () => {
    const uninitializedClient = createMLSClient({
      ...
      keyRegistryAddress: '0x2222222222222222222222222222222222222222' as Address,
      relayUrl: 'http://localhost:3000',
      rpcUrl: 'http://localhost:8545',
      network: 'testnet',
    });
    
    expect(() => uninitializedClient.listGroups()).toThrow('Client not initialized');
  });
});
...
describe('MLS Group', () => {
  ...
  test('removes members', async () => {
    const memberToRemove = '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' as Address;
  ...
  test('paginates messages', async () => {
    ...
    const secondPage = await group.getMessages({ limit: 2, after: firstPage[1]!.id });
...
describe('Content Types', () => {
  ...
  test('generates content preview', () => {
    ...
    expect(getContentPreview(image({ url: '', width: 0, height: 0, mimeType: '' }))).toBe('📷 Image');
    expect(getContentPreview(file({ url: '', name: 'doc.pdf', size: 0, mimeType: '' }))).toBe('📎 doc.pdf');
    expect(getContentPreview(reaction({ emoji: '👍', messageId: '' }))).toBe('👍 reaction');
  });
});

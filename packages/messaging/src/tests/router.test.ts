...
describe('Average Latency Calculation', () => {
  ...
  test('calculates average for first delivery', () => {
    ...
    const newAvg = (currentAvg * (totalDeliveries - 1) + newLatency) / totalDeliveries;
...
describe('Node Selection Algorithm', () => {
  ...
  test('selects node with lowest latency', () => {
    const nodes: RelayNode[] = [
      { id: 'node-1', url: 'wss://n1.example.com', region: 'us-east', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
      { id: 'node-2', url: 'wss://n2.example.com', region: 'us-west', latencyMs: 50, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
      { id: 'node-3', url: 'wss://n3.example.com', region: 'eu-west', latencyMs: 150, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
    ];
  ...
  test('filters out unhealthy nodes', () => {
    const nodes: RelayNode[] = [
      { id: 'node-1', url: 'wss://n1.example.com', region: 'us-east', latencyMs: 10, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: false },
      { id: 'node-2', url: 'wss://n2.example.com', region: 'us-west', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
    ];
  ...
  test('prefers node in preferred region', () => {
    const nodes: RelayNode[] = [
      { id: 'node-1', url: 'wss://n1.example.com', region: 'us-east', latencyMs: 50, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
      { id: 'node-2', url: 'wss://n2.example.com', region: 'eu-west', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
    ];
  ...
  test('falls back to all nodes when no preferred region matches', () => {
    ...
    const healthyNodes = nodes.filter(n => n.isHealthy);
    let candidates = healthyNodes.filter(n => preferredRegions.includes(n.region));
  ...
  test('returns null when no healthy nodes available', () => {
    const nodes: RelayNode[] = [
      { id: 'node-1', url: 'wss://n1.example.com', region: 'us-east', latencyMs: 50, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: false },
      { id: 'node-2', url: 'wss://n2.example.com', region: 'us-west', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: false },
    ];
  ...
  test('handles equal latencies consistently', () => {
    const nodes: RelayNode[] = [
      { id: 'node-a', url: 'wss://a.example.com', region: 'us-east', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
      { id: 'node-b', url: 'wss://b.example.com', region: 'us-east', latencyMs: 100, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
    ];
...
describe('Statistics Tracking', () => {
  ...
  test('tracks successful vs failed deliveries', () => {
    ...
    expect(stats.totalMessages).toBe(10);
    expect(stats.successfulDeliveries + stats.failedDeliveries).toBe(stats.totalMessages);
  });
...
describe('Edge Cases', () => {
  ...
  test('handles zero latency nodes', () => {
    const nodes: RelayNode[] = [
      { id: 'local', url: 'ws://localhost:8080', region: 'local', latencyMs: 0, activeConnections: 0, lastHealthCheck: Date.now(), isHealthy: true },
    ];
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      if (Math.random() > 0.1) {
        ...
        stats.averageLatencyMs = 
          (stats.averageLatencyMs * (stats.successfulDeliveries - 1) + latency) / stats.successfulDeliveries;
      } else {
        stats.failedDeliveries++;
      }
    }

...
describe('Key Generation', () => {
  ...
  test('generates unique key pairs each time', () => {
    ...
    expect(publicKeysEqual(keyPair1.privateKey, keyPair2.privateKey)).toBe(false);
  });
...
describe('Wallet Key Derivation', () => {
  ...
  test('address case is normalized (lowercase)', () => {
    ...
    const keyPair1 = deriveKeyPairFromWallet('0xABCDEF1234567890ABCDEF1234567890ABCDEF12', signature);
    const keyPair2 = deriveKeyPairFromWallet('0xabcdef1234567890abcdef1234567890abcdef12', signature);
  ...
  test('different addresses produce different keys', () => {
    ...
    const keyPair1 = deriveKeyPairFromWallet('0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', signature);
    const keyPair2 = deriveKeyPairFromWallet('0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', signature);
...
describe('Encryption and Decryption', () => {
  ...
  test('encrypts and decrypts string message correctly', () => {
    ...
    const encrypted = encryptMessage(originalMessage, recipient.publicKey, sender.privateKey);
  ...
  test('produces different ciphertext for same message (random nonce)', () => {
    ...
    expect(decryptMessageToString(encrypted1, recipient.privateKey)).toBe(message);
    expect(decryptMessageToString(encrypted2, recipient.privateKey)).toBe(message);
  });
  ...
  test('uses ephemeral key pair when sender key not provided', () => {
    ...
    expect(encrypted1.ephemeralPublicKey).not.toEqual(encrypted2.ephemeralPublicKey);
  });
  ...
  test('decryption fails with tampered ciphertext', () => {
    ...
    const tampered: EncryptedMessage = {
      ...encrypted,
      ciphertext: new Uint8Array(encrypted.ciphertext.map((b, i) => i === 0 ? b ^ 1 : b)),
    };
  ...
  test('decryption fails with tampered nonce', () => {
    ...
    const tampered: EncryptedMessage = {
      ...encrypted,
      nonce: new Uint8Array(encrypted.nonce.map((b, i) => i === 0 ? b ^ 1 : b)),
    };
...
describe('Property-Based Encryption Tests', () => {
  ...
  test('encrypt/decrypt with random key pairs', () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const encrypted = encryptMessage(message, recipient.publicKey, sender.privateKey);
      const decrypted = decryptMessageToString(encrypted, recipient.privateKey);
  ...
  test('encryption preserves message length patterns', () => {
    ...
    const lengths = [0, 1, 15, 16, 17, 31, 32, 33, 127, 128, 129, 1023, 1024, 1025];
...
describe('Message Envelope', () => {
  test('creates envelope with encrypted content', () => {
    ...
    const envelope = createMessageEnvelope(from, to, content, recipient.publicKey);
  ...
  test('envelope ID is unique', () => {
    ...
    for (let i = 0; i < 100; i++) {
      const envelope = createMessageEnvelope('from', 'to', 'msg', recipient.publicKey);
      ids.add(envelope.id);
    }

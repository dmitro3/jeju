...
import {
  KeyRegistrySync,
  createKeyRegistrySync,
} from '../tee/registry-sync';
...
describe('TEE Key Manager', () => {
  ...
  test('performs ECDH in TEE', async () => {
    ...
    const sharedSecret = await keyManager.sharedSecret(preKey.keyId, theirPublicKey);
  ...
  test('exports encrypted backup', async () => {
    ...
    const backup = await keyManager.exportEncrypted(identityKey.keyId, 'test-password');
  ...
  test('imports from encrypted backup', async () => {
    ...
    const imported = await keyManager.importFromBackup(backup, password, 'imported-key');

...
export interface MessageHandler {
  (envelope: XMTPEnvelope): Promise<void>;
}
...
export class JejuXMTPNode {
   ...
   */
  async lookupIdentities(addresses: Address[]): Promise<Map<Address, XMTPIdentity>> {
  ...
  private async startSync(): Promise<void> {
    ...
    console.log(`[XMTP Node] Starting sync from block ${this.syncState.lastSyncedBlock}`);
  ...
  private async flushPendingMessages(): Promise<void> {
    ...
    console.log(`[XMTP Node] Flushing ${this.syncState.pendingMessages} pending messages`);
  }
 ...
 */
export async function createXMTPNode(config: XMTPNodeConfig): Promise<JejuXMTPNode> {

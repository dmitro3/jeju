...
import { OAuthInitResponseSchema, parseStoredSession } from './schemas'
import {
  createSIWEMessage,
  formatSIWEMessage,
  verifySIWESignature,
...
export function AuthProvider({
  ...
  const loginWithSocial = useCallback(
    async (provider: SocialProvider): Promise<AuthSession> => {
      setState((s) => ({ ...s, isLoading: true, error: null }))
      ...
      if (!initResult.success) {
        throw new Error(
          `Invalid OAuth init response: ${initResult.error.message}`,
        )
      }
  ...
  const linkProvider = useCallback(
    async (provider: SocialProvider | 'farcaster'): Promise<LinkedProvider> => {
      if (!state.session) {
        throw new Error('Not authenticated')
      }

      // Check if already linked
      const existingLink = state.session.linkedProviders.find(
      ...
      if (provider === 'farcaster') {
        ...
        const response = await pollAuthChannel({
          channelToken: channelResponse.channelToken,
        })
        if (!response) {
      ...
      // Initiate OAuth flow
      const authUrl =
        `${oauth3AgentUrl}/oauth/${provider}/authorize?` +
      ...
      // Wait for callback
      const linkedProvider = await new Promise<LinkedProvider>(
        (resolve, reject) => {
  ...
  const unlinkProvider = useCallback(
    async (provider: SocialProvider | 'farcaster'): Promise<void> => {
      if (!state.session) {
        throw new Error('Not authenticated')
      }

      const updatedProviders = state.session.linkedProviders.filter(
        (p) => p.provider !== provider,
      )
  ...
  const registerNewPasskey = useCallback(
    async (name?: string): Promise<PasskeyCredential> => {
      if (!state.session) {
        throw new Error('Not authenticated')
      }

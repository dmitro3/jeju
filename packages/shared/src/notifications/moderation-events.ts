...
export class ModerationNotificationService extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    console.log(`[ModerationNotifications] Starting from block ${this.lastProcessedBlock}`);
    
    // Start polling loop
    this.pollTimer = setInterval(() => this.pollEvents(), this.config.pollInterval);
    ...
    for (const subscriber of this.subscribers.values()) {
      if (this.matchesFilter(event, subscriber.filter)) {
        try {
          ...
          console.error(`[ModerationNotifications] Subscriber ${subscriber.id} error:`, error);
        }
      }
    }
   ...
   */
  private matchesFilter(event: ModerationEvent, filter?: Subscriber['filter']): boolean {
    ...
    if (filter.addresses) {
      ...
      if (!eventAddresses.some(addr => 
        filter.addresses?.map(a => a.toLowerCase()).includes(addr.toLowerCase())
      )) {
        return false;
      }
    }
  ...
  private async sendWebhooks(event: ModerationEvent): Promise<void> {
    for (const webhookUrl of this.config.webhookUrls) {
      try {
        ...
        console.error(`[ModerationNotifications] Webhook failed for ${webhookUrl}:`, error);
      }
    }
  }
  ...
  getAddressSubscription(address: Address): Subscriber | undefined {
    return Array.from(this.subscribers.values()).find(
      sub => sub.filter?.addresses?.map(a => a.toLowerCase()).includes(address.toLowerCase())
    );
  }
}
...
export function createModerationNotifications(config: NotificationConfig): ModerationNotificationService {

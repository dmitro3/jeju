    ...
    test('output is a valid number string or starts with <', () => {
      for (let i = 0; i < 100; i++) {
        ...
        if (!formatted.startsWith('<')) {
          expect(isNaN(parseFloat(formatted))).toBe(false);
        }
      }
    });
  });
  ...
  describe('property-based tests', () => {
    test('higher amounts are more likely to be significant', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const smallSignificant = isSignificantAmount(smallAmount, 18, minUsd, priceUsd);
        const largeSignificant = isSignificantAmount(largeAmount, 18, minUsd, priceUsd);

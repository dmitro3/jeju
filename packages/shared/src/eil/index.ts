/**
 * EIL (Economic Interoperability Layer) Utilities
 *
 * Shared utilities for EIL integration:
...
export function validateSwapParams(params: CrossChainSwapParams): {
  ...
  if (destChain && !destChain.isDestination) {
    return {
      valid: false,
      error: `${destChain.name} is not a supported destination`,
    }
  }
...
export function getBestGasTokenForApp(
  ...
  const viable = userBalances.filter(
    (opt) => opt.userBalance >= opt.cost && opt.availableLiquidity >= opt.cost,
  )
  if (viable.length === 0) return null

  // Priority 1: App's preferred token
  if (
    appPreference?.isActive &&
    appPreference.preferredToken !==
      '0x0000000000000000000000000000000000000000'
  ) {
    const preferred = viable.find(
      (opt) =>
        opt.token.toLowerCase() ===
  ...
  for (const defaultToken of globalDefaults) {
    const defaultOpt = viable.find(
      (opt) => opt.token.toLowerCase() === defaultToken.toLowerCase(),
    )
    if (defaultOpt) {

...
export class CSAMHashProvider {
  ...
  constructor(config: CSAMHashProviderConfig = {}) {
    this.arachnidApiKey =
      config.arachnid?.apiKey ?? process.env.ARACHNID_API_KEY
    this.arachnidBaseUrl =
      config.arachnid?.baseUrl ?? 'https://api.arachnid-shield.ca/v1'
  ...
  private async loadInternalHashes(path: string): Promise<void> {
    try {
      ...
      logger.info('[CSAMHashProvider] Loaded internal hashes', {
        count: internalHashes.size,
      })
    } catch (err) {
      logger.error('[CSAMHashProvider] Failed to load internal hashes', {
        error: String(err),
      })
    }
  }
  ...
  async initialize(): Promise<void> {
    ...
    if (!hasArachnid && !hasInternal) {
      logger.error(
        '[CSAMHashProvider] CRITICAL: No CSAM hash sources configured!',
      )
      logger.error(
        '[CSAMHashProvider] Set ARACHNID_API_KEY or provide internalHashPath',
      )
    }
  ...
  async checkImage(buffer: Buffer): Promise<HashMatchResult> {
    ...
    if (internalHashes.has(contentHash)) {
      logger.warn('[CSAMHashProvider] MATCH: Internal hash database', {
  ...
  private async checkArachnid(
    ...
    try {
      ...
      if (!response.ok) {
        logger.warn('[CSAMHashProvider] Arachnid API error', {
      ...
      logger.error('[CSAMHashProvider] Arachnid API failed', {
        error: String(err),
      })
      return {
        matched: false,
...
export function getCSAMHashProvider(
  config?: CSAMHashProviderConfig,
): CSAMHashProvider {
  if (!instance) {

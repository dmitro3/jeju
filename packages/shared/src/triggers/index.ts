...
class DecentralizedTriggerClient implements TriggerClient {
  private config: Required<
    Omit<TriggerConfig, 'rpcUrl' | 'registryAddress' | 'privateKey'>
  > &
    TriggerConfig
  ...
  async get(id: string): Promise<Trigger | null> {
    const response = await fetch(
      `${this.config.computeEndpoint}/triggers/${id}`,
      {
        signal: AbortSignal.timeout(this.config.timeout),
      },
    )
  ...
  async setActive(id: string, active: boolean): Promise<void> {
    const response = await fetch(
      `${this.config.computeEndpoint}/triggers/${id}/active`,
      {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async executeWebhook(
    path: string,
    body: WebhookBody,
  ): Promise<TriggerProof | null> {
    const response = await fetch(
      `${this.config.computeEndpoint}/webhook${path}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async depositPrepaid(amount: string): Promise<string> {
    const response = await fetch(
      `${this.config.computeEndpoint}/prepaid/deposit`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async withdrawPrepaid(amount: string): Promise<string> {
    const response = await fetch(
      `${this.config.computeEndpoint}/prepaid/withdraw`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount }),
  ...
  async getPrepaidBalance(address: Address): Promise<string> {
    const response = await fetch(
      `${this.config.computeEndpoint}/prepaid/${address}`,
      {
        signal: AbortSignal.timeout(this.config.timeout),
      },
    )
 ...
 */
export async function registerAppTriggers(
  config: AppTriggersConfig,
): Promise<Trigger[]> {
  ...
  for (const triggerConfig of config.triggers) {
    ...
    if (existing) {
      console.log(
        `[Triggers] Trigger ${triggerConfig.name} already exists: ${existing.id}`,
      )
    ...
    const trigger = await client.create({
      ...
      description: triggerConfig.description
        ...
        : `${config.appName} ${triggerConfig.name} trigger`,
      ...
      registerOnChain:
        triggerConfig.registerOnChain !== undefined
          ? triggerConfig.registerOnChain
          : true,
    })
...
export function getTriggerClient(): TriggerClient {
  ...
  const computeEndpoint =
    process.env.COMPUTE_ENDPOINT ?? process.env.TRIGGER_SERVICE_URL
  if (!computeEndpoint) {
    throw new Error(
      'COMPUTE_ENDPOINT or TRIGGER_SERVICE_URL environment variable is required',
    )
  }

  const rpcUrl = process.env.COMPUTE_RPC_URL ?? process.env.RPC_URL
  const registryAddress = process.env.TRIGGER_REGISTRY_ADDRESS as
    | Address
    | undefined

 ...
 *
 * Tracks cache, database, storage, and system performance metrics.
 * Identifies bottlenecks and generates optimization recommendations.
 *
 * @module @jejunetwork/shared/monitoring
 */
...
export type BottleneckType = 'cache' | 'database' | 'storage' | 'memory' | 'cpu'
...
class PerformanceMonitor {
  ...
  takeSnapshot(): PerformanceSnapshot {
    ...
    const dbOperationBreakdown: DatabaseMetrics['operationBreakdown'] = {}
    ...
    const timeWindowSeconds = Math.min(60, uptime || 1)
    ...
    const snapshot: PerformanceSnapshot = {
      ...
      system: {
        cpuUsagePercent: hasCpuUsage ? process.cpuUsage().user / 1000000 : 0,

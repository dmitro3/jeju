 ...
 * Includes profanity detection, prompt injection prevention, and spam filtering.
 *
 * For production, consider integrating with OpenAGI Moderation API or similar service.
 *
 * @module @jejunetwork/shared/content-safety
 */
...
export type ContentCategory = 'profanity' | 'abuse' | 'injection' | 'spam'
...
/**
 * Check content for specific category of issues
 *
 * @param content - Content to check
 * @param category - Category to check for
 * @returns True if content has issues of the specified category
 */
export function hasContentIssue(
  content: string,
  category: ContentCategory,
): boolean {
  const result = checkUserInput(content)
  return !result.safe && result.category === category
}

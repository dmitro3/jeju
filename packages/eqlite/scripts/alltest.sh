...
main() {
  go test -tags "${UNITTESTTAGS:-}" -race -failfast -parallel 16 -cpu 16 -coverprofile main.cover.out $(go list ./... | grep -v EQLite/api)
  ...
  go test -tags "${UNITTESTTAGS:-}" -bench=^BenchmarkMiner$ -benchtime=5s -run ^$ ./cmd/eqlite-minerd/ -bench-miner-count=2
  bash cleanupDB.sh || true
}

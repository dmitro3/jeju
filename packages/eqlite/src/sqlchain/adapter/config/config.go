...
import (
	...
	"eqlite/src/conf"
	"eqlite/src/sqlchain/adapter/storage"
...
type Config struct {
	...
	StorageDriver   string          `yaml:"StorageDriver"` // sqlite3 or eqlite
...
func LoadConfig(configPath string) (config *Config, err error) {
	...
	if len(config.StorageDriver) == 0 {
		config.StorageDriver = "eqlite"
	}
	if config.StorageDriver == "eqlite" {
	...
	if config.CertificatePath == "" || config.PrivateKeyPath == "" {
		...
		if config.VerifyCertificate && config.ClientCAPath != "" {
			...
			var caCert []byte
			if caCert, err = os.ReadFile(clientCAPath); err != nil {
				return
			}
	...
	case "eqlite":
		config.StorageInstance = storage.NewEQLiteStorage(config.MirrorServer)
	case "sqlite3":
...
func loadCert(pemFile string) (cert *x509.Certificate, err error) {
	...
	if certBytes, err = os.ReadFile(pemFile); err != nil {
		return
	}

This doc introduce the usage of EQLite client. Client is used for creating, querying, updating, and deleting the SQLChain and database adhere to the SQLChain.
...
```bash
$ go get github.com/EQLite/EQLite/client
$ go get github.com/EQLite/EQLite/cmd/eqlite
```

and import `client` package if you want to use it in your code.


## Initialize a EQLite Client
...
Run `eqlite` like below. Enter a master key (like a password) for generating local key pair. After that, it may take a few seconds with a private key file and config.yaml file generated in `~/.eqlite/` folder.

```bash
$ eqlite generate config
Generating key pair...
Enter master key(press Enter for default: ""):
⏎
Private key file: ~/.eqlite/private.key
...
After you prepare your master key and config file, EQLite client can be initialized by:

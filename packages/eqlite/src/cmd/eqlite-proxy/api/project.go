...
func projectUserList(c *gin.Context) {
	r := struct {
		DB              proto.DatabaseID `json:"db" form:"db" uri:"db" binding:"required,len=64"`
...
func getProjectOAuthCallback(c *gin.Context) {
	...
	for _, h := range cfg.Hosts {
		// project alias happy and host api.eqlite.io will produce happy.api.eqlite.io as service host
...
func updateProjectOAuthConfig(c *gin.Context) {
	...
	if err != nil {
		...
		_, err = model.AddProjectConfig(projectDB, model.ProjectConfigOAuth, r.Provider, cfg)
		if err != nil {
			_ = c.Error(err)
...
func updateProjectMiscConfig(c *gin.Context) {
	...
	if err != nil {
		// not exists, create
		_, err = model.AddProjectConfig(projectDB, model.ProjectConfigMisc, "", cfg)
		if err != nil {
			_ = c.Error(err)

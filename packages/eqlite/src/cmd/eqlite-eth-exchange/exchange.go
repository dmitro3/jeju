...
import (
	...
	cinter "eqlite/src/blockproducer/interfaces"
	"eqlite/src/client"
...
func (e *Exchange) saveTx(blockNumber *big.Int, tx *types.Transaction) (r *TxRecord, err error) {
	...
	var eqliteAmount uint64
	if eqliteAmount, err = e.getExchangedAmount(tx.Value()); err != nil {
		_ = AddAuditRecord(e.metaDatabase, &AuditRecord{
			Hash: tx.Hash().String(),
			Op:   "get_amount",
			Data: tx,
	...
	r, err = UpsertTx(e.metaDatabase, &TxRecord{
		...
		// EQLite
		EQLiteAccount: accountAddr.String(),
		EQLiteAmount:  eqliteAmount,
...
func (e *Exchange) getExchangedAmount(ethAmount *big.Int) (eqliteAmount uint64, err error) {
	ethAmountFloat := new(big.Float).SetInt(ethAmount)
	eqliteAmount, accuracy := new(big.Float).Mul(ethAmountFloat, e.cfg.ExchangeRate).Uint64()
...
func (e *Exchange) processPendingTxs() {
	...
	for _, r := range records {
		if r.EQLiteAmount > balance {
			// skip this tx, account
			_ = AddAuditRecord(e.metaDatabase, &AuditRecord{
				Hash: r.Hash,
				Op:   "check_balance",
				Data: map[string]interface{}{
					"required": r.EQLiteAmount,
					"balance":  balance,
				},
			})
			continue
		...
		// parse eqlite account
		var target proto.AccountAddress
		err = hash.Decode((*hash.Hash)(&target), r.EQLiteAccount)
		if err != nil {
			_ = AddAuditRecord(e.metaDatabase, &AuditRecord{
				Hash: r.Hash,
				Op:   "invalid_target_account",
				Data: map[string]interface{}{
					"account": r.EQLiteAccount,
				},
			})
		}

		var txHash hash.Hash
		txHash, err = e.transferToken(target, nonce, r.EQLiteAmount)
		_ = AddAuditRecord(e.metaDatabase, &AuditRecord{
			Hash: r.Hash,
			Op:   "transfer_token",
			Data: map[string]interface{}{
...
func (e *Exchange) updateTransferringTxState() {
	...
	if err != nil {
		log.WithError(err).Warning("failed to get transferring tx")
		return
	}
	...
	for _, r := range records {
		...
		if err = hash.Decode(&req.Hash, r.EQLiteTxHash); err != nil {
			continue
		}
		...
		if resp.State == cinter.TransactionStateConfirmed {
			// update state
			if err = SetTxConfirmed(e.metaDatabase, r); err != nil {
				log.WithError(err).Warning("failed to set tx confirmed")
			}
			_ = AddAuditRecord(e.metaDatabase, &AuditRecord{
				Hash: r.Hash,

...
func startNodes() {
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/eqlited.test"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/leader.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/follower1.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/follower2.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/eqlite-minerd.test"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/miner0.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/miner1.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/eqlite-minerd/miner2.cover.out"),
...
func startNodesProfile(bypassSign bool) {
	...
	os.RemoveAll(FJ(testWorkingDir, "./integration/node_miner_0/data"))
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/eqlite-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_0/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/eqlite-minerd/miner0.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/eqlite-minerd/miner0.trace"),
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/eqlite-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_1/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/eqlite-minerd/miner1.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/eqlite-minerd/miner1.trace"),
			"-metric-graphite-server", "192.168.2.100:2003",
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/eqlite-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_2/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/eqlite-minerd/miner2.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/eqlite-minerd/miner2.trace"),
			"-metric-graphite-server", "192.168.2.100:2003",
...
func TestFullProcess(t *testing.T) {
	...
	Convey("test full process", t, func(c C) {
		...
		dbMix, err := sql.Open("eqlite", dsnCfgMix.FormatDSN())
		So(err, ShouldBeNil)
		defer dbMix.Close()
		...
		dbFollower, err := sql.Open("eqlite", dsnCfgFollower.FormatDSN())
		So(err, ShouldBeNil)
...
func benchMiner(b *testing.B, minerCount uint16) {
	...
	testDataDir, err := os.MkdirTemp(testWorkingDir, "eqlite")
	if err != nil {
		panic(err)
	}
	...
	dsnFile := FJ(baseDir, "./cmd/eqlite-minerd/.dsn")
	var dsn string
	if minerCount > 0 {
		// create
		meta := client.ResourceMeta{

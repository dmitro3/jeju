...
// CmdCreate is eqlite create command entity.
var CmdCreate = &Command{
	UsageLine: "eqlite create [common params] [-wait-tx-confirm] [db_meta_params]",
	Short:     "create a database",
	Long: `
Create command creates a EQLite database by database meta params. The meta info must include
node count.
e.g.
    eqlite create -db-node 2
...
e.g.
    eqlite create -wait-tx-confirm -db-node 2
`,
...
func runCreate(cmd *Command, args []string) {
	...
	storeOneDSN(dsn)
	fmt.Printf("The connecting string beginning with 'eqlite://' could be used as a dsn for `eqlite console`\n or any command, or be used in website like https://web.eqlite.io\n")
}

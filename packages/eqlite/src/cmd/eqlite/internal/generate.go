...
// CmdGenerate is eqlite generate command entity.
var CmdGenerate = &Command{
	UsageLine: "eqlite generate [common params] [-source template_file] [-miner listen_addr] [-private existing_private_key] [dest_path]",
	Short:     "generate a folder contains config file and private key",
	Long: `
Generate generates private.key and config.yaml for EQLite.
...
func runGenerate(cmd *Command, args []string) {
	...
	if len(args) == 0 {
		workingRoot = utils.HomeDirExpand("~/.eqlite")
	} else if args[0] == "" {
		workingRoot = utils.HomeDirExpand("~/.eqlite")
	} else {
	...
	if source == "" {
		...
		sourceConfig, err := os.ReadFile(source)
		if err != nil {
	...
	if fileinfo, err = os.Stat(workingRoot); err == nil {
		if fileinfo.IsDir() {
			_ = filepath.Walk(workingRoot, func(filepath string, f os.FileInfo, err error) error {
				if f == nil {
					return err
				}
				if f.IsDir() {
...
You can get some free PTC from:
	https://testnet.eqlite.io/wallet/`)

...
// CmdRPC is eqlite rpc command entity.
var CmdRPC = &Command{
	UsageLine: "eqlite rpc [common params] [-wait-tx-confirm] [-endpoint rpc_endpoint | -bp] -name rpc_name -req rpc_request",
	Short:     "make a rpc request",
	Long: `
RPC makes a RPC request to the target endpoint.
e.g.
    eqlite rpc -name 'MCC.QuerySQLChainProfile' \

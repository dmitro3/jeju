...
func TestStorage(t *testing.T) {
	...
	// test failed query
	_, _, _, err = st.Query(context.Background(), []Query{newQuery("SQL???? WHAT!!!!")})
	...
	// test non-read query
	_, _, _, _ = st.Query(context.Background(),
	...
	// test again
	_, _, data, err = st.Query(context.Background(), []Query{newQuery("SELECT `key` FROM `kv`")})
	if err != nil {
		t.Fatalf("query failed: %v", err.Error())
	} else if len(data) != 3 {
	...
	// test with select
	_, _, data, err = st.Query(context.Background(),

...
func TestInvalidKeystoreFileRecover(t *testing.T) {
	Convey("invalid file recover", t, func() {
		...
		defer utils.RemoveAll(dbFile + "*")
		var err error
		err = os.WriteFile(dbFile, []byte("UNKNOWN_DATA_MUST_NOT_BE_A_SQLITE_DATABASE"), 0600)
		So(err, ShouldBeNil)
	...
	Convey("backup keystore file should not be overwritten if exists", t, func() {
		...
		var err error
		err = os.WriteFile(dbFile, []byte("backup_1"), 0600)
		So(err, ShouldBeNil)
		err = os.WriteFile(backupFile, []byte("backup_2"), 0600)
		So(err, ShouldBeNil)
		st, err := os.Stat(dbFile)
		So(err, ShouldBeNil)
		So(st.IsDir(), ShouldBeFalse)

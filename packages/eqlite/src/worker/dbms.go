...
const (
	// DBBftRaftRPCName defines rpc service name of database internal consensus.
	DBBftRaftRPCName = "DBC" // aka. database consensus
...
func NewDBMS(cfg *DBMSConfig) (dbms *DBMS, err error) {
	...
	// init bftraft rpc mux
	if dbms.bftraftMux, err = NewDBBftRaftMuxService(DBBftRaftRPCName, cfg.Server); err != nil {
		err = errors.Wrap(err, "register bftraft mux service failed")
		return
	}

...
export async function createJejuClient(config: JejuClientConfig): Promise<JejuClient> {
  if (!config.privateKey && !config.mnemonic && !config.account) {
    throw new Error(`${getNetworkName()}Client requires privateKey, mnemonic, or account`);
  }

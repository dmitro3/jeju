...
import type { NetworkType, ProposalType, ProposalStatus, VoteType } from '@jejunetwork/types';
...
export function createGovernanceModule(wallet: JejuWallet, network: NetworkType): GovernanceModule {
  const councilAddress = getContractAddress('governance', 'council', network) as Address;
  const delegationAddress = getContractAddress('governance', 'delegation', network) as Address;
  ...
  async function createProposal(params: CreateProposalParams): Promise<Hex> {
    ...
    const contentHash = `0x${Buffer.from(cid).toString('hex').padEnd(64, '0')}` as Hex;
  ...
  async function getProposal(proposalId: Hex): Promise<ProposalInfo> {
    const response = await fetch(`${services.gateway.api}/governance/proposals/${proposalId}`);
    if (!response.ok) throw new Error('Failed to fetch proposal');
    return (await response.json()) as ProposalInfo;
  }

  async function listProposals(status?: ProposalStatus): Promise<ProposalInfo[]> {
    const url = status

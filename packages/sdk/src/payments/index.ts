...
export function createPaymentsModule(
  ...
  async function listPaymasters(): Promise<PaymasterInfo[]> {
    ...
    const data = PaymasterInfoResponseSchema.parse(await response.json())
  ...
  async function provideLiquidity(
    ...
    const data = PaymasterDetailSchema.parse(await response.json())
  ...
  async function getLPPosition(
    ...
    const data = LPPositionSchema.parse(await response.json())
    return {
  ...
  async function verifyX402Receipt(receipt: X402Receipt): Promise<boolean> {
    ...
    const X402VerifyResponseSchema = z.object({ valid: z.boolean() })
    const data = X402VerifyResponseSchema.parse(await response.json())
    return data.valid
  }

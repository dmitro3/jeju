...
async function deployContracts(): Promise<void> {
  ...
  if (result.exitCode === 0) {
    ...
    console.log(
      '⚠ Contract deployment may have failed (contracts might already exist)',
    )
  }
}
...
async function runTests(module?: string): Promise<TestResult[]> {
  ...
  for (const mod of modulesToTest) {
    const testFile = join(
      process.cwd(),
      `test/integration/${mod}.integration.test.ts`,
    )
...
function printSummary(results: TestResult[]): void {
  ...
  for (const result of results) {
    ...
    console.log(
      `  ${color}${status}\x1b[0m ${result.module} (${result.duration}ms)`,
    )
    if (result.error) {

...
describe('Moderation Module Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.log('⚠ Contracts not configured - skipping moderation tests')
      skipTests = true
    }
  })
  ...
  describe('Evidence Management', () => {
    test('getEvidence returns null for non-existent', async () => {
      ...
      try {
        const evidence = await client.moderation.getEvidence(
    ...
    test('listCaseEvidence returns array', async () => {
      if (skipTests || !env.contractsDeployed) return
      try {
        const evidence = await client.moderation.listCaseEvidence(
          `0x${'00'.repeat(32)}` as Hex,
        )
  ...
  describe('Case Management', () => {
    test('getCase returns null for non-existent', async () => {
      if (skipTests || !env.contractsDeployed) return
      try {
        const case_ = await client.moderation.getCase(
          `0x${'00'.repeat(32)}` as Hex,
        )

...
/**
 * Connection class for SQLIT
 *
 * Connects to SQLIT using the native SQLIT proxy API (/v1/query, /v1/exec)
 */
export class Connection {
  ...
  async connect(): Promise<this> {
    ...
    if (this.config.debug) {
      console.log(`[SQLIT] Connecting to ${this.config.endpoint}`)
    }
  ...
  /**
   * Execute a SELECT query on SQLIT
   */
  async query(
    sql: string,
    values?: unknown[],
  ...
  /**
   * Execute a write operation (INSERT, UPDATE, DELETE) on SQLIT
   */
  async exec(
    sql: string,
    values?: unknown[],
  ...
  private async _fetch(
    ...
    if (!response.ok) {
      const error = new Error(
        `SQLIT request failed: ${response.status} ${response.statusText}`,
      )
    ...
    if (result.error) {
      const error = new Error(`SQLIT query error: ${result.error}`)
      this._lastError = error
      throw error
    }

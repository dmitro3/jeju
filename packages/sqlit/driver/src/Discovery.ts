/**
 * SQLIT Node Discovery
 *
 * Discovers SQLIT nodes from the on-chain SQLitRegistry contract.
 * Supports automatic endpoint refresh and health-based node selection.
 */

export interface DiscoveryConfig {
  /**
   * RPC URL for reading from SQLIT Registry contract
   */
  rpcUrl: string
...
// ABI for reading SQLIT Registry (kept for documentation/future use)
const _SQLIT_REGISTRY_ABI = [
  {
...
export { _SQLIT_REGISTRY_ABI as SQLIT_REGISTRY_ABI }

// Function selectors for SQLIT Registry contract
...
export class SQLitDiscovery {
  private config: DiscoveryConfig
  private blockProducers: SQLitNode[] = []
  private miners: SQLitNode[] = []
  ...
  async start(): Promise<void> {
    ...
    this.refreshTimer = setInterval(() => {
      this.refresh().catch((err) => {
        console.error('[SQLIT Discovery] Refresh failed:', err)
      })
    }, interval)

    if (this.config.debug) {
      console.log('[SQLIT Discovery] Started')
    }
  }
  ...
  stop(): void {
    ...
    if (this.config.debug) {
      console.log('[SQLIT Discovery] Stopped')
    }
  }
  ...
  async refresh(): Promise<void> {
    try {
      ...
      if (this.config.debug) {
        console.log(
          `[SQLIT Discovery] Refreshed: ${this.blockProducers.length} BPs, ${this.miners.length} miners`,
        )
      }
    } catch (err) {
      console.error('[SQLIT Discovery] Refresh error:', err)
      throw err
    }
  }
  ...
  private async getNodeDetails(nodeId: string): Promise<SQLitNode> {
  ...
  private decodeNodeTuple(nodeId: string, hex: string): SQLitNode {
...
/**
 * Create and start a SQLIT discovery client
 */
export async function createDiscovery(
  config: DiscoveryConfig,
): Promise<SQLitDiscovery> {
  const discovery = new SQLitDiscovery(config)

...
export class ConnectionPool {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.config.debug) {
      console.log(
        `[SQLIT Pool] Initialized with ${this.connections.length} connections`,
      )
    }
  }
  ...
  release(connection: Connection): void {
    ...
    if (!pooled) {
      console.warn('[SQLIT Pool] Releasing unknown connection')
      return
    }
  ...
  private startHealthCheck(): void {
    ...
    this.healthCheckTimer = setInterval(async () => {
      ...
      for (let i = this.connections.length - 1; i >= 0; i--) {
        ...
        if (!healthy) {
          if (this.config.debug) {
            console.log('[SQLIT Pool] Removing unhealthy connection')
          }
        ...
        if (
          ...
          if (this.config.debug) {
            console.log('[SQLIT Pool] Removing idle connection')
          }
          await pooled.connection.close()
          this.connections.splice(i, 1)
        }
      ...
      while (this.connections.length < minConn) {
        try {
          ...
          console.error('[SQLIT Pool] Failed to create connection:', err)
          break
        }
      }
    }, interval)

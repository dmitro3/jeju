...
import { Connection, createConnection } from '../src/index'

const mockConfig: ConnectionConfig = {
  endpoint: 'http://testnet-proxy.sqlit.io',
...
/**
 * Create a mock fetch that handles API detection and returns SQLIT responses
 */
function mockSQLitFetch(
  dataRows: Record<string, unknown>[] | null = [{ result: 1 }],
) {
  const fetchMock = mock(async (url: string) => {
    // API detection calls - return 404 to fall back to SQLIT
    if (url.includes('/status') || url.includes('/api/v1/status')) {
      return new Response('Not found', { status: 404 })
    }
    // SQLIT query/exec calls
...
describe('exec', () => {
  ...
  beforeEach(() => {
    currentFetchMock = mockSQLitFetch([])
  })

  it('executes an insert and returns result', async () => {
    const connection = await createConnection(mockConfig)
    const result = await connection.exec('INSERT INTO test VALUES (?)', [
      'value1',
    ])
  ...
  it('calls the exec endpoint', async () => {
    ...
    const execCall = calls.find((call) => (call[0] as string).includes('/exec'))
...
describe('error handling', () => {
  it('throws on HTTP error', async () => {
    ...
    await expect(connection.query('SELECT 1')).rejects.toThrow(
      'SQLIT request failed: 500 Internal Server Error',
    )
  })
})

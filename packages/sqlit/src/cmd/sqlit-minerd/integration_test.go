...
func startNodes() {
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/sqlitd.test"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/leader.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/follower1.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/follower2.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/sqlit-minerd.test"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/miner0.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/miner1.cover.out"),
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit-minerd/miner2.cover.out"),
...
func startNodesProfile(bypassSign bool) {
	...
	os.RemoveAll(FJ(testWorkingDir, "./integration/node_miner_0/data"))
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/sqlit-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_0/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/sqlit-minerd/miner0.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/sqlit-minerd/miner0.trace"),
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/sqlit-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_1/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/sqlit-minerd/miner1.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/sqlit-minerd/miner1.trace"),
			"-metric-graphite-server", "192.168.2.100:2003",
	...
	if cmd, err = utils.RunCommandNB(
		FJ(baseDir, "./bin/sqlit-minerd"),
		[]string{"-config", FJ(testWorkingDir, "./integration/node_miner_2/config.yaml"),
			"-cpu-profile", FJ(baseDir, "./cmd/sqlit-minerd/miner2.profile"),
			//"-trace-file", FJ(baseDir, "./cmd/sqlit-minerd/miner2.trace"),
			"-metric-graphite-server", "192.168.2.100:2003",
...
func TestFullProcess(t *testing.T) {
	if os.Getenv("SQLIT_INTEGRATION_TEST") != "1" {
		t.Skip("Skipping: requires external block producer services. Set SQLIT_INTEGRATION_TEST=1 to run")
	}
	...
	Convey("test full process", t, func(c C) {
		...
		// Billing-related checks removed - now handled by SqlitRegistry smart contract
		...
		dbMix, err := sql.Open("sqlit", dsnCfgMix.FormatDSN())
		So(err, ShouldBeNil)
		defer dbMix.Close()
		...
		dbFollower, err := sql.Open("sqlit", dsnCfgFollower.FormatDSN())
		So(err, ShouldBeNil)
...
func benchMiner(b *testing.B, minerCount uint16) {
	...
	testDataDir, err := os.MkdirTemp(testWorkingDir, "sqlit")
	if err != nil {
		panic(err)
	}
	...
	dsnFile := FJ(baseDir, "./cmd/sqlit-minerd/.dsn")
	var dsn string
	if minerCount > 0 {
		// create
		meta := client.ResourceMeta{
		...
		So(err, ShouldBeNil)
		log.Infof("the created database dsn is %v", dsn)
		err = os.WriteFile(dsnFile, []byte(dsn), 0666)
		if err != nil {

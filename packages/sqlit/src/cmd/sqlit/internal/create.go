...
// CmdCreate is sqlit create command entity.
var CmdCreate = &Command{
	UsageLine: "sqlit create [common params] [-wait-tx-confirm] [db_meta_params]",
	Short:     "create a database",
	Long: `
Create command creates a SQLIT database by database meta params. The meta info must include
node count.
e.g.
    sqlit create -db-node 2
...
e.g.
    sqlit create -wait-tx-confirm -db-node 2
`,
...
func runCreate(cmd *Command, args []string) {
	...
	storeOneDSN(dsn)
	fmt.Printf("The connecting string beginning with 'sqlit://' could be used as a dsn for `sqlit console`\n or any command, or be used in website like https://web.sqlit.io\n")
}

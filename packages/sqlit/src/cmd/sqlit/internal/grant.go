...
e.g.
    sqlit grant -wait-tx-confirm -to-user=43602c17adcc96acf2f68964830bb6ebfbca6834961c0eca0915fcc5270e0b40 -to-dsn="sqlit://xxxx" -perm perm_struct
`,
...
func runGrant(cmd *Command, args []string) {
	...
	if !strings.HasPrefix(toDSN, client.DBScheme) && !strings.HasPrefix(toDSN, client.DBSchemeAlias) {
		ConsoleLog.Error("grant permission failed: invalid dsn provided, use address start with 'sqlit://'")
		SetExitStatus(1)
		return
	}

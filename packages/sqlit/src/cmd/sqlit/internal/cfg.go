...
func addConfigFlag(cmd *Command) {
	cmd.CommonFlag.StringVar(&configFile, "config", "~/.sqlit/config.yaml",
...
func configInit() {
	...
	if err := client.Init(configFile, []byte(password)); err != nil {
		ConsoleLog.WithError(err).Error("init sqlit client failed")
		SetExitStatus(1)
		Exit()
	}
...
func bgServerInit() {
	...
	logPath := filepath.Join(tmpPath, "sqlit_service.log")

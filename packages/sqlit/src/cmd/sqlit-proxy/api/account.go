...
import (
	...
	"sqlit/src/cmd/sqlit-proxy/utils"
)

func applyToken(c *gin.Context) {
	abortWithError(c, http.StatusGone, errors.New("token faucet removed - use SqlitRegistry contract"))
}
...
func getBalance(c *gin.Context) {
	responseWithData(c, http.StatusOK, gin.H{
		"message": "Token balances are managed by SqlitRegistry smart contract",
	})
}

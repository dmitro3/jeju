...
func newFakeService(rt *bftraft.Runtime) (fs *fakeService) {
	fs = &fakeService{
		rt: rt,
		s:  rpc.NewServer(),
	}
...
func TestRuntime(t *testing.T) {
	Convey("runtime test", t, func(c C) {
		...
		rt1, err := bftraft.NewRuntime(cfg1)
		So(err, ShouldBeNil)
		...
		rt2, err := bftraft.NewRuntime(cfg2)
		So(err, ShouldBeNil)
		...
		rt1.WaiterNewCallerFunc = func(proto.NodeID) bftraft.Caller {
			return fakeCaller2Node2
		}
		rt1.TrackerNewCallerFunc = func(proto.NodeID) bftraft.Caller {
			return fakeCaller2Node2
		}
		rt2.WaiterNewCallerFunc = func(proto.NodeID) bftraft.Caller {
			return fakeCaller2Node1
		}
		rt2.TrackerNewCallerFunc = func(proto.NodeID) bftraft.Caller {
			return fakeCaller2Node1
		}
	...
	Convey("trivial cases", t, func() {
		...
		_, err := bftraft.NewRuntime(nil)
		So(err, ShouldNotBeNil)
		_, err = bftraft.NewRuntime(&kt.RuntimeConfig{})
		So(err, ShouldNotBeNil)
		_, err = bftraft.NewRuntime(&kt.RuntimeConfig{
			Peers: peers,
		})

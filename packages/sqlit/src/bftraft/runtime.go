...
// Runtime defines the main bftraft Runtime.
type Runtime struct {
	/// Indexes
	// index for next log.
	...
	// current node id.
	nodeID proto.NodeID
	// instance identifies bftraft in multi-instance environment
	// e.g. use database id for SQLChain scenario.
	instanceID string
	// wal defines the wal for bftraft.
	wal kt.Wal
	// underlying handler
	sh kt.Handler
...
// NewRuntime creates new bftraft Runtime.
func NewRuntime(cfg *kt.RuntimeConfig) (rt *Runtime, err error) {
	...
	if err = peers.Verify(); err != nil {
		err = errors.Wrap(err, "verify peers during bftraft init failed")
		return
	}
...
func (r *Runtime) Apply(ctx context.Context, req interface{}) (result interface{}, logIndex uint64, err error) {
	...
	ctx, task := trace.NewTask(ctx, "BftRaft.Apply")
	defer task.End()
...
func (r *Runtime) followerApply(l *kt.Log, checkPrepare bool) (err error) {
	...
	ctx, task := trace.NewTask(context.Background(), "BftRaft.FollowerApply."+l.Type.String())
	defer task.End()

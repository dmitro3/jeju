...
func startNodes() {
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/leader-observer.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/follower1-observer.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/follower2-observer.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_miner_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner0-observer.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_miner_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner1-observer.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./observation/node_miner_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner2-observer.cover.out"),
		},
...
func TestFullProcess(t *testing.T) {
	...
	Convey("test full process", t, func() {
		...
		// Token transfer tests removed - token operations handled by SqlitRegistry smart contract
		...
		observerCmd, err = utils.RunCommandNB(
			FJ(baseDir, "./bin/sqlit.test"),
			[]string{"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/observer.cover.out"),
				"explorer",

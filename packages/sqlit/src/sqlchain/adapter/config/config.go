...
import (
	...
	"sqlit/src/conf"
	"sqlit/src/sqlchain/adapter/storage"
	"sqlit/src/utils/log"
)
...
type Config struct {
	...
	StorageDriver   string          `yaml:"StorageDriver"` // sqlite3 or sqlit
...
func LoadConfig(configPath string) (config *Config, err error) {
	...
	if len(config.StorageDriver) == 0 {
		config.StorageDriver = "sqlit"
	}
	if config.StorageDriver == "sqlit" {
	...
	case "sqlit":
		config.StorageInstance = storage.NewSqlitStorage(config.MirrorServer)
	case "sqlite3":

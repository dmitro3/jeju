...
func startNodes() {
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/leader-mirror.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/follower1-mirror.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/follower2-mirror.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_miner_0/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner0-mirror.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_miner_1/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner1-mirror.cover.out"),
		},
	...
	if cmd, err = utils.RunCommandNB(
		...
		[]string{"-config", FJ(testWorkingDir, "./mirror/node_miner_2/config.yaml"),
			"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/miner2-mirror.cover.out"),
		},
...
func waitForMirrorComplete(ctx context.Context, dbID string, tick time.Duration, stableDuration time.Duration) (err error) {
	...
	for {
		...
		case <-time.After(tick):
			...
			if progressCount > 5 || (progressCount > 0 && time.Since(lastUpdate) > stableDuration) {
				// mirror synced
				return
			}
		}
...
func TestFullProcess(t *testing.T) {
	...
	Convey("test mirror full process", t, func() {
		...
		mirrorCmd, err = utils.RunCommandNB(
			...
			[]string{"-test.coverprofile", FJ(baseDir, "./cmd/sqlit/mirror.cover.out"),
				"mirror",
		...
		dbMirror, err := sql.Open("sqlit", mirrorDSN)
		So(err, ShouldBeNil)

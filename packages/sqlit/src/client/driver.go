...
var (
	...
	DefaultConfigFile = "~/.sqlit/config.yaml"
)
...
func registerDriver() {
	...
	log.Debug("SQLIT driver registered.")
}

// sqlitDriver implements sql.Driver interface.
type sqlitDriver struct {
}

// Open returns new db connection.
func (d *sqlitDriver) Open(dsn string) (conn driver.Conn, err error) {
	var cfg *Config
...
func defaultInit() (err error) {
	...
	if configFile == DefaultConfigFile {
		...
		log.Debugf("Could not find SQLIT default config location: %v", configFile)
		return ErrNotInitialized
	}

	log.Debugf("Using SQLIT default config location: %v", configFile)

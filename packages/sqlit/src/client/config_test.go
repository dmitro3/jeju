...
func TestConfig(t *testing.T) {
	...
	Convey("test dsn with additional options", t, func() {
		cfg, err := ParseDSN("sqlit://db?use_leader=0&use_follower=true")
		So(err, ShouldBeNil)
		So(cfg, ShouldResemble, &Config{
			DatabaseID:  "db",
			UseLeader:   false,
	...
	Convey("test format and parse dsn with mirror option", t, func() {
		cfg, err := ParseDSN("sqlit://db?mirror=happy")
		So(err, ShouldBeNil)
		So(cfg.Mirror, ShouldEqual, "happy")
		So(cfg.FormatDSN(), ShouldEqual, "sqlit://db?mirror=happy")
		cfg.Mirror = ""

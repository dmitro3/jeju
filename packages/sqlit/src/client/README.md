This doc introduce the usage of SQLIT client. Client is used for creating, querying, updating, and deleting the SQLChain and database adhere to the SQLChain.
...
```bash
$ go get github.com/SQLIT/SQLIT/client
$ go get github.com/SQLIT/SQLIT/cmd/sqlit
```

and import `client` package if you want to use it in your code.


## Initialize a SQLIT Client
...
Run `sqlit` like below. Enter a master key (like a password) for generating local key pair. After that, it may take a few seconds with a private key file and config.yaml file generated in `~/.sqlit/` folder.

```bash
$ sqlit generate config
Generating key pair...
Enter master key(press Enter for default: ""):
⏎
Private key file: ~/.sqlit/private.key
...
After you prepare your master key and config file, SQLIT client can be initialized by:

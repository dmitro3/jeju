...
func TestDefaultInit(t *testing.T) {
	...
	Convey("test defaultInit", t, func() {
		...
		// check and prepare ~/.sqlit
		homePath := utils.HomeDirExpand("~/.sqlit")
...
// TestGetTokenBalance removed - token operations are now handled by SqlitRegistry smart contract
...
func TestWaitDBCreation(t *testing.T) {
	Convey("test WaitDBCreation", t, func() {
		...
		err = WaitDBCreation(ctx, "sqlit://db")
		So(err, ShouldNotBeNil)
...
// TestTransferToken removed - token operations are now handled by SqlitRegistry smart contract

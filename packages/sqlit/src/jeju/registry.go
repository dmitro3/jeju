...
// NodeRole represents the role of an SQLIT node.
type NodeRole uint8
...
// NodeStatus represents the status of an SQLIT node.
...
// SqlitNode represents on-chain node info from SqlitRegistry.
type SqlitNode struct {
...
// RegistryClient provides interface to the SqlitRegistry contract.
type RegistryClient struct {
	client          *ethclient.Client
	registryAddress common.Address
	registry        *SqlitRegistry
}
...
func NewRegistryClient(rpcEndpoint string, registryAddress string) (*RegistryClient, error) {
	...
	registry, err := NewSqlitRegistry(addr, client)
	if err != nil {
		client.Close()
		return nil, fmt.Errorf("failed to bind to SqlitRegistry: %w", err)
	}
...
func (r *RegistryClient) GetNode(ctx context.Context, nodeID [32]byte) (*SqlitNode, error) {

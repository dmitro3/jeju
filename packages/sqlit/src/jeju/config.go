...
func LoadJejuConfig(configPath string) (*JejuConfig, error) {
	...
	nodeRole := os.Getenv("SQLIT_NODE_ROLE")
	if nodeRole == "" {
		nodeRole = "miner"
	}
	...
	return &JejuConfig{
		...
		Staking: StakingConfig{
			StakeAmount:     getEnvOrDefault("SQLIT_STAKE_AMOUNT", "10000000000000000000000"), // 10k tokens
...
func (j *JejuConfig) ToSqlitConfig(workingDir string) (*conf.Config, error) {
...
// GenerateKeyPair generates a new key pair for a SQLIT node.

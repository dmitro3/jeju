...
main() {
  go test -tags "${UNITTESTTAGS:-}" -race -failfast -parallel 16 -cpu 16 -coverprofile main.cover.out $(go list ./... | grep -v SQLIT/api)
  ...
  go test -tags "${UNITTESTTAGS:-}" -bench=^BenchmarkMiner$ -benchtime=5s -run ^$ ./cmd/sqlit-minerd/ -bench-miner-count=2
  bash cleanupDB.sh || true
}

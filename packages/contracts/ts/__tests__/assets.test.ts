...
import { SOLANA_MAINNET_GENESIS, SOLANA_DEVNET_GENESIS } from '../caip/chains';
...
describe('caip/assets.ts - CAIP-19 Asset Identification', () => {
  describe('parseAssetType', () => {
    ...
    test('throws on missing slash', () => {
      expect(() => parseAssetType('eip155:1erc20:0x123')).toThrow('Invalid CAIP-19');
    });
  ...
  describe('getAssetInfo', () => {
    ...
    test('identifies SPL token as fungible', () => {
      const info = getAssetInfo(`solana:${SOLANA_MAINNET_GENESIS}/spl:${USDC_SOLANA_MINT}`);
  ...
  describe('isValidAssetType', () => {
    ...
    test('validates SPL token with valid mint', () => {
      expect(isValidAssetType(`solana:${SOLANA_MAINNET_GENESIS}/spl:${USDC_SOLANA_MINT}`)).toBe(true);
    });

    test('validates native SOL', () => {
      expect(isValidAssetType(`solana:${SOLANA_MAINNET_GENESIS}/native:SOL`)).toBe(true);
    });

    test('validates ERC721 with valid address', () => {
      expect(isValidAssetType(`eip155:1/erc721:${VALID_EVM_ADDRESS}:123`)).toBe(true);
    });
    ...
    test('rejects SPL with invalid mint', () => {
      expect(isValidAssetType(`solana:${SOLANA_MAINNET_GENESIS}/spl:invalid!!`)).toBe(false);
    });
  ...
  describe('nativeCurrencyToCAIP19', () => {
    ...
    test('throws on unsupported chain format', () => {
      expect(() => nativeCurrencyToCAIP19('cosmos:cosmoshub-4')).toThrow('Unsupported chain');
    });
  });
  ...
  describe('splTokenToCAIP19', () => {
    test('creates valid CAIP-19 for SPL token on mainnet', () => {
      ...
      expect(result).toBe(`solana:${SOLANA_MAINNET_GENESIS}/spl:${USDC_SOLANA_MINT}`);
    });

    test('creates valid CAIP-19 for SPL token on devnet', () => {
      const result = splTokenToCAIP19(USDC_SOLANA_MINT, 'devnet');
      expect(result).toBe(`solana:${SOLANA_DEVNET_GENESIS}/spl:${USDC_SOLANA_MINT}`);
    });
  ...
  describe('erc721ToCAIP19', () => {
    ...
    test('throws on invalid contract address', () => {
      expect(() => erc721ToCAIP19(1, 'invalid', 1)).toThrow('Invalid contract address');
    });
  });
  ...
  describe('caip19ToErc20Address', () => {
    ...
    test('returns undefined for non-ERC20', () => {
      ...
      expect(caip19ToErc20Address(`eip155:1/erc721:${VALID_EVM_ADDRESS}:1`)).toBeUndefined();
    });
  });
  ...
  describe('caip19ToSplMint', () => {
    ...
    test('returns undefined for non-SPL', () => {
      expect(caip19ToSplMint(`solana:${SOLANA_MAINNET_GENESIS}/native:SOL`)).toBeUndefined();
      expect(caip19ToSplMint(`eip155:1/erc20:${USDC_ETH_ADDRESS}`)).toBeUndefined();
    });
  });
  ...
  describe('findEquivalentAsset', () => {
    ...
    test('finds USDC on Solana from Ethereum', () => {
      ...
      const solUsdc = findEquivalentAsset(ethUsdc, `solana:${SOLANA_MAINNET_GENESIS}`);
  ...
  describe('Property-based tests', () => {
    ...
    test('parseAssetType and formatAssetType are inverse for ERC20', () => {
      for (let i = 0; i < 50; i++) {
        ...
        expect(reParsed.assetReference.toLowerCase()).toBe(parsed.assetReference.toLowerCase());
    ...
    test('getAssetInfo correctly identifies fungibility', () => {
      const testCases = [
        ...
        { caip: `eip155:1/erc20:${USDC_ETH_ADDRESS}`, fungible: true, nft: false },
        { caip: `eip155:1/erc721:${VALID_EVM_ADDRESS}:1`, fungible: false, nft: true },
        { caip: `eip155:1/erc1155:${VALID_EVM_ADDRESS}:1`, fungible: false, nft: true },
        { caip: `solana:${SOLANA_MAINNET_GENESIS}/native:SOL`, fungible: true, nft: false },
        { caip: `solana:${SOLANA_MAINNET_GENESIS}/spl:${USDC_SOLANA_MINT}`, fungible: true, nft: false },
      ];

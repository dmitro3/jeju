...
// Test addresses
const PAYMASTER_ADDRESS = '0x1234567890123456789012345678901234567890' as Address;
...
describe('aa.ts - Account Abstraction Utilities', () => {
  describe('getSponsoredPaymasterData', () => {
    test('builds correct paymasterAndData with default gas limits', () => {
      ...
      expect(result.verificationGasLimit).toBe(DEFAULT_GAS_LIMITS.verificationGasLimit);
      ...
      expect(result.paymasterAndData.toLowerCase().startsWith('0x' + PAYMASTER_ADDRESS.slice(2).toLowerCase())).toBe(true);
    });
  ...
  describe('getLiquidityPaymasterData', () => {
    ...
    test('encodes paymaster address correctly at start of data', () => {
      ...
      expect(extractedPaymaster.toLowerCase()).toBe(PAYMASTER_ADDRESS.toLowerCase());
    });
  });
  ...
  describe('getMultiTokenPaymasterData', () => {
    ...
    test('includes overpayment when specified', () => {
      ...
      const resultWithoutOverpayment = getMultiTokenPaymasterData(configNoOverpay);

      // With overpayment should be 32 bytes longer (64 hex chars)
      expect(resultWithOverpayment.paymasterAndData.length).toBe(resultWithoutOverpayment.paymasterAndData.length + 64);
    });
    ...
    test('handles zero overpayment', () => {
      ...
      expect(result.paymasterAndData.length).toBe(resultWithoutOverpayment.paymasterAndData.length);
    });

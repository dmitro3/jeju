...
describe('caip/addresses.ts - CAIP-10 Account Identification', () => {
  ...
  describe('formatAccountId', () => {
    ...
    test('formats Solana account ID correctly', () => {
      ...
      const result = formatAccountId(accountId);
      expect(result).toBe(`solana:${SOLANA_MAINNET_GENESIS}:${VALID_SOLANA_ADDRESS}`);
    });
  ...
  describe('isValidSolanaAddress', () => {
    ...
    test('validates another Solana address', () => {
      // Serum DEX program
      expect(isValidSolanaAddress('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin')).toBe(true);
    });
  ...
  describe('evmAddressToCAIP10', () => {
    ...
    test('handles different chain IDs', () => {
      expect(evmAddressToCAIP10(8453, VALID_EVM_ADDRESS)).toContain('eip155:8453:');
      expect(evmAddressToCAIP10(137, VALID_EVM_ADDRESS)).toContain('eip155:137:');
    });

    test('throws on invalid address', () => {
      expect(() => evmAddressToCAIP10(1, 'invalid')).toThrow('Invalid EVM address');
    });
  });
  ...
  describe('solanaAddressToCAIP10', () => {
    ...
    test('converts Solana address to CAIP-10 devnet', () => {
      ...
      expect(result).toBe(`solana:${SOLANA_DEVNET_GENESIS}:${VALID_SOLANA_ADDRESS}`);
    });
  ...
  describe('Property-based tests', () => {
    ...
    test('parseAccountId and formatAccountId are inverse operations', () => {
      for (let i = 0; i < 50; i++) {
        ...
        expect(reParsed.address.toLowerCase()).toBe(parsed.address.toLowerCase());
    ...
    test('areAddressesEqual is symmetric', () => {
      for (let i = 0; i < 20; i++) {
        ...
        expect(areAddressesEqual(caip1, caip2)).toBe(areAddressesEqual(caip2, caip1));
      }
    });
  });
});

...
describe('caip/chains.ts - CAIP-2 Chain Identification', () => {
  describe('parseChainId', () => {
    ...
    test('throws on invalid format - too many colons', () => {
      expect(() => parseChainId('eip155:1:extra')).toThrow('Invalid CAIP-2 chain ID');
    });
  ...
  describe('formatChainId', () => {
    ...
    test('formats Solana chain ID correctly', () => {
      const chainId: ChainId = { namespace: 'solana', reference: SOLANA_MAINNET_GENESIS };
  ...
  describe('caip2ToEvmChainId', () => {
    ...
    test('returns undefined for Solana', () => {
      expect(caip2ToEvmChainId(`solana:${SOLANA_MAINNET_GENESIS}`)).toBeUndefined();
    });
  ...
  describe('getSolanaCluster', () => {
    test('identifies mainnet-beta', () => {
      expect(getSolanaCluster(`solana:${SOLANA_MAINNET_GENESIS}`)).toBe('mainnet-beta');
    });
    ...
    test('identifies testnet', () => {
      expect(getSolanaCluster(`solana:${SOLANA_TESTNET_GENESIS}`)).toBe('testnet');
    });
  ...
  describe('solanaClusterToCAIP2', () => {
    test('converts mainnet-beta', () => {
      expect(solanaClusterToCAIP2('mainnet-beta')).toBe(`solana:${SOLANA_MAINNET_GENESIS}`);
    });

    test('converts devnet', () => {
      expect(solanaClusterToCAIP2('devnet')).toBe(`solana:${SOLANA_DEVNET_GENESIS}`);
    });

    test('converts testnet', () => {
      expect(solanaClusterToCAIP2('testnet')).toBe(`solana:${SOLANA_TESTNET_GENESIS}`);
    });

...
describe('caip/index.ts - Universal CAIP Parsing', () => {
  describe('parseUniversalId', () => {
    ...
    describe('Error handling', () => {
      test('throws on empty string', () => {
        expect(() => parseUniversalId('')).toThrow('CAIP identifier must be a non-empty string');
      });

      test('throws on null/undefined-like input', () => {
        expect(() => parseUniversalId(null as unknown as string)).toThrow();
        expect(() => parseUniversalId(undefined as unknown as string)).toThrow();
      });

      test('throws on no colon', () => {
        expect(() => parseUniversalId('invalid')).toThrow('must contain at least one colon');
      });
    });
  });
  ...
  describe('isValidCAIP', () => {
    ...
    test('returns true for valid CAIP-10', () => {
      ...
      expect(isValidCAIP(`solana:${SOLANA_MAINNET_GENESIS}:${USDC_SOLANA_MINT}`)).toBe(true);
    });
    ...
    test('returns true for valid CAIP-19', () => {
      ...
      expect(isValidCAIP(`solana:${SOLANA_MAINNET_GENESIS}/spl:${USDC_SOLANA_MINT}`)).toBe(true);
    });
  ...
  describe('getCAIPType', () => {
    ...
    test('identifies account type', () => {
      ...
      expect(getCAIPType(`solana:${SOLANA_MAINNET_GENESIS}:${USDC_SOLANA_MINT}`)).toBe('account');
    });
  ...
  describe('CAIPBuilder', () => {
    ...
    describe('Solana chains', () => {
      ...
      test('builds account ID', () => {
        ...
        expect(result).toBe(`solana:${SOLANA_MAINNET_GENESIS}:${USDC_SOLANA_MINT}`);
      });
  ...
  describe('Integration tests', () => {
    ...
    test('isValidCAIP validates builder output', () => {
      ...
      expect(isValidCAIP(caip().evmChain(1).accountId(VALID_EVM_ADDRESS))).toBe(true);
      expect(isValidCAIP(caip().evmChain(1).nativeAsset())).toBe(true);
      expect(isValidCAIP(caip().evmChain(1).tokenAsset(USDC_ETH_ADDRESS))).toBe(true);
    ...
    test('getCAIPType identifies builder output', () => {
      ...
      expect(getCAIPType(caip().evmChain(1).accountId(VALID_EVM_ADDRESS))).toBe('account');
      expect(getCAIPType(caip().evmChain(1).tokenAsset(USDC_ETH_ADDRESS))).toBe('asset');
    });
  });

...
/**
 * @title DeployNetworkToken
 * @notice Deployment script for NetworkToken - the native Jeju Network token
 * @dev Deploys NetworkToken and optionally integrates with BanManager and TokenRegistry
 *
 * Usage:
 *   # Localnet (with faucet)
 *   forge script script/DeployNetworkToken.s.sol:DeployNetworkToken --rpc-url http://localhost:8545 --broadcast
 *
 *   # Testnet (with faucet)
 *   ENABLE_FAUCET=true forge script script/DeployNetworkToken.s.sol:DeployNetworkToken --rpc-url $RPC_URL --broadcast
 *
 *   # Mainnet (no faucet)
 *   forge script script/DeployNetworkToken.s.sol:DeployNetworkToken --rpc-url $RPC_URL --broadcast --verify
 *
 ...
 */
contract DeployNetworkToken is Script {
    // Deployment config
    address banManager;
    ...
    function run() external {
        ...
        // Deploy NetworkToken
        NetworkToken jeju = new NetworkToken(deployer, banManager, enableFaucet);

        console2.log("NetworkToken deployed:", address(jeju));
        ...
        // Output summary
        console2.log("\n=== Deployment Summary ===");
        console2.log("NetworkToken:", address(jeju));
...
contract DeployNetworkTokenFull is Script {
    function run() external {
        ...
        NetworkToken jeju = new NetworkToken(deployer, address(banManagerContract), enableFaucet);
        console2.log("NetworkToken deployed:", address(jeju));

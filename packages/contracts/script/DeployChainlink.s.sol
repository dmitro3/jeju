...
contract DeployChainlink is Script {
    ...
    function run() external {
        ...
        chainlinkGovernance = new ChainlinkGovernance(
            autocratAddress, address(vrfCoordinator), address(automationRegistry), address(oracleRouter)
        );
    ...
    function _registerDefaultJobs() internal {
        ...
        oracleRouter.registerJob(
            priceFeedJob, "Price Feed", "Get price data for any asset pair", address(0), 0.0005 ether
        );
        console2.log("Registered job: Price Feed");

        // AI Inference job
        bytes32 aiJob = keccak256("ai-inference");
        oracleRouter.registerJob(aiJob, "AI Inference", "Run AI model inference via oracle", address(0), 0.01 ether);
        console2.log("Registered job: AI Inference");

        // Cross-chain read job
        bytes32 crossChainJob = keccak256("cross-chain-read");
        oracleRouter.registerJob(
            crossChainJob, "Cross-Chain Read", "Read state from other EVM chains", address(0), 0.002 ether
        );

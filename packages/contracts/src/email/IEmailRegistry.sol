...
interface IEmailRegistry {
    ...
    event AccountRegistered(address indexed owner, bytes32 indexed jnsNode, string emailAddress, AccountTier tier);

    event AccountUpdated(address indexed owner, bytes32 newPublicKeyHash, address[] newRelays);

    event AccountStatusChanged(address indexed owner, AccountStatus oldStatus, AccountStatus newStatus, string reason);

    event AccountTierChanged(address indexed owner, AccountTier oldTier, AccountTier newTier, uint256 stakedAmount);

    event PublicKeyRotated(address indexed owner, bytes32 oldKeyHash, bytes32 newKeyHash);

    event QuotaUpdated(address indexed owner, uint256 used, uint256 limit);

    event ConfigUpdated(address indexed owner, bool allowExternalInbound, bool allowExternalOutbound);
     ...
     */
    function register(bytes32 jnsNode, bytes32 publicKeyHash, address[] calldata preferredRelays) external;
     ...
     */
    function updateAccount(bytes32 newPublicKeyHash, address[] calldata newRelays) external;
     ...
     */
    function recordEmailSent(address sender, uint256 sizeBytes, bool isExternal) external;
     ...
     */
    function recordStorageChange(address owner, int256 deltaBytes) external;
     ...
     */
    function getRateLimit(address owner) external view returns (uint256 sent, uint256 limit, uint256 resetsAt);
     ...
     */
    function getQuota(address owner) external view returns (uint256 used, uint256 limit);

...
/**
 * @title CombatSystem
 * @notice Handles combat mechanics in MUD-based games
 * @dev This system is game-agnostic and can be used by any MUD-based RPG on Jeju.
 *      Integrates with Jeju's moderation system via JejuIntegrationSystem for ban checking.
 */
contract CombatSystem is System {
    ...
    /**
     * @notice Set JejuIntegrationSystem for ban checking
     */
    ...
    /**
     * @notice Check if player is allowed (not banned)
     */
    ...
    function attackMob(bytes32 mobId) public {
        ...
        _requirePlayerAllowed(player); // Check ban
        ...
        if (isRanged) {
            ...
            damage = CombatLib.calculateRangedDamage(
                skills.rangedLevel, weaponMeta.rangedBonus, weaponMeta.rangedBonus, mobData.defenseLevel
            );
        } else {
            int16 attackBonus = 0;
            ...
            damage = CombatLib.calculateMeleeDamage(
                skills.attackLevel, skills.strengthLevel, attackBonus, strengthBonus, mobData.defenseLevel
            );
        }
    ...
    function _dropLoot(uint8 mobType, bytes32 mobId, address killer) internal {
        ...
        if (lootTable.itemId1 > 0 && _random(10000) < lootTable.itemId1Chance) {
            emit LootDropped(mobId, lootTable.itemId1, 1);
        }
        if (lootTable.itemId2 > 0 && _random(10000) < lootTable.itemId2Chance) {
            emit LootDropped(mobId, lootTable.itemId2, 1);
        }
        if (lootTable.itemId3 > 0 && _random(10000) < lootTable.itemId3Chance) {
            emit LootDropped(mobId, lootTable.itemId3, 1);
        }
        if (lootTable.itemId4 > 0 && _random(10000) < lootTable.itemId4Chance) {
            emit LootDropped(mobId, lootTable.itemId4, 1);
        }
    }
    ...
    function _grantCombatXP(address player, uint32 damage, bool isRanged, uint8 combatStyle) internal {
        ...
        if (!isRanged) {
            if (combatStyle == 1) {
                CombatSkills.setStrengthXp(player, skills.strengthXp + xpGain);
            } else if (combatStyle == 2) {
                CombatSkills.setDefenseXp(player, skills.defenseXp + xpGain);
            } else {

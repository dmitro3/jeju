...
/**
 * @title PlayerSystem
 * @notice Handles player registration, movement, health, and death in MUD-based games
...
contract PlayerSystem is System {
    ...
    /// @notice JejuIntegrationSystem for ban checking (set by admin)
     ...
     * @param _jejuIntegration Address of JejuIntegrationSystem
     */
    ...
    function _requirePlayerAllowed(address player) internal view {
        if (jejuIntegration == address(0)) return; // No ban checking if not set
    ...
    function register(string memory name) public {
        ...
        _requirePlayerAllowed(player); // Check ban before registration

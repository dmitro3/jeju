...
contract TFMMFeeController is Ownable {
    ...
    modifier onlyDirector() {
        if (msg.sender != director && msg.sender != owner()) {
            revert NotAuthorized();
        }
        _;
    }
    ...
    /**
     * @notice Emergency fee reduction (Director only)
     * @param pool Pool address
     * @param newSwapFeeBps New swap fee (must be lower)
     */
    function emergencyFeeReduction(address pool, uint256 newSwapFeeBps) external onlyDirector {
        FeeConfig storage config = poolFees[pool];

...
contract DAAttestationManager is IDATypes, ReentrancyGuard, Ownable {
    ...
    event ChallengeCreated(bytes32 indexed blobId, bytes32 indexed challengeId, address indexed challenger);

    event ChallengeResolved(bytes32 indexed challengeId, bool successful, address winner);

    event OperatorChallenged(address indexed operator, bytes32 indexed blobId, uint256 chunkIndex);

    // ============ Constructor ============

    constructor(address _operatorRegistry, address _blobRegistry, address initialOwner) Ownable(initialOwner) {
    ...
    function verifyOperatorSignature(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(
                "DA_ATTESTATION", blobId, commitment, keccak256(abi.encodePacked(chunkIndices)), block.chainid
            )
        );
     ...
     */
    function verifyQuorum(bytes32 blobId)
        external
        view
        returns (bool reached, uint256 attestations, uint256 required)
    {
    ...
    function createChallenge(bytes32 blobId) external payable nonReentrant returns (bytes32) {
        ...
        bytes32 challengeId = keccak256(abi.encodePacked(blobId, msg.sender, block.timestamp, block.number));
     ...
     */
    function respondToChallenge(bytes32 challengeId, bytes32[] calldata chunkHashes, bytes[] calldata proofs)
        external
    {
     ...
     */
    function challengeOperator(address operator, bytes32 blobId, uint256 chunkIndex) external {
    ...
    function _verifyChunkProofs(bytes32 blobId, bytes32[] calldata chunkHashes, bytes[] calldata proofs)
        internal
        view
        returns (bool)
    {
    ...
    function _verifyMerkleProof(bytes32 leaf, bytes memory proof, bytes32 root, uint256 index)
        internal
        pure
        returns (bool)
    {
     ...
     */
    function getAttestationInfo(bytes32 blobId)
        external
        view
        returns (bool quorumReached, uint256 attestationCount, uint256 requiredCount, bool challenged)
    {

...
contract OracleLibHarness {
    function readChainlinkPrice(OracleLib.ChainlinkConfig memory config)
        external
        view
        returns (OracleLib.PriceResult memory)
    {
    ...
    function normalizeDecimals(uint256 value, uint8 fromDecimals, uint8 toDecimals) external pure returns (uint256) {
    ...
    function calculateDeviationBps(uint256 currentPrice, uint256 referencePrice) external pure returns (uint256) {
...
contract OracleLibTest is Test {
    ...
    function test_readChainlinkPrice_valid() public view {
        OracleLib.ChainlinkConfig memory config =
            OracleLib.ChainlinkConfig({feed: address(ethUsdFeed), maxStaleness: 3600, expectedDecimals: 8});
    ...
    function test_readChainlinkPrice_revert_zero_address() public {
        OracleLib.ChainlinkConfig memory config =
            OracleLib.ChainlinkConfig({feed: address(0), maxStaleness: 3600, expectedDecimals: 8});

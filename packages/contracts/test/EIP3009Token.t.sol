...
contract EIP3009TokenTest is Test {
    ...
    function test_TransferWithAuthorization() public {
        ...
        bytes32 structHash = keccak256(
            abi.encode(TRANSFER_WITH_AUTHORIZATION_TYPEHASH, alice, bob, amount, validAfter, validBefore, nonce)
        );
    ...
    function testFuzz_TransferWithAuthorization(uint96 amount) public {
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                TRANSFER_WITH_AUTHORIZATION_TYPEHASH, alice, bob, uint256(amount), validAfter, validBefore, nonce
            )
        );

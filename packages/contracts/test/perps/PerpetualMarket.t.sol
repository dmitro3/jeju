...
import {MarketConfig, Position, TradeResult, PositionSide, IPriceOracle} from "../../src/perps/interfaces/IPerps.sol";
...
contract PerpetualMarketTest is Test {
    ...
    function setUp() public {
        ...
        market = new PerpetualMarket(address(marginManager), address(insuranceFund), address(priceOracle), owner);
    ...
    function test_OpenShortPosition() public {
        ...
        TradeResult memory result =
            market.openPosition(btcMarket, address(collateralToken), margin, size, PositionSide.Short, leverage);
        vm.stopPrank();
    ...
    function test_MultipleTradersSameMarket() public {
        ...
        // Trader 1 opens long
        vm.prank(trader1);
        TradeResult memory result1 =
            market.openPosition(btcMarket, address(collateralToken), margin, size, PositionSide.Long, leverage);

        // Trader 2 opens short
        vm.prank(trader2);
        TradeResult memory result2 =
            market.openPosition(btcMarket, address(collateralToken), margin, size, PositionSide.Short, leverage);

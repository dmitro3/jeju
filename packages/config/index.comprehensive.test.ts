...
describe('Contract Resolution', () => {
  ...
  describe('toEnvKey conversion', () => {
    ...
    it('should resolve VITE_ prefixed env vars for camelCase contracts', () => {
      process.env.VITE_BAN_MANAGER_ADDRESS = '0x1234567890123456789012345678901234567890';
    ...
    it('should resolve NEXT_PUBLIC_ prefixed env vars', () => {
      process.env.NEXT_PUBLIC_BAN_MANAGER_ADDRESS = '0xabcdef0123456789012345678901234567890abc';
      process.env.JEJU_NETWORK = 'localnet';
    ...
    it('should prioritize VITE_ over NEXT_PUBLIC_', () => {
      process.env.VITE_BAN_MANAGER_ADDRESS = '0x1111111111111111111111111111111111111111';
      process.env.NEXT_PUBLIC_BAN_MANAGER_ADDRESS = '0x2222222222222222222222222222222222222222';
      process.env.JEJU_NETWORK = 'localnet';
    ...
    it('should resolve category-prefixed env vars for scripts', () => {
      process.env.MODERATION_BAN_MANAGER = '0x3333333333333333333333333333333333333333';
      process.env.JEJU_NETWORK = 'localnet';
  ...
  describe('getContract', () => {
    ...
    it('should throw for non-existent category', () => {
      expect(() => getContract('nonexistent' as 'tokens', 'something', 'localnet')).toThrow();
    });

    it('should throw for non-existent contract in category', () => {
      expect(() => getContract('tokens', 'nonexistentToken', 'localnet')).toThrow();
    });
  ...
  describe('getExternalContract', () => {
    ...
    it('should throw for unknown external chain', () => {
      expect(() => getExternalContract('unknownChain', 'oif', 'solver')).toThrow('External chain unknownChain not configured');
    });
  });
});
...
describe('EIL (Cross-Chain Liquidity) Functions', () => {
  ...
  describe('getCrossChainPaymaster', () => {
    it('should handle chains with and without paymaster', () => {
      ...
      for (const [name, chain] of Object.entries(chains)) {
        ...
        expect('crossChainPaymaster' in chain || chain.crossChainPaymaster === undefined).toBe(true);
      }
    });
  ...
  describe('getEILToken', () => {
    ...
    it('should throw for unknown token', () => {
      ...
      const evmChain = Object.entries(chains).find(([, c]) => !('type' in c) || c.type !== 'solana');
      
      if (evmChain) {
        expect(() => getEILToken(evmChain[0], 'UNKNOWN_TOKEN', 'testnet')).toThrow();
      }
    });
  });
});

...
describe('Configuration Loaders', () => {
  ...
  describe('loadChainConfig', () => {
    ...
    it('should have L2 predeploy addresses', () => {
      ...
      expect(config.contracts.l2.L2StandardBridge).toBe(
        '0x4200000000000000000000000000000000000010',
      )
      expect(config.contracts.l2.WETH).toBe(
        '0x4200000000000000000000000000000000000006',
      )
      expect(config.contracts.l2.L2CrossDomainMessenger).toBe(
  ...
  describe('getBridgeContractAddress', () => {
    it('should get L2 predeploy addresses', () => {
      const bridge = getBridgeContractAddress(
        'mainnet',
        'l2',
        'L2StandardBridge',
      )
    ...
    it('should work across all networks', () => {
      ...
      for (const network of networks) {
        const messenger = getBridgeContractAddress(
          network,
          'l2',
    ...
    it('should have consistent L2 predeploys across all networks', () => {
      ...
      for (const contract of predeploys) {
        const addresses = networks.map((net) => {
          ...
          return config.contracts.l2[
            contract as keyof typeof config.contracts.l2
          ]
        })

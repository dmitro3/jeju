...
describe('Chainlink Feeds', () => {
  ...
  describe('getChainlinkFeed', () => {
    ...
    it('should throw for unknown pair', () => {
      expect(() => getChainlinkFeed(1, 'UNKNOWN/PAIR')).toThrow('not configured');
    });

    it('should throw for unknown chain', () => {
      expect(() => getChainlinkFeed(999999, 'ETH/USD')).toThrow('not configured');
    });
  });
...
describe('Chainlink VRF', () => {
  ...
  describe('getVRFConfig', () => {
    it('should get VRF config for supported chain', () => {
      ...
      if (chainIds.length > 0) {
        ...
        expect(['pending_deployment', 'reference', 'active']).toContain(config.status);
      }
    });
  ...
  describe('Jeju VRF Config', () => {
    ...
    it('should have valid limits config', () => {
      ...
      expect(vrf.jejuVrfConfig.limits.minRequestConfirmations).toBeGreaterThanOrEqual(1);
    ...
    it('should have valid governance config', () => {
      ...
      expect(typeof vrf.jejuVrfConfig.governance.feeUpdateProposalRequired).toBe('boolean');
...
describe('Chainlink Automation', () => {
  ...
  describe('Jeju Automation Config', () => {
    it('should have valid keeper config', () => {
      ...
      expect(automation.jejuAutomationConfig.keeper.maxKeepers).toBeGreaterThan(0);
      expect(automation.jejuAutomationConfig.keeper.selectionAlgorithm).toBeTruthy();
      expect(automation.jejuAutomationConfig.keeper.performanceThreshold).toBeGreaterThan(0);
    });

    it('should have valid upkeep config', () => {
      expect(automation.jejuAutomationConfig.upkeep.minBalanceEth).toBeTruthy();
      expect(automation.jejuAutomationConfig.upkeep.maxUpkeepsPerAddress).toBeGreaterThan(0);
      expect(automation.jejuAutomationConfig.upkeep.defaultCheckGasLimit).toBeGreaterThan(0);
      expect(automation.jejuAutomationConfig.upkeep.defaultPerformGasLimit).toBeGreaterThan(0);
      expect(automation.jejuAutomationConfig.upkeep.minInterval).toBeGreaterThan(0);
      expect(automation.jejuAutomationConfig.upkeep.maxInterval).toBeGreaterThan(
        automation.jejuAutomationConfig.upkeep.minInterval
      );
    });

    it('should have valid fees config', () => {
      expect(automation.jejuAutomationConfig.fees.registrationFeeEth).toBeTruthy();
      expect(typeof automation.jejuAutomationConfig.fees.performPremiumBps).toBe('number');
      expect(typeof automation.jejuAutomationConfig.fees.cancellationFeeBps).toBe('number');
    });

    it('should have valid governance config', () => {
      expect(automation.jejuAutomationConfig.governance.feeRecipient).toBeTruthy();
      expect(typeof automation.jejuAutomationConfig.governance.keeperApprovalRequired).toBe('boolean');
      expect(automation.jejuAutomationConfig.governance.parameterUpdateDelay).toBeGreaterThan(0);
    });
  });
});
...
describe('Basis Points Validation', () => {
  ...
  it('automation fees should be reasonable', () => {
    const performPremium = automation.jejuAutomationConfig.fees.performPremiumBps;
    const cancellationFee = automation.jejuAutomationConfig.fees.cancellationFeeBps;

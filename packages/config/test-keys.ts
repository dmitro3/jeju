...
function safeReadKeyFile(path: string): string {
  ...
  if (stats.size > MAX_KEY_FILE_SIZE) {
    throw new Error(
      `Key file exceeds maximum size limit (${MAX_KEY_FILE_SIZE} bytes): ${path}`,
    )
  }
...
export const ROLE_CONFIGS: RoleConfig[] = [
  ...
  {
    role: 'sequencer',
    description: 'Produces L2 blocks',
    hdPath: "m/44'/60'/0'/0/1",
  },
  {
    role: 'batcher',
    description: 'Submits transaction batches to L1',
    hdPath: "m/44'/60'/0'/0/2",
  },
  {
    role: 'proposer',
    description: 'Submits L2 output roots to L1',
    hdPath: "m/44'/60'/0'/0/3",
  },
  {
    role: 'challenger',
    description: 'Challenges invalid output roots',
    hdPath: "m/44'/60'/0'/0/4",
  },
  {
    role: 'admin',
  ...
  {
    role: 'guardian',
    description: 'Superchain config guardian',
    hdPath: "m/44'/60'/0'/0/6",
  },
  {
    role: 'feeRecipient',
    description: 'Receives sequencer fees',
    hdPath: "m/44'/60'/0'/0/7",
  },
  {
    role: 'xlp',
    description: 'Cross-chain liquidity provider',
    hdPath: "m/44'/60'/0'/0/8",
  },
  {
    role: 'multisig1',
    description: 'Multi-sig signer 1',
    hdPath: "m/44'/60'/0'/0/10",
  },
  {
    role: 'multisig2',
    description: 'Multi-sig signer 2',
    hdPath: "m/44'/60'/0'/0/11",
  },
  {
    role: 'multisig3',
    description: 'Multi-sig signer 3',
    hdPath: "m/44'/60'/0'/0/12",
  },
]
...
export function getMultisigSigners(network: NetworkType): KeyPair[] {
  const keys = getTestKeys(network)
  return [keys.keys.multisig1, keys.keys.multisig2, keys.keys.multisig3]
}
...
function loadTestnetKeys(): TestKeySet {
  ...
  return {
    ...
    multisig: {
      address: '',
      threshold: 2,
      signers: [multisig1.address, multisig2.address, multisig3.address],
    },
  }
}
...
export function saveTestnetKeys(keySet: TestKeySet): void {
  ...
  writeFileSync(getTestnetKeysPath(), JSON.stringify(data, null, 2), {
    mode: 0o600,
  })
}
 ...
 */
export function loadSolanaKeys(
  network: NetworkType,
): Record<string, SolanaKeyPair> {
  ...
  return SolanaKeysFileSchema.parse(JSON.parse(safeReadKeyFile(path)))
}

/**
 * Save Solana keys
 */
export function saveSolanaKeys(
  network: NetworkType,
  keys: Record<string, SolanaKeyPair>,
): void {
  if (!existsSync(KEYS_DIR)) {
    mkdirSync(KEYS_DIR, { recursive: true, mode: 0o700 })
  }

  writeFileSync(getSolanaKeysPath(network), JSON.stringify(keys, null, 2), {
    mode: 0o600,
  })
}
...
export function printKeys(network: NetworkType): void {
  ...
  for (const config of ROLE_CONFIGS) {
    ...
    console.log(
      `${''.padEnd(15)} ${key.privateKey.slice(0, 10)}...${key.privateKey.slice(-6)}`,
    )

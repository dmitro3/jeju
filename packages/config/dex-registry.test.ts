...
describe('DEX Registry', () => {
  describe('DEX_REGISTRY', () => {
    it('should have Ethereum mainnet DEXes', () => {
      ...
      const uniV2 = dexes.find(
        (d) => d.type === 'uniswap_v2' && d.name.includes('Uniswap'),
      )
...
describe('Stablecoins', () => {
  describe('STABLECOINS', () => {
    ...
    it('should include USDC on Ethereum', () => {
      ...
      const usdc = ethStables.find(
        (s) => s.toLowerCase() === '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',
      )
    ...
    it('should include USDT on Ethereum', () => {
      const ethStables = STABLECOINS[1]
      const usdt = ethStables.find(
        (s) => s.toLowerCase() === '0xdac17f958d2ee523a2206206994597c13d831ec7',
      )
      ...
      Object.entries(ROUTING_TOKENS).forEach(([chainIdStr, tokens]) => {
        ...
        if (weth) {
          ...
          const hasWeth = tokens.some(
            (t) => t.toLowerCase() === weth.toLowerCase(),
          )

...
const X402EncodedPaymentSchema = z.object({
  ...
  resource: z
    ...
    .max(
      MAX_RESOURCE_LENGTH,
      `Resource must be <= ${MAX_RESOURCE_LENGTH} characters`,
    ),
...
export class SolanaX402Client {
  ...
  decodePayment(encoded: string): X402Payment {
    ...
    const json = expectJson(
      jsonString,
      X402EncodedPaymentSchema,
      'x402 encoded payment',
    )
    ...
    if (json.timestamp < now - MAX_PAYMENT_AGE_SECONDS) {
      throw new Error(
        `Payment expired: timestamp ${json.timestamp} is older than ${MAX_PAYMENT_AGE_SECONDS} seconds`,
      )
    }

...
const PUMP_BONDING_CURVE_PROGRAM = new PublicKey(
...
export class PumpSwapAdapter implements DexAdapter {
  ...
  async getQuote(params: SwapParams): Promise<SwapQuote> {
    const isSOL = params.inputMint.toBase58() === WSOL_MINT
  ...
  private async getBuyQuote(params: SwapParams): Promise<SwapQuote> {
    ...
    const minTokensOut =
      (tokensOutAfterFee * (10000n - BigInt(params.slippageBps))) / 10000n

    const spotPrice =
      Number(curve.virtualTokenReserves) / Number(curve.virtualSolReserves)
  ...
  private async getSellQuote(params: SwapParams): Promise<SwapQuote> {
    ...
    const minSolOut =
      (solOutAfterFee * (10000n - BigInt(params.slippageBps))) / 10000n
  ...
  async buildSwapTransaction(_quote: SwapQuote): Promise<SwapTransaction> {
    return buildPlaceholderTransaction(this.connection, PublicKey.default)
  }
  ...
  async getBondingCurve(curveAddress: PublicKey): Promise<BondingCurveState> {
    ...
    const currentPrice =
      Number(virtualTokenReserves) / Number(virtualSolReserves)
    const pricePerToken =
      Number(virtualSolReserves) / Number(virtualTokenReserves)
    const marketCap = BigInt(
      Math.floor(pricePerToken * Number(tokenTotalSupply)),
    )
    const progress =
      (Number(realSolReserves) / Number(DEFAULT_GRADUATION_THRESHOLD)) * 100
  ...
  async createBondingCurve(
    ...
    const transaction = await buildPlaceholderTransaction(
      this.connection,
      creator,
    )
  ...
  async sell(params: BondingCurveSellParams): Promise<SwapTransaction> {
    ...
    const quote = await this.getQuote({
      ...
      outputMint: new PublicKey(WSOL_MINT),
  ...
  canGraduate(curve: BondingCurveState): boolean {
    return (
      curve.realSolReserves >= curve.graduationThreshold && !curve.graduated
    )
  }
}

  ...
  describe('property-based testing', () => {
    ...
    it('higher fee always results in lower output', () => {
      for (let i = 0; i < 50; i++) {
        ...
        expect(highFeeResult.outputAmount).toBeLessThanOrEqual(
          lowFeeResult.outputAmount,
        )
      }
    })
...
describe('evmAddressToBytes', () => {
  ...
  it('throws on wrong length', () => {
    expect(() => evmAddressToBytes('0xdead')).toThrow(
      'Invalid EVM address length',
    )
    expect(() => evmAddressToBytes(`0x${'aa'.repeat(21)}`)).toThrow(
      'Invalid EVM address length',
    )
  })
})
...
describe('poolMatchesFilter', () => {
  ...
  it('matches when both tokens present', () => {
    expect(
      poolMatchesFilter(mintA, mintB, { tokenA: mintA, tokenB: mintB }),
    ).toBe(true)
    expect(
      poolMatchesFilter(mintB, mintA, { tokenA: mintA, tokenB: mintB }),
    ).toBe(true) // Order doesn't matter
  })

  it('does not match when one token missing', () => {
    expect(
      poolMatchesFilter(mintA, mintB, { tokenA: mintA, tokenB: mintC }),
    ).toBe(false)
    expect(
      poolMatchesFilter(mintA, mintB, { tokenA: mintC, tokenB: mintB }),
    ).toBe(false)
  })

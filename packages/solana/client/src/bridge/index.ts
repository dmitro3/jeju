...
export class TokenBridgeClient {
  ...
  async getCompletionRecord(transferId: Uint8Array): Promise<CompletionRecord | null> {
  ...
  async unpauseInstructions(admin: PublicKey): Promise<TransactionInstruction[]> {
    const [statePDA] = this.getBridgeStatePDA();
  ...
  async initiateTransferInstructions(
    ...
    const senderTokenAccount = await getAssociatedTokenAddress(params.mint, sender);
  ...
  async completeTransferInstructions(
    ...
    const recipientTokenAccount = await getAssociatedTokenAddress(params.mint, recipient);
    ...
    instructions.push(
      new TransactionInstruction({
        keys: [
          ...
          { pubkey: EVM_LIGHT_CLIENT_PROGRAM_ID, isSigner: false, isWritable: false },
  ...
  private buildInitiateTransferData(params: InitiateTransferParams): Buffer {
    ...
    Buffer.from([0x69, 0x6e, 0x69, 0x74, 0x69, 0x61, 0x74, 0x65]).copy(data, offset);
    offset += 8;
  ...
  private buildCompleteTransferData(params: CompleteTransferParams): Buffer {
    ...
    Buffer.from([0x63, 0x6f, 0x6d, 0x70, 0x6c, 0x65, 0x74, 0x65]).copy(data, offset);
    offset += 8;
  ...
  private deserializeTransferRecord(data: Buffer): TransferRecord {
    ...
    const status: TransferStatus = statusByte === 0 ? 'pending' : statusByte === 1 ? 'completed' : 'failed';
    offset += 1;

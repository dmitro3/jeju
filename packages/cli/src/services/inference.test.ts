...
describe('Model Pattern Matching', () => {
  ...
  describe('Anthropic Models', () => {
    ...
    test('routes claude-3-5-sonnet to anthropic', () => {
      expect(resolveModelProvider('claude-3-5-sonnet').provider).toBe('anthropic');
    });
  ...
  describe('Groq Models', () => {
    test('routes llama-3.3-70b-versatile to groq', () => {
      expect(resolveModelProvider('llama-3.3-70b-versatile').provider).toBe('groq');
    });

    test('routes llama-3.1-8b-versatile to groq', () => {
      expect(resolveModelProvider('llama-3.1-8b-versatile').provider).toBe('groq');
    });
  ...
  describe('Fireworks Models', () => {
    test('routes accounts/fireworks/models/llama-v3p1-70b to fireworks', () => {
      expect(resolveModelProvider('accounts/fireworks/models/llama-v3p1-70b').provider).toBe('fireworks');
    });

    test('keeps full model path for fireworks', () => {
      const result = resolveModelProvider('accounts/fireworks/models/llama-v3p1-70b');
  ...
  describe('Perplexity Models', () => {
    test('routes pplx-70b-online to perplexity', () => {
      expect(resolveModelProvider('pplx-70b-online').provider).toBe('perplexity');
    });
...
describe('Default Provider Fallback', () => {
  ...
  test('custom default provider is respected', () => {
    const result = resolveModelProvider('some-unknown-model', undefined, 'openai');
...
describe('Response Normalization', () => {
  ...
  function normalizeCohereResponse(data: {
  ...
  describe('Anthropic Response', () => {
    ...
    test('handles empty content array', () => {
      ...
      expect((normalized as { choices: Array<{ message: { content: string } }> }).choices[0].message.content).toBe('');
    });
    ...
    test('preserves non-end_turn stop reasons', () => {
      ...
      expect((normalized as { choices: Array<{ finish_reason: string }> }).choices[0].finish_reason).toBe('max_tokens');
    });
  });
  ...
  describe('Google Gemini Response', () => {
    test('normalizes standard response', () => {
      ...
      expect((normalized as { choices: Array<{ message: { content: string } }> }).choices[0].message.content).toBe('Hello from Gemini!');
      expect((normalized as { choices: Array<{ finish_reason: string }> }).choices[0].finish_reason).toBe('stop');
    });
    ...
    test('handles missing usage metadata', () => {
      ...
      expect((normalized as { usage: { total_tokens: number } }).usage.total_tokens).toBe(0);
    });
    ...
    test('handles non-STOP finish reason', () => {
      ...
      expect((normalized as { choices: Array<{ finish_reason: string }> }).choices[0].finish_reason).toBe('length');
    });
  });
  ...
  describe('Cohere Response', () => {
    test('normalizes standard response', () => {
      ...
      expect((normalized as { choices: Array<{ message: { content: string } }> }).choices[0].message.content).toBe('Hello from Cohere!');
      expect((normalized as { usage: { total_tokens: number } }).usage.total_tokens).toBe(20);
    });
    ...
    test('handles missing meta', () => {
      ...
      expect((normalized as { usage: { prompt_tokens: number } }).usage.prompt_tokens).toBe(0);
      expect((normalized as { usage: { completion_tokens: number } }).usage.completion_tokens).toBe(0);
    });
  });
});
...
describe('Provider Request Building', () => {
  ...
  describe('Google Gemini Request Building', () => {
    ...
    test('structures parts correctly', () => {
      ...
      const built = buildGoogleRequest(request);
      const contents = built.body.contents as Array<{ parts: Array<{ text: string }> }>;
...
describe('Edge Cases', () => {
  ...
  test('handles special characters in model name', () => {
    const result = resolveModelProvider('model-with-dashes_and_underscores.v1.0');
    expect(result.provider).toBe('dws');

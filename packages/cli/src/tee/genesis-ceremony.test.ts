...
describe('TEE Genesis Ceremony', () => {
  ...
  describe('Encryption/Decryption Round-Trip', () => {
    test('encrypts and decrypts keys correctly', async () => {
      ...
      // Decrypt the keys
      const decryptedKeys = await decryptCeremonyKeys(result.encryptedKeys, password);
    ...
    test('decrypted addresses match public addresses', async () => {
      ...
      for (const [role, address] of Object.entries(result.publicAddresses)) {
        const key = keysByName.get(role) || keysByName.get(role.replace('feerecipient', 'feerecipient'));
    ...
    test('decryption fails with corrupted data', async () => {
      ...
      await expect(
        decryptCeremonyKeys(corrupted, password)
      ).rejects.toThrow();
    });

    test('different passwords produce different encrypted outputs', async () => {
      const password1Hash = createHash('sha256').update('Password1!').digest('hex');
      const password2Hash = createHash('sha256').update('Password2!').digest('hex');
  ...
  describe('Attestation Verification', () => {
    ...
    test('accepts valid non-simulated attestation', async () => {
      ...
      const measurementHash = createHash('sha256').update(JSON.stringify({
        network: 'testnet',
        timestamp,
        addresses,
      })).digest('hex');
  ...
  describe('Derivation Path Uniqueness', () => {
    ...
    test('derivation paths include network', async () => {
      ...
      const testnetKeys = await decryptCeremonyKeys(testnetResult.encryptedKeys, password);
      const mainnetKeys = await decryptCeremonyKeys(mainnetResult.encryptedKeys, password);

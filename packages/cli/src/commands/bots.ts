...
const BACKTEST_STRATEGIES = ['momentum', 'mean-reversion', 'volatility', 'composite'] as const;
...
export const botsCommand = new Command('bots')
  ...
  .addCommand(
    new Command('start')
      ...
      .option('--strategy <name>', 'Strategy name (tfmm-rebalancer, cross-chain-arbitrage, funding-arb, liquidation)', 'tfmm-rebalancer')
  ...
  .addCommand(
    new Command('backtest')
      ...
      .option('--strategy <name>', 'Strategy: momentum, mean-reversion, volatility, composite', 'composite')
  ...
  .addCommand(
    new Command('prices')
      ...
      .option('--tokens <list>', 'Comma-separated token symbols', 'ETH,BTC,USDC')
      .action(async (options) => {
...
async function startBot(options: {
  ...
  if (!privateKey) {
    logger.error('PRIVATE_KEY or DEPLOYER_PRIVATE_KEY environment variable required');
    process.exit(1);
  }
  ...
  const args = ['run', 'src/cli.ts', 'start', chainId.toString(), rpcUrl, privateKey];
...
async function runBacktest(options: {
  ...
  const proc = spawn({
    cmd: ['bun', 'run', 'src/cli.ts', 'backtest', options.strategy, options.startDate, options.endDate, options.initialCapital],
    cwd: botsDir,
    stdout: 'inherit',
    stderr: 'inherit',
  });
...
async function listStrategies(): Promise<void> {
  ...
  console.log('  liquidation           Liquidation bot for undercollateralized positions');

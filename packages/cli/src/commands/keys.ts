...
const setupTestnetSubcommand = new Command('setup-testnet')
  ...
  .action(async (options) => {
    ...
    const scriptPath = join(
      rootDir,
      'packages/deployment/scripts/deploy/setup-testnet-deployer.ts',
    )
...
async function runTeeGenesis(network: NetworkType, endpoint?: string) {
  ...
  logger.info(
    '║  Keys will be derived inside hardware-isolated enclave        ║',
  )
  logger.info(
    '║  Attestation will prove genuine TEE execution                 ║',
  )
  logger.info(
    '╚═══════════════════════════════════════════════════════════════╝',
  )
  ...
  // Get TEE endpoint
  const teeEndpoint =
    endpoint || process.env.DSTACK_ENDPOINT || process.env.PHALA_ENDPOINT
  ...
  if (!teeEndpoint) {
    ...
    const { provider } = await prompts({
      ...
      choices: [
        {
          title: 'Phala Network (dstack)',
          value: 'phala',
          description: 'Public TEE network',
        },
        {
          title: 'GCP Confidential VM',
          value: 'gcp',
          description: 'Google Cloud TDX/SEV',
        },
        {
          title: 'Custom endpoint',
          value: 'custom',
          description: 'Self-hosted dstack',
        },
        {
  ...
  try {
    ...
    writeFileSync(
      addressesPath,
      JSON.stringify(result.publicAddresses, null, 2),
    )
    ...
    logger.info(
      '  Measurement: ' +
        result.attestation.measurementHash.slice(0, 40) +
        '...',
    )
    ...
    logger.newline()
    logger.warn(
      'To decrypt keys: Use your password with the TEE decryption tool',
    )
  } catch (error) {
...
async function runDistributedCeremony(options: DistributedCeremonyOptions) {
  ...
  logger.info(
    '║  Keys distributed across multiple TEE enclaves                ║',
  )
  logger.info(
    '║  No single party (human or TEE) can reconstruct               ║',
  )
  logger.info(
    '╚═══════════════════════════════════════════════════════════════╝',
  )
  ...
  if (threshold > providers.length) {
    logger.error(
      `Threshold ${threshold} cannot exceed provider count ${providers.length}`,
    )
    return
  }
  ...
  try {
    ...
    writeFileSync(
      attestationsPath,
      JSON.stringify(result.attestations, null, 2),
    )
    ...
    if (options.register) {
      ...
      if (registryAddress && rpcUrl) {
        const txHash = await registerCeremonyOnChain(
          result,
          registryAddress,
          rpcUrl,
        )
    ...
    logger.list([
      ...
      'Fund operator addresses',
      `Deploy chain: jeju deploy ${network}`,
      'For signing operations, use threshold signing with ' +
        threshold +
        ' TEEs',
    ])
  } catch (error) {
...
async function runGenesis(network: NetworkType) {
  ...
  if (network === 'mainnet') {
    ...
    logger.error(
      '║  MAINNET DEPLOYMENT - THIS IS PRODUCTION                      ║',
    )
    logger.error(
      '║  Keys generated here will control real funds                  ║',
    )
    logger.error(
      '╚═══════════════════════════════════════════════════════════════╝',
    )
  ...
  const securityChecks = [
    {
      q: 'Is this machine air-gapped (no network connection)?',
      required: network === 'mainnet',
    },
    { q: 'Is your screen private (no cameras, no observers)?', required: true },
    {
      q: 'Do you have a hardware wallet or secure cold storage ready?',
      required: true,
    },
    { q: 'Do you have paper/metal backup materials ready?', required: true },
    {
      q: 'Have you verified the integrity of this CLI installation?',
      required: network === 'mainnet',
    },
  ]
    ...
    choices: [
      {
        ...
        value: 'generate',
        description: 'Create fresh cryptographic keys',
      },
      {
        title: 'Import existing keys',
        value: 'import',
        description: 'Use keys from hardware wallet or file',
      },
    ],
  })
  ...
  logger.error(
    '║  CRITICAL: Write down ALL keys below                          ║',
  )
  logger.error(
    '║  This is your ONLY opportunity to see private keys            ║',
  )
  logger.error(
    '║  After confirmation, keys will be encrypted and cleared       ║',
  )
  logger.error(
    '╚═══════════════════════════════════════════════════════════════╝',
  )
...
async function showKeys(network: NetworkType, showPrivate: boolean) {
  ...
  if (network === 'localnet') {
    ...
    if (existsSync(addressesPath)) {
      logger.subheader(
        `${network.charAt(0).toUpperCase() + network.slice(1)} Addresses`,
      )

...
async function checkEscapeHatch(network: NetworkType): Promise<CheckResult> {
  ...
  const code = await client.getCode({
    address: forcedInclusionAddress as `0x${string}`,
  })
...
async function checkDAVerification(network: NetworkType): Promise<CheckResult> {
  ...
  const code = await client.getCode({
    address: l2OutputOracleAdapter as `0x${string}`,
  })
...
export const verifyStage2Command = new Command('verify-stage2').description(
  'Verify Stage 2 decentralization readiness',
)
...
verifyStage2Command
  ...
  .action(async (options) => {
    ...
    const checks: Array<{
      name: string
      check: (n: NetworkType) => Promise<CheckResult>
    }> = [
        ...
        if (!result.passed) {
          switch (name) {
            case 'Fraud Proofs Active':
              console.log(
                `  • Run: jeju deploy-mips --network ${network} --use-optimism`,
              )
              break
            case 'Ownership Transferred':
              console.log(
                `  • Run: jeju decentralize --network ${network} --timelock <ADDRESS>`,
              )
              break
            case 'Escape Hatch Deployed':
              console.log(
                `  • Run: jeju deploy decentralization --network ${network}`,
              )
              break
...
verifyStage2Command
  ...
  .action(() => {
    ...
    console.log(
      '  • Sequencer failure: Users can force transactions within 7 days',
    )

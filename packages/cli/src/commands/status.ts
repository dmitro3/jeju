...
import { DEFAULT_PORTS, type NetworkType, WELL_KNOWN_KEYS } from '../types'
...
async function quickStatus(network: NetworkType): Promise<void> {
  ...
  try {
    ...
    logger.debug(
      `App discovery skipped: ${error instanceof Error ? error.message : 'not in workspace'}`,
    )
  }
...
async function fullCheck(network: NetworkType): Promise<void> {
    ...
    logger.table([
      {
        label: 'Chain',
        value: 'Not running',
        status: network === 'localnet' ? 'warn' : 'ok',
      },
    ])
  ...
  logger.table([
    ...
    {
      label: 'Testnet',
      value: hasKeys('testnet') ? 'configured' : 'not set',
      status: hasKeys('testnet') ? 'ok' : 'warn',
    },
    {
      label: 'Mainnet',
      value: hasKeys('mainnet') ? 'configured' : 'not set',
      status: hasKeys('mainnet') ? 'ok' : 'warn',
    },
  ])

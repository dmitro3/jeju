...
export function validateFilename(filename: string): string {
  ...
  if (!SAFE_FILENAME_PATTERN.test(filename)) {
    throw new Error('Invalid filename: must start with alphanumeric and contain only alphanumeric, dots, hyphens, underscores');
  }
...
export function validateKeyName(name: string): string {
  ...
  if (!SAFE_KEY_NAME_PATTERN.test(name)) {
    throw new Error('Invalid key name: must start with letter and contain only alphanumeric, hyphens, underscores');
  }
...
export function safePath(baseDir: string, ...segments: string[]): string {
  ...
  for (const segment of segments) {
    ...
    if (segment === '..' || segment.includes('/../') || segment.startsWith('../') || segment.endsWith('/..')) {
...
export function validateAppName(name: string): string {
  ...
  if (!APP_NAME_PATTERN.test(name)) {
    throw new Error('App name must be lowercase, start with a letter, and contain only letters, numbers, and hyphens');
  }
  
  // Reject reserved names
  const reservedNames = ['node_modules', 'dist', 'build', 'src', 'test', 'tests', 'lib'];
...
export function validateHex(value: string, expectedLength?: number): string {
  ...
  if (expectedLength !== undefined && hex.length !== expectedLength) {
    throw new Error(`Hex value must be ${expectedLength} characters (got ${hex.length})`);
  }

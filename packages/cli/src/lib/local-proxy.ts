...
export async function ensureHostsFile(
  ...
  if (status.exists && !options.force) {
    if (status.missingDomains.length > 0) {
      console.log(
        `   ⚠️  Hosts file has ${status.missingDomains.length} missing domain(s) - run 'jeju proxy hosts:add' to update`,
      )
    }
    console.log('   ✅ Hosts file configured (partial)')
    return true
  }

  if (status.exists) {
    console.log(
      `   🔄 Hosts file missing ${status.missingDomains.length} domain(s): ${status.missingDomains.slice(0, 3).join(', ')}${status.missingDomains.length > 3 ? '...' : ''}`,
    )
  } else {
  ...
  if (platform === 'win32') {
    console.log('   ⚠️  Windows detected - need Administrator privileges')
...
export async function removeHostsBlock(): Promise<boolean> {
  ...
  if (!status.exists) {
    console.log('   ℹ️  No Jeju block in hosts file')
    return true
  }
  ...
  if (process.platform === 'win32') {
    console.log('   ⚠️  Run in Administrator PowerShell:')
    console.log(`   Copy-Item "${tempFile}" -Destination "${hostsPath}"`)
    return false
  }
  ...
  if (result.exitCode !== 0) {
    console.error('   ❌ Failed to remove hosts block')
    return false
  }

  console.log('   ✅ Hosts block removed')
  return true
}
...
`
    ...
    if (result.exitCode !== 0) {
      ...
      if (loadResult.exitCode !== 0) {
        console.log('   ⚠️  Could not set up port forwarding (sudo required)')
        return false
      }
    }
    return true
...
export async function uninstallPortForwarding(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    ...
    if (pfConf.includes('anchor "jeju"')) {
      ...
      console.log('   ✅ Removed anchor from pf.conf')
    }

    // Remove anchor file
    await $`sudo rm -f /etc/pf.anchors/jeju`.nothrow()
    console.log('   ✅ Removed /etc/pf.anchors/jeju')

    // Reload pf
    await $`sudo pfctl -f /etc/pf.conf`.nothrow()
    console.log('   ✅ Reloaded packet filter')
    ...
    console.log('   ✅ Removed iptables rules')
    ...
    console.log('   ✅ Removed port forwarding rule')
...
export async function startProxy(config: ProxyConfig = {}): Promise<boolean> {
  ...
  if (!hostsOk) {
    console.log('   ⚠️  Hosts file not configured - URLs will not resolve')
  ...
  // Always run Caddy on 8080, then optionally forward port 80 → 8080
  ...
  console.log(`   ✅ Caddy running on port ${caddyPort}`)

  // Set up port forwarding for port 80 if requested
  if (usePortForwarding) {
    console.log('')
    console.log('5. Checking port forwarding (80 → 8080)...')

    // First check if persistent forwarding is already set up
    if (await isPortForwardingActive()) {
      console.log('   ✅ Port forwarding already active')
      actualPort = 80
    } else if (await hasSudoAccess()) {
      // Try to set up temporary forwarding
      const forwarded = await setupPortForwarding(caddyPort)
      if (forwarded) {
        console.log('   ✅ Port 80 forwarded to 8080')
        actualPort = 80
      } else {
        console.log('   ⚠️  Port forwarding failed - using port 8080')
      }
    } else {
      console.log('   ⚠️  Port forwarding not set up - using port 8080')
  ...
  console.log('🎉 Local proxy ready. Available URLs:')

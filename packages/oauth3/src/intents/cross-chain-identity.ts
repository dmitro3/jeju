...
const SUPPORTED_CHAINS: SupportedChain[] = [
  {
    ...
    entryPointAddress: '0x0000000000000000000000000000000000005FF137D4b0FDCD49DcA30c7CF57E578a026d2789' as Address,
  },
  {
...
export class CrossChainIdentityManager {
  ...
  computeSmartAccountAddress(
    ...
    const salt = keccak256(encodeAbiParameters(
      ...
      [identityId, owner, BigInt(chainId)]
    ));

    const initCodeHash = keccak256(toBytes(
      `account_bytecode:${identityId}:${owner}:${chainId}`
    ));

    const create2Hash = keccak256(encodeAbiParameters(
  ...
  async submitIntent(intent: CrossChainAuthIntent | IdentitySyncIntent): Promise<{
    intentId: Hex;

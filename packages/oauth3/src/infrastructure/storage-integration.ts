...
import type { OAuth3Session, VerifiableCredential, OAuth3App, OAuth3Identity } from '../types.js';
...
export class OAuth3StorageService {
  ...
  constructor(config: StorageConfig = {}) {
    this.ipfsApi = config.ipfsApiEndpoint || process.env.IPFS_API_ENDPOINT || DEFAULT_IPFS_API;
    this.ipfsGateway = config.ipfsGatewayEndpoint || process.env.IPFS_GATEWAY || DEFAULT_IPFS_GATEWAY;
    this.encryptionKey = config.encryptionKey ? toBytes(config.encryptionKey) : null;
    this.defaultTier = config.defaultTier || 'hot';
  }

  async storeSession(session: OAuth3Session): Promise<StoredSession> {
    if (!this.encryptionKey) {
      throw new Error('Encryption key required for session storage. Use MPC-derived key.');
    }

    const encryptedData = await this.encrypt(JSON.stringify(session));
    const result = await this.upload(encryptedData, `session-${session.sessionId}.enc`, { tier: 'hot' });
  ...
  async listSessionsForIdentity(identityId: Hex): Promise<StoredSession[]> {
    ...
    for (const [sessionId, cid] of this.sessionCache.entries()) {
      ...
      if (session?.identityId === identityId) {
        results.push({ sessionId, cid, encryptedData: '0x' as Hex, expiresAt: session.expiresAt, createdAt: 0 });
      }
    }
    return results;
  }

  async storeCredential(credential: VerifiableCredential): Promise<StoredCredential> {
  ...
  async retrieveCredentialByCid(cid: string): Promise<VerifiableCredential | null> {
    const data = await this.retrieve(cid);
    if (!data) return null;
    const credential = JSON.parse(new TextDecoder().decode(data)) as VerifiableCredential;
    this.credentialCache.set(credential.id, cid);
    return credential;
  }

  async listCredentialsForSubject(subjectDid: string): Promise<StoredCredential[]> {
    ...
    for (const [id, cid] of this.credentialCache.entries()) {
      ...
      if (cred?.credentialSubject.id === subjectDid) {
        results.push({
          credentialId: id, cid, subjectDid, issuerDid: cred.issuer.id, type: cred.type,
          issuedAt: new Date(cred.issuanceDate).getTime(), expiresAt: new Date(cred.expirationDate).getTime(),
        });
      }
    }
    return results;
  ...
  async storeAppMetadata(app: OAuth3App): Promise<StorageResult> {
    return this.upload(new TextEncoder().encode(JSON.stringify({
      appId: app.appId, name: app.name, description: app.description, owner: app.owner,
      jnsName: app.jnsName, redirectUris: app.redirectUris, allowedProviders: app.allowedProviders,
      metadata: app.metadata, createdAt: app.createdAt,
    })), `app-${app.appId}.json`, { tier: 'warm' });
  }
  ...
  async storeIdentityMetadata(identity: OAuth3Identity): Promise<StorageResult> {
    return this.upload(new TextEncoder().encode(JSON.stringify({
      id: identity.id, owner: identity.owner, smartAccount: identity.smartAccount,
      metadata: identity.metadata, createdAt: identity.createdAt, updatedAt: identity.updatedAt,
    })), `identity-${identity.id}.json`, { tier: 'warm' });
  }

  private async upload(data: Uint8Array, name: string, options: { tier?: StorageTier }): Promise<StorageResult> {
    ...
    return { cid: result.Hash, size: parseInt(result.Size, 10), url: `${this.ipfsGateway}/${result.Hash}`, tier: options.tier || this.defaultTier };
  }

  private async retrieve(cid: string): Promise<Uint8Array | null> {
    const response = await fetch(`${this.ipfsGateway}/${cid}`, { signal: AbortSignal.timeout(30000) })
      .catch((e: Error) => { throw e.name === 'AbortError' ? new Error(`Timeout: ${cid}`) : e; });
    if (response.status === 404) return null;
    if (!response.ok) throw new Error(`IPFS retrieve failed: ${response.status}`);
    return new Uint8Array(await response.arrayBuffer());
  }

  private async unpin(cid: string): Promise<void> {
    const response = await fetch(`${this.ipfsApi}/pin/rm?arg=${cid}`, { method: 'POST' })
      .catch((error: Error) => {
  ...
  private async decrypt(data: Uint8Array): Promise<string> {
    ...
    const plaintext = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: data.slice(0, 12) }, key, ciphertextBuffer);
    return new TextDecoder().decode(plaintext);
  }

  async saveSessionIndex(): Promise<string> {
    const sessions = Array.from(this.sessionCache.entries()).map(([sessionId, cid]) => ({ sessionId, cid }));
    const result = await this.upload(new TextEncoder().encode(JSON.stringify({ version: 1, sessions, lastUpdated: Date.now() })), 'session-index.json', { tier: 'hot' });
    return result.cid;
  }
  ...
  async loadSessionIndex(indexCid: string): Promise<void> {
    ...
    const index = JSON.parse(new TextDecoder().decode(data)) as { sessions: Array<{ sessionId: Hex; cid: string }> };
    index.sessions.forEach(e => this.sessionCache.set(e.sessionId, e.cid));
  }

  async saveCredentialIndex(): Promise<string> {
    const credentials = Array.from(this.credentialCache.entries()).map(([credentialId, cid]) => ({ credentialId, cid }));
    const result = await this.upload(new TextEncoder().encode(JSON.stringify({ version: 1, credentials, lastUpdated: Date.now() })), 'credential-index.json', { tier: 'permanent' });
    return result.cid;
  }
  ...
  async loadCredentialIndex(indexCid: string): Promise<void> {
    const data = await this.retrieve(indexCid);
    if (!data) throw new Error(`Index not found: ${indexCid}`);
    const index = JSON.parse(new TextDecoder().decode(data)) as { credentials: Array<{ credentialId: string; cid: string }> };
    index.credentials.forEach(e => this.credentialCache.set(e.credentialId, e.cid));
  }
  ...
  getGatewayUrl(cid: string): string { return `${this.ipfsGateway}/${cid}`; }
}

let instance: OAuth3StorageService | null = null;

export function createOAuth3StorageService(config: StorageConfig = {}): OAuth3StorageService {
  if (!instance) instance = new OAuth3StorageService(config);
  return instance;
}

export function resetOAuth3StorageService(): void { instance = null; }

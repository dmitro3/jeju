...
export class EmailProvider {
  ...
  constructor(config: EmailAuthConfig) {
    this.config = {
      ...config,
      magicLinkExpiryMinutes: config.magicLinkExpiryMinutes ?? DEFAULT_MAGIC_LINK_EXPIRY,
   ...
   */
  async sendMagicLink(email: string): Promise<{ token: Hex; magicLink: string }> {
    this.validateEmail(email);

    const tokenBytes = crypto.getRandomValues(new Uint8Array(32));
    const token = toHex(tokenBytes);
    const expiresAt = Date.now() + (this.config.magicLinkExpiryMinutes ?? DEFAULT_MAGIC_LINK_EXPIRY) * 60 * 1000;
    ...
    await this.sendEmail(email, 'Sign in to your account', this.getMagicLinkEmailTemplate(magicLink));
  ...
  async sendOTP(email: string): Promise<{ sent: boolean }> {
    ...
    const expiresAt = Date.now() + (this.config.otpExpiryMinutes ?? DEFAULT_OTP_EXPIRY) * 60 * 1000;
    ...
    await this.sendEmail(email, 'Your verification code', this.getOTPEmailTemplate(code));
  ...
  async login(email: string, password: string): Promise<EmailAuthResult> {
    ...
    if (!user.emailVerified) {
      return { success: false, error: 'Email not verified', requiresVerification: true };
    }
   ...
   * Change password
   */
  async changePassword(email: string, currentPassword: string, newPassword: string): Promise<EmailAuthResult> {
    const normalizedEmail = email.toLowerCase();
    const user = this.users.get(normalizedEmail);
   ...
   */
  async resetPassword(token: Hex, newPassword: string): Promise<EmailAuthResult> {
    const magicLinkToken = this.pendingMagicLinks.get(token);

    if (!magicLinkToken || magicLinkToken.used || Date.now() > magicLinkToken.expiresAt) {
  ...
  private generateUserId(email: string): string {
    return keccak256(toBytes(`email:${email}:${Date.now()}:${Math.random()}`)).slice(0, 18);
  }
  ...
  private async verifyPassword(password: string, storedHash: Hex): Promise<boolean> {
  ...
  private async sendEmail(_to: string, _subject: string, _html: string): Promise<void> {
    if (this.config.devMode || process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
      return;
    }

    const smtpConfig = {
      host: this.config.smtpHost ?? process.env.SMTP_HOST,
      port: this.config.smtpPort ?? parseInt(process.env.SMTP_PORT ?? '587'),
      user: this.config.smtpUser ?? process.env.SMTP_USER,
      password: this.config.smtpPassword ?? process.env.SMTP_PASSWORD,
    };

    if (!smtpConfig.host || !smtpConfig.user || !smtpConfig.password) {
      throw new Error('SMTP configuration required for production email sending');
    }

    throw new Error('Email provider not configured. Set SMTP_* environment variables.');
  }

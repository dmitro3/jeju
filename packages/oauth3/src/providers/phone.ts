...
export class PhoneProvider {
  ...
  async verifyOTP(phone: string, code: string): Promise<PhoneAuthResult> {
    ...
    if (!otp) {
      return { success: false, error: 'No pending verification for this phone number' };
    }
  ...
  private generateUserId(phone: string): string {
    return keccak256(toBytes(`phone:${phone}:${Date.now()}:${Math.random()}`)).slice(0, 18);
  }
  ...
  private checkRateLimit(phone: string): { allowed: boolean; reason?: string } {
    ...
    if (limit.blockedUntil && Date.now() < limit.blockedUntil) {
      const remainingMinutes = Math.ceil((limit.blockedUntil - Date.now()) / 60000);
      return { allowed: false, reason: `Too many attempts. Please try again in ${remainingMinutes} minutes.` };
    }
    ...
    // Check daily limit
    const maxAttempts = this.config.maxDailyAttempts ?? DEFAULT_MAX_DAILY_ATTEMPTS;
    if (limit.dailyAttempts >= maxAttempts) {
      limit.blockedUntil = limit.lastAttempt + RATE_LIMIT_WINDOW;
      return { allowed: false, reason: 'Daily SMS limit reached. Please try again tomorrow.' };
    }
  ...
  private async sendViaTwilio(phone: string, message: string): Promise<void> {
    ...
    const fromPhone = this.config.twilioPhoneNumber ?? process.env.TWILIO_PHONE_NUMBER;

    if (!accountSid || !authToken || !fromPhone) {
      throw new Error('Twilio configuration required. Set TWILIO_* environment variables.');
    }
...
export function createPhoneProvider(config: PhoneAuthConfig = {}): PhoneProvider {

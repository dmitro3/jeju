...
describe('FROST Threshold Signing - Lagrange Interpolation', () => {
  ...
  test('Lagrange interpolation reconstructs polynomial', () => {
    ...
    const reconstructed = ((l1 * y1 + l2 * y2) % CURVE_ORDER + CURVE_ORDER) % CURVE_ORDER;
...
describe('FROST Threshold Signing - Key Generation', () => {
  ...
  test('throws for invalid threshold > total', () => {
    expect(() => generateKeyShares(5, 3)).toThrow('Threshold cannot exceed total parties');
  });

  test('throws for threshold < 2', () => {
    expect(() => generateKeyShares(1, 3)).toThrow('Threshold must be at least 2');
  });
  ...
  test('different calls produce different shares (randomness)', () => {
    ...
    expect(shares1[0].groupPublicKey.equals(shares2[0].groupPublicKey)).toBe(false);
  });
});
...
describe('FROST Threshold Signing - Signing Protocol', () => {
  ...
  test('throws when computing group commitment without binding factors', () => {
    ...
    expect(() => computeGroupCommitment(commitments, emptyFactors)).toThrow('Missing binding factor');
  });

  test('throws when computing group commitment with empty commitments', () => {
    const bindingFactors = new Map<number, bigint>();
    expect(() => computeGroupCommitment([], bindingFactors)).toThrow('No commitments provided');
  });
...
describe('FROST Threshold Signing - Full Signing Flow', () => {
  ...
  test('fails when not enough participants', async () => {
    ...
    await expect(coordinator.sign(message, [1, 2])).rejects.toThrow('Need at least 3 participants');
  });
...
describe('FROST Threshold Signing - Signature Verification', () => {
  ...
  test('verifySignature rejects invalid r value', async () => {
    ...
    const cluster = coordinator.getCluster();
    const groupPubKey = secp256k1.ProjectivePoint.fromHex(cluster.groupPublicKey.slice(2));
...
import { TOTPManager, createTOTPManager } from '../src/mfa/totp.js';
...
import { BackupCodesManager, createBackupCodesManager } from '../src/mfa/backup-codes.js';
...
import { PasskeyManager, createPasskeyManager } from '../src/mfa/passkeys.js';
...
describe('Passkeys - Challenge Management', () => {
  ...
  beforeEach(() => {
    manager = createPasskeyManager({ rpId: 'example.com', rpName: 'Example App' });
  });
...
describe('Passkeys - Credential Management', () => {
  ...
  test('updateCredentialName returns false for non-existent', () => {
    const result = manager.updateCredentialName('user', 'nonexistent', 'New Name');
    expect(result).toBe(false);
  });
});
...
describe('Validation - Hex Schemas', () => {
  ...
  test('AddressSchema accepts valid addresses', () => {
    ...
    expect(AddressSchema.safeParse('0xabcdefABCDEF1234567890123456789012345678').success).toBe(true);
  });

  test('AddressSchema rejects invalid addresses', () => {
    expect(AddressSchema.safeParse('0x123').success).toBe(false); // Too short
    expect(AddressSchema.safeParse('0x12345678901234567890123456789012345678901').success).toBe(false); // Too long
    expect(AddressSchema.safeParse('1234567890123456789012345678901234567890').success).toBe(false); // No prefix
  });
...
describe('Validation - Helper Functions', () => {
  ...
  test('expectEndpoint throws for missing endpoint', () => {
    expect(() => expectEndpoint({ endpoint: '' })).toThrow('no endpoint configured');
  });
});
...
describe('Validation - Schema Response Validation', () => {
  ...
  test('validateResponse throws with context for invalid input', () => {
    expect(() => validateResponse(TestSchema, 'invalid', 'test hex'))
      .toThrow('Invalid test hex');
  });
});

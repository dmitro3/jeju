...
} from '../src/infrastructure/compute-integration.js'
import {
  CACHE_EXPIRY_MS,
  CHAIN_IDS,
  CONTRACTS,
...
} from '../src/infrastructure/discovery.js'
import {
  createOAuth3JNSService,
  OAuth3JNSService,
  resetOAuth3JNSService,
} from '../src/infrastructure/jns-integration.js'
import {
  createOAuth3StorageService,
  OAuth3StorageService,
  resetOAuth3StorageService,
} from '../src/infrastructure/storage-integration.js'
import {
  deriveLocalEncryptionKey,
  ThresholdEncryptionService,
} from '../src/infrastructure/threshold-encryption.js'
import {
  calculateComputeFee,
  calculateStorageFee,
  ...
  X402PaymentClient,
} from '../src/infrastructure/x402-payments.js'
import {
  AuthProvider,
  type OAuth3Session,
  TEEProvider,
...
describe('Config Module', () => {
  ...
  describe('getContracts', () => {
    it('should return correct contracts for each network', () => {
      ...
      expect(localnetContracts.jnsRegistry).not.toBe(
        testnetContracts.jnsRegistry,
      )
    })

    it('should throw for mainnet until deployed', () => {
      expect(() => getContracts(CHAIN_IDS.mainnet)).toThrow(
...
describe('OAuth3 JNS Integration', () => {
  ...
  describe('address getters', () => {
    ...
    it('should use testnet addresses for testnet chain', () => {
      ...
      expect(testnetJns.getRegistryAddress()).toBe(
        CONTRACTS.testnet.jnsRegistry,
      )
      // Testnet addresses should be different from localnet
      expect(testnetJns.getRegistryAddress()).not.toBe(
        CONTRACTS.localnet.jnsRegistry,
      )
    })
  })
  ...
  describe('on-chain operations (require contracts)', () => {
    ...
    it('should resolve non-existent app as null', async () => {
      try {
        const app = await jns.resolveApp(
          'definitely-nonexistent-12345.oauth3.jeju',
        )
        expect(app).toBeNull()
      } catch (error) {
        expect(isChainError(error as Error)).toBe(true)
    ...
    it('should handle various name formats', async () => {
      const formats = ['myapp', 'myapp.oauth3', 'myapp.oauth3.jeju']
...
describe('OAuth3 Decentralized Storage', () => {
  ...
  const createMockSession = (
    overrides: Partial<OAuth3Session> = {},
  ): OAuth3Session => ({
    sessionId:
  ...
  describe('encryption key requirements', () => {
    it('should throw when storing session without encryption key', async () => {
      ...
      await expect(
        noKeyStorage.storeSession(createMockSession()),
  ...
  describe('session storage', () => {
    ...
    it('should handle expired session retrieval', async () => {
      const expiredSession = createMockSession({
        sessionId:
          '0xexpired0000000000000000000000000000000000000000000000000000000' as Hex,
    ...
    it('should return null for non-existent session', async () => {
      const result = await storage.retrieveSession(
        '0xnonexistent000000000000000000000000000000000000000000000000000' as Hex,
      )
      expect(result).toBeNull()
    })
  })
  ...
  describe('credential storage', () => {
    const createMockCredential = (): VerifiableCredential => ({
      ...
      issuer: {
        id: 'did:ethr:420691:0x1234567890123456789012345678901234567890',
        name: 'Test',
      },
      ...
      proof: {
        ...
        verificationMethod:
          'did:ethr:420691:0x1234567890123456789012345678901234567890#controller',
...
describe('OAuth3 Compute Integration', () => {
  ...
  describe('TEE verifier address', () => {
    ...
    it('should match config', () => {
      expect(compute.getTeeVerifierAddress()).toBe(
        CONTRACTS.localnet.teeVerifier,
      )
    })
  })
  ...
  describe('attestation retrieval', () => {
    it('should return null for unreachable endpoint', async () => {
      const attestation = await compute.getNodeAttestation(
...
describe('OAuth3 Decentralized Discovery', () => {
  ...
  describe('app discovery (require contracts)', () => {
    ...
    it('should discover app by hex ID', async () => {
      try {
        const app = await discovery.discoverApp(
...
describe('Threshold Encryption', () => {
  describe('deriveLocalEncryptionKey', () => {
    ...
    it('should produce different keys for different seeds', () => {
      const key1 = deriveLocalEncryptionKey(
        '0x1111111111111111111111111111111111111111111111111111111111111111' as Hex,
      )
      const key2 = deriveLocalEncryptionKey(
  ...
  describe('ThresholdEncryptionService', () => {
    ...
    it('should encrypt data with valid public key', async () => {
      ...
      const publicKeyHex = ('0x' +
        Array.from(new Uint8Array(publicKeyRaw))
...
describe('Edge Cases and Error Handling', () => {
  describe('invalid inputs', () => {
    ...
    it('should throw for negative chain ID (maps to mainnet which is not deployed)', () => {
      ...
      expect(() => createOAuth3JNSService({ chainId: -1 })).toThrow(
        'Mainnet contracts not yet deployed',
      )
    })
  })
...
describe('Storage Index Operations', () => {
  ...
  describe('session index', () => {
    ...
    it('should throw on load for non-existent index', async () => {
      await expect(
        storage.loadSessionIndex('QmNonExistent123'),
      ).rejects.toThrow()
    })
  })
  ...
  describe('credential index', () => {
    ...
    it('should throw on load for non-existent index', async () => {
      await expect(
        storage.loadCredentialIndex('QmNonExistent456'),
      ).rejects.toThrow()
    })
  })
})
  ...
  describe('verifyNodeSignature', () => {
    it('should verify signature on-chain (require contracts)', async () => {
      try {
        ...
        const valid = await compute.verifyNodeSignature(
          nodeId,
          messageHash,
          signature,
        )

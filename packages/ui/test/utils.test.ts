...
function createMockClient(): { address: `0x${string}`; network: string; chainId: number; isSmartAccount: boolean } {
  return {
...
describe("requireClient", () => {
  describe("success cases", () => {
    test("returns the client when it is not null", () => {
      ...
      const result = requireClient(mockClient as Parameters<typeof requireClient>[0]);
  ...
  describe("type narrowing", () => {
    test("narrows type from nullable to non-null", () => {
      const mockClient = createMockClient();
      const nullableClient: typeof mockClient | null = Math.random() > -1 ? mockClient : null;
      
      // After requireClient, TypeScript should know it's not null
      const result = requireClient(nullableClient as Parameters<typeof requireClient>[0]);

...
describe('percentToTokens - Property-Based Testing (Fuzzing)', () => {
  ...
  const randomPercentages = Array.from({ length: 100 }, () => Math.random() * 100);
...
describe('formatTokens - Edge Cases', () => {
  ...
  test('boundary at 1 million', () => {
    expect(formatTokens(999_999n)).toBe('1000.0K');
    expect(formatTokens(1_000_000n)).toBe('1.0M');
  });
});
...
describe('calculateVestingSchedule - Mathematical Properties', () => {
  ...
  test('final cumulative equals total amount', () => {
    const testCases: [VestingSchedule, bigint][] = [
      [{ cliffDuration: 0, vestingDuration: ONE_YEAR, tgeUnlockPercent: 10, vestingType: 'linear' as const }, 1_000_000n],
      [{ cliffDuration: 6 * ONE_MONTH, vestingDuration: 2 * ONE_YEAR, tgeUnlockPercent: 0, vestingType: 'linear' as const }, 2_400_000n],
      [{ cliffDuration: ONE_YEAR, vestingDuration: ONE_YEAR, tgeUnlockPercent: 20, vestingType: 'linear' as const }, 1_200_000n],
    ];
...
describe('validateFeeDistribution - Valid Cases', () => {
  test('accepts default fee distribution', () => {
    expect(() => validateFeeDistribution(DEFAULT_FEE_DISTRIBUTION)).not.toThrow();
  });
...
describe('createTokenEconomics - Factory Function', () => {
  ...
  const validVesting: VestingConfig = {
    team: { cliffDuration: ONE_YEAR, vestingDuration: 3 * ONE_YEAR, tgeUnlockPercent: 0, vestingType: 'linear' },
    advisors: { cliffDuration: 6 * ONE_MONTH, vestingDuration: 2 * ONE_YEAR, tgeUnlockPercent: 0, vestingType: 'linear' },
    presale: { cliffDuration: 3 * ONE_MONTH, vestingDuration: ONE_YEAR, tgeUnlockPercent: 10, vestingType: 'linear' },
    ecosystem: { cliffDuration: 0, vestingDuration: 4 * ONE_YEAR, tgeUnlockPercent: 5, vestingType: 'linear' },
  };
  ...
  test('creates token economics with custom options', () => {
    const result = createTokenEconomics(
      ...
      DEFAULT_FEE_DISTRIBUTION,
      {
        ...
        maxWalletPercent: 5,
        maxTxPercent: 1,
        feeExemptAddresses: ['0x1234567890123456789012345678901234567890' as Address],
      }
    );
...
describe('DEFAULT_FEE_DISTRIBUTION', () => {
  test('sums to 100%', () => {
    const total = Object.values(DEFAULT_FEE_DISTRIBUTION).reduce((sum, val) => sum + val, 0);

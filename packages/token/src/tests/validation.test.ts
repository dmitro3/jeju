...
describe('addressSchema - Invalid Addresses', () => {
  ...
  test('rejects address with wrong length (too short)', () => {
    const result = addressSchema.safeParse('0x123456789012345678901234567890123456789');
    expect(result.success).toBe(false);
  });

  test('rejects address with wrong length (too long)', () => {
    const result = addressSchema.safeParse('0x12345678901234567890123456789012345678901');
    expect(result.success).toBe(false);
  });
...
describe('bytes32Schema', () => {
  test('accepts valid bytes32', () => {
    const validBytes32 = '0x1234567890123456789012345678901234567890123456789012345678901234';
    const result = bytes32Schema.safeParse(validBytes32);
    expect(result.success).toBe(true);
  });

  test('rejects bytes32 with wrong length', () => {
    const tooShort = '0x123456789012345678901234567890123456789012345678901234567890123';
    const tooLong = '0x12345678901234567890123456789012345678901234567890123456789012345';
...
describe('validate function', () => {
  test('returns validated data on success', () => {
    const result = validate(addressSchema, '0x1234567890123456789012345678901234567890');
...
describe('safeParse function', () => {
  test('returns success result for valid data', () => {
    const result = safeParse(addressSchema, '0x1234567890123456789012345678901234567890');
    expect(result.success).toBe(true);
    if (result.success) {
...
describe('tokenEconomicsSchema - Full Integration', () => {
  const validVesting = {
    team: { cliffDuration: 31536000, vestingDuration: 94608000, tgeUnlockPercent: 0, vestingType: 'linear' as const },
    advisors: { cliffDuration: 15768000, vestingDuration: 63072000, tgeUnlockPercent: 0, vestingType: 'linear' as const },
    presale: { cliffDuration: 7884000, vestingDuration: 31536000, tgeUnlockPercent: 10, vestingType: 'linear' as const },
    ecosystem: { cliffDuration: 0, vestingDuration: 126144000, tgeUnlockPercent: 5, vestingType: 'linear' as const },
  };

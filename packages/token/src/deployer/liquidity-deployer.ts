...
export async function addLiquidityV3(
  ...
  // Mint position with slippage protection (allow 1% slippage)
  // Setting min amounts to 0 would allow sandwich attacks
...
export async function lockLPTokens(params: LPLockParams): Promise<{
  ...
  console.log(
    `Locking ${amount} LP tokens until ${new Date(Number(unlockTime) * 1000).toISOString()}...`,
  )
...
export function getFeeTierTickSpacing(fee: number): number {
  ...
  if (spacing === undefined) {
    throw new Error(
      `Unknown fee tier: ${fee}. Valid tiers are: 500, 3000, 10000`,
    )
  }
  return spacing
}

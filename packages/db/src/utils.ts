 ...
 */
export function validateSQLIdentifier(identifier: string, type: 'table' | 'column' | 'index' = 'table'): string {
  ...
  const RESERVED_WORDS = new Set([
    ...
    'CONSTRAINT', 'CASCADE', 'RESTRICT', 'SET', 'ADD', 'COLUMN', 'DATABASE',
  ]);
  
  if (RESERVED_WORDS.has(identifier.toUpperCase())) {
    throw new Error(`Invalid SQL ${type} name "${identifier}": cannot use SQL reserved word`);
  }
 ...
 */
export function validateSQLIdentifiers(identifiers: string[], type: 'table' | 'column' | 'index' = 'column'): string[] {

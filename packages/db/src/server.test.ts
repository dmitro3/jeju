...
import { CQLServer, createCQLServer } from './server';
...
describe('CQL Server', () => {
  ...
  test('executes SQL queries', async () => {
    ...
    // Create table
    const createRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        database: dbId,
    ...
    // Insert data
    const insertRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        database: dbId,
    ...
    // Query data
    const selectRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        database: dbId,
  ...
  test('handles ACL operations', async () => {
    ...
    // Grant permissions
    const grantRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/databases/${dbId}/acl/grant`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        address,
    ...
    // List ACL
    const listRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/databases/${dbId}/acl`);
    ...
    // Revoke permissions
    const revokeRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/databases/${dbId}/acl/revoke`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // Verify revocation
    const verifyRes = await fetch(`http://localhost:${TEST_PORT}/api/v1/databases/${dbId}/acl`);

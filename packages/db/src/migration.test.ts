...
describe('addColumn', () => {
  describe('basic column addition', () => {
    ...
    it('should handle different data types', () => {
      expect(addColumn('t', 'c', 'TEXT').up).toBe(
        'ALTER TABLE t ADD COLUMN c TEXT',
      )
      expect(addColumn('t', 'c', 'INTEGER').up).toBe(
        'ALTER TABLE t ADD COLUMN c INTEGER',
      )
      expect(addColumn('t', 'c', 'REAL').up).toBe(
        'ALTER TABLE t ADD COLUMN c REAL',
      )
      expect(addColumn('t', 'c', 'BLOB').up).toBe(
  ...
  describe('with DEFAULT', () => {
    it('should add DEFAULT value', () => {
      const { up } = addColumn('users', 'status', 'TEXT', {
        default: "'active'",
      })
    ...
    it('should add DEFAULT with numeric value', () => {
      const { up } = addColumn('accounts', 'balance', 'REAL', {
        default: '0.0',
      })

      expect(up).toBe(
        'ALTER TABLE accounts ADD COLUMN balance REAL DEFAULT 0.0',
      )
    })
  })
  ...
  describe('with combined options', () => {
    it('should combine NOT NULL and DEFAULT', () => {
      const { up } = addColumn('users', 'role', 'TEXT', {
        notNull: true,
        default: "'user'",
      })
...
describe('createIndex', () => {
  describe('basic indexes', () => {
    ...
    it('should create index on multiple columns', () => {
      const { up, down } = createIndex('idx_posts_user_date', 'posts', [
        'user_id',
        'created_at',
      ])
  ...
  describe('unique indexes', () => {
    ...
    it('should create unique index on multiple columns', () => {
      ...
      expect(up).toBe(
        'CREATE UNIQUE INDEX idx_unique_combo ON items (col1, col2, col3)',
      )
    })
  })
  ...
  describe('edge cases', () => {
    it('should handle column names with underscores', () => {
      const { up } = createIndex('idx_test', 'items', [
        'first_name',
        'last_name',
      ])
...
describe('integration: full migration workflow', () => {
  it('should generate a complete migration set for a blog schema', () => {
    ...
    const userEmailIndex = createIndex(
      'idx_users_email',
      'users',
      ['email'],
      true,
    ...
    const addSlug = addColumn('posts', 'slug', 'TEXT', {
      notNull: true,
      default: "''",
    })

...
describe('CQLClient', () => {
  ...
  describe('Query Execution', () => {
    ...
    it('should execute parameterized query', async () => {
      ...
      await client.query('SELECT * FROM test WHERE id = ? AND name = ?', [
        1,
        'test',
      ])
    ...
    it('should handle bigint parameters', async () => {
      ...
      await client.query('SELECT * FROM test WHERE id = ?', [
    ...
    it('should handle Uint8Array parameters', async () => {
      ...
      await client.query('SELECT * FROM test WHERE hash = ?', [
  ...
  describe('Error Handling', () => {
    it('should require database ID for connect', async () => {
      const clientNoDB = new CQLClient({
        blockProducerEndpoint: 'http://localhost:4020',
      })

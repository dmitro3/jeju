...
export class FarcasterSignerManager {
  ...
  async importSigner(params: {
    ...
    const privateKey = typeof params.privateKey === 'string'
  ...
  async sign(keyId: string, message: Uint8Array): Promise<Uint8Array> {
    ...
    if (stored.info.status !== 'active') {
      throw new Error(`Signer not active: ${keyId} (status: ${stored.info.status})`);
    }
...
export function getSignerManager(config?: SignerManagerConfig): FarcasterSignerManager {
  if (!defaultManager) {

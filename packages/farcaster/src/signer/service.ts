...
export class FarcasterSignerService {
  ...
  async createSigner(params: {
    ...
    if (params.appFid) {
      ...
      approvalLink = this.registration.generateSimpleApprovalLink(signer.publicKey);
    }
  ...
  async getSignerForPosting(fid: number): Promise<{
    ...
    const publicKeyBytes = await this.manager.getSignerPublicKeyBytes(signer.keyId);
   ...
   */
  async getPoster(fid: number, network?: 'mainnet' | 'testnet'): Promise<FarcasterPoster | null> {
    const signer = await this.manager.getActiveSignerForFid(fid);
    if (!signer) return null;
...
export function getSignerService(config?: SignerServiceConfig): FarcasterSignerService {
  if (!defaultService) {

...
describe('Key Derivation', () => {
  describe('deriveEncryptionKeys', () => {
    it('derives 32-byte X25519 keys from Ed25519 key', () => {
      const { privateKey, publicKey } = deriveEncryptionKeys(ALICE_ED25519_PRIVATE);
    ...
    it('produces different keys for different inputs', () => {
      ...
      expect(bytesToHex(aliceKeys.privateKey)).not.toBe(bytesToHex(bobKeys.privateKey));
      expect(bytesToHex(aliceKeys.publicKey)).not.toBe(bytesToHex(bobKeys.publicKey));
    });
...
describe('Encryption/Decryption', () => {
  ...
  describe('decrypt', () => {
    it('recovers original plaintext', () => {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(plaintext, bobKeys.publicKey);

      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);
    ...
    it('works with UTF-8 content', () => {
      const testMessages = ['Hello 🌍', '你好世界', '🎉🚀🌟', 'Mixed: Hello 世界 🎉'];
    ...
    it('works with special characters', () => {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(specialChars, bobKeys.publicKey);
      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);
  ...
  describe('bidirectional communication', () => {
    it('Alice can send to Bob', () => {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(message, bobKeys.publicKey);
      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);
    ...
    it('Bob can send to Alice', () => {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(message, aliceKeys.publicKey);
      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, aliceKeys.privateKey);
      expect(decrypted).toBe(message);
    });
...
describe('Security Properties', () => {
  ...
  describe('ciphertext indistinguishability', () => {
    ...
    it('ciphertext length does not reveal exact plaintext length', () => {
      ...
      const long = encrypt('Hello World, this is a longer message!', bobKeys.publicKey);

      // Both should have 16-byte auth tag
      const shortPlainLen = new TextEncoder().encode('Hi').length;
      const longPlainLen = new TextEncoder().encode('Hello World, this is a longer message!').length;
  ...
  describe('authentication', () => {
    it('rejects tampered ciphertext', () => {
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt('Secret', bobKeys.publicKey);
    ...
    it('rejects tampered nonce', () => {
      ...
      expect(() => {
        decrypt(ciphertext, tamperedNonce, ephemeralPublicKey, bobKeys.privateKey);
      }).toThrow();
    });
    ...
    it('rejects truncated ciphertext', () => {
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt('Hello World', bobKeys.publicKey);
...
describe('HKDF Key Derivation', () => {
  it('produces 32-byte output for 32-byte request', () => {
    ...
    const derived = hkdf(sha256, secret, new Uint8Array(0), new TextEncoder().encode('test'), 32);
  ...
  it('different info strings produce different keys', () => {
    ...
    const key1 = hkdf(sha256, secret, new Uint8Array(0), new TextEncoder().encode('info1'), 32);
    const key2 = hkdf(sha256, secret, new Uint8Array(0), new TextEncoder().encode('info2'), 32);
...
describe('Property-Based Tests', () => {
  it('encryption is always reversible', () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(plaintext, recipientKeys.publicKey);
      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, recipientKeys.privateKey);
  ...
  it('different sender keys work with same recipient', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const { ciphertext, nonce, ephemeralPublicKey } = encrypt(message, recipientKeys.publicKey);
      const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, recipientKeys.privateKey);
      expect(decrypted).toBe(message);
    }
  });
...
describe('Edge Cases', () => {
  ...
  it('handles binary data in plaintext', () => {
    ...
    const binaryStr = Array.from({ length: 256 }, (_, i) => String.fromCharCode(i)).join('');

    // Note: This will produce invalid UTF-8, but should still roundtrip
    // In practice, messages should be valid text
    const { ciphertext, nonce, ephemeralPublicKey } = encrypt(binaryStr, bobKeys.publicKey);
    const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);
  ...
  it('handles very long messages', () => {
    ...
    const { ciphertext, nonce, ephemeralPublicKey } = encrypt(longMessage, bobKeys.publicKey);
    const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);
  ...
  it('handles null characters', () => {
    ...
    const { ciphertext, nonce, ephemeralPublicKey } = encrypt(withNull, bobKeys.publicKey);
    const decrypted = decrypt(ciphertext, nonce, ephemeralPublicKey, bobKeys.privateKey);

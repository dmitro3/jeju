...
import { verifyMessage, MessageType } from '../hub/message-builder';
...
describe('splitTextForThread', () => {
  ...
  it('respects word boundaries', () => {
    ...
    for (const chunk of chunks) {
      const withoutMarkers = chunk.replace(/^\.\.\./g, '').replace(/\.\.\.$/g, '').trim();
...
describe('CastBuilder', () => {
  ...
  describe('buildCast', () => {
    ...
    it('throws for text exceeding 320 bytes', async () => {
      ...
      await expect(builder.buildCast(longText)).rejects.toThrow('exceeds 320 bytes');
    });
    ...
    it('builds cast with URL embeds', async () => {
      ...
      expect(message.data.castAddBody?.embeds?.[0].url).toBe('https://example.com');
    });

    it('builds cast with cast embeds', async () => {
      const options: CastOptions = {
        embedCasts: [{ fid: 100, hash: '0xabcdef1234567890abcdef1234567890abcdef12' as Hex }],
      };
    ...
    it('builds reply with parent cast ID', async () => {
      const options: CastOptions = {
        replyTo: { fid: 999, hash: '0xabcdef1234567890abcdef1234567890abcdef12' as Hex },
      };
    ...
    it('builds channel cast with parent URL', async () => {
      ...
      expect(message.data.castAddBody?.parentUrl).toBe('https://farcaster.group/developers');
    });

    it('does not set both parentCastId and parentUrl', async () => {
      // replyTo takes precedence over channelUrl
      const options: CastOptions = {
        replyTo: { fid: 1, hash: '0xabcdef1234567890abcdef1234567890abcdef12' as Hex },
  ...
  describe('buildChannelCast', () => {
    it('builds channel cast with parent URL', async () => {
      ...
      expect(message.data.castAddBody?.parentUrl).toBe('https://farcaster.group/general');
    });
  });
  ...
  describe('buildDeleteCast', () => {
    ...
    it('produces verifiable delete message', async () => {
      const message = await builder.buildDeleteCast('0xabcdef1234567890abcdef1234567890abcdef12' as Hex);
      expect(verifyMessage(message)).toBe(true);
    });
  });
  ...
  describe('buildThread', () => {
    ...
    it('applies channel URL only to first cast', async () => {
      ...
      expect(messages[0].data.castAddBody?.parentUrl).toBe('https://farcaster.group/test');
    ...
    it('all thread messages are verifiable', async () => {
      const messages = await builder.buildThread(['One', 'Two', 'Three', 'Four']);
  ...
  describe('buildQuoteCast', () => {
    it('builds quote cast with embedded cast reference', async () => {
      const quotedCast = { fid: 789, hash: '0xabcdef1234567890abcdef1234567890abcdef12' as Hex };
      const message = await builder.buildQuoteCast('My thoughts on this:', quotedCast);
...
describe('Convenience Functions', () => {
  describe('createCast', () => {
    ...
    it('creates cast with options', async () => {
      const message = await createCast(TEST_FID, TEST_PRIVATE_KEY, 'With embed', {
        embeds: ['https://example.com'],
        network: 'testnet',
      });
  ...
  describe('createReply', () => {
    it('creates reply message', async () => {
      const message = await createReply(TEST_FID, TEST_PRIVATE_KEY, 'Reply text', {
        fid: 100,
...
describe('Property-Based Tests', () => {
  ...
  it('thread messages form proper chain', async () => {
    ...
    for (let threadLength = 1; threadLength <= 10; threadLength++) {
      const texts = Array.from({ length: threadLength }, (_, i) => `Cast ${i + 1}`);
      const messages = await builder.buildThread(texts);

...
describe('Timestamp Functions', () => {
  ...
  describe('fromFarcasterTimestamp', () => {
    ...
    it('is inverse of toFarcasterTimestamp', () => {
      const testTimestamps = [FARCASTER_EPOCH, 1700000000, 1704067200, 1750000000];
...
describe('Message Encoding', () => {
  ...
  describe('varint encoding via encodeMessageData', () => {
    it('encodes small FIDs (single byte)', () => {
      ...
      expect(Math.abs(encoded1.length - encoded127.length)).toBeLessThanOrEqual(1);
    });
    ...
    it('message types affect encoding', async () => {
      ...
      for (const type of types) {
        const data: MessageData = {
          ...
          timestamp: 100,
          network: FarcasterNetwork.MAINNET,
          castAddBody: type === MessageType.CAST_ADD ? { text: 'Test' } : undefined,
          reactionBody:
            type === MessageType.REACTION_ADD
              ? { type: ReactionType.LIKE, targetCastId: { fid: 1, hash: new Uint8Array(20) } }
              : undefined,
          linkBody:
            type === MessageType.LINK_ADD ? { type: 'follow', targetFid: 1 } : undefined,
          userDataBody:
            type === MessageType.USER_DATA_ADD

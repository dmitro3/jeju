  ...
  if (body.embeds) {
    for (const embed of body.embeds) {
      if (embed.url) {
        ...
        const embedContent = new Uint8Array([0x0a, urlBytes.length, ...urlBytes]);
        ...
        const embedContent = new Uint8Array([0x12, castIdBytes.length, ...castIdBytes]);
        parts.push(new Uint8Array([0x12, embedContent.length, ...embedContent]));
      }
    }
  }
...
export function encodeMessageData(data: MessageData): Uint8Array {
  ...
  if (data.linkBody) {
    ...
    const bodyBytes = new Uint8Array([
      0x0a, typeBytes.length, ...typeBytes,
      0x10, ...encodeVarint(body.targetFid),
    ]);
  ...
  if (data.userDataBody) {
    ...
    const bodyBytes = new Uint8Array([
      0x08, body.type,
      0x12, valueBytes.length, ...valueBytes,
    ]);
...
export function serializeMessage(message: Message): Uint8Array {
  ...
  // Field 4: signature
  parts.push(new Uint8Array([0x22, message.signature.length, ...message.signature]));

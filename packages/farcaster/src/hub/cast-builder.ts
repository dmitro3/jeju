...
export class CastBuilder {
  ...
  async buildCast(text: string, options?: CastOptions): Promise<Message> {
    ...
    if (options?.replyTo) {
      castAddBody.parentCastId = createCastId(options.replyTo.fid, options.replyTo.hash);
  ...
  async buildThread(
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      if (previousMessage) {
        const hashHex = `0x${Array.from(previousMessage.hash).map(b => b.toString(16).padStart(2, '0')).join('')}` as Hex;
 ...
 */
export function splitTextForThread(text: string, maxBytes: number = 320): string[] {
  const chunks: string[] = [];
  let remaining = text;
  ...
  while (remaining.length > 0) {
    ...
    for (const word of wordsWithSpaces) {
      ...
      if (currentBytes + wordBytes > maxBytes - 3) { // -3 for "..."
        break;
      }

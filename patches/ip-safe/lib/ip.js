...
ip.isLoopback = function (addr) {
  // SECURITY FIX: GHSA-2p57-rm9w-gvfp
  // Normalize IPv4 addresses to prevent bypass via alternate formats (hex, octal, etc.)
  
  // Handle IPv6 addresses (must have : in them)
  if (/:/.test(addr)) {
    return /^fe80::1$/i.test(addr)
      || /^::1$/.test(addr)
      || /^::$/.test(addr)
      || /^(::f{4}:)?127\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})/.test(addr);
  }
  
  // For IPv4, normalize the address first to catch all formats
  // This handles: decimal, hex (0x7f), octal (0177), and long integer forms
  let normalized = -1;
  
  // If it's a plain number (no dots), convert from long form
  if (!/\./.test(addr)) {
    ...
    // Normalize addresses with dots (handles hex/octal octets)
    normalized = ip.normalizeToLong(addr);
  }
  
  if (normalized >= 0) {
    addr = ip.fromLong(normalized);
  }
  
  // Now check against normalized form - 127.0.0.0/8 range
  return /^127\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/.test(addr);
};

...
function PositionRow({ position }: { position: Position }) {
  ...
  return (
    <tr className="hover:bg-[var(--bg-secondary)] transition-colors">
      ...
      <td className="px-4 md:px-6 py-4 hidden sm:table-cell">
        <div className="flex gap-2 flex-wrap">
          {position.yesShares > 0n && (
            <span className="badge-success">YES {formatEthValue(position.yesShares)}</span>
          )}
          {position.noShares > 0n && (
            <span className="badge-error">NO {formatEthValue(position.noShares)}</span>
          )}
        </div>
      </td>
      ...
      <td className="px-4 md:px-6 py-4">
        <span className={pnl >= 0n ? 'text-bazaar-success' : 'text-bazaar-error'}>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8">
            ...
            <div className="stat-card">
              ...
              <div className="stat-value">{countActivePositions(positions)}</div>
            </div>
          </div>

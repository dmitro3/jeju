...
class LMSR {
  ...
  cost(yesShares: number, noShares: number): number {
    return (
      this.b *
      Math.log(Math.exp(yesShares / this.b) + Math.exp(noShares / this.b))
    )
  }
...
describe('LMSR Pricing Model', () => {
  ...
  test('should calculate initial 50/50 price', async () => {
    ...
    console.log(
      `   Initial prices: YES=${(priceYes * 100).toFixed(1)}%, NO=${(priceNo * 100).toFixed(1)}%`,
    )
...
describe('Market Dynamics', () => {
  ...
  test('should handle extreme probabilities', async () => {
    ...
    console.log(
      `   High YES shares: price = ${(priceAtHigh * 100).toFixed(2)}%`,
    )

/**
 * Swap Tests with Wallet
 * Tests token swapping with MetaMask confirmation including XLP V2/V3 pools
 */
...
async function selectSwapTokens(
  page: Page,
  inputToken: string,
  outputToken: string,
): Promise<void> {
...
test.describe('Swap with Wallet', () => {
  ...
  test('calculates output amount for ETH to USDC swap', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
  ...
  test('shows cross-chain toggle when EIL is available', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)

    const crossChainToggle = page
      .locator('button')
      .filter({ hasText: /ON|OFF/i })
    if (await crossChainToggle.isVisible()) {
      await crossChainToggle.click()
      await page.waitForTimeout(WAIT_SHORT)
      const chainSelects = page.locator('select').filter({
        has: page.locator('option', { hasText: /Ethereum|Base|Arbitrum/i }),
      })
      expect(await chainSelects.count()).toBeGreaterThanOrEqual(0)
    }
  })

  test('swaps tokens (input to output direction)', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
    ...
    if (await swapButton.isEnabled()) {
      ...
      expect(
        body?.includes('Swap') ||
          body?.includes('error') ||
          body?.includes('initiated'),
      ).toBe(true)
    }
  })

  test('shows fee and rate information', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
    ...
    const body = await page.textContent('body')
    expect(
      body?.includes('Rate') || body?.includes('Fee') || body?.includes('ETH'),
    ).toBe(true)
  })

  test('handles token swap direction reversal', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
...
test.describe('Cross-Chain Swap', () => {
  test.beforeEach(async ({ context, page, metamaskPage, extensionId }) => {
    await connectWallet(page, context, metamaskPage, extensionId)
  })
  ...
  test('enables cross-chain mode', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
  ...
  test('selects different source and destination chains', async ({ page }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
    ...
    if (await enableCrossChain(page)) {
      ...
      if (selectCount >= 4) {
        ...
        if (await sourceChain.isVisible())
          await sourceChain.selectOption({ index: 0 })
        if (await destChain.isVisible())
          await destChain.selectOption({ index: 1 })
      }
    }
  })

  test('shows estimated time and fees for cross-chain swap', async ({
    page,
  }) => {
    await page.goto('/swap')
    await page.waitForTimeout(WAIT_MEDIUM)
...
test.describe('Liquidity Page with Wallet', () => {
  test.beforeEach(async ({ context, page, metamaskPage, extensionId }) => {
    await connectWallet(page, context, metamaskPage, extensionId)
  })

  test('loads liquidity page with V4 and XLP sections', async ({ page }) => {
  ...
  test('shows V4 liquidity add form', async ({ page }) => {
    await page.goto('/liquidity')
    await page.waitForTimeout(WAIT_MEDIUM)
  ...
  test('shows user positions when connected', async ({ page }) => {
    await page.goto('/liquidity')
    await page.waitForTimeout(1000)

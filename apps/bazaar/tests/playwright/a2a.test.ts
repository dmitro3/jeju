...
test.describe('A2A Protocol Implementation', () => {
  ...
  test('should execute list-tokens skill without payment', async ({
    request,
  }) => {
    const response = await request.post(A2A_ENDPOINT, {
      data: {
    ...
    const dataPart = body.result.parts.find(
      (p: { kind: string }) => p.kind === 'data',
    )
    expect(dataPart?.data?.tokens).toBeInstanceOf(Array)
  })

  test('should execute get-latest-blocks skill without payment', async ({
    request,
  }) => {
    const response = await request.post(A2A_ENDPOINT, {
      data: {
  ...
  test('should require payment for token creation', async ({ request }) => {
    ...
    expect(
      BigInt(body.error.data.accepts[0].maxAmountRequired),
    ).toBeGreaterThan(0n)
  })
  ...
  test('should require payment for swap', async ({ request }) => {
    ...
    const expectedFee =
      (BigInt('1000000000000000000') * BigInt(30)) / BigInt(10000)
    expect(BigInt(body.error.data.accepts[0].maxAmountRequired)).toBe(
      expectedFee,
    )
  })

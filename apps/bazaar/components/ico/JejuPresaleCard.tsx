...
export function JejuPresaleCard() {
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.success('Contribution successful', {
        description: 'Your tokens will be claimable at TGE',
      })
      setAmount('')
    }
  }, [isSuccess])
  ...
  const tokenPrice =
    contractTokenPrice && contractTokenPrice > 0n
      ? contractTokenPrice
      : JEJU_TOKENOMICS.presale.tokenPrice
  const tokensReceived = amount
    ? (parseEther(amount) * 10n ** 18n) / tokenPrice
    : 0n
  const bonus =
  ...
  const handleContribute = () => {
    ...
    if (presaleAddress === '0x0000000000000000000000000000000000000000') {
      toast.info('Presale contract not deployed yet', {
        description: 'Coming soon on testnet',
      })
      return
    }
    ...
    <div className="card p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <span
          className={`text-xs px-2 py-1 rounded-full ${
            stats.phase === 'PUBLIC'
              ? 'bg-bazaar-primary/20 text-bazaar-primary'
              : stats.phase === 'WHITELIST'
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        >
          ...
          >
            {contractTokenPrice
              ? `${formatEther(contractTokenPrice)} ETH`
              : `${formatEther(JEJU_TOKENOMICS.presale.tokenPrice)} ETH`}
          </div>
          <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>
            Price
          </div>
      ...
      <div className="space-y-4">
        ...
        {amount && parseFloat(amount) > 0 && (
          ...
          >
            ...
            >
              ...
              >
                {Number(
                  (tokensReceived + bonusTokens) / 10n ** 18n,
                ).toLocaleString()}{' '}
                JEJU
              </span>
            </div>
        ...
        <p
          ...
          style={{ color: 'var(--text-tertiary)' }}
        >
          Min: {formatEther(JEJU_TOKENOMICS.presale.minContribution)} ETH · Max:{' '}
          {formatEther(JEJU_TOKENOMICS.presale.maxContribution)} ETH
        </p>
      </div>
    </div>
  )

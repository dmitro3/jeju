...
import { BBLN_ADDRESSES } from '../../lib/stubs/bbln'
...
export function BBLNPresaleCard() {
  ...
  const { isLoading: isConfirmingBid, isSuccess: bidSuccess } =
  ...
  useEffect(() => {
    if (claimSuccess) {
      toast.success('Claim successful', {
        description: 'BBLN tokens have been sent to your wallet',
      })
    }
  }, [claimSuccess])
    ...
    <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-950/20 via-black to-orange-950/20 p-6 shadow-xl">
      ...
      <div className="mb-6 rounded-lg bg-gradient-to-r from-amber-500/10 to-orange-500/10 p-4">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-amber-300/50">
          ...
          <span>
            ~${(Number(formatEther(stats.currentPrice)) * 3000).toFixed(2)} per
            BBLN
          </span>
        </div>
      </div>
      ...
      {userContribution && userContribution.ethAmount > 0n && (
        <div className="mb-6 rounded-lg border border-amber-500/20 bg-amber-950/20 p-4">
          <h3 className="mb-3 text-sm font-medium text-amber-300">
            Your Contribution
          </h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-amber-100">
                {formatEther(userContribution.ethAmount)} ETH
              </span>
            </div>
            {userContribution.tokenAllocation > 0n && (
              <div className="flex justify-between">
                ...
                <span className="text-amber-100">
                  {formatTokens(userContribution.tokenAllocation)} BBLN
                </span>
              </div>
            )}
            {userContribution.bonusTokens > 0n && (
              <div className="flex justify-between">
                ...
                <span className="text-green-400">
                  +{formatTokens(userContribution.bonusTokens)} BBLN
                </span>
              </div>
            )}
            {userContribution.claimable > 0n && (
              <div className="flex justify-between">
                ...
                <span className="text-amber-100">
                  {formatTokens(userContribution.claimable)} BBLN
                </span>
              </div>
            )}
          </div>
      ...
      <div className="mt-6 rounded-lg bg-amber-950/20 p-4 text-xs text-amber-300/50">
        ...
        <p className="mt-1">
          Continuous Clearing Auction - price starts high and decreases over
          time. All successful bidders pay the same final clearing price. ELIZA
          OS holders get 1.5x allocation bonus.
        </p>
      </div>
    </div>
  )

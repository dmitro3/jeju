...
function useXLPLiquidity(_paymasterAddress: Address | undefined) {
  return {
    addLiquidity: async (): Promise<void> => {
      /* stub */
    },
    removeLiquidity: async (): Promise<void> => {
      /* stub */
    },
    ethBalance: 0n,
    depositETH: async (_amount: bigint): Promise<void> => {
      /* stub */
    },
    withdrawETH: async (_amount: bigint): Promise<void> => {
      /* stub */
    },
    depositToken: async (_token: Address, _amount: bigint): Promise<void> => {
      /* stub */
    },
    isLoading: false,
    isAddSuccess: false,
...
function useXLPRegistration(_paymasterAddress: Address | undefined) {
  return {
    register: async (_chains: number[], _amount: bigint): Promise<void> => {
      /* stub */
    },
    unregister: async (): Promise<void> => {
      /* stub */
    },
    addStake: async (_amount: bigint): Promise<void> => {
      /* stub */
    },
    startUnbonding: async (_amount: bigint): Promise<void> => {
      /* stub */
    },
    completeUnbonding: async (): Promise<void> => {
      /* stub */
    },
    isLoading: false,
    isRegistered: false,
...
async function fetchXLPVoucherHistory(
  xlpAddress: string,
): Promise<VoucherHistoryItem[]> {
  const query = `
  ...
  const response = await fetch(INDEXER_URL, {
    ...
    body: JSON.stringify({
      query,
      variables: { xlp: xlpAddress.toLowerCase() },
    }),
  })
  ...
  return fulfillments.map(
    (f: {
      voucherRequest: VoucherHistoryItem
      feeEarned: string
      fulfilledAt: string
    }) => ({
  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: '1rem' }}>
        <h2
          style={{ fontSize: '1.25rem', margin: '0 0 1.5rem', fontWeight: 700 }}
        >
          XLP Dashboard
        </h2>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      {history.map((item) => {
        ...
        const feeEth = item.feeEarned
          ? (Number(BigInt(item.feeEarned)) / 1e18).toFixed(6)
          : '0'

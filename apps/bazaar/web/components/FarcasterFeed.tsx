/**
 * Bazaar Farcaster Feed Component
 *
 * Displays the Bazaar channel feed for marketplace updates
 */

import { ExternalLink, MessageCircle, RefreshCw, User } from 'lucide-react'
import { type BazaarFeedCast, useBazaarFeed } from '../hooks/useMessaging'
...
export function FarcasterFeed() {
  const { data, isLoading, refetch, isRefetching } = useBazaarFeed({
    limit: 20,
  })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1">
            Latest from /bazaar on Farcaster
          </p>
        </div>
        <button
          type="button"
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-text-secondary">No updates in /bazaar yet</p>
          <p className="text-text-tertiary text-sm mt-2">
            Check back later for marketplace updates
          </p>
        </div>
      ) : (
        <div>

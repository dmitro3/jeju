...
export function useNFTAuction(auctionId?: bigint) {
  ...
  const { data: auction, refetch: refetchAuction } = useReadContract({
    ...
    query: {
      enabled: !!auctionId && auctionId > 0n && MARKETPLACE_ADDRESS !== '0x0',
    },
  })
  ...
  const createAuction = (
    ...
    const buyoutPrice = buyoutPriceETH
      ? parseEther(NonEmptyStringSchema.parse(buyoutPriceETH))
      : 0n
  ...
  const placeBid = (auctionId: bigint, bidAmountETH: string) => {
    ...
    const [, , , reservePrice, highestBid, highestBidder, endTime, settled] =
      validatedAuction as AuctionData
    ...
    const minBid =
      highestBid > 0n ? highestBid + highestBid / BigInt(20) : reservePrice
  ...
  const settleAuction = (auctionId: bigint) => {
    ...
    if (validatedAuction) {
      ...
      expectTrue(
        Number(endTime) < now,
        `Auction ends in ${Math.floor((Number(endTime) - now) / 60)} minutes`,
      )
      expectTrue(!settled, 'Auction already settled')
    }

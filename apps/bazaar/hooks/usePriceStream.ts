...
export function usePriceStream(options?: UsePriceStreamOptions): UsePriceStreamReturn {
  ...
  const connect = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      // Auto-reconnect
      if (config.autoReconnect && reconnectAttempts.current < config.maxReconnectAttempts) {
        reconnectAttempts.current++;
        console.log(`[PriceStream] Reconnecting in ${config.reconnectDelay}ms (attempt ${reconnectAttempts.current})`);
        setTimeout(connect, config.reconnectDelay);
      }
    };
  }, [wsEndpoint, config.autoReconnect, config.reconnectDelay, config.maxReconnectAttempts]);
  ...
  const unsubscribe = useCallback((msg: Omit<SubscriptionMessage, 'type'>) => {
    ...
    if (msg.tokens) {
      for (const t of msg.tokens) {
        subscriptionsRef.current.delete(`${t.chainId}:${t.address.toLowerCase()}`);
      }
    }
    if (msg.chains) {
      for (const c of msg.chains) {
   ...
   */
  const getPrice = useCallback((chainId: number, address: string): PriceUpdate | undefined => {
   ...
   */
  const fetchPrice = useCallback(async (chainId: number, address: string): Promise<TokenPrice | null> => {
 ...
 */
export function useTokenPrice(chainId: number, address: string): {
  price: PriceUpdate | null;
  isLoading: boolean;
  error: Error | null;
} {
  const { prices, subscribe, unsubscribe, isConnected, error } = usePriceStream();
 ...
 */
export function useTokenPrices(tokens: Array<{ chainId: number; address: string }>): {
  prices: Map<string, PriceUpdate>;
  isLoading: boolean;
  error: Error | null;
} {
  const { prices: allPrices, subscribe, unsubscribe, isConnected, error } = usePriceStream();
  const [isLoading, setIsLoading] = useState(true);
...
export async function fetchTokenPrices(
  ...
  const url = endpoint ?? (process.env.NEXT_PUBLIC_DWS_URL ?? 'http://localhost:4030');

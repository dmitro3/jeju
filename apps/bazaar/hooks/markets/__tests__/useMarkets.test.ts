...
describe('useMarkets Hook', () => {
  ...
  test('should calculate prices for each market', () => {
    ...
    const balancedYesPrice = calculateYesPrice(balancedYes, balancedNo, liquidityB);
    const balancedNoPrice = calculateNoPrice(balancedYes, balancedNo, liquidityB);
  ...
  test('should sort markets by createdAt descending', () => {
    ...
    const sorted = markets.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());

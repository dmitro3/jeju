...
async function gql<T>(
  ...
  // GraphQLResponseSchema validates the structure, and we trust that
  // the data field matches our expected type T when properly queried
...
const mapToken = (c: RawTokenData): Token => {
  ...
  // Validate addresses using schema
  const validatedAddress = AddressSchema.parse(c.address)
  const creatorAddress = c.creator?.address
    ? AddressSchema.parse(c.creator.address)
    : ZERO_ADDRESS

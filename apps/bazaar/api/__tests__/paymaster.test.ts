...
describe('Paymaster Integration', () => {
  ...
  test('should get paymaster options for UI', async () => {
    ...
    if (options.length > 0) {
      ...
      expect(option.estimatedCostFormatted).toContain(
  ...
  test('should mark USDC and JEJU as recommended', async () => {
    ...
    if (options.length > 0) {
      const recommended = options.filter(
        (opt: PaymasterOption) => opt.isRecommended,
      )
      expect(recommended.length).toBeGreaterThanOrEqual(0)

      if (recommended.length > 0) {
        const symbols = recommended.map(
          (opt: PaymasterOption) => opt.paymaster.tokenSymbol,
        )
        expect(
          symbols.some((s: string) => s === 'USDC' || s.includes('eliza')),
        ).toBe(true)
      }
    } else {
  ...
  test('should format estimated cost with correct decimals', async () => {
    ...
    options.forEach((option: PaymasterOption) => {

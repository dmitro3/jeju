...
import { type OttoUser, OttoUserSchema, type Platform } from '../../lib'
import {
  DEFAULT_CHAIN_ID,
  getChainId,
  getChainName,
...
export const swapAction: Action = {
  ...
  handler: async (
    ...
    if (!validation.valid) {
      callback?.({
        text:
          validation.error ??
          'Please specify what to swap. Example: "swap 1 ETH to USDC" or "exchange 100 USDC for ETH on base"',
      })
      return
    }
    ...
    const chainId = params.chain
      ? (getChainId(params.chain) ?? user.settings.defaultChainId)
      : user.settings.defaultChainId
    const fromToken = await getTradingService().getTokenInfo(
      params.from ?? '',
      chainId,
    )
    const toToken = await getTradingService().getTokenInfo(
      params.to ?? '',
      chainId,
    )
  ...
  examples: [
    [
      ...
      { name: 'Otto', content: { text: 'Getting quote...' } },
    ],
  ],
}
...
export const bridgeAction: Action = {
  ...
  handler: async (
    ...
    if (!sourceChainId || !destChainId) {
      callback?.({
        text: `Unknown chain: ${!sourceChainId ? params.fromChain : params.toChain}`,
      })
      return
    }
    ...
    const sourceToken = await getTradingService().getTokenInfo(
      params.token,
      sourceChainId,
    )
    const destToken = await getTradingService().getTokenInfo(
      params.token,
      destChainId,
    )
    ...
    const outputAmount = getTradingService().formatAmount(
      quote.outputAmount,
      destToken.decimals,
    )
    const channelId = getRoomId(message)
...
export const balanceAction: Action = {
  ...
  handler: async (
    ...
    if (balances.length === 0) {
      callback?.({
        text: `No tokens found on ${getChainName(user.settings.defaultChainId)}`,
      })
      return
    }
  ...
  examples: [
    [
      ...
      { name: 'Otto', content: { text: 'Fetching balances...' } },
    ],
  ],
}
...
export const priceAction: Action = {
  ...
  examples: [
    [
      ...
      { name: 'Otto', content: { text: 'ETH: $2500' } },
    ],
  ],
}
...
export const connectAction: Action = {
  ...
  examples: [
    [
      { name: 'user', content: { text: 'connect wallet' } },
      { name: 'Otto', content: { text: 'Connect: https://...' } },
    ],
  ],
}
...
export const confirmAction: Action = {
  ...
  handler: async (
    ...
    if (pending.type === 'swap' && pending.quote) {
      ...
      callback?.({
        text: `Executing swap: ${swapParams.amount} ${swapParams.from} → ${swapParams.to}...`,
      })
    ...
    } else if (pending.type === 'launch' && pending.params) {
      const launchParams = pending.params as LaunchRequest
      callback?.({
        text: `Launching ${launchParams.token.name} ($${launchParams.token.symbol})...`,
      })
      ...
      if (result.success) {
        let socialMsg = ''
        if (result.farcasterCastHash) {
          socialMsg += `\n📣 Farcaster: ${result.farcasterCastHash}`
        }
        if (result.twitterTweetId) {
          socialMsg += `\n🐦 Twitter: ${result.twitterTweetId}`
        }

        callback?.({
          text: `🚀 Token Launched.\n\n📛 ${launchParams.token.name} ($${launchParams.token.symbol})\n📍 Contract: ${result.tokenAddress}\n${result.poolAddress ? `💧 Pool: ${result.poolAddress}` : ''}${result.bondingCurveAddress ? `📈 Bonding Curve: ${result.bondingCurveAddress}` : ''}\n🔗 Tx: ${result.txHash}${socialMsg}`,
        })
      } else {
  ...
  examples: [
    [
      ...
      { name: 'Otto', content: { text: 'Executing...' } },
    ],
  ],
}
...
export const cancelAction: Action = {
  ...
  examples: [
    [
      ...
      { name: 'Otto', content: { text: 'Cancelled.' } },
    ],
  ],
}
...
export const launchAction: Action = {
  name: 'OTTO_LAUNCH',
  description: 'Launch a new token with bonding curve or ICO',
  similes: [
    'launch',
    'create token',
    'new token',
  ...
  handler: async (
    ...
    // Parse launch parameters from natural language
    const params = parseLaunchParams(text)

    if (!params.name || !params.symbol) {
      callback?.({
        text: `**Launch a Token**\n\nUsage: "launch MyToken MTK"\n\nOptions:\n• Add supply: "launch MyToken MTK 1000000"\n• Add liquidity: "launch MyToken MTK 1000000 1ETH"\n• Bonding curve: "launch MyToken MTK bonding"\n• ICO/Presale: "launch MyToken MTK ico"\n\nExample: "launch Moon Token MOON 1000000000 bonding"`,
      })
      return
    }
    ...
    const launchType = params.launchType ?? 'bonding'

    // Build launch request
    const launchRequest: LaunchRequest = {
      userId: user.id,
      walletAddress: user.smartAccountAddress ?? user.primaryWallet,
      ...
      token: {
        ...
        imageUrl:
          params.imageUrl ??
          `https://placehold.co/400x400/png?text=${encodeURIComponent(params.symbol.toUpperCase())}`,
        description: params.description,
        initialSupply: params.supply ?? '1000000000000000000000000000', // 1 billion default
        antiSnipe: false,
        antiSnipeBlocks: 0,
      ...
      launchType,
      bondingConfig:
        launchType === 'bonding' ? DEFAULT_BONDING_CONFIG : undefined,
      icoConfig: launchType === 'ico' ? DEFAULT_ICO_CONFIG : undefined,
      initialLiquidity: params.liquidity,
      social: {
        farcasterEnabled: platform === 'farcaster',
        farcasterCastOnLaunch: platform === 'farcaster',
        twitterEnabled: platform === 'twitter',
        twitterTweetOnLaunch: platform === 'twitter',
        discordEnabled: platform === 'discord',
        telegramEnabled: platform === 'telegram',
      },
    }
    ...
    callback?.({
      text: `**Token Launch Preview**\n\n📛 Name: ${params.name}\n💎 Symbol: $${params.symbol.toUpperCase()}\n📊 Supply: ${params.supply ?? '1B'}\n🎯 Type: ${typeLabel}\n⛓️ Chain: ${getChainName(chainId)}\n\n${launchType === 'bonding' ? '💡 Bonding curve: Price increases as more people buy' : ''}${launchType === 'ico' ? '💡 ICO: Presale with vesting and LP lock' : ''}\n\nReply "confirm" to launch or "cancel" to abort.`,
    })
  },
  examples: [
    [
      { name: 'user', content: { text: 'launch Moon Token MOON' } },
      { name: 'Otto', content: { text: 'Token Launch Preview...' } },
    ],
    [
      {
        name: 'user',
        content: { text: 'create token called Degen DEGEN bonding' },
      },
      { name: 'Otto', content: { text: 'Token Launch Preview...' } },
    ],
  ],
...
export const helpAction: Action = {
  ...
  examples: [
    [
      ...
      {
        name: 'Otto',
        content: { text: 'I can help with swap, bridge, launch...' },
      },
    ],
  ],
}
...
export const ottoWalletProvider: Provider = {
  ...
  get: async (_runtime: IAgentRuntime, message: Memory, _state: State) => {
    ...
    if (!user) {
      return {
        text: 'User not connected. Use "connect wallet" to link your wallet.',
      }
    }
...
export const ottoPlugin: Plugin = {
  name: 'otto',
  description:
    'Otto Trading Agent - Swap, bridge, launch tokens, and manage portfolio',
  actions: [
    swapAction,
    bridgeAction,
    launchAction,

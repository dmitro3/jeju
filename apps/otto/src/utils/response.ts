...
export function createStatusResponse(
  config: {
    ...
    farcaster: { enabled: boolean }
    ai: { enabled: boolean }
    trading: { supportedChains: number[] }
  },
...
export function createChainsResponse(config: {
  trading: { supportedChains: number[]; defaultChainId: number }
}): z.infer<typeof ChainsResponseSchema> {
  const response = {
    chains: config.trading.supportedChains,
...
export function createInfoResponse(config: {
  baseUrl: string
  discord: { applicationId?: string }
  telegram: { token?: string }
}): z.infer<typeof InfoResponseSchema> {
  const response = {
    name: 'Otto',

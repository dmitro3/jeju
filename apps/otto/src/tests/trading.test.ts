...
describe('TradingService', () => {
  ...
  describe('parseAmount', () => {
    ...
    test('parses large amounts', () => {
      expect(service.parseAmount('1000000', 18)).toBe('1000000000000000000000000');
      expect(service.parseAmount('1000000', 6)).toBe('1000000000000');
    });

    test('throws for empty string', () => {
      expect(() => service.parseAmount('', 18)).toThrow('Amount must be a non-empty string');
    });
  ...
  describe('limitOrders', () => {
    ...
    test('throws when cancelling with missing orderId', async () => {
      await expect(service.cancelLimitOrder('', 'user-123')).rejects.toThrow('Order ID and user ID are required');
    });

    test('throws when cancelling with missing userId', async () => {
      await expect(service.cancelLimitOrder('order-123', '')).rejects.toThrow('Order ID and user ID are required');
    });

...
describe('StateManager', () => {
  ...
  describe('chat sessions', () => {
    ...
    test('updates session', () => {
      const manager = getStateManager();
      const walletAddress = '0xabcdef1234567890123456789012345678901234' as const;
      const session = manager.createSession();
  ...
  describe('limit orders', () => {
    ...
    test('retrieves user limit orders', () => {
      ...
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-a', userId: 'user-x', status: 'open' as const });
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-b', userId: 'user-x', status: 'open' as const });
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-c', userId: 'user-y', status: 'open' as const });
    ...
    test('filters out non-open orders from user orders', () => {
      ...
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-open', userId: 'filter-user', status: 'open' as const });
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-filled', userId: 'filter-user', status: 'filled' as const });
      manager.addLimitOrder({ ...baseOrder, orderId: 'order-cancelled', userId: 'filter-user', status: 'cancelled' as const });
    ...
    test('updates limit order', () => {
      ...
      manager.addLimitOrder(order);
      manager.updateLimitOrder('order-update', { status: 'filled', filledAt: Date.now() });

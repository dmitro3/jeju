...
test.describe('Proposal Wizard', () => {
  ...
  test('summary field has character limit', async ({ page }) => {
    const summaryTextarea = page.locator(
      'textarea[placeholder*="1-2 sentence summary"]',
    )
    await expect(summaryTextarea).toBeVisible()

    // Type summary
    await summaryTextarea.fill(
      'This is a test summary that describes what the proposal does.',
    )
  ...
  test('description field shows character count', async ({ page }) => {
    ...
    await descriptionTextarea.fill(
      'This is a detailed description of the proposal. It includes motivation, solution, implementation plan, timeline, and risk assessment.',
    )
  ...
  test('can proceed with valid draft', async ({ page }) => {
    ...
    await page
      .locator('input[placeholder*="Clear, descriptive title"]')
      .fill('Increase Staking Rewards by 5%')
    await page
      .locator('textarea[placeholder*="1-2 sentence summary"]')
      .fill(
        'This proposal aims to increase the staking rewards from 10% to 15% APY to attract more validators and improve network security.',
      )
...
- Mitigation: Gradual rollout over 30 days
  ...
  test('back button returns to previous step', async ({ page }) => {
    ...
    await page
      .locator('textarea[placeholder*="1-2 sentence summary"]')
      .fill('This is a test summary that is at least fifty characters long.')
...
test.describe('Proposal Wizard - Quality Step', () => {
  test('can navigate to quality step', async ({ page }) => {
    ...
    await page
      .locator('input[placeholder*="Clear, descriptive title"]')
      .fill('Increase Staking Rewards')
    await page
      .locator('textarea[placeholder*="1-2 sentence summary"]')
      .fill(
        'This proposal aims to increase staking rewards to improve network security.',
      )
    await page.locator('textarea[placeholder*="Include"]').fill(`
## Problem Statement

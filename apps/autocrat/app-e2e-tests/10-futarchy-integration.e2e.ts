...
const API_BASE = process.env.AUTOCRAT_API_URL || 'http://localhost:8010'
...
test.describe('Futarchy API Integration', () => {
  ...
  test('GET /api/v1/futarchy/market/:proposalId returns 404 for unknown proposal', async () => {
    const response = await request.get(
      '/api/v1/futarchy/market/0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
    )
  ...
  test('GET /api/v1/futarchy/sentiment/:proposalId returns 404 for unknown proposal', async () => {
    const response = await request.get(
      '/api/v1/futarchy/sentiment/0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
    )
...
test.describe('Futarchy UI Integration', () => {
  ...
  test('root endpoint lists futarchy in available endpoints', async ({
    page,
  }) => {
...
test.describe('Futarchy Flow Simulation', () => {
  ...
  test('full escalation flow with deployed contracts', async ({ request }) => {
    ...
    const vetoedResponse = await request.get(
      `${API_BASE}/api/v1/futarchy/vetoed`,
    )
    ...
    const marketResponse = await request.get(
      `${API_BASE}/api/v1/futarchy/market/${proposalId}`,
    )
    ...
    if (marketResponse.ok()) {
      ...
      // 3. Check sentiment
      const sentimentResponse = await request.get(
        `${API_BASE}/api/v1/futarchy/sentiment/${proposalId}`,
      )
      ...
      const escalateResponse = await request.post(
        `${API_BASE}/api/v1/futarchy/escalate`,
        {
          data: { proposalId },
        },
      )
      ...
      if (escalateResult.success) {
        ...
        const newMarketResponse = await request.get(
          `${API_BASE}/api/v1/futarchy/market/${proposalId}`,
        )

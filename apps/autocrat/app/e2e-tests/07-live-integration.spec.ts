...
test.beforeAll(async () => {
  try {
    const response = await fetch(`${AUTOCRAT_API}/health`)
    if (!response.ok) {
      test.skip()
    }
  } catch {
...
test.describe('Live Backend Integration', () => {
  ...
  test('create page assesses proposal with real AI', async ({ page }) => {
    ...
    if (await summaryInput.isVisible()) {
      await summaryInput.fill(
        'Deploy bridge contracts to enable seamless token transfers between Ethereum mainnet and Base L2.',
      )
    }
  ...
  test('proposals page loads and displays list', async ({ page }) => {
    ...
    const proposalCards = page.locator(
      '.proposal-card, [data-testid="proposal-card"]',
    )
    const emptyState = page.getByText(/No proposals|No active proposals/i)

    const hasProposals = await proposalCards
      .first()
      .isVisible()
      .catch(() => false)
  ...
  test('navigation between pages works', async ({ page }) => {
    ...
    await page
      .getByRole('link', { name: /Proposals/i })
      .first()
      .click()

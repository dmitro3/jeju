...
export const daoRoutes = new Elysia({ prefix: '/api/v1/dao' })
  ...
  .post('/', async ({ body }) => {
    ...
    body: t.Object({
      ...
      board: t.Optional(t.Array(t.Object({
        address: t.String(),
        agentId: t.String(),
        role: t.String(),
        weight: t.Number(),
  ...
  .patch('/:daoId', async ({ params, body }) => {
    ...
    params: t.Object({ daoId: t.String() }),
    body: t.Object({
      ceoPersona: t.Optional(t.Object({
        name: t.String(),
        pfpCid: t.Optional(t.String()),
        description: t.String(),
        personality: t.String(),
  ...
  .patch('/:daoId/governance', async ({ params, body }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
  ...
  // Get CEO persona
  .get('/:daoId/persona', async ({ params }) => {
    const service = getService()
  ...
  .get('/:daoId/council', async ({ params }) => {
    const service = getService()
  ...
  .post('/:daoId/agents', async ({ params, body }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
    ...
    const members = await service.getCouncilMembers(params.daoId)
    const newMember = members.find((m) => m.memberAddress.toLowerCase() === body.address.toLowerCase())
  ...
  // Get single agent
  .get('/:daoId/agents/:agentId', async ({ params }) => {
    const service = getService()
    const members = await service.getCouncilMembers(params.daoId)
    const member = members.find((m) => m.agentId.toString() === params.agentId)
  ...
  .patch('/:daoId/agents/:agentId', async ({ params, body }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
  ...
  .delete('/:daoId/agents/:agentId', async ({ params }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
    ...
    const txHash = await service.removeCouncilMember(params.daoId, member.memberAddress as Address)
  ...
  .get('/:daoId/proposals', async ({ params, query }) => {
  ...
  // Get single proposal
  .get('/:daoId/proposals/:proposalId', async ({ params }) => {
  ...
  .post('/:daoId/proposals', async ({ params, body }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
    ...
    if (assessment.overallScore < 60) {
      throw new Error(`Proposal quality too low (${assessment.overallScore}). Improve before submitting.`)
    }
  ...
  // Get treasury data
  .get('/:daoId/treasury', async ({ params }) => {
    const service = getService()
    const exists = await service.daoExists(params.daoId)
    if (!exists) throw new Error('DAO not found')
    ...
    const transactions = await blockchain.getTreasuryTransactions(dao.treasury, 20)

    const totalUsdValue = balances.reduce((sum, b) => sum + Number(b.usdValue), 0)
  ...
  // Get linked packages
  .get('/:daoId/packages', async ({ params }) => {
    const service = getService()

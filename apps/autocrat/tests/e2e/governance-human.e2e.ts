...
const CACHE_FILE = join(
  process.cwd(),
  'test-results',
  'governance-verification-cache.json',
)
...
test.describe('Director Dashboard', () => {
  test('loads with all sections', async ({ page }) => {
    ...
    await verifyScreenshot(
      path,
      'Director Dashboard with pending decisions and proposal list',
      '/director',
    )
  })
  ...
  test('proposal selection shows context', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath(SCREENSHOT_DIR, 'Director-Context'),
      fullPage: true,
    })
  })
  ...
  test('decision form validates input', async ({ page }) => {
    ...
    await page
      .locator('textarea[placeholder*="Explain"]')
      .fill('Valid reasoning for approval.')
...
test.describe('Governance Tab', () => {
  test('loads with filters', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath(SCREENSHOT_DIR, 'Governance-Tab'),
      fullPage: true,
    })
  })
...
test.describe('Treasury Tab', () => {
  test('shows financial data', async ({ page }) => {
    ...
    await verifyScreenshot(
      path,
      'Treasury dashboard with balance and transactions',
      '/dao/*/treasury',
    )
  })
})
...
test.describe('Settings Tab', () => {
  test('shows governance parameters', async ({ page }) => {
    ...
    await page.waitForTimeout(500)
    await page.screenshot({
      path: screenshotPath(SCREENSHOT_DIR, 'Settings-Tab'),
      fullPage: true,
    })
  })
})
...
test.describe('Mobile Responsiveness', () => {
  test('Director Dashboard renders on mobile', async ({ page }) => {
    ...
    await verifyScreenshot(
      path,
      'Mobile Director Dashboard with stacked layout',
      '/director/mobile',
    )
  })
  ...
  test('Governance tab renders on mobile', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath(SCREENSHOT_DIR, 'Mobile-Governance'),
      fullPage: true,
    })
  })
})

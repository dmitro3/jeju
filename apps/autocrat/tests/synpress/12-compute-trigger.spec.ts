...
test.describe('Compute Trigger', () => {
  ...
  test('GET trigger history returns executions', async ({ request }) => {
    const response = await request.get(`${AUTOCRAT_URL}/api/v1/triggers/history`);
  ...
  test('POST trigger execute runs cycle manually', async ({ request }) => {
    const response = await request.post(`${AUTOCRAT_URL}/api/v1/triggers/execute`);
  ...
  test('trigger can be called multiple times', async ({ request }) => {
    // First call
    const response1 = await request.post(`${AUTOCRAT_URL}/trigger/orchestrator`);
    ...
    // Second call
    const response2 = await request.post(`${AUTOCRAT_URL}/trigger/orchestrator`);

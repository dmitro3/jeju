...
test.describe('ElizaOS Integration', () => {
  test('health reports service status', async ({ request }) => {
    ...
    expect(data.service).toBe('jeju-autocrat');
  ...
  test('agent card includes A2A capabilities', async ({ request }) => {
    const response = await request.get(`${AUTOCRAT_URL}/a2a/.well-known/agent-card.json`);
    expect(response.ok()).toBeTruthy();
  ...
  test('multiple skills can be called sequentially', async ({ request }) => {
    // First call
    const response1 = await request.post(`${AUTOCRAT_URL}/a2a`, {
      data: {
        jsonrpc: '2.0',
        id: 1,
        method: 'message/send',
    ...
    // Second call
    const response2 = await request.post(`${AUTOCRAT_URL}/a2a`, {
      data: {
        jsonrpc: '2.0',
        id: 2,
        method: 'message/send',

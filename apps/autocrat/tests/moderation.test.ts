...
function updateModeratorStatsOnResolution(
  ...
  newStats.accuracy =
    (newStats.flagsUpheld / Math.max(1, newStats.flagsUpheld + newStats.flagsRejected)) * 100;

  return newStats;
}

function shouldAutoReject(score: ModerationScore): { reject: boolean; reason?: string } {
  if (score.visibilityScore < 10) {
    const top = score.flags.sort((a, b) => WEIGHT[b.flagType] - WEIGHT[a.flagType])[0];
    return { reject: true, reason: top?.reason ?? 'Too many flags' };
  }

  if (
    score.flags.filter((f) => f.flagType === FlagType.SPAM && f.upvotes > f.downvotes * 2)
      .length >= 3
  ) {
    return { reject: true, reason: 'Multiple spam flags' };
  }

  if (score.flags.some((f) => f.flagType === FlagType.HARMFUL && f.upvotes > 10)) {
...
describe('Moderation System', () => {
  describe('Visibility Score Calculation', () => {
    ...
    test('HARMFUL flag has highest weight', () => {
      ...
      const spamScore = calculateVisibilityScore(spamFlags, getDefaultModeratorStats);
      const harmfulScore = calculateVisibilityScore(harmfulFlags, getDefaultModeratorStats);

      expect(harmfulScore.visibilityScore).toBeLessThan(spamScore.visibilityScore);
    });
    ...
    test('upvotes increase flag weight', () => {
      ...
      const withUpvotes = [
        createFlag('prop-1', 'mod-1', FlagType.SPAM, { upvotes: 10, downvotes: 0 }),
      ];

      const noVotesScore = calculateVisibilityScore(noVotes, getDefaultModeratorStats);
      const upvotesScore = calculateVisibilityScore(withUpvotes, getDefaultModeratorStats);

      expect(upvotesScore.visibilityScore).toBeLessThan(noVotesScore.visibilityScore);
    });

    test('downvotes decrease flag weight', () => {
      const noVotes = [createFlag('prop-1', 'mod-1', FlagType.SPAM)];
      const withDownvotes = [
        createFlag('prop-1', 'mod-1', FlagType.SPAM, { upvotes: 0, downvotes: 10 }),
      ];

      const noVotesScore = calculateVisibilityScore(noVotes, getDefaultModeratorStats);
      const downvotesScore = calculateVisibilityScore(withDownvotes, getDefaultModeratorStats);

      expect(downvotesScore.visibilityScore).toBeGreaterThan(noVotesScore.visibilityScore);
    });
  ...
  describe('Moderator Stats Update on Resolution', () => {
    test('upheld flag increases reputation', () => {
      ...
      const updated = updateModeratorStatsOnResolution(initial, FlagType.SPAM, true);
    ...
    test('rejected flag decreases reputation', () => {
      const initial = getDefaultModeratorStats('mod-1');
      const updated = updateModeratorStatsOnResolution(initial, FlagType.SPAM, false);
  ...
  describe('Auto-Rejection Logic', () => {
    test('visibility below 10 triggers auto-reject', () => {
      const score: ModerationScore = {
        proposalId: 'prop-1',
        visibilityScore: 5,
        flags: [createFlag('prop-1', 'mod-1', FlagType.HARMFUL, { reason: 'Very harmful' })],
    ...
    test('3+ upvoted spam flags trigger auto-reject', () => {
      const score: ModerationScore = {
        ...
        visibilityScore: 50,
        flags: [
          createFlag('prop-1', 'mod-1', FlagType.SPAM, { upvotes: 5, downvotes: 1 }),
          createFlag('prop-1', 'mod-2', FlagType.SPAM, { upvotes: 5, downvotes: 1 }),
          createFlag('prop-1', 'mod-3', FlagType.SPAM, { upvotes: 5, downvotes: 1 }),
        ],
    ...
    test('harmful flag with >10 upvotes triggers auto-reject', () => {
      const score: ModerationScore = {
        proposalId: 'prop-1',
        visibilityScore: 50,
        flags: [createFlag('prop-1', 'mod-1', FlagType.HARMFUL, { upvotes: 15, downvotes: 0 })],
        trustWeightedFlags: 50,
        recommendation: 'REVIEW',
      };
    ...
    test('harmful flag with exactly 10 upvotes does not trigger', () => {
      const score: ModerationScore = {
        proposalId: 'prop-1',
        visibilityScore: 50,
        flags: [createFlag('prop-1', 'mod-1', FlagType.HARMFUL, { upvotes: 10, downvotes: 0 })],
        trustWeightedFlags: 50,
        recommendation: 'REVIEW',
      };

...
function calculateQuadraticAllocation(
  ...
  const matchingShare = (matchingPool * quadraticScore) / (quadraticScore * BigInt(totalProjects) + 1n);
...
describe('FundingOracle', () => {
  ...
  describe('calculateQuadraticAllocation', () => {
    const mockStaker = '0x1234567890abcdef1234567890abcdef12345678' as `0x${string}`;
    ...
    test('multiple equal stakes benefits from quadratic funding', () => {
      ...
      const fourStakers = [
        { staker: '0x0000000000000000000000000000000000000001' as `0x${string}`, amount: parseEther('1') },
        { staker: '0x0000000000000000000000000000000000000002' as `0x${string}`, amount: parseEther('1') },
        { staker: '0x0000000000000000000000000000000000000003' as `0x${string}`, amount: parseEther('1') },
        { staker: '0x0000000000000000000000000000000000000004' as `0x${string}`, amount: parseEther('1') },
      ];
      const oneStaker = [{ staker: mockStaker, amount: parseEther('4') }];

      const matchingPool = parseEther('100');
      const quadratic4 = calculateQuadraticAllocation(fourStakers, matchingPool, 2);
      const quadratic1 = calculateQuadraticAllocation(oneStaker, matchingPool, 2);
    ...
    test('more stakers = more matching funds (quadratic property)', () => {
      ...
      const allocationA = calculateQuadraticAllocation(projectA, matchingPool, 2);
      const allocationB = calculateQuadraticAllocation(projectB, matchingPool, 2);
  ...
  describe('calculateHeuristicWeight', () => {
    ...
    test('base weight from stake (capped at 2000)', () => {
      expect(calculateHeuristicWeight(10, false, false, MAX_CEO_WEIGHT)).toBe(1000);
      expect(calculateHeuristicWeight(20, false, false, MAX_CEO_WEIGHT)).toBe(2000);
      expect(calculateHeuristicWeight(30, false, false, MAX_CEO_WEIGHT)).toBe(2000); // Capped
    });

    test('linked project adds 1000 weight', () => {
      expect(calculateHeuristicWeight(0, true, false, MAX_CEO_WEIGHT)).toBe(1000);
      expect(calculateHeuristicWeight(10, true, false, MAX_CEO_WEIGHT)).toBe(2000);
    });

    test('funded project adds 500 weight', () => {
      expect(calculateHeuristicWeight(0, false, true, MAX_CEO_WEIGHT)).toBe(500);
      expect(calculateHeuristicWeight(10, false, true, MAX_CEO_WEIGHT)).toBe(1500);
    });

    test('combined bonuses stack', () => {
      // stake: 10 ETH = 1000, linked: +1000, funded: +500 = 2500
      expect(calculateHeuristicWeight(10, true, true, MAX_CEO_WEIGHT)).toBe(2500);
    });
    ...
    test('fractional stake values work', () => {
      expect(calculateHeuristicWeight(0.5, false, false, MAX_CEO_WEIGHT)).toBe(50);
      expect(calculateHeuristicWeight(1.5, false, false, MAX_CEO_WEIGHT)).toBe(150);
    });
  });
  ...
  describe('Quadratic Funding Property Tests', () => {
    ...
    test('splitting a stake increases quadratic score (core QF property)', () => {
      ...
      const manyContributors = Array(100).fill(null).map(() => ({ amount: 10n ** 18n }));
    ...
    test('doubling all stakes quadruples the score (sqrt property)', () => {
      ...
      const doubledScore = calculateQuadraticAllocation(doubled, matchingPool, 1);

...
/** Type guard for agent object with role */
function isAgentWithRole(obj: JsonValue): obj is { role: string } {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    !Array.isArray(obj) &&
...
test.describe('Council Status', () => {
  test('get-autocrat-status returns all council roles', async ({ request }) => {
    ...
    const agentsWithRole = agents.filter(isAgentWithRole)
    expect(agentsWithRole.length).toBe(4)

    const roles = agentsWithRole.map((a) => a.role)

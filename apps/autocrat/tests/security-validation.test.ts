...
function createSandboxResult(overrides: Partial<SandboxResult> = {}): SandboxResult {
  return {
    success: true,
...
describe('SecurityValidationAgent', () => {
  describe('calculateConfidence', () => {
    test('base confidence is 50 without static analysis or sandbox', () => {
      const result = calculateConfidence({ isLikelyValid: false, notes: [] }, null);
      // 50 - 20 (not valid) = 30
      expect(result).toBe(30);
    });

    test('static analysis likely valid adds 20', () => {
      const result = calculateConfidence({ isLikelyValid: true, notes: [] }, null);
    ...
    test('successful sandbox execution adds 10', () => {
      const sandbox = createSandboxResult({ success: true, exploitTriggered: false });
      const result = calculateConfidence({ isLikelyValid: true, notes: [] }, sandbox);
      // 50 + 20 + 10 = 80
      expect(result).toBe(80);
    });

    test('failed sandbox execution subtracts 10', () => {
      const sandbox = createSandboxResult({ success: false, exploitTriggered: false });
      const result = calculateConfidence({ isLikelyValid: true, notes: [] }, sandbox);
      // 50 + 20 - 10 = 60
    ...
    test('exploit triggered sets confidence to at least 90', () => {
      const sandbox = createSandboxResult({ success: true, exploitTriggered: true });

      // Even with negative static analysis, exploit confirmation overrides
      const result = calculateConfidence({ isLikelyValid: false, notes: [] }, sandbox);
  ...
  describe('determineResult', () => {
    test('exploit triggered = VALID regardless of confidence', () => {
      ...
      const result = determineResult({ isLikelyValid: false, notes: [] }, sandbox, 10);
  ...
  describe('assessSeverity', () => {
    test('FUNDS_AT_RISK with exploit = CRITICAL', () => {
      expect(
        assessSeverity(BountySeverity.MEDIUM, VulnerabilityType.FUNDS_AT_RISK, true)
      ).toBe(BountySeverity.CRITICAL);
    });

    test('WALLET_DRAIN with exploit = CRITICAL', () => {
      expect(
        assessSeverity(BountySeverity.LOW, VulnerabilityType.WALLET_DRAIN, true)
      ).toBe(BountySeverity.CRITICAL);
    });

    test('REMOTE_CODE_EXECUTION with exploit = CRITICAL', () => {
      expect(
        assessSeverity(BountySeverity.MEDIUM, VulnerabilityType.REMOTE_CODE_EXECUTION, true)
      ).toBe(BountySeverity.CRITICAL);
    });

    test('TEE_BYPASS with exploit = HIGH', () => {
      expect(
        assessSeverity(BountySeverity.MEDIUM, VulnerabilityType.TEE_BYPASS, true)
      ).toBe(BountySeverity.HIGH);
    });

    test('MPC_KEY_EXPOSURE with exploit = HIGH', () => {
      expect(
        assessSeverity(BountySeverity.LOW, VulnerabilityType.MPC_KEY_EXPOSURE, true)
      ).toBe(BountySeverity.HIGH);
    });

    test('CONSENSUS_ATTACK with exploit = HIGH', () => {
      expect(
        assessSeverity(BountySeverity.MEDIUM, VulnerabilityType.CONSENSUS_ATTACK, true)
      ).toBe(BountySeverity.HIGH);
    });

    test('without exploit, uses claimed severity', () => {
      expect(
        assessSeverity(BountySeverity.LOW, VulnerabilityType.FUNDS_AT_RISK, false)
    ...
    test('other vuln types with exploit use claimed severity', () => {
      expect(
        assessSeverity(BountySeverity.MEDIUM, VulnerabilityType.DATA_LEAK, true)
  ...
  describe('Reward Calculation Edge Cases', () => {
    ...
    test('reward ordering is consistent', () => {
      ...
      for (const confidence of [25, 50, 75, 100]) {
        ...
        const critical = calculateReward(BountySeverity.CRITICAL, confidence, false);

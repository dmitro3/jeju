...
// ============ Pure Functions for Testing (extracted from encryption.ts) ============
...
async function deriveKey(encryptionKey: string, policyHash: string, extractable = false): Promise<CryptoKey> {
...
async function deriveKeyExtractable(encryptionKey: string, policyHash: string): Promise<CryptoKey> {
...
describe('Encryption Module', () => {
  describe('Access Control Conditions', () => {
    ...
    test('second condition is timestamp check for 30 days', () => {
      ...
      const thirtyDaysLater = encryptedAt + 30 * 24 * 60 * 60;
      expect(timestampCondition.returnValueTest.value).toBe(thirtyDaysLater.toString());
    });
  ...
  describe('Encrypt/Decrypt Round Trip', () => {
    test('can encrypt and decrypt simple string', async () => {
      ...
      const { ciphertext, iv, tag } = await encrypt(plaintext, policyHash, TEST_ENCRYPTION_KEY);
      const decrypted = await decrypt(ciphertext, iv, tag, policyHash, TEST_ENCRYPTION_KEY);
    ...
    test('wrong policy hash fails to decrypt', async () => {
      ...
      const { ciphertext, iv, tag } = await encrypt(plaintext, correctHash, TEST_ENCRYPTION_KEY);
    ...
    test('wrong encryption key fails to decrypt', async () => {
      ...
      const { ciphertext, iv, tag } = await encrypt(plaintext, policyHash, 'key-1');

      await expect(decrypt(ciphertext, iv, tag, policyHash, 'key-2')).rejects.toThrow();
    });
  ...
  describe('Policy Hash', () => {
    ...
    test('different proposal IDs produce different policy hashes', () => {
      ...
      expect(computePolicyHash(conditions1)).not.toBe(computePolicyHash(conditions2));
    });
  ...
  describe('IV and Tag Format', () => {
    ...
    test('ciphertext is hex encoded', async () => {
      const { ciphertext } = await encrypt('test', '0xpolicy', TEST_ENCRYPTION_KEY);

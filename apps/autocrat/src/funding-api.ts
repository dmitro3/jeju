...
export const fundingApi = {
  ...
  async getEpochVotes(daoId: string, epochId: number): Promise<ApiResult<WeightVote[]>> {
    const service = getDeepFundingService();
    const votes = await service.getEpochVotes(daoId, epochId);
    return { success: true, data: votes };
  },

  async getDAOFundingConfig(daoId: string): Promise<ApiResult<FeeDistributionConfig>> {
    const service = getDeepFundingService();
  ...
  async getContributor(contributorId: string): Promise<ApiResult<ContributorProfile>> {
  ...
  async getContributorByWallet(wallet: Address): Promise<ApiResult<ContributorProfile>> {
    const service = getContributorService();
  ...
  async getContributorProfile(contributorId: string): Promise<ApiResult<{
    ...
    return { success: true, data: { profile, socialLinks, repoClaims, depClaims } };
  },

  async getPendingContributorRewards(daoId: string, contributorId: string): Promise<ApiResult<bigint>> {
    const service = getDeepFundingService();
    const rewards = await service.getPendingContributorRewards(daoId, contributorId);
  ...
  async getContributorShare(
    ...
    const service = getDeepFundingService();
    const share = await service.getContributorShare(daoId, epochId, contributorId);
    if (!share) {
  ...
  async getDependencyShare(daoId: string, depHash: string): Promise<ApiResult<DependencyShare>> {
    const service = getDeepFundingService();
  ...
  async scanRepository(owner: string, repo: string): Promise<ApiResult<ScanResult>> {
    const scanner = getDependencyScanner();
    const result = await scanner.scanRepository(owner, repo);
    return { success: true, data: result };
  },

  async generateContributorRecommendations(daoId: string): Promise<ApiResult<Array<{
    contributorId: string;
    contributorProfile: ContributorProfile | null;
    suggestedWeight: number;
    reason: string;
    contributions: { bounties: number; paymentRequests: number; repos: number; deps: number };
  }>>> {
    const service = getDeepFundingService();
    const recommendations = await service.generateContributorRecommendations(daoId);
  ...
  async generateDependencyRecommendations(
    ...
    const service = getDeepFundingService();
    const recommendations = await service.generateDependencyRecommendations(daoId, owner, repo);
    return { success: true, data: recommendations };
  },
  ...
  async getPendingPaymentRequests(daoId: string): Promise<ApiResult<PaymentRequest[]>> {
    const service = getPaymentRequestService();
  ...
  async getCEODecision(requestId: string): Promise<ApiResult<CEODecision | null>> {
    const service = getPaymentRequestService();
  ...
  async voteOnWeight(
    ...
    try {
      const service = getDeepFundingService();
      const hash = await service.voteOnWeight(daoId, targetId, adjustment, reason, reputation);
      return { success: true, data: hash };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      if (message.includes('Wallet client required')) {
        return { success: false, error: 'Wallet not connected. This operation requires a wallet.' };
      }
   ...
   */
  async depositFees(daoId: string, source: string, amount: bigint): Promise<ApiResult<Hash>> {
    try {
      const service = getDeepFundingService();
  ...
  async claimContributorRewards(
    ...
    try {
      const service = getDeepFundingService();
      const hash = await service.claimContributorRewards(daoId, contributorId, epochs, recipient);
      return { success: true, data: hash };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      if (message.includes('Wallet client required')) {

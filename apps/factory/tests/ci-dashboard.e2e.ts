...
test.describe('Build Detail', () => {
  ...
  test('lists jobs', async ({ page }) => {
    ...
    const jobButtons = page.locator('button:has-text("Build"), button:has-text("Test"), button:has-text("Deploy")');
  ...
  test('copies logs to clipboard', async ({ page }) => {
    ...
    if ((await copyButton.count()) > 0) {
      ...
      await expect(page.locator('[data-lucide="check"]')).toBeVisible({ timeout: 3000 });
    }
  });
});
...
test.describe('Build Actions', () => {
  test('shows cancel button for running builds', async ({ page }) => {
    ...
    const runningBuild = page.locator('[href^="/ci/"]:has(.animate-spin)').first();
...
test.describe('Artifacts', () => {
  test('displays artifacts section when present', async ({ page }) => {
    ...
    if ((await artifactsSection.count()) > 0) {
      ...
      const downloadButton = page.locator('button:has([data-lucide="download"])');

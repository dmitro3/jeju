...
async function startLocalBlockchain(): Promise<void> {
  ...
  // Verify it's running
  const { stdout } = await execAsync(`curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' ${RPC_URL}`);
...
async function deployContracts(): Promise<Record<string, string>> {
  ...
  const { stdout } = await execAsync('cd /home/shaw/Documents/jeju && bun run scripts/deploy-contracts.ts --network localnet');
  
  // Parse deployed addresses from output
  const addresses: Record<string, string> = {};
  const addressMatches = stdout.matchAll(/(\w+) deployed at: (0x[a-fA-F0-9]{40})/g);
...
async function startDevServer(): Promise<void> {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const result = await execAsync('curl -s http://localhost:3000').catch(() => ({ stdout: '' }));
...
async function verifyOnChainState(contracts: Record<string, string>): Promise<VerificationResult[]> {
...
async function runJejuCLIVerification(): Promise<VerificationResult[]> {
  ...
  for (const test of cliTests) {
    ...
    verifications.push({
      ...
      verified: result.stdout.includes(test.expected) || !('stderr' in result && result.stderr),
    });
  }
...
async function printSummary(report: ValidationReport): Promise<void> {
  ...
  if (report.tests.failed > 0) {
    console.log('\nFailed Tests:');
    for (const test of report.tests.results.filter((t) => t.status === 'failed')) {
  ...
  console.log(`Contracts Deployed: ${Object.keys(report.blockchain.contracts).length}`);
...
async function main(): Promise<void> {
  ...
  const report = await generateReport(testResults, contracts, verifications, startTime);

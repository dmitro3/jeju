...
export function VerificationStatusBadge({ status }: { status: VerificationStatus }) {
  return (
    <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${VERIFICATION_STATUS_STYLES[status]}`}>
      {status}
    </span>
  );
}
...
export function PaymentStatusBadge({ status }: { status: PaymentRequestStatus }) {
  ...
  return (
    <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${style.bgClass} ${style.textClass}`}>
      {style.label}
    </span>
  );
}
...
export function StatusBadge({ 
  ...
  return (
    <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${VARIANT_STYLES[variant]}`}>
      {label}
    </span>
  );
}
...
export function ContributorTypeBadge({ type }: { type: ContributorType }) {
  return (
    <span className={`px-2 py-0.5 text-xs font-medium rounded ${CONTRIBUTOR_TYPE_STYLES[type]}`}>
      {type}
    </span>
  );
}
...
export function RegistryTypeBadge({ type }: { type: RegistryType }) {
  return (
    <span className={`px-1.5 py-0.5 text-xs font-medium rounded ${REGISTRY_TYPE_STYLES[type]}`}>
      {type}
    </span>
  );
}

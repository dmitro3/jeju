...
export function useDAOPool(daoId: string | undefined) {
  ...
  const pool: DAOPool | null = data && (data as [string])[0] !== '0x' + '0'.repeat(64) ? {
    ...
    dependencyPool: (data as [string, Address, bigint, bigint, bigint])[4],
    reservePool: (data as [string, Address, bigint, bigint, bigint, bigint])[5],
    lastDistributedEpoch: Number((data as [string, Address, bigint, bigint, bigint, bigint, bigint])[6]),
    epochStartTime: Number((data as [string, Address, bigint, bigint, bigint, bigint, bigint, bigint])[7]),
  } : null;
...
export function useCurrentEpoch(daoId: string | undefined) {
  ...
  const epoch: FundingEpoch | null = data && (data as [bigint])[0] !== 0n ? {
    ...
    totalContributorRewards: (data as [bigint, string, bigint, bigint, bigint])[4],
    totalDependencyRewards: (data as [bigint, string, bigint, bigint, bigint, bigint])[5],
    totalDistributed: (data as [bigint, string, bigint, bigint, bigint, bigint, bigint])[6],
    finalized: (data as [bigint, string, bigint, bigint, bigint, bigint, bigint, boolean])[7],
  } : null;

  return { epoch, isLoading, error, refetch };
}

export function useEpoch(daoId: string | undefined, epochId: number | undefined) {
  const { data, isLoading, error, refetch } = useReadContract({
    address: getAddress(),
    abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
    functionName: 'getEpoch',
    args: daoId && epochId !== undefined ? [daoId as `0x${string}`, BigInt(epochId)] : undefined,
...
export function useContributorShare(daoId: string | undefined, epochId: number | undefined, contributorId: string | undefined) {
  const { data, isLoading, error, refetch } = useReadContract({
    address: getAddress(),
    abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
    functionName: 'getContributorShare',
    args: daoId && epochId !== undefined && contributorId 
      ? [daoId as `0x${string}`, BigInt(epochId), contributorId as `0x${string}`] 
      : undefined,
    query: { enabled: !!daoId && epochId !== undefined && !!contributorId },
  });

  const share: ContributorShare | null = data && (data as [string])[0] !== '0x' + '0'.repeat(64) ? {
    ...
    lastClaimEpoch: Number((data as [string, bigint, bigint, bigint, bigint])[4]),
  } : null;

  return { share, isLoading, error, refetch };
}

export function useDependencyShare(daoId: string | undefined, depHash: string | undefined) {
  const { data, isLoading, error, refetch } = useReadContract({
    address: getAddress(),
    abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
    functionName: 'getDependencyShare',
    args: daoId && depHash ? [daoId as `0x${string}`, depHash as `0x${string}`] : undefined,
    query: { enabled: !!daoId && !!depHash },
  });

  const share: DependencyShare | null = data && (data as [string])[0] !== '0x' + '0'.repeat(64) ? {
    depHash: (data as [string])[0],
    contributorId: (data as [string, string])[1],
    weight: Number((data as [string, string, bigint])[2]),
    transitiveDepth: Number((data as [string, string, bigint, bigint])[3]),
    usageCount: Number((data as [string, string, bigint, bigint, bigint])[4]),
    pendingRewards: (data as [string, string, bigint, bigint, bigint, bigint])[5],
    claimedRewards: (data as [string, string, bigint, bigint, bigint, bigint, bigint])[6],
    isRegistered: (data as [string, string, bigint, bigint, bigint, bigint, bigint, boolean])[7],
  } : null;
...
export function useDAOFundingConfig(daoId: string | undefined) {
  ...
  const config: FeeDistributionConfig | null = data ? {
    ...
    reserveBps: Number((data as [bigint, bigint, bigint, bigint, bigint, bigint])[5]),
  } : null;

  return { config, isLoading, error };
}

export function useEpochVotes(daoId: string | undefined, epochId: number | undefined) {
  const { data, isLoading, error, refetch } = useReadContract({
    address: getAddress(),
    abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
...
export function usePendingContributorRewards(daoId: string | undefined, contributorId: string | undefined) {
  const { data, isLoading, error, refetch } = useReadContract({
    address: getAddress(),
    abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
    functionName: 'getPendingContributorRewards',
    args: daoId && contributorId ? [daoId as `0x${string}`, contributorId as `0x${string}`] : undefined,
...
export function useVoteOnWeight() {
  ...
  const vote = (daoId: string, targetId: string, adjustment: number, reason: string, reputation: number) => {
    writeContract({
      address: getAddress(),
      abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
      functionName: 'voteOnWeight',
      args: [daoId as `0x${string}`, targetId as `0x${string}`, BigInt(adjustment), reason, BigInt(reputation)],
    });
  };
...
export function useClaimContributorRewards() {
  ...
  const claim = (daoId: string, contributorId: string, epochs: number[], recipient: Address) => {
    writeContract({
      address: getAddress(),
      abi: DEEP_FUNDING_DISTRIBUTOR_ABI,
      functionName: 'claimContributorRewards',
      args: [daoId as `0x${string}`, contributorId as `0x${string}`, epochs.map(BigInt), recipient],
    });
  };

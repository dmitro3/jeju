...
import type {
  ContainerImage as BaseContainerImage,
...
export interface DWSContainerImage
  extends Omit<BaseContainerImage, 'id' | 'downloads'> {
  architecture: string
  os: string
  pushedAt: number
  manifestUri: string
...
class DecentralizedDWSClient {
  ...
  private async request<T>(
    ...
    if (!response?.ok && this.nodes.size > 1) {
      ...
      for (const node of nodes) {
        ...
        response = await fetch(failoverUrl, { ...options, headers }).catch(
          () => null,
        )
  ...
  async createTrainingJob(params: {
    ...
    return this.request<ComputeJob>(
      '/compute/training',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async runInference(
    ...
    return this.request<InferenceResult>(
  ...
  async triggerWorkflow(params: {
    ...
    return this.request<CIWorkflow>(

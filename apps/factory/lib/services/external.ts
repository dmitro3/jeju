...
export class GitHubService {
  ...
  async syncRepoToJeju(
    owner: string,
    repo: string,
  ): Promise<{ success: boolean; jejuRepoId?: string }> {
    const repoData = await this.getRepo(owner, repo)

    const response = await fetch(
      `${process.env.NEXT_PUBLIC_DWS_URL}/api/git/import`,
      {
        method: 'POST',
...
export class LinearService {
  ...
  async createIssue(params: {
    ...
    `,
      { input: params },
    )) as { issueCreate: { issue: LinearIssue } }
  ...
  async syncIssueToJeju(
    issueId: string,
  ): Promise<{ success: boolean; jejuTaskId?: string }> {
...
export class NpmService {
  ...
  async mirrorToJeju(
    name: string,
    version?: string,
  ): Promise<{ success: boolean; jejuPackageId?: string }> {
    const pkg = await this.getPackage(name)
    ...
    formData.append(
      'tarball',
      tarball,
      `${name.replace('/', '-')}-${targetVersion}.tgz`,
    )
    formData.append(
      'metadata',
      JSON.stringify({
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_DWS_URL}/api/packages`,
      {
        method: 'POST',
        body: formData,
      },

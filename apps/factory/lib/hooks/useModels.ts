...
export function useModel(organization: string, name: string) {
  ...
  const loadModel = useCallback(async () => {
    ...
    const response = await fetch(
      `${dwsUrl}/models/${organization}/${name}`,
    ).catch((err: Error) => {
      setError(err)
      return null
    })
...
export function useModelActions() {
  ...
  const uploadFiles = useCallback(
    async (organization: string, name: string, files: File[]) => {
      setIsPending(true)
      const dwsUrl = process.env.NEXT_PUBLIC_DWS_URL || 'http://localhost:4030'
      ...
      const response = await fetch(
        `${dwsUrl}/models/${organization}/${name}/upload`,
        {
          method: 'POST',
          body: formData,
        },
  ...
  const publishVersion = useCallback(
    async (
      ...
      const response = await fetch(
        `${dwsUrl}/models/${organization}/${name}/versions`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const toggleStar = useCallback(async (organization: string, name: string) => {
    ...
    const response = await fetch(
      `${dwsUrl}/models/${organization}/${name}/star`,
      {
        method: 'POST',
      },
    )
  ...
  const runInference = useCallback(
    async (
      ...
      const response = await fetch(
        `${dwsUrl}/models/${organization}/${name}/inference`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
export function useDatasets(params?: {
  ...
  const loadDatasets = useCallback(async () => {
    ...
    const response = await fetch(`${dwsUrl}/datasets?${searchParams}`).catch(
      (err: Error) => {
        setError(err)
        return null
      },
...
export function useDataset(organization: string, name: string) {
  ...
  const [files, setFiles] = useState<
    { filename: string; cid: string; size: number; split?: string }[]
  >([])
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<Error | null>(null)
  ...
  const loadDataset = useCallback(async () => {
    ...
    const response = await fetch(
      `${dwsUrl}/datasets/${organization}/${name}`,
    ).catch((err: Error) => {
      setError(err)
      return null
    })

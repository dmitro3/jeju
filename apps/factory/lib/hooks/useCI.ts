...
export function useWorkflows(repoId: string) {
  ...
  const fetch = useCallback(async () => {
    ...
    const response = await globalThis.fetch(
      `${DWS_API_URL}/ci/workflows/${repoId}`,
    )
...
export function useWorkflowRuns(
  repoId: string,
  options?: { limit?: number; branch?: string; status?: string },
) {
  ...
  const fetchRuns = useCallback(async () => {
    ...
    const response = await globalThis.fetch(
      `${DWS_API_URL}/ci/repos/${repoId}/runs?${params}`,
    )
...
export function useRunLogs(
  runId: string,
  options?: { jobId?: string; stepId?: string },
) {
  ...
  useEffect(() => {
    ...
    globalThis
      .fetch(`${DWS_API_URL}/ci/runs/${runId}/logs?${params}`)
      .then((r) => r.json())
      .then((data) => {
        if (!isMountedRef.current) return
        // Limit initial logs to MAX_LOG_ENTRIES
        const initialLogs = Array.isArray(data.logs)
    ...
    const eventSource = new EventSource(
      `${DWS_API_URL}/ci/runs/${runId}/logs/stream`,
    )
...
export function useCIActions() {
  ...
  const triggerWorkflow = useCallback(
    async (
      ...
      const response = await globalThis.fetch(
        `${DWS_API_URL}/ci/runs/${repoId}/${workflowId}`,
        {
          method: 'POST',
          headers: {
  ...
  const cancelRun = useCallback(
    async (runId: string) => {
      const response = await globalThis.fetch(
        `${DWS_API_URL}/ci/runs/${runId}/cancel`,
        {
          method: 'POST',
  ...
  const rerunWorkflow = useCallback(
    async (runId: string) => {
      const run = await globalThis
        .fetch(`${DWS_API_URL}/ci/runs/${runId}`)
        .then((r) => r.json())
...
export function useSecrets(repoId: string) {
  ...
  const deleteSecret = useCallback(
    async (secretId: string) => {
      const response = await globalThis.fetch(
        `${DWS_API_URL}/ci/secrets/${secretId}`,
        {
          method: 'DELETE',
          headers: { 'x-jeju-address': address || '' },
        },
...
export function useEnvironments(repoId: string) {
  ...
  const fetchEnvironments = useCallback(async () => {
    const response = await globalThis.fetch(
      `${DWS_API_URL}/ci/environments/${repoId}`,
    )
...
export function useArtifacts(runId: string) {
  ...
  const downloadArtifact = useCallback(
    async (name: string) => {
      const response = await globalThis.fetch(
        `${DWS_API_URL}/ci/artifacts/${runId}/${name}`,
      )

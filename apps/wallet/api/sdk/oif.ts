...
import { OIFQuoteResponseSchema } from '../../lib/api-responses'
import { jsonToHex } from '../../lib/buffer'
...
const SWAP_ORDER_TYPE: Hex =
  '0x43726f7373436861696e53776170000000000000000000000000000000000000'
...
export class OIFClient {
  ...
  async createGaslessIntent(
    ...
    // Generate intent ID (browser-safe, no Buffer)
    const intentId =
      `0x${jsonToHex({ order, nonce: nonce.toString() }).slice(0, 64)}` as Hex

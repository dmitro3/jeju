...
const mockFetch = mock((input: RequestInfo | URL) => {
  ...
  if (url.includes('mychain') || url.includes('chain1'))
    chainIdResult = '0x3039' // 12345
  if (url.includes('duplicate')) chainIdResult = '0x3039'

  return Promise.resolve(
    new Response(JSON.stringify({ result: chainIdResult }), {
      status: 200,
...
describe('CustomRPCService', () => {
  ...
  describe('testRPC', () => {
    it('should test RPC health', async () => {
      const isHealthy = await customRPCService.testRPC(

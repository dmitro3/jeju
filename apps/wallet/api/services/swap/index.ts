...
import { EMPTY_CALLDATA } from '../../guards'
import type { Token } from '../../sdk/types'
import { isSupportedChainId, type SupportedChainId } from '../rpc'
...
interface TransactionSigner {
  signTransaction: (tx: TransactionData) => Promise<Hex>
}
...
class SwapService {
  ...
  async getCrossChainQuote(
    ...
    return validated.map((q) => {
      if (!isSupportedChainId(q.sourceChainId)) {
        throw new Error(`Unsupported source chain: ${q.sourceChainId}`)
      }
      if (!isSupportedChainId(q.destinationChainId)) {
        throw new Error(
          `Unsupported destination chain: ${q.destinationChainId}`,
        )
      }
      return {
        ...q,
  ...
  async executeCrossChainSwap(
    ...
    const response = await postApi<{
      intentData: TransactionData
      intentId: Hex
    }>(JEJU_SOLVER_URL, '/cross-chain/swap', { quoteId: quote.id })
    ...
    const txData: TransactionData = {
      to: validated.intentData.to
        ? expectAddress(validated.intentData.to, 'to')
        : ZERO_ADDRESS,
      data: validated.intentData.data
        ? expectHex(validated.intentData.data, 'data')
        : EMPTY_CALLDATA,
      value: validated.intentData.value
        ? BigInt(validated.intentData.value)
        : 0n,
    }
    const intentId = validated.intentId

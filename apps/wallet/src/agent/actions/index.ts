...
import { type RiskLevel, securityEngine } from '../../services/security'
...
export const getPortfolioAction = {
  name: 'GET_PORTFOLIO',
  description:
    'Get the user portfolio showing all token balances across supported chains',
  similes: [
    'show portfolio',
    'my balances',
    'show balances',
...
export const swapTokensAction = {
  ...
  async execute(
    ...
    params: { amount?: string; fromToken?: string; toToken?: string },
    ...
    if (!params.amount) {
      return {
        success: false,
        message:
          'Please specify the amount you want to swap. Example: "Swap 0.1 ETH for USDC"',
      }
    }
  ...
  async execute(
    ...
    if (!params.to) {
      return {
        success: false,
        message:
          'Please provide a valid recipient address. Example: "Send 0.1 ETH to 0x1234...5678"',
      }
    }
...
export const checkApprovalsAction = {
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    if (summary.highRiskApprovals > 0) {
      ...
      for (const approval of summary.tokenApprovals.filter(
        (a) => a.riskLevel === 'high',
      )) {
...
export const getHistoryAction = {
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    const transactions = await historyService.getHistory(walletAddress, {
      limit: 10,
    })
...
export const getGasPriceAction = {
  ...
  async execute(_context: ActionContext): Promise<ActionResult> {
    const chainIds = Object.keys(SUPPORTED_CHAINS).map(
      Number,
    ) as SupportedChainId[]
    const gasPrices = await Promise.all(
      chainIds.map((id) => oracleService.getGasPrice(id)),
    )
...
export const helpAction = {
  ...
  async execute(): Promise<ActionResult> {
    return {
      success: true,
      message:
        `**Network Wallet - Available Commands**\n\n` +
        `**Portfolio & Balances:**\n` +
...
export async function dispatchAction(
  ...
  const action = walletActions.find(
    (a) =>
      a.name === actionName ||

...
export class WalletService {
  ...
  private _balanceCache: Map<
    string,
    { balance: TokenBalance[]; timestamp: number }
  > = new Map()
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    for (const [chainId, chain] of Object.entries(SUPPORTED_CHAINS)) {
      const rpcUrl =
        this.getNetworkRpcUrl(Number(chainId)) || chain.rpcUrls.default.http[0]
    ...
    runtime.logger.info(
      '[WalletService] Initialized with network infrastructure',
    )
  }
  ...
  async importWallet(options: {
    ...
    this.runtime?.logger.info(
      `[WalletService] Importing ${options.type} wallet`,
    )
  ...
  private async initializeWalletClients(
    _address: Address,
    mnemonic: string,
  ): Promise<void> {
    const account = mnemonicToAccount(mnemonic)
  ...
  private async initializeWalletClientsFromPrivateKey(
    privateKey: Hex,
  ): Promise<void> {
  ...
  async sendTransaction(options: {
    ...
    this.runtime?.logger.info(
      `[WalletService] Sending transaction on chain ${options.chainId}`,
    )

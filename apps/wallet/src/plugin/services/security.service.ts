...
export class SecurityService {
  ...
  async analyzeTransaction(options: {
    ...
    this.runtime?.logger.info(
      `[SecurityService] Analyzing transaction to ${options.to}`,
    )
  ...
  private async simulateTransaction(options: {
    ...
    const from =
      options.from || ('0x0000000000000000000000000000000000000001' as Address)
    ...
    if (options.typedData) {
      ...
      if (primaryType === 'Permit') {
        ...
        risks.push({
          ...
          details: {
            spender: spenderAddr.startsWith('0x')
              ? (spenderAddr as `0x${string}`)
              : undefined,
            amount:
      ...
      if (domain.name && typeof domain.name === 'string') {
        if (
          domain.name.toLowerCase().includes('claim') ||
          ...
          risks.push({
            ...
            severity: 'high',
            description:
              'Signature request appears to be from a potential phishing site',
          })
          riskLevel = 'high'
        }
      }

...
export const walletInfoAction = {
  ...
  description: 'Get wallet information including address, balances, and account details',
  similes: ['SHOW_WALLET', 'WALLET_STATUS', 'MY_WALLET', 'CHECK_BALANCE', 'VIEW_PORTFOLIO'],
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    if (!state.isInitialized) {
      return { success: true, message: 'No wallet connected. Would you like to create or import one?' };
    }
    
    if (state.isLocked) {
      return { success: true, message: 'Wallet is locked. Please unlock to view details.' };
    }

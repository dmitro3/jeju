...
export const crossChainSwapAction = {
  ...
  parseParams(text: string): CrossChainParams {
    ...
    const bridgeMatch = text.match(
      /(\d+\.?\d*)\s*(\w+)\s*(?:from\s+(\w+)\s+)?(?:to|on)\s+(\w+)/i,
    )
    if (bridgeMatch) {
    ...
    const swapMatch = text.match(
      /(\d+\.?\d*)\s*(\w+)\s*for\s*(\w+)\s*(?:on|to)\s*(\w+)/i,
    )
    if (swapMatch) {
  ...
  async execute(
    ...
    const sourceChainId = params.sourceChain
      ? CHAIN_NAME_TO_ID[params.sourceChain.toLowerCase()] ||
        state.activeChainId
      : state.activeChainId
    const destChainId =
      CHAIN_NAME_TO_ID[params.destChain?.toLowerCase() || ''] || 8453

...
import { launchpadService, LaunchType } from '../../services';
...
export const viewLaunchesAction = {
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    if (launches.length === 0) {
      return { 
        success: true, 
        message: 'No recent token launches found.\n\nSay "Launch token" to create your own.',
      };
    }
    ...
    for (const launch of launches) {
      ...
      if (launch.launchType === LaunchType.BondingCurve && launch.bondingCurve !== '0x0000000000000000000000000000000000000000') {
        const curveInfo = await launchpadService.getBondingCurveInfo(launch.bondingCurve);
        if (curveInfo) {
...
export const viewMyLaunchesAction = {
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    const launches = await launchpadService.getCreatorLaunches(state.currentAccount.address as Address);

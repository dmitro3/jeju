...
import { perpsService, PositionSide, MARKET_IDS } from '../../services';
...
export const openPerpPositionAction = {
  ...
  async execute(context: ActionContext, params?: { market?: string; side?: 'long' | 'short'; leverage?: number }): Promise<ActionResult> {
    ...
    const marketSymbol = params.market?.toUpperCase().includes('BTC') ? 'BTC-PERP' : 'ETH-PERP';
    const marketId = MARKET_IDS[marketSymbol as keyof typeof MARKET_IDS];
    const market = await perpsService.getMarket(marketId);
    
    if (!market) {
      return { success: false, message: `Perp market ${marketSymbol} is not available on this network.` };
    }
    
    const leverage = params.leverage || 5;
    if (leverage > market.maxLeverage) {
      return { success: false, message: `Maximum leverage for ${marketSymbol} is ${market.maxLeverage}x.` };
    }
    
    const markPrice = perpsService.formatPrice(market.markPrice || 0n);
    const fundingRate = market.fundingRate ? (Number(market.fundingRate) / 1e18 * 100).toFixed(4) : '0';
    
    return {
      success: true,
      message: `**${params.side === 'long' ? 'Long' : 'Short'} ${marketSymbol}**\n\n` +
...
export const closePerpPositionAction = {
  ...
  async execute(context: ActionContext, params?: { market?: string }): Promise<ActionResult> {
    const state = context.walletService.getState();
    if (!state.currentAccount) {
      return { success: false, message: 'Please connect your wallet first.' };
    }
    
    const positions = await perpsService.getPositions(state.currentAccount.address as Address);
    
    if (positions.length === 0) {
      return { success: true, message: 'You don\'t have any open perpetual positions.' };
    }
    ...
    const pnlStr = position.unrealizedPnl >= 0n 
      ? `+$${formatUnits(position.unrealizedPnl, 18)}`
      : `-$${formatUnits(-position.unrealizedPnl, 18)}`;
    
    return {
      success: true,
      message: `**Close ${position.side === PositionSide.Long ? 'Long' : 'Short'} ${position.symbol}**\n\n` +
...
export const viewPerpPositionsAction = {
  ...
  async execute(context: ActionContext): Promise<ActionResult> {
    ...
    for (const pos of positions) {
      const pnlStr = pos.unrealizedPnl >= 0n 
...
export const viewPerpMarketsAction = {
  ...
  async execute(_context: ActionContext): Promise<ActionResult> {
    ...
    if (markets.length === 0) {
      return { success: true, message: 'No perpetual markets available on this network.' };
    }
    
    let response = '**Available Perpetual Markets**\n\n';
    
    for (const market of markets) {
      const markPrice = market.markPrice ? `$${perpsService.formatPrice(market.markPrice)}` : 'N/A';

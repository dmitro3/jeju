...
/**
 * Minimal mock of IAgentRuntime for testing - only includes methods we actually use
 * The plugin only calls runtime.getSetting() so that's all we need to mock
 */
type MockableRuntime = Pick<IAgentRuntime, 'getSetting'>

/**
 * Minimal mock of Memory for testing - only includes fields we actually use
 * The plugin only reads message.content.text
 */
interface MockableMemory {
  content: { text: string }
}

// Mock runtime - satisfies the IAgentRuntime interface for methods used by the plugin
...
// Mock message - satisfies the Memory interface for fields used by the plugin
const createMockMessage = (text: string): MockableMemory => ({
  content: { text },
})
...
describe('walletStateProvider', () => {
  it('should return connected state when wallet is configured', async () => {
    ...
    const result = await walletStateProvider.get(
      runtime as IAgentRuntime,
      createMockMessage('') as Memory,
      {} as State,
    )
...
describe('portfolioProvider', () => {
  it('should return portfolio info when wallet connected', async () => {
    ...
    const result = await portfolioProvider.get(
      runtime as IAgentRuntime,
      createMockMessage('') as Memory,
      {} as State,
    )
...
describe('sendTokenAction', () => {
  ...
  it('should validate when wallet is connected', async () => {
    ...
    const isValid = await sendTokenAction.validate(
      runtime as IAgentRuntime,
      message as Memory,
    )
  ...
  it('should parse address and amount from message', async () => {
    ...
    expect(callArg.content.params).toHaveProperty(
      'recipient',
...
describe('portfolioAction', () => {
  ...
  it('should always validate (to show connect prompt)', async () => {
    ...
    const isValid = await portfolioAction.validate(
      runtime as IAgentRuntime,
      message as Memory,
    )
    expect(isValid).toBe(true)
...
describe('registerNameAction', () => {
  ...
  it('should parse .jeju name from message', async () => {
    ...
    const callback = mock(() => {})
    await registerNameAction.handler(
      runtime as IAgentRuntime,
      message as Memory,
      undefined,
      undefined,

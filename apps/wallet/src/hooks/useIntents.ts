  ...
  const createIntent = useCallback(
    async (params: IntentParams) => {
      ...
      try {
        ...
        const message =
          e instanceof Error ? e.message : 'Failed to create intent'
        setError(message)
  ...
  const refundIntent = useCallback(
    async (intentId: Hex) => {
      ...
      try {
        ...
        const message =
          e instanceof Error ? e.message : 'Failed to refund intent'
        setError(message)
        throw new Error(message)
      }
    },
...
export function useIntentHistory() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Intent history fetch failed: ${response.status} ${response.statusText}`,
        )
      }

...
const JNS_REGISTRAR_ABI = [
  ...
  { inputs: [{ name: 'name', type: 'string' }, { name: 'owner', type: 'address' }, { name: 'duration', type: 'uint256' }], name: 'register', outputs: [{ type: 'bytes32' }], stateMutability: 'payable', type: 'function' },
  { inputs: [{ name: 'name', type: 'string' }, { name: 'duration', type: 'uint256' }], name: 'renew', outputs: [], stateMutability: 'payable', type: 'function' },
  { inputs: [{ name: 'labelhash', type: 'bytes32' }], name: 'nameExpires', outputs: [{ type: 'uint256' }], stateMutability: 'view', type: 'function' },
] as const;
...
const JNS_REVERSE_ABI = [
  { inputs: [{ name: 'name', type: 'string' }], name: 'setName', outputs: [{ type: 'bytes32' }], stateMutability: 'nonpayable', type: 'function' },
  { inputs: [{ name: 'addr', type: 'address' }], name: 'node', outputs: [{ type: 'bytes32' }], stateMutability: 'pure', type: 'function' },
] as const;
...
export class JNSService {
  ...
  async getPrice(name: string, durationYears: number = 1): Promise<JNSPricing> {
    ...
    if (!registrar) {
      ...
      if (len === 3) pricePerYear = BigInt(0.1e18); // 0.1 ETH
      else if (len === 4) pricePerYear = BigInt(0.01e18); // 0.01 ETH
   ...
   */
  buildRegisterTx(params: JNSRegistrationParams): { to: Address; data: Hex; value: bigint } | null {
    const { registrar } = this.getContracts();

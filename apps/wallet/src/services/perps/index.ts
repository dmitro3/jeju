...
export const MARKET_IDS = {
  'BTC-PERP': ('0x' +
    Buffer.from('BTC-PERP').toString('hex').padEnd(64, '0')) as Hex,
  'ETH-PERP': ('0x' +
    Buffer.from('ETH-PERP').toString('hex').padEnd(64, '0')) as Hex,
}
...
export class PerpsService {
  ...
  async getMarkets(): Promise<PerpMarket[]> {
    ...
    for (const marketId of marketIds) {
      ...
      const [fundingRate, _cumFunding, _lastTime] = fundingResult as [
        bigint,
        bigint,
        bigint,
      ]
  ...
  async getPositions(trader: Address): Promise<PerpPosition[]> {
    ...
    const positionIds = await client
      .readContract({
        address: perpMarket,
        abi: PERP_MARKET_ABI,

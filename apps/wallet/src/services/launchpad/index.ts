...
import { rpcService, SUPPORTED_CHAINS, type SupportedChainId } from '../rpc'
...
const TOKEN_LAUNCHPAD_ABI = [
  // Launch functions
  {
    inputs: [
      ...
      { name: 'communityVault', type: 'address' },
      {
        components: [
          { name: 'virtualEthReserves', type: 'uint256' },
  ...
  },
  {
    inputs: [
      ...
      {
        components: [
          { name: 'presaleAllocationBps', type: 'uint256' },
          { name: 'presalePrice', type: 'uint256' },
          { name: 'lpFundingBps', type: 'uint256' },
          { name: 'lpLockDuration', type: 'uint256' },
          { name: 'buyerLockDuration', type: 'uint256' },
...
const ICO_PRESALE_ABI = [
  {
    inputs: [{ name: 'tokensRequested', type: 'uint256' }],
    name: 'buy',
    outputs: [],
    stateMutability: 'payable',
    type: 'function',
...
export class LaunchpadService {
  ...
  async getBondingCurveInfo(
    ...
    const progress =
      graduationTarget > 0n ? Number((realEth * 100n) / graduationTarget) : 0
  ...
  async getPresaleInfo(
    ...
    const [totalRaised, tokensSold, softCap, hardCap, endTime, finalized] =
      await Promise.all([
        client.readContract({
   ...
   */
  buildLaunchBondingCurveTx(
    params: LaunchBondingCurveParams,

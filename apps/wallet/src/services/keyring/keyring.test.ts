...
const FOUNDRY_PRIVATE_KEY_1 =
  '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80' as Hex
const FOUNDRY_ADDRESS_1 =
  '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' as Address
const FOUNDRY_PRIVATE_KEY_2 =
  '0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d' as Hex
const FOUNDRY_ADDRESS_2 =
...
describe('KeyringService', () => {
  ...
  describe('Mnemonic Import', () => {
    it('should derive correct address from known mnemonic', async () => {
      const password = 'testpassword123'
      const address = await service.importMnemonic(
        TEST_VECTORS.mnemonic,
        password,
      )

      // The expected address from the test mnemonic at m/44'/60'/0'/0/0
      expect(address.toLowerCase()).toBe(
        TEST_VECTORS.expectedAddress0.toLowerCase(),
      )
    })
    ...
    it('should reject invalid mnemonic', async () => {
      ...
      await expect(
        service.importMnemonic(invalidMnemonic, password),
    ...
    it('should reject mnemonic with wrong word count', async () => {
      ...
      await expect(
        service.importMnemonic(shortMnemonic, password),
      ).rejects.toThrow('Invalid mnemonic phrase')
    })
    ...
    it('should prevent duplicate mnemonic import', async () => {
      ...
      await expect(
        service.importMnemonic(TEST_VECTORS.mnemonic, password),
  ...
  describe('Private Key Import', () => {
    it('should import valid private key and derive correct address', async () => {
      const address = await service.importPrivateKey(
        FOUNDRY_PRIVATE_KEY_1,
        'password',
      )
    ...
    it('should prevent duplicate private key import', async () => {
      ...
      await expect(
        service.importPrivateKey(FOUNDRY_PRIVATE_KEY_1, 'password'),
      ).rejects.toThrow('Account already exists')
    })
  })
  ...
  describe('Private Key Export', () => {
    it('should export imported private key correctly', async () => {
      ...
      expect(exportedKey.toLowerCase()).toBe(
        FOUNDRY_PRIVATE_KEY_1.toLowerCase(),
      )
    })
    ...
    it('should export HD wallet private key correctly', async () => {
      ...
      const service2 = new KeyringService()
      const reimportedAddress = await service2.importPrivateKey(
        exportedKey,
        password,
      )
    ...
    it('should throw for watch-only account export', async () => {
      ...
      await expect(
        service.exportPrivateKey(watchAddress, 'password'),
    ...
    it('should throw for non-existent account', async () => {
      ...
      await expect(
        service.exportPrivateKey(nonExistent, 'password'),
  ...
  describe('Message Signing', () => {
    ...
    it('should throw when signing with watch-only account', async () => {
      ...
      await expect(
        service.signMessage(watchAddress, 'test', 'password'),
  ...
  describe('Transaction Signing', () => {
    ...
    it('should throw when signing tx with watch account', async () => {
      ...
      await expect(
        service.signTransaction(watchAddress, tx, 'password'),
      ).rejects.toThrow('Cannot sign with watch-only account')
    })
  })
  ...
  describe('Watch Address Management', () => {
    ...
    it('should throw for duplicate watch address', () => {
      ...
      expect(() => service.addWatchAddress(address)).toThrow(
  ...
  describe('Encryption Security', () => {
    ...
    it('should fail decryption with wrong password', async () => {
      ...
      await expect(
        service.exportPrivateKey(address, wrongPassword),
      ).rejects.toThrow()
    })
  })
})

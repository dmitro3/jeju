...
const TypeIcon: React.FC<{ type: Transaction['type']; isOutgoing: boolean }> = ({ type, isOutgoing }) => {
  const icons: Record<string, string> = {
    send: '↑',
    receive: '↓',
    swap: '⇄',
...
const formatAmount = (value: string, type: Transaction['type'], isOutgoing: boolean): string => {
...
export const TransactionHistory: React.FC<TransactionHistoryProps> = ({
  ...
  const [filter, setFilter] = useState<'all' | 'sent' | 'received' | 'swaps' | 'approvals'>('all');
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex gap-1 bg-zinc-800 p-1 rounded-lg">
          {(['all', 'sent', 'received', 'swaps', 'approvals'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-zinc-500">{tx.chainName}</span>
                          </div>
                          
                          <div className="flex items-center gap-2 text-sm text-zinc-400">
                            {tx.type === 'approve' ? (
                              <span>
                                {tx.token?.symbol || 'Token'} → {tx.to?.slice(0, 6)}...{tx.to?.slice(-4)}
                              </span>
                            ) : tx.nft ? (
                              <span>{tx.nft.name} #{tx.nft.tokenId}</span>
                            ) : (
                              <span>
                                {isOutgoing ? 'To: ' : 'From: '}
                                {isOutgoing 
                        ...
                        <div className="text-right">
                          {tx.type !== 'approve' && (
                            <>
                              <p className={`font-medium ${isOutgoing ? 'text-red-400' : 'text-green-400'}`}>
                                {formatAmount(tx.valueFormatted, tx.type, isOutgoing)} {tx.token?.symbol || 'ETH'}
                              </p>
                              {tx.valueUsd !== undefined && (
                                <p className="text-xs text-zinc-500">${tx.valueUsd.toFixed(2)}</p>
                              )}
                            </>
                          )}
                          <p className="text-xs text-zinc-500 mt-1">{formatDate(tx.timestamp)}</p>
                        </div>
                      </div>
                    </button>
                  );

...
const RiskBadge: React.FC<{ level: SimulationResult['risk']['level'] }> = ({
  level,
}) => {
...
const ChangeIcon: React.FC<{
  type: 'send' | 'receive' | 'approve' | 'revoke'
}> = ({ type }) => {
  ...
  if (simulation) {
    try {
      expectSchema(
        ...
        'TransactionSimulation props.simulation',
      )
    } catch (e) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between p-4 bg-zinc-900 rounded-lg">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${simulation.success ? 'bg-green-500' : 'bg-red-500'}`}
          />
            ...
            {simulation.nativeChange && (
              <div className="flex items-center justify-between p-3 bg-zinc-800 rounded">
                ...
                <div className="text-right">
                  <p
                    className={
                      simulation.nativeChange.type === 'send'
                        ? 'text-red-400'
                        : 'text-green-400'
                    }
                  >
                    {simulation.nativeChange.type === 'send' ? '-' : '+'}
                    {simulation.nativeChange.amountFormatted}
                  </p>
                  <p className="text-xs text-zinc-500">
                    ${simulation.nativeChange.usdValue.toFixed(2)}
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 font-mono">
                      {change.token.address.slice(0, 6)}...
                      {change.token.address.slice(-4)}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p
                    className={
                      change.type === 'send' ? 'text-red-400' : 'text-green-400'
                    }
                  >
                    {change.type === 'send' ? '-' : '+'}
                    {change.amountFormatted}
                  </p>
                  <p className="text-xs text-zinc-500">
            ...
            {simulation.approvalChanges.map((approval, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <ChangeIcon
                      type={approval.isRevoke ? 'revoke' : 'approve'}
                    />
                    <div>
                      <p className="font-medium">
                        {approval.isRevoke ? 'Revoke' : 'Approve'}{' '}
                        {approval.symbol}
                      </p>
                      <p className="text-xs text-zinc-500">
                        Spender:{' '}
        ...
        <div className="p-4 bg-zinc-900 rounded-lg">
          ...
          <div className="p-3 bg-zinc-800 rounded">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-sm">
                {simulation.contractInteraction.method}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="p-4 bg-zinc-900 rounded-lg">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="font-medium">
              {(Number(simulation.gas.totalCost) / 1e18).toFixed(6)} ETH
            </p>
            <p className="text-xs text-zinc-500">
              ${simulation.gas.totalCostUsd.toFixed(2)}
            </p>
          </div>
        </div>
      </div>

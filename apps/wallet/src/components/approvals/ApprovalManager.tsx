...
const RiskBadge: React.FC<{ level: TokenApproval['spender']['riskLevel'] }> = ({ level }) => {
  ...
  return (
    <span className={`px-2 py-0.5 text-xs font-medium rounded ${styles[level]}`}>
      {level}
    </span>
  );
};
...
export const ApprovalManager: React.FC<ApprovalManagerProps> = ({
  ...
  const filteredApprovals = approvals.filter(approval => {
    ...
    if (filter === 'risky') return approval.spender.riskLevel === 'high' || approval.spender.riskLevel === 'medium';
    return true;
  });
  ...
  const riskyCount = approvals.filter(a => 
    a.spender.riskLevel === 'high' || a.spender.riskLevel === 'medium'
  ).length;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <button
          ...
          className={`p-3 rounded-lg text-center transition-colors ${
            filter === 'all' ? 'bg-emerald-600' : 'bg-zinc-800 hover:bg-zinc-700'
          }`}
        >
        ...
        <button
          ...
          className={`p-3 rounded-lg text-center transition-colors ${
            filter === 'unlimited' ? 'bg-yellow-600' : 'bg-zinc-800 hover:bg-zinc-700'
          }`}
        >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            {selected.size === sortedApprovals.length ? 'Deselect All' : 'Select All'}
          </button>
          {selected.size > 0 && (
            <button
            ...
            return (
              <div
                ...
                className={`p-4 rounded-lg transition-colors ${
                  isSelected ? 'bg-zinc-700 ring-1 ring-emerald-500' : 'bg-zinc-800'
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-zinc-400 truncate">
                      Spender: {approval.spender.name || `${approval.spender.address.slice(0, 6)}...${approval.spender.address.slice(-4)}`}
                    </p>

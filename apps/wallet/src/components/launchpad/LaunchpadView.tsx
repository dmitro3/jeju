...
import { RefreshCw, Rocket, TrendingUp, Users, DollarSign, Plus } from 'lucide-react';
import { launchpadService, LaunchType, type Launch, type BondingCurveInfo } from '../../services';
...
export function LaunchpadView({ address }: LaunchpadViewProps) {
  ...
  const [curveInfos, setCurveInfos] = useState<Map<string, BondingCurveInfo>>(new Map());
  ...
  const fetchData = useCallback(async () => {
    ...
    for (const launch of [...recent, ...mine]) {
      if (launch.launchType === LaunchType.BondingCurve && 
          launch.bondingCurve !== '0x0000000000000000000000000000000000000000') {
        const info = await launchpadService.getBondingCurveInfo(launch.bondingCurve);
  ...
  const renderLaunchCard = (launch: Launch) => {
    ...
    const typeStr = launch.launchType === LaunchType.BondingCurve ? 'Bonding Curve' : 'ICO Presale';
    
    return (
      <div key={launch.id.toString()} className="bg-card border border-border rounded-xl p-4 hover:border-purple-500/50 transition-colors">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className={`px-2 py-1 rounded-full text-xs font-medium ${
            launch.graduated ? 'bg-emerald-500/20 text-emerald-400' : 'bg-purple-500/20 text-purple-400'
          }`}>
        ...
        {curveInfo && (
          <>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">{curveInfo.progress}% to graduation</span>
              <span className="font-medium">{formatUnits(curveInfo.realEthReserves, 18)} ETH raised</span>
            </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Launch and trade tokens on bonding curves</p>
          </div>
          <button
            onClick={fetchData}
            disabled={isLoading}
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <div className="text-2xl font-bold mt-1">{launches.filter(l => !l.graduated).length}</div>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground flex items-center gap-1">
        ...
        <div className="flex gap-2 border-b border-border pb-2">
          {[
            ...
            { id: 'my-launches' as const, label: `My Launches (${myLaunches.length})`, icon: Users },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {isLoading ? (
              ...
              <div className="col-span-full text-center py-12 bg-card border border-border rounded-xl">
                ...
                <p className="text-muted-foreground mt-2">Be the first to launch a token</p>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl p-6 hover:border-purple-500/50 transition-colors cursor-pointer">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Pump.fun style fair launch. Price increases as more people buy. 
                  Auto-graduates to AMM when target reached.
                </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {myLaunches.length === 0 ? (
              <div className="col-span-full text-center py-12 bg-card border border-border rounded-xl">
                ...
                <h3 className="text-lg font-medium">No Launches Yet</h3>
                <p className="text-muted-foreground mt-2">You haven't launched any tokens</p>
                <button
                  onClick={() => setTab('create')}
                  className="mt-4 px-6 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-xl"
                >

...
import { RefreshCw, TrendingUp, TrendingDown, Activity, DollarSign, AlertTriangle } from 'lucide-react';
import { perpsService, PositionSide, type PerpMarket, type PerpPosition } from '../../services';
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <div className={`text-2xl font-bold mt-1 ${totalPnlNum >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              {totalPnlNum >= 0 ? '+' : ''}{totalPnlNum.toFixed(2)} USD
            </div>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            <div className="text-sm text-muted-foreground">Available Markets</div>
        ...
        {tab === 'trade' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                  ...
                  {['ETH-PERP', 'BTC-PERP'].map(m => (
                    <button
                      ...
                      className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${
                        selectedMarket === m ? 'bg-primary text-primary-foreground' : 'bg-secondary hover:bg-secondary/80'
                      }`}
                    >
                      {m.replace('-PERP', '')}
                    </button>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-colors ${
                      side === 'long' ? 'bg-emerald-500 text-white' : 'bg-secondary hover:bg-secondary/80'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-colors ${
                      side === 'short' ? 'bg-red-500 text-white' : 'bg-secondary hover:bg-secondary/80'
                    }`}
                  >
              ...
              {/* Leverage */}
              <div className="mb-4">
                <label className="text-sm text-muted-foreground mb-2 block">Leverage: {leverage}x</label>
                <input
                  type="range"
                  min={1}
                  max={20}
              ...
              <p className="text-xs text-muted-foreground mt-4 text-center">
                Or use chat: "{side === 'long' ? 'Long' : 'Short'} {selectedMarket.replace('-PERP', '')} {leverage}x with 100 USDC"
              </p>
            </div>
            
            {/* Market Info */}
            <div className="bg-card border border-border rounded-xl p-6">
              <h3 className="text-lg font-semibold mb-4">{selectedMarket} Info</h3>
              
              {selectedMarketData ? (
                <div className="space-y-4">
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Mark Price</span>
                    <span className="font-medium">${perpsService.formatPrice(selectedMarketData.markPrice || 0n)}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Funding Rate (8h)</span>
                    <span className={`font-medium ${Number(selectedMarketData.fundingRate || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {selectedMarketData.fundingRate ? (Number(selectedMarketData.fundingRate) / 1e18 * 100).toFixed(4) : '0'}%
                    </span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-border">
                  ...
                  <div className="flex justify-between py-2 border-b border-border">
                    ...
                    <span className="font-medium">{formatUnits(selectedMarketData.currentOpenInterest, 8)}</span>
                  </div>
                  <div className="flex justify-between py-2">
                    <span className="text-muted-foreground">Taker Fee</span>
                    <span className="font-medium">{selectedMarketData.takerFeeBps / 100}%</span>
                  </div>
                </div>
              ) : (
          ...
          <div className="space-y-4">
            {positions.length === 0 ? (
              <div className="text-center py-12 bg-card border border-border rounded-xl">
                ...
                <p className="text-muted-foreground mt-2">Open a position to start trading</p>
                <button
              ...
              positions.map(pos => {
                ...
                return (
                  <div key={pos.positionId} className="bg-card border border-border rounded-xl p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{pos.leverage.toFixed(1)}x leverage</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className={`font-bold ${pnlNum >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{formatUnits(pos.size, 8)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Entry</p>
                        <p className="font-medium">${perpsService.formatPrice(pos.entryPrice)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Mark</p>
                        <p className="font-medium">${perpsService.formatPrice(pos.markPrice)}</p>
                      </div>
                      <div>
                        ...
                        <p className="font-medium text-yellow-500">${perpsService.formatPrice(pos.liquidationPrice)}</p>
                      </div>
                    </div>
            ...
            <table className="w-full">
              <thead className="bg-secondary/50">
                <tr>
                  ...
                  <th className="text-right p-4 text-sm font-medium text-muted-foreground">Funding (8h)</th>
                  <th className="text-right p-4 text-sm font-medium text-muted-foreground">Max Lev.</th>
                  <th className="text-right p-4 text-sm font-medium text-muted-foreground">Open Interest</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {markets.map(market => (
                  <tr key={market.marketId} className="hover:bg-secondary/30">
                    ...
                    <td className="p-4 text-right">${perpsService.formatPrice(market.markPrice || 0n)}</td>
                    <td className={`p-4 text-right ${Number(market.fundingRate || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {market.fundingRate ? (Number(market.fundingRate) / 1e18 * 100).toFixed(4) : '0'}%
                    </td>
                    <td className="p-4 text-right">{market.maxLeverage}x</td>
                    <td className="p-4 text-right">{formatUnits(market.currentOpenInterest, 8)}</td>
                  </tr>
                ))}
                {markets.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-muted-foreground">
                      {isLoading ? 'Loading markets...' : 'No markets available on this network'}
                    </td>
                  </tr>
                )}
              </tbody>

...
import { AAClient, createAAClient, type Call } from './account-abstraction';
...
const createMockPublicClient = (): MockPublicClientWithFns => {
  ...
  return {
    client: { readContract, getCode, estimateFeesPerGas, getGasPrice } as unknown as PublicClient,
    readContract,
    getCode,
    estimateFeesPerGas,
    getGasPrice,
...
describe('AAClient', () => {
  ...
  describe('buildBatchCallData', () => {
    ...
    it('should handle single call in batch', () => {
      const calls: Call[] = [
        { to: TEST_RECIPIENT, data: '0xabcdef' as Hex },
      ];
    ...
    it('should preserve order of calls', () => {
      const calls: Call[] = [
        { to: '0x1111111111111111111111111111111111111111' as Address, data: '0xaa' as Hex },
        { to: '0x2222222222222222222222222222222222222222' as Address, data: '0xbb' as Hex },
        { to: '0x3333333333333333333333333333333333333333' as Address, data: '0xcc' as Hex },
      ];
      ...
      const pos1 = callDataLower.indexOf('1111111111111111111111111111111111111111');
      const pos2 = callDataLower.indexOf('2222222222222222222222222222222222222222');
      const pos3 = callDataLower.indexOf('3333333333333333333333333333333333333333');
  ...
  describe('buildInitCode', () => {
    it('should build valid init code', () => {
      ...
      expect(initCode.toLowerCase()).toContain(TEST_FACTORY.toLowerCase().slice(2));
  ...
  describe('isAccountDeployed', () => {
    it('should return true for deployed account', async () => {
      ...
      expect(mockPublicClient.getCode).toHaveBeenCalledWith({ address: TEST_SMART_ACCOUNT });
    });
  ...
  describe('getSmartAccountAddress', () => {
    it('should compute counterfactual address', async () => {
      ...
      const address = await client.getSmartAccountAddress(TEST_OWNER, 0n, TEST_FACTORY);
    ...
    it('should throw without factory address', async () => {
      await expect(client.getSmartAccountAddress(TEST_OWNER, 0n))
  ...
  describe('Factory function', () => {
    ...
    it('should accept custom entry point address', () => {
      const customEntryPoint = '0x0000000071727De22E5E9d8BAf0edAc6f37da032' as Address;
      const aaClient = createAAClient({
        chainId: 1,
        publicClient: mockPublicClient.client,

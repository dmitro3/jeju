...
const createMockPublicClient = (): MockPublicClientWithFns => {
  ...
  return {
    client: { readContract, waitForTransactionReceipt } as unknown as PublicClient,
    readContract,
...
const createMockWalletClient = (): MockWalletClientWithFns => {
  ...
  return {
    client: { account: { address: TEST_USER }, writeContract } as unknown as WalletClient,
    writeContract,
  };
};
...
describe('EILClient', () => {
  ...
  describe('buildPaymasterData', () => {
    it('should build mode 0 (token payment) data correctly', () => {
      ...
      expect(paymasterData.slice(4, 44).toLowerCase()).toBe(USDC_ADDRESS.slice(2).toLowerCase());
      
      // App address should be zero address when not specified
      expect(paymasterData.slice(44).toLowerCase()).toBe(ZERO_ADDRESS.slice(2).toLowerCase());
    });

    it('should include app address when provided', () => {
      const paymasterData = client.buildPaymasterData(0, USDC_ADDRESS, TEST_APP);
      
      expect(paymasterData.slice(44).toLowerCase()).toBe(TEST_APP.slice(2).toLowerCase());
    });

    it('should throw for unsupported mode', () => {
      expect(() => client.buildPaymasterData(1, USDC_ADDRESS))
        .toThrow('Only mode 0 (token payment) is supported');
    });
    ...
    it('should handle different token addresses', () => {
      ...
      expect(usdcData.slice(4, 44).toLowerCase()).toBe(USDC_ADDRESS.slice(2).toLowerCase());
      expect(usdtData.slice(4, 44).toLowerCase()).toBe(USDT_ADDRESS.slice(2).toLowerCase());
    });
  });
  ...
  describe('canSponsor', () => {
    it('should check if paymaster can sponsor with token', async () => {
      ...
      mockPublicClient.readContract.mockResolvedValueOnce([true, tokenCost, userBalance]);
    ...
    it('should return false when user has insufficient balance', async () => {
      mockPublicClient.readContract.mockResolvedValueOnce([false, 5000000n, 1000000n]);
  ...
  describe('getBestGasToken', () => {
    it('should find best gas payment token', async () => {
      ...
      mockPublicClient.readContract.mockResolvedValueOnce([USDC_ADDRESS, 5000000n]);
    ...
    it('should return zero address when no suitable token', async () => {
      ...
      const result = await client.getBestGasToken(TEST_USER, 100000n, [USDC_ADDRESS]);
  ...
  describe('getBestPaymentTokenForApp', () => {
    it('should consider app preferences when finding best token', async () => {
      ...
      mockPublicClient.readContract.mockResolvedValueOnce([USDC_ADDRESS, 5000000n, 'App prefers USDC']);
  ...
  describe('previewTokenCost', () => {
    it('should calculate token cost for gas', async () => {
      ...
      const cost = await client.previewTokenCost(estimatedGas, gasPrice, USDC_ADDRESS);
  ...
  describe('getSwapQuote', () => {
    it('should get swap quote with price impact', async () => {
      ...
      mockPublicClient.readContract.mockResolvedValueOnce([expectedOut, priceImpactBps]);
      
      const result = await client.getSwapQuote(USDC_ADDRESS, USDT_ADDRESS, amountIn);
  ...
  describe('Cross-chain operations', () => {
    it('should throw when creating transfer on unconfigured chain', async () => {
      ...
      await expect(unconfiguredClient.createCrossChainTransfer({
    ...
    it('should throw when wallet not connected', async () => {
      ...
      await expect(clientNoWallet.createCrossChainTransfer({
        sourceToken: ZERO_ADDRESS,
        amount: 1000000000000000000n,
        destinationToken: ZERO_ADDRESS,
        destinationChainId: 8453,
  ...
  describe('getCurrentFee', () => {
    it('should get current fee for request', async () => {
      const requestId = '0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef' as Hex;

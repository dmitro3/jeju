...
import { GasAbstractionService, createGasService, type GasConfig } from './gas-abstraction';
...
type MockFn<T extends (...args: Parameters<T>) => ReturnType<T>> = ReturnType<typeof vi.fn<Parameters<T>, ReturnType<T>>>;

// Create mock public client with accessible mock functions
interface MockPublicClientWithFns {
  client: PublicClient;
  getBalance: MockFn<(args: { address: Address }) => Promise<bigint>>;
  getGasPrice: MockFn<() => Promise<bigint>>;
  readContract: MockFn<(args: object) => Promise<unknown>>;
}

const createMockPublicClient = (): MockPublicClientWithFns => {
  const getBalance = vi.fn<[{ address: Address }], Promise<bigint>>();
  const getGasPrice = vi.fn<[], Promise<bigint>>();
  const readContract = vi.fn<[object], Promise<unknown>>();
  
  return {
    client: { getBalance, getGasPrice, readContract } as unknown as PublicClient,
    getBalance,
    getGasPrice,
    readContract,
  };
...
describe('GasAbstractionService', () => {
  ...
  describe('getGasStatus', () => {
    it('should detect native balance availability', async () => {
      ...
      mockClient1.getBalance.mockResolvedValueOnce(nativeBalance);
      mockClient1.getGasPrice.mockResolvedValueOnce(20000000000n);
    ...
    it('should detect no native balance', async () => {
      mockClient1.getBalance.mockResolvedValueOnce(0n);
      mockClient1.getGasPrice.mockResolvedValueOnce(20000000000n);
    ...
    it('should throw for unsupported chain', async () => {
      await expect(service.getGasStatus(999999, TEST_USER, []))
    ...
    it('should not need bridging when has native balance', async () => {
      mockClient1.getBalance.mockResolvedValueOnce(1000000000000000000n);
      mockClient1.getGasPrice.mockResolvedValueOnce(20000000000n);
  ...
  describe('getBestGasOption', () => {
    it('should return null when no EIL client', async () => {
      ...
      const unconfiguredService = createGasService({
        publicClients: new Map([[999999, createMockPublicClient(999999)]]),
    ...
    it('should return null when no token balances', async () => {
      const option = await service.getBestGasOption(
        1,
        TEST_USER,
        [],
        100000n

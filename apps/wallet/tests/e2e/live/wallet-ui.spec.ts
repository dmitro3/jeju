...
test.describe('Wallet UI (Live)', () => {
  ...
  test('should load CSS and JS resources', async ({ page }) => {
    ...
    const _hasJs = resources.some(
      (r) => r.includes('.js') || r.includes('.tsx') || r.includes('.ts'),
    )
    const _hasCss = resources.some(
      (r) => r.includes('.css') || r.includes('style'),
    )
  ...
  test('should handle navigation to invalid paths gracefully', async ({
    page,
  }) => {

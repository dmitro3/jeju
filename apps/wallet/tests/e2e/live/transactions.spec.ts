...
test.describe('Transactions (Live)', () => {
  ...
  test('should track transaction receipt', async () => {
    ...
    expect(receipt.from.toLowerCase()).toBe(TEST_CONFIG.testAccount.address.toLowerCase());
    expect(receipt.to?.toLowerCase()).toBe('0x70997970C51812dc3A010C7d01b50e0d17dc79C8'.toLowerCase());
    expect(receipt.status).toBe('success');

...
const VIEWS = [
  ...
  { name: 'Messages', expectedContent: 'Message', description: 'Messages interface.' },
  { name: 'Portfolio', expectedContent: 'Portfolio', description: 'Wallet portfolio view.' },
  { name: 'Pools', expectedContent: 'Pool', description: 'Liquidity pools.' },
  { name: 'Perps', expectedContent: 'Perp', description: 'Perpetuals trading.' },
  { name: 'Launch', expectedContent: 'Launch', description: 'Token launchpad.' },
  { name: 'NFTs', expectedContent: 'NFT', description: 'NFT gallery.' },
  { name: 'Names', expectedContent: 'Name', description: 'JNS names management.' },
  { name: 'Security', expectedContent: 'Security', description: 'Approvals and security.' },
]
...
test.describe('Wallet - Main Page', () => {
  test('Wallet loads', async ({ page }) => {
    ...
    if (errors.length > 0 && !hasKnownBug) { await page.screenshot({ path: join(SCREENSHOT_DIR, 'Wallet-ERROR.png'), fullPage: true }); throw new Error(`Errors: ${errors.join(', ')}`) }
    ...
    if (isLLMConfigured?.() && verifyImage) {
      ...
      const verification = cached ? cached.result : await verifyImage(screenshotPath, 'Multi-chain wallet with chat interface')
      if (!cached) { verificationCache[hash] = { result: verification, timestamp: new Date().toISOString(), route: '/' }; saveCache() }
      console.log(`${cached ? '📦' : '🔍'} Wallet: ${verification.quality} (${Math.round(verification.confidence * 100)}%)`)
    }
  })
})
...
test.afterAll(() => { saveCache(); console.log(`📊 Wallet tests completed`) })

...
fn verify_signature(address: &str, message: &str, signature: &str) -> Result<bool, String> {
    // Parse the address
    let expected_addr: Address = address.parse()
...
pub async fn login_with_wallet(
    ...
    let timestamp_str = message.strip_prefix(AUTH_MESSAGE_PREFIX)
    ...
    if age > MAX_MESSAGE_AGE_SECS {
        return Err(format!("Message expired. Age: {}s, max: {}s", age, MAX_MESSAGE_AGE_SECS));
    }

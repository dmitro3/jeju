...
describe('expectValid', () => {
  ...
  test('throws on missing object property', () => {
    expect(() => expectValid(objectSchema, { name: 'Alice' })).toThrow(
      'Validation failed',
    )
  })

  test('includes context in error message', () => {
    expect(() => expectValid(stringSchema, 'ab', 'user input')).toThrow(
      'in user input',
    )
  })
  ...
  test('includes path in error message', () => {
    ...
    expect(() =>
      expectValid(schema, { user: { email: 'not-an-email' } }),
    ).toThrow('user.email')
  })
...
describe('expectExists', () => {
  ...
  test('throws for null', () => {
    expect(() => expectExists(null, 'Value cannot be null')).toThrow(
      'Value cannot be null',
    )
  })

  test('throws for undefined', () => {
    expect(() => expectExists(undefined, 'Value cannot be undefined')).toThrow(
...
describe('expect (assertCondition)', () => {
  ...
  test('does not throw for truthy expressions', () => {
    ...
    expect(() =>
      assertCondition('hello'.length > 0, 'String has length'),
    ).not.toThrow()
    expect(() =>
      assertCondition([1, 2, 3].includes(2), 'Array includes value'),
    ).not.toThrow()
  })

  test('throws for false condition', () => {
    expect(() => assertCondition(false, 'Condition failed')).toThrow(
      'Condition failed',
    )
  })
  ...
  test('throws for falsy expressions', () => {
    ...
    expect(() => assertCondition(''.length > 0, 'Empty string')).toThrow(
      'Empty string',
    )
  })

  test('uses custom error message', () => {
    expect(() => assertCondition(false, 'Custom assertion error')).toThrow(
...
describe('getExists', () => {
  ...
  test('throws for undefined', () => {
    expect(() => getExists(undefined, 'Value is undefined')).toThrow(
      'Value is undefined',
    )
  })

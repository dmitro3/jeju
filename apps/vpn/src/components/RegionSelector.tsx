import { Check, ChevronDown, Wifi, Zap } from 'lucide-react'
...
export function RegionSelector({
  ...
  const validatedSelectedNode = selectedNode
    ? VPNNodeSchema.parse(selectedNode)
    : null

  // Group nodes by country
  const nodesByCountry = validatedNodes.reduce(
    (acc: Record<string, VPNNode[]>, node: VPNNode) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-medium">
              {validatedSelectedNode
                ? COUNTRY_NAMES[validatedSelectedNode.country_code] ||
                  validatedSelectedNode.country_code
                : 'Select Region'}
            </div>
            {validatedSelectedNode && (
              <div className="flex items-center gap-2 text-xs text-[#606070]">
                <span
                  className={getLatencyColor(validatedSelectedNode.latency_ms)}
                >
        ...
        <ChevronDown
          className={`w-5 h-5 text-[#606070] transition-transform ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-[#12121a] border border-[#2a2a35] rounded-2xl overflow-hidden z-50 max-h-80 overflow-y-auto">
          ...
          {/* Countries */}
          {Object.entries(nodesByCountry).map(
            ([countryCode, countryNodes]: [string, VPNNode[]]) => {
              if (countryNodes.length === 0) {
                return null
              }
              const bestNode = findBestClientNode(countryNodes)
              const isSelected =
                validatedSelectedNode?.country_code === countryCode
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                      <Wifi
                        className={`w-4 h-4 ${getLatencyColor(bestNode.latency_ms)}`}
                      />
                      <span
                        className={`text-sm ${getLatencyColor(bestNode.latency_ms)}`}
                      >

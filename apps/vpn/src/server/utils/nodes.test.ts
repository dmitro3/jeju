...
describe('validateCountryCode', () => {
  ...
  test('throws on non-alpha characters', () => {
    expect(() => validateCountryCode('U1')).toThrow('Invalid country code format');
    expect(() => validateCountryCode('1U')).toThrow('Invalid country code format');
    expect(() => validateCountryCode('12')).toThrow('Invalid country code format');
  });
});
...
describe('filterNodesByCountry', () => {
  ...
  test('throws on invalid country code', () => {
    expect(() => filterNodesByCountry(nodes, 'USA')).toThrow('Invalid country code');
  });
});
...
describe('sortNodesByStatusAndLoad', () => {
  test('puts online nodes first', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'offline', status: 'offline', activeConnections: 0 }),
      createTestNode({ nodeId: 'online', status: 'online', activeConnections: 50 }),
  ...
  test('sorts online nodes by active connections', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'high-load', status: 'online', activeConnections: 90 }),
      createTestNode({ nodeId: 'low-load', status: 'online', activeConnections: 10 }),
      createTestNode({ nodeId: 'mid-load', status: 'online', activeConnections: 50 }),
    ];
    const result = sortNodesByStatusAndLoad(nodes);
...
describe('sortNodesByLoad', () => {
  test('sorts by load percentage (lowest first)', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'high', activeConnections: 80, maxConnections: 100 }),
      createTestNode({ nodeId: 'low', activeConnections: 10, maxConnections: 100 }),
      createTestNode({ nodeId: 'mid', activeConnections: 50, maxConnections: 100 }),
    ];
  ...
  test('handles different max connections', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'small-full', activeConnections: 10, maxConnections: 10 }), // 100%
      createTestNode({ nodeId: 'big-half', activeConnections: 50, maxConnections: 100 }), // 50%
    ];
    const result = sortNodesByLoad(nodes);
  ...
  test('throws on zero maxConnections', () => {
    ...
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'bad', maxConnections: 0, activeConnections: 0 }),
      createTestNode({ nodeId: 'good', maxConnections: 100, activeConnections: 50 }),
    ];
    expect(() => sortNodesByLoad(nodes)).toThrow('Invalid node load calculation');
  });
});

describe('calculateNodeLoad', () => {
  test('calculates load percentage correctly', () => {
    expect(calculateNodeLoad(createTestNode({ activeConnections: 50, maxConnections: 100 }))).toBe(50);
    expect(calculateNodeLoad(createTestNode({ activeConnections: 25, maxConnections: 100 }))).toBe(25);
    expect(calculateNodeLoad(createTestNode({ activeConnections: 75, maxConnections: 100 }))).toBe(75);
  });

  test('returns 0 for empty node', () => {
    expect(calculateNodeLoad(createTestNode({ activeConnections: 0, maxConnections: 100 }))).toBe(0);
  });

  test('returns 100 for full node', () => {
    expect(calculateNodeLoad(createTestNode({ activeConnections: 100, maxConnections: 100 }))).toBe(100);
  });

  test('returns 100 for zero max connections', () => {
    expect(calculateNodeLoad(createTestNode({ activeConnections: 0, maxConnections: 0 }))).toBe(100);
  });

  test('rounds to nearest integer', () => {
    expect(calculateNodeLoad(createTestNode({ activeConnections: 33, maxConnections: 100 }))).toBe(33);
    expect(calculateNodeLoad(createTestNode({ activeConnections: 1, maxConnections: 3 }))).toBe(33);
  });

  test('throws on invalid load (over 100%)', () => {
    expect(() => calculateNodeLoad(createTestNode({ activeConnections: 150, maxConnections: 100 }))).toThrow(
      'Invalid load calculation'
    );
  });
});
...
describe('findBestNode', () => {
  test('returns lowest load online node', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'high', status: 'online', activeConnections: 90, maxConnections: 100 }),
      createTestNode({ nodeId: 'low', status: 'online', activeConnections: 10, maxConnections: 100 }),
    ];
  ...
  test('filters by country when provided', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'us-low', countryCode: 'US', activeConnections: 10, maxConnections: 100 }),
      createTestNode({ nodeId: 'de-lower', countryCode: 'DE', activeConnections: 5, maxConnections: 100 }),
    ];
    const ctx = createTestContext(nodes);
  ...
  test('ignores offline nodes', () => {
    const nodes: VPNNodeState[] = [
      createTestNode({ nodeId: 'offline-low', status: 'offline', activeConnections: 0, maxConnections: 100 }),
      createTestNode({ nodeId: 'online-high', status: 'online', activeConnections: 90, maxConnections: 100 }),
    ];
    const ctx = createTestContext(nodes);
    const result = findBestNode(ctx);
...
describe('getNodeById', () => {
  ...
  test('throws when node not found', () => {
    const ctx = createTestContext([]);
    expect(() => getNodeById(ctx, 'missing-node')).toThrow('Node not found: missing-node');
  });
});

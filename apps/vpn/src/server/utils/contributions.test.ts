...
function createTestContribution(overrides: Partial<ContributionState> = {}): ContributionState {
  const now = Date.now();
  return {
...
describe('getQuotaRemaining', () => {
  ...
  test('throws when contribution exceeds cap', () => {
    ...
    expect(() => getQuotaRemaining(contribution)).toThrow('Quota remaining cannot be negative');
  });
});
...
describe('resetContributionPeriod', () => {
  test('resets all counters and sets new period', () => {
    ...
    expect(contribution.periodEnd).toBe(contribution.periodStart + 30 * 24 * 60 * 60 * 1000);
  });
});
...
describe('addUsage', () => {
  ...
  test('throws on negative usage', () => {
    const contribution = createTestContribution();
    expect(() => addUsage(contribution, BigInt(-100))).toThrow('Usage cannot be negative');
  });
});
...
describe('addContribution', () => {
  ...
  test('throws on negative contribution', () => {
    ...
    expect(() => addContribution(contribution, BigInt(-100))).toThrow('Contribution cannot be negative');
  });
  ...
  test('throws when contribution exceeds cap', () => {
    ...
    expect(() => addContribution(contribution, BigInt(600))).toThrow('Quota remaining cannot be negative');
  });
});

...
export function validateProxyUrl(urlString: string): void {
  ...
  const localhostPatterns = ['localhost', '127.0.0.1', '::1', '0.0.0.0', '[::1]', '[::ffff:127.0.0.1]'];
  if (localhostPatterns.some(p => hostname === p)) {
    throw new Error('Access to localhost is not allowed');
  }
  
  // Block internal network hostnames
  const internalPatterns = ['.local', '.internal', '.corp', '.lan', '.intranet', '.localhost', '.home', '.private'];
 ...
 */
export async function validateProxyUrlWithDNS(urlString: string): Promise<{ url: URL; resolvedIP: string }> {
  ...
  for (const addr of addresses) {
    if (addr.includes(':')) {
      ...
      if (isPrivateIPv6(addr)) {
        throw new Error(`DNS rebinding detected: ${hostname} resolves to private IPv6 ${addr}`);
      }
    } else {
      // IPv4
      if (isPrivateIPv4(addr)) {
        throw new Error(`DNS rebinding detected: ${hostname} resolves to private IP ${addr}`);
      }
    }
  }

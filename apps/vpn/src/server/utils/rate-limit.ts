...
function cleanupExpired(): void {
  ...
  if (cleanedCount > 0) {
    console.log(`Rate limiter: cleaned ${cleanedCount} expired entries. Remaining: ${rateLimitStore.size}`);
  }
}
...
export function checkRateLimit(
  ...
  const effectiveConfig = config ?? DEFAULT_CONFIGS[type] ?? DEFAULT_CONFIGS.default;
  ...
  if (rateLimitStore.size >= MAX_ENTRIES) {
    ...
    return { allowed: false, remaining: 0, resetAt: now + effectiveConfig.windowMs };
  }
 ...
 */
export function createRateLimitMiddleware(type: string, config?: RateLimitConfig) {
  return async (c: { req: { header: (name: string) => string | undefined }; json: (body: object, status: number) => Response }, next: () => Promise<void>) => {
    ...
    const identifier = jejuAddress ?? forwardedFor?.split(',')[0]?.trim() ?? realIp ?? 'unknown';

...
describe('createSession', () => {
  ...
  test('throws when node not found', () => {
    ...
    expect(() => createSession(ctx, clientAddress, 'missing-node')).toThrow(
...
describe('getSessionsForAddress', () => {
  test('returns all sessions for address', () => {
    ...
    expect(
      sessions.every(
        (s) => s.clientAddress.toLowerCase() === address1.toLowerCase(),
      ),
    ).toBe(true)
  })
  ...
  test('returns empty array when no sessions', () => {
    const ctx = createTestContext([])
    const sessions = getSessionsForAddress(
      ctx,
      '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' as Address,
    )

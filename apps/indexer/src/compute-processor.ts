...
export async function processComputeEvents(
  ...
  for (const block of ctx.blocks) {
    ...
    for (const log of block.logs) {
      ...
      if (!eventSig || !COMPUTE_EVENT_SIGNATURES.has(eventSig as `0x${string}`))
        continue
      ...
      if (eventSig === SETTLED) {
        ...
        const request = new InferenceRequestEntity({
          ...
          maxTokens:
            BigInt(decoded[1].toString()) + BigInt(decoded[2].toString()), // inputTokens + outputTokens
          tokensUsed:
            BigInt(decoded[1].toString()) + BigInt(decoded[2].toString()),

/**
 * DEX Processor - Indexes DEX swap events for price discovery
 * 
 * Processes:
 * - Pool creation events (PairCreated, PoolCreated)
 * - Swap events for price calculation
 * - Sync events for reserve updates
 * - Mint/Burn events for liquidity tracking
 * 
 * Generates:
 * - Real-time token prices from swap ratios
 * - OHLCV candles at multiple intervals
 * - Pool liquidity and volume stats
 * 
...
const SYNC_V2 = '0x1c411e9a96e071241c2f21f7726b17ae89e3cab4c78be50e062b03a9fffbbad1';
...
const MINT_V3 = '0x7a53080ba414158be7ec69b987b5fb7d07dee101fe85488f0853ae16239d0bde';
// keccak256('Burn(address,int24,int24,uint128,uint256,uint256)')
const BURN_V3 = '0x0c396cd989a39f4459b5fa1aed6a9a8dcdbc45908acfd67e028cd568da98982c';

// All DEX event signatures
const DEX_EVENT_SIGNATURES = new Set([
  PAIR_CREATED_V2, SWAP_V2, SYNC_V2, MINT_V2, BURN_V2,
  POOL_CREATED_V3, SWAP_V3, MINT_V3, BURN_V3,
]);

// Known DEX factory addresses per chain
const DEX_FACTORIES: Record<number, Record<string, { name: string; version: string }>> = {
  1: {
    '0x5c69bee701ef814a2b6a3edd4b1652cb9cc5aa6f': { name: 'Uniswap V2', version: 'v2' },
    '0x1f98431c8ad98523631ae4a59f267346ea31f984': { name: 'Uniswap V3', version: 'v3' },
    '0xc0aee478e3658e2610c5f7a4a2e1777ce9e4f2ac': { name: 'Sushiswap', version: 'v2' },
  },
  42161: {
    '0x1f98431c8ad98523631ae4a59f267346ea31f984': { name: 'Uniswap V3', version: 'v3' },
    '0x6eccab422d763ac031210895c81787e87b43a652': { name: 'Camelot', version: 'v2' },
  },
  8453: {
    '0x33128a8fc17869897dce68ed026d694621f6fdfd': { name: 'Uniswap V3', version: 'v3' },
    '0x420dd381b31aef6683db6b902084cb0ffece40da': { name: 'Aerodrome', version: 'v2' },
  },
  10: {
    '0x1f98431c8ad98523631ae4a59f267346ea31f984': { name: 'Uniswap V3', version: 'v3' },
    '0xf1046053aa5682b4f9a81b5481394da16be5ff5a': { name: 'Velodrome', version: 'v2' },
  },
};

// Stablecoins for USD price calculation
...
// WETH addresses for ETH price derivation
...
function getPeriodStart(timestamp: Date, interval: CandleInterval): Date {
  ...
  switch (interval) {
    case CandleInterval.MINUTE_1: periodMs = 60 * 1000; break;
    case CandleInterval.MINUTE_5: periodMs = 5 * 60 * 1000; break;
    case CandleInterval.MINUTE_15: periodMs = 15 * 60 * 1000; break;
    case CandleInterval.HOUR_1: periodMs = 60 * 60 * 1000; break;
    case CandleInterval.HOUR_4: periodMs = 4 * 60 * 60 * 1000; break;
    case CandleInterval.DAY_1: periodMs = 24 * 60 * 60 * 1000; break;
    case CandleInterval.WEEK_1: periodMs = 7 * 24 * 60 * 60 * 1000; break;
    default: periodMs = 60 * 60 * 1000;
  }
...
export async function processDEXEvents(ctx: ProcessorContext<Store>): Promise<void> {
    ...
    for (const log of block.logs) {
      ...
      if (topic0 === PAIR_CREATED_V2) {
        await processPairCreatedV2(
          ctx, logData, header, timestamp, chainId,
          tokens, pools, dexes, accountFactory
        );
        ...
        await processSwapV2(
          ctx, logData, header, timestamp, chainId,
          tokens, pools, swaps, candles, hourlyCandles, dailyCandles, accountFactory
        );
        ...
        await processSyncV2(ctx, logData, chainId, pools);
      }
    }
  }
...
async function processPairCreatedV2(
  ...
  const token0 = await getOrCreateToken(ctx, tokens, chainId, token0Addr, timestamp, accountFactory);
  const token1 = await getOrCreateToken(ctx, tokens, chainId, token1Addr, timestamp, accountFactory);
  token0.poolCount += 1;
  token1.poolCount += 1;
  ...
  ctx.log.info(`New pool: ${dex.name} ${token0.symbol}/${token1.symbol} at ${pairAddr}`);
}
...
async function processPoolCreatedV3(
  ...
  pools.set(poolId, pool);
  ctx.log.info(`New V3 pool: ${dex.name} ${token0.symbol}/${token1.symbol} fee=${fee}`);
}
...
async function processSwapV2(
  ...
  const [senderAddr, amount0In, amount1In, amount0Out, amount1Out, toAddr] = decoded;
  ...
  const token1 = tokens.get(pool.token1.id) || await ctx.store.get(Token, pool.token1.id);
  ...
  const price = calculatePriceFromAmounts(
    amountIn, amountOut, tokenIn.decimals, tokenOut.decimals
  );
  ...
  if (amount0In > 0n && amount1Out > 0n) {
    pool.token0Price = calculatePriceFromAmounts(amount0In, amount1Out, token0.decimals, token1.decimals);
  } else if (amount1In > 0n && amount0Out > 0n) {
    pool.token1Price = calculatePriceFromAmounts(amount1In, amount0Out, token1.decimals, token0.decimals);
  }
  pools.set(poolId, pool);
  ...
  if (isStablecoin(chainId, tokenIn.address)) {
    ...
    const priceUSD = Number(amountIn) / Number(amountOut) * (10 ** (tokenOut.decimals - tokenIn.decimals));
    tokenOut.priceUSD = priceUSD.toString();
  } else if (isStablecoin(chainId, tokenOut.address)) {
    // tokenIn price = amountOut / amountIn
    const priceUSD = Number(amountOut) / Number(amountIn) * (10 ** (tokenIn.decimals - tokenOut.decimals));
  ...
  // Update candles
  await updateCandles(
    candles, hourlyCandles, dailyCandles,
    tokenIn, tokenOut, pool,
    price, amountIn, amountOut, amountInUSD,
    timestamp, chainId
  );
}
...
async function processSwapV3(
  ...
  const [senderAddr, recipientAddr, amount0, amount1, sqrtPriceX96, liquidity, tick] = decoded;
  const poolAddr = log.address.toLowerCase();
  const poolId = `${chainId}-${poolAddr}`;
  ...
  const swapId = `${txHash}-${log.logIndex}`;
  const sender = accountFactory.getOrCreate(senderAddr, header.height, timestamp);
  const recipient = accountFactory.getOrCreate(recipientAddr, header.height, timestamp);
...
async function updateCandles(
  ...
  for (const interval of intervals) {
    ...
    if (parseFloat(currentPrice) < parseFloat(candleIn.low) || candleIn.low === '0') {
    ...
    if (parseFloat(candleIn.open) > 0) {
      const change = (parseFloat(candleIn.close) - parseFloat(candleIn.open)) / parseFloat(candleIn.open);
  ...
  if (parseFloat(pool.token0Price) < parseFloat(hourly.low) || hourly.low === '0') {

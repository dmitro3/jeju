    ...
    for (const log of block.logs) {
      ...
      if (eventSig === NODE_REGISTERED) {
        ...
        if (node) {
          ...
          node.currentRequestsServed = BigInt(
            decoded.args.requestsServed.toString(),
          )
          node.currentAvgResponseTime = BigInt(
            decoded.args.avgResponseTime.toString(),
          )
        ...
        if (node) {
          ...
          rewardClaims.push(
            new RewardClaim({
              ...
              rewardAmount: amount,
              paymasterFeesETH: BigInt(
                decoded.args.paymasterFeesETH.toString(),
              ),
              timestamp: BigInt(block.header.timestamp),
              blockNumber: BigInt(block.header.height),
              transactionHash: txHash,
        ...
        if (proposal) {
          const decoded = decodeEventLog({
            ...
            eventName: string
            args: { proposalId: string; outcome: boolean }
          }
        ...
        if (proposal) {
          const decoded = decodeEventLog({
            ...
            eventName: string
            args: { proposalId: string; admin: string; reason: string }
          }

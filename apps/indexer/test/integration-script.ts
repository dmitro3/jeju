...
async function startDatabase(): Promise<boolean> {
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (available) {
        ...
        try {
          ...
          try {
            await execAsync(
              'docker exec squid-db-1 psql -U postgres -c "CREATE DATABASE indexer;" 2>&1',
            )
          } catch {
            // Database might already exist, check if we can connect
            try {
              await execAsync(
                'docker exec squid-db-1 psql -U postgres -d indexer -c "SELECT 1;" 2>&1',
              )
            } catch {
    ...
    console.log(
      `⚠️  Failed to start database: ${error instanceof Error ? error.message : String(error)}`,
    )
    return false
  }
}
...
async function queryDatabase(sql: string): Promise<string> {
  ...
  if (!isAvailable) {
    throw new Error(
      'Database container (squid-db-1) is not running. Start it with: cd apps/indexer && bun run db:up',
    )
  }
  ...
  try {
    ...
    if (errorMsg.includes('database "indexer" does not exist')) {
      throw new Error(
        'Database "indexer" does not exist. Run migrations: cd apps/indexer && bun run db:migrate',
      )
    }
...
async function main() {
  ...
  if (!dbAvailable) {
    ...
    if (!dbAvailable) {
      console.log(
        '⚠️  Database container (squid-db-1) is not running and could not be started.',
      )
  ...
  await runTest('Database Schema', async () => {
    ...
    if (parseInt(count, 10) < 15)
      throw new Error(`Expected 15 tables, found ${count}`)
  ...
  await runTest('Blocks Indexed', async () => {
    ...
    if (parseInt(count, 10) === 0) throw new Error('No blocks indexed')
  ...
  await runTest('Transactions Indexed', async () => {
    ...
    if (parseInt(count, 10) === 0) throw new Error('No transactions indexed')
  ...
  await runTest('Event Logs Captured', async () => {
    ...
    if (parseInt(count, 10) === 0)
      throw new Error('No logs captured - event processing not working')
  ...
  await runTest('Events Decoded', async () => {
    ...
    if (parseInt(count, 10) === 0)
      throw new Error('No events decoded - event decoding not working')
  ...
  await runTest('Token Transfers Detected', async () => {
    ...
    if (parseInt(count, 10) === 0)
      throw new Error('No token transfers - token detection not working')
  ...
  await runTest('Contracts Detected', async () => {
    ...
    if (parseInt(count, 10) === 0)
      throw new Error('No contracts - contract detection not working')
  ...
  await runTest('ERC20 Token Detection', async () => {
    const count = await queryDatabase(
      'SELECT COUNT(*) FROM contract WHERE is_erc20 = true',
    )
    if (parseInt(count, 10) === 0) throw new Error('No ERC20 tokens detected')
  ...
  await runTest('ERC721 NFT Detection', async () => {
    const count = await queryDatabase(
      'SELECT COUNT(*) FROM contract WHERE is_erc721 = true',
    )
  ...
  await runTest('ERC1155 Multi-Token Detection', async () => {
    const count = await queryDatabase(
      'SELECT COUNT(*) FROM contract WHERE is_erc1155 = true',
    )
  ...
  await runTest('Account Tracking', async () => {
    ...
    if (parseInt(count, 10) === 0) throw new Error('No accounts tracked')
  ...
  await runTest('Event Type Diversity', async () => {
    const count = await queryDatabase(
      'SELECT COUNT(DISTINCT event_name) FROM decoded_event',
    )
    if (parseInt(count, 10) === 0) throw new Error('No event types decoded')
  ...
  await runTest('Token Standard Coverage', async () => {
    const count = await queryDatabase(
      'SELECT COUNT(DISTINCT token_standard) FROM token_transfer',
    )
    if (parseInt(count, 10) < 2)
      throw new Error('Not enough token standards detected')

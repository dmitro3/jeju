...
describe('Agent ID Validation', () => {
  const isValidAgentId = (id: string): boolean => {
    ...
    return !Number.isNaN(num) && num >= 0 && String(num) === id.trim()
  }
...
describe('Service ID Validation', () => {
  ...
  it('should reject invalid service IDs', () => {
    ...
    expect(
      parseServiceId('unknown-0x1234567890abcdef1234567890abcdef12345678').type,
    ).toBeNull()
    expect(
      parseServiceId('-0x1234567890abcdef1234567890abcdef12345678').type,
    ).toBeNull()
  })
  ...
  const validateA2ARequest = (
    ...
    if (body.method === 'tasks/send') {
      ...
      if (!params?.content || typeof params.content !== 'string') {
        return {
          valid: false,
          error: 'tasks/send requires params.content string',
        }
      }
    }
    ...
    if (body.method === 'tasks/get' || body.method === 'tasks/cancel') {
      ...
      if (!params?.task_id || typeof params.task_id !== 'string') {
        return {
          valid: false,
          error: `${body.method} requires params.task_id string`,
        }
      }
    }

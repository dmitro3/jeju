...
describe('Agent Mappers', () => {
  ...
  describe('mapAgentSummary', () => {
    ...
    it('should throw on missing registeredAt', () => {
      const invalidAgent = { ...validAgent, registeredAt: null as unknown as Date };
      expect(() => mapAgentSummary(invalidAgent)).toThrow('Agent registeredAt is required');
    });
  });
...
describe('Block Mappers', () => {
  ...
  describe('mapBlockSummary', () => {
    ...
    it('should throw on null block', () => {
      expect(() => mapBlockSummary(null as unknown as MockBlock)).toThrow('Block is required');
    });

    it('should throw on negative block number', () => {
      const invalidBlock = { ...validBlock, number: -1 };
      expect(() => mapBlockSummary(invalidBlock)).toThrow('Invalid block number');
    });
    ...
    it('should throw on missing timestamp', () => {
      ...
      expect(() => mapBlockSummary(invalidBlock)).toThrow('Block timestamp is required');
    });
  ...
  describe('mapBlockDetail', () => {
    it('should include all detail fields', () => {
      ...
      expect(result.number).toBe(12345678);
      expect(result.hash).toBe('0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890');
      expect(result.parentHash).toBe('0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef');
      expect(result.timestamp).toBe('2024-06-15T08:30:00.000Z');
      expect(result.transactionCount).toBe(150);
      expect(result.gasUsed).toBe('15000000');
...
describe('Transaction Mappers', () => {
  ...
  describe('mapTransactionSummary', () => {
    it('should map all summary fields correctly', () => {
      ...
      expect(result.hash).toBe('0xdeadbeef1234567890abcdef1234567890abcdef1234567890abcdef12345678');
...
describe('Provider Mappers', () => {
  ...
  describe('mapProviderSummary', () => {
    ...
    it('should throw on null provider', () => {
      expect(() => mapProviderSummary(null as unknown as MockProvider, 'compute')).toThrow('Provider is required');
    });

    it('should throw on invalid type', () => {
      expect(() => mapProviderSummary(computeProvider, 'invalid' as 'compute')).toThrow('Invalid provider type');
    });

    it('should throw on invalid address', () => {
      const invalidProvider = { ...computeProvider, address: null as unknown as string };
      expect(() => mapProviderSummary(invalidProvider, 'compute')).toThrow('Invalid provider address');
    });

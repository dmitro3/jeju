...
function buildBlockWhereClause(identifier: BlockIdentifier): { hash?: string; number?: number } {
...
describe('parseBlockIdentifier', () => {
  describe('Block Numbers', () => {
    it('should parse valid block numbers', () => {
      ...
      expect(parseBlockIdentifier('100')).toEqual({ type: 'number', value: 100 });
      expect(parseBlockIdentifier('12345678')).toEqual({ type: 'number', value: 12345678 });
      expect(parseBlockIdentifier('999999999')).toEqual({ type: 'number', value: 999999999 });
    });
    ...
    it('should throw on purely non-numeric strings', () => {
      ...
      expect(() => parseBlockIdentifier('abc12')).toThrow('Invalid block number');
    });
  ...
  describe('Block Hashes', () => {
    ...
    it('should throw on hash with wrong length', () => {
      // Too short
      expect(() => parseBlockIdentifier('0xabcdef')).toThrow('Invalid block hash format');
      // Too long
      expect(() => parseBlockIdentifier('0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef12345678901234')).toThrow('Invalid block hash format');
    });

    it('should throw on hash with invalid characters', () => {
      expect(() => parseBlockIdentifier('0xghijkl1234567890abcdef1234567890abcdef1234567890abcdef1234567890')).toThrow('Invalid block hash format');
    });

    it('should throw on hash without 0x prefix', () => {
      // This would be treated as a block number and fail
      expect(() => parseBlockIdentifier('abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890')).toThrow('Invalid block number');
    });
  });

  describe('Input Validation', () => {
    it('should throw on empty string', () => {
      expect(() => parseBlockIdentifier('')).toThrow('Block identifier is required');
    });

    it('should throw on null', () => {
      expect(() => parseBlockIdentifier(null as unknown as string)).toThrow('Block identifier is required');
    });

    it('should throw on undefined', () => {
      expect(() => parseBlockIdentifier(undefined as unknown as string)).toThrow('Block identifier is required');
    });

    it('should throw on non-string type', () => {
      expect(() => parseBlockIdentifier(123 as unknown as string)).toThrow('Block identifier is required');
    });
...
describe('buildBlockWhereClause', () => {
  ...
  describe('Number Identifier', () => {
    ...
    it('should handle large block numbers', () => {
      const identifier: BlockIdentifier = { type: 'number', value: 999999999999 };
      const result = buildBlockWhereClause(identifier);

...
function validateContainerParams(params: ContainerParams): {
  ...
  return {
    limit: Math.min(100, parseInt(params.limit || '50', 10) || 50),
  ...
  it('should calculate compute provider stats', () => {
    const providers: Provider[] = [
      ...
      },
      {
        ...
        totalEarnings: 1000000000000000000n,
      },
  ...
  function findFullStackProviders(
    ...
    for (const storage of storageProviders) {
      if (
        storage.agentId &&
        storage.isActive &&
        computeByAgent.has(storage.agentId)
      ) {
        ...
        fullStack
          .find((f) => f.agentId === storage.agentId)

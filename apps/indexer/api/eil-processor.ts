/**
 * EIL cross-chain event processor
 */
...
const VOUCHER_REQUESTED = keccak256(
  stringToHex(
    'VoucherRequested(bytes32,address,address,uint256,uint256,address,uint256,uint256)',
  ),
)
const VOUCHER_ISSUED = keccak256(
  stringToHex('VoucherIssued(bytes32,bytes32,address,uint256)'),
)
const VOUCHER_FULFILLED = keccak256(
  stringToHex('VoucherFulfilled(bytes32,address,uint256)'),
)
const VOUCHER_EXPIRED = keccak256(
  stringToHex('VoucherExpired(bytes32,address)'),
)
const FUNDS_REFUNDED = keccak256(
  stringToHex('FundsRefunded(bytes32,address,uint256)'),
)
const XLP_DEPOSIT = keccak256(
  stringToHex('XLPDeposit(address,address,uint256)'),
)
const XLP_WITHDRAW = keccak256(
  stringToHex('XLPWithdraw(address,address,uint256)'),
)
const SOURCE_FUNDS_CLAIMED = keccak256(
  stringToHex('SourceFundsClaimed(bytes32,address,uint256,uint256)'),
)

// Event signatures for L1StakeManager
const XLP_REGISTERED = keccak256(
  stringToHex('XLPRegistered(address,uint256,uint256[])'),
)
const STAKE_DEPOSITED = keccak256(
  stringToHex('StakeDeposited(address,uint256,uint256)'),
)
const UNBONDING_STARTED = keccak256(
  stringToHex('UnbondingStarted(address,uint256,uint256)'),
)
const STAKE_WITHDRAWN = keccak256(
  stringToHex('StakeWithdrawn(address,uint256)'),
)
const XLP_SLASHED = keccak256(
  stringToHex('XLPSlashed(address,bytes32,uint256,address)'),
)

const EIL_EVENT_SIGNATURES: Set<Hex> = new Set([
  VOUCHER_REQUESTED,
  VOUCHER_ISSUED,
  VOUCHER_FULFILLED,
  VOUCHER_EXPIRED,
...
export async function processEILEvents(
  ...
  if (totalEvents > 0) {
    ctx.log.info(
      `EIL: Processed ${voucherRequests.size} requests, ${vouchers.size} vouchers, ${transfers.size} transfers`,
    )
  }
}
...
async function processVoucherRequested(
  ...
  const requester = await getOrCreateAccount(
    ctx,
    requesterAddr,
    header.height,
    timestamp,
...
async function processVoucherIssued(
  ...
  const request =
    voucherRequests.get(requestId) ||
    (await ctx.store.get(CrossChainVoucherRequest, requestId))
...
async function processVoucherFulfilled(
  ...
  const voucher =
    ...
    (await ctx.store.get(CrossChainVoucher, voucherId))
  if (voucher) {
    ...
    if (voucher.xlp) {
      const xlp =
        xlps.get(voucher.xlp.id) || (await ctx.store.get(XLP, voucher.xlp.id))
      if (xlp) {
    ...
    if (requestId) {
      ...
      const transfer =
        ...
        (await ctx.store.get(EILTransfer, transferId))
      if (transfer) {
...
async function processFundsRefunded(
  ...
  const transferId = `${requestId}-transfer`
  const transfer =
    transfers.get(transferId) || (await ctx.store.get(EILTransfer, transferId))
  if (transfer) {
...
async function processXLPWithdraw(
  ...
  if (deposit) {
    ...
    if (isETH) {
      deposit.ethAmount =
        deposit.ethAmount > amount ? deposit.ethAmount - amount : 0n
    } else {
...
async function processXLPSlashed(
  ...
  const decoded = decodeLogData(
    [{ type: 'uint256' }, { type: 'address' }] as const,
    log.data,
  )
  const amount = decoded[0]
...
async function updateGlobalStats(ctx: ProcessorContext<Store>): Promise<void> {
  ...
  const activeXLPCount = await ctx.store.count(XLP, {
    where: { isActive: true },
  })
  const transferCount = await ctx.store.count(EILTransfer)
  const completedCount = await ctx.store.count(EILTransfer, {
  ...
  stats.successRate =
    transferCount > 0 ? Math.floor((completedCount / transferCount) * 100) : 100

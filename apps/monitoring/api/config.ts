...
const { config, configure: setMonitoringConfig } = createAppConfig<MonitoringConfig>({
  // Server
  port: getEnvNumber('PORT') ?? 9091,
  a2aPort: getEnvNumber('A2A_PORT') ?? 9091,
  isProduction: isProductionEnv(),
  corsOrigins: (getEnvVar('CORS_ORIGINS') ?? 'http://localhost:3000,http://localhost:4020').split(',').map(s => s.trim()).filter(Boolean),

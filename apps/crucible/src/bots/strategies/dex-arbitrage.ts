...
export class DexArbitrageStrategy {
  ...
  initialize(pools: Pool[]): void {
    console.log(
      `🔄 Initializing DEX arbitrage strategy with ${pools.length} pools`,
    )
    ...
    console.log(
      `   Indexed ${this.pools.size} pools, ${this.tokenGraph.size} unique tokens`,
    )
  }
  ...
  private evaluateCrossPoolArb(
    ...
    const buyOutput = getAmountOut(
      clampedInput,
      buyPool.reserve0,
      buyPool.reserve1,
    )
    const sellOutput = getAmountOut(
      buyOutput,
      sellPool.reserve1,
      sellPool.reserve0,
    )
  ...
  private findTriangularArbitrage(startToken: string): void {
    ...
    for (const [token1, edge1] of startNode.pools) {
      ...
      for (const [token2, edge2] of node1.pools) {
        ...
        this.evaluateTriangularArb(start, token1, token2, edge1, edge2, edge3)
      }
    }
  }
  ...
  private evaluateTriangularArb(
    ...
    const { optimalInput, expectedProfit } =
      calculateOptimalTriangularArbitrage(r1In, r1Out, r2In, r2Out, r3In, r3Out)
  ...
  private findMultiHopArbitrage(startToken: string): void {
    ...
    while (queue.length > 0) {
      ...
      for (const [nextToken, edge] of currentNode.pools) {
        ...
        if (
          state.pools.some(
            (e) => e.pool.pool.address === edge.pool.pool.address,
          )
        )
          continue
        ...
        if (nextToken === start && state.pools.length >= 2) {
          ...
          !state.visited.has(nextToken) &&
          state.pools.length < MAX_PATH_LENGTH - 1
        ) {
          // Continue exploring
  ...
  private evaluateMultiHopArb(startToken: string, edges: PoolEdge[]): void {
    ...
    const { optimalInput, expectedProfit } =
      calculateOptimalMultiHopArbitrage(pools)
  ...
  private hasMinLiquidity(poolState: PoolState): boolean {
    return (
      poolState.reserve0 >= MIN_LIQUIDITY && poolState.reserve1 >= MIN_LIQUIDITY
    )
  }

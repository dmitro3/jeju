...
export class OracleKeeperStrategy {
  ...
  private async checkToken(token: string, source: PriceSource): Promise<void> {
    ...
    if (
      ...
      Date.now() - lastUpdate < UPDATE_COOLDOWN_MS
    )
      return
  ...
  private async fetchPythPrice(
    ...
    try {
      ...
      const { price, expo, publishTime } = result as {
        price: bigint
        conf: bigint
        expo: number
        publishTime: bigint
  ...
  private async fetchRedstonePrice(symbol: string): Promise<TokenPrice | null> {
    try {
      ...
      const response = await fetch(
        `${endpoint}?symbols=${symbol}&provider=redstone`,
      )
      ...
      // Check staleness
      const ageSeconds =
        Math.floor(Date.now() / 1000) - Math.floor(priceData.timestamp / 1000)
      if (ageSeconds > STALE_THRESHOLD_SEC) {

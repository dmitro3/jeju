...
export class SolanaArbStrategy extends EventEmitter {
  ...
  private async executeSolanaToEvm(opportunity: SolanaArbOpportunity): Promise<{
    ...
    console.log(`   Jupiter route: ${route.routePlan.map(r => r.swapInfo.label).join(' → ')}`);
  ...
  private async executeEvmToSolana(opportunity: SolanaArbOpportunity): Promise<{
    ...
    if (!tokenAddress) throw new Error(`No token address for ${opportunity.token.symbol} on chain ${evmChainId}`);
  ...
  private async bridgeSolanaToEvm(
    ...
    if (!zkBridgeEndpoint) {
      throw new Error('ZK_BRIDGE_ENDPOINT not configured - bridge integration required');
    }

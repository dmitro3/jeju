...
function calculateHealthFactor(
  ...
  const maintenanceMargin = (position.size * BigInt(maintenanceMarginBps)) / 10000n;
...
function calculateLiquidationProfit(
  ...
  const liquidationBonus = (effectiveMargin > 0n ? effectiveMargin : 0n) * BigInt(market.liquidationBonus) / 10000n;
...
describe('Cascade Detection Logic', () => {
  ...
  test('should simulate cascade scenario', () => {
    ...
    const priceAfterDrop = BigInt(2000e18) * BigInt(Math.floor((1 - priceDropPercent) * 10000)) / 10000n;

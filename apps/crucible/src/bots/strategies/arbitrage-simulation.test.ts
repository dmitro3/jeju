...
import type { ChainId, Pool, StrategyConfig, Token } from '../autocrat-types'
...
describe('DEX Arbitrage Profitability', () => {
  ...
  describe('getAmountOut calculation correctness', () => {
    ...
    test('should handle large trades with price impact', () => {
      ...
      const smallEffectivePrice =
        Number((smallOut * 10000n) / smallTrade) / 10000

      // Large trade: 10% of pool
      const largeTrade = BigInt(10e18)
      const largeOut = calculateAmountOut(largeTrade, reserveIn, reserveOut)
      const largeEffectivePrice =
        Number((largeOut * 10000n) / largeTrade) / 10000
      ...
      console.log(
        `Price impact: ${((1 - largeEffectivePrice / smallEffectivePrice) * 100).toFixed(2)}%`,
      )
    })
  ...
  describe('Triangular arbitrage profitability', () => {
    test('should detect profitable triangular arbitrage', () => {
      ...
      console.log(
        `${Number(usdcOut) / 1e6} USDC -> ${Number(btcOut) / 1e8} BTC`,
      )
  ...
  describe('Cross-pool arbitrage profitability', () => {
    test('should detect cross-pool arbitrage between two pools with same pair', () => {
      ...
      console.log(
        `  Buy ETH in Pool A with USDC -> ${Number(ethFromA) / 1e18} ETH`,
      )
      console.log(`  Profit: ${Number(profit) / 1e18} ETH (${profitBps} bps)`)
    ...
    test('should account for gas costs in profitability', () => {
      ...
      console.log(
        `  Gross profit: ${Number(grossProfit) / 1e18} ETH (${grossProfitBps} bps)`,
      )
      console.log(`  Gas cost: ${Number(gasCost) / 1e18} ETH`)
      console.log(
        `  Net profit: ${Number(netProfit) / 1e18} ETH (${netProfitBps} bps)`,
      )
...
describe('Cross-Chain Arbitrage Profitability', () => {
  ...
  beforeEach(() => {
    strategy = new CrossChainArbStrategy(
      [1, 42161, 8453, 420691] as ChainId[],
      config,
    )
  })
  ...
  describe('Cross-chain profit calculation', () => {
    test('should calculate profit correctly with bridge costs', () => {
      ...
      if (opportunities.length > 0) {
        ...
        console.log(
          `  Buy on: Chain ${opp.sourceChainId} at $${Number(opp.sourcePrice) / 1e18}`,
        )
        console.log(
          `  Sell on: Chain ${opp.destChainId} at $${Number(opp.destPrice) / 1e18}`,
        )
        console.log(`  Price diff: ${opp.priceDiffBps} bps`)
        console.log(`  Input amount: ${Number(opp.inputAmount) / 1e18} tokens`)
        console.log(
          `  Expected profit: ${Number(opp.expectedProfit) / 1e18} tokens`,
        )
        ...
        const expectedPriceDiffBps = Number(
          (priceDiff * 10000n) / arbitrumPrice,
        )
        expect(opp.priceDiffBps).toBeGreaterThanOrEqual(
          expectedPriceDiffBps - 1,
        )
      }
    })
    ...
    test('should NOT profit on small price differences due to bridge costs', () => {
      ...
      if (opportunities.length > 0) {
        for (const opp of opportunities) {
          ...
          console.log(
            `Small diff opp: ${opp.priceDiffBps} bps, net: ${opp.netProfitWei}`,
          )
        }
      }
...
describe('Sandwich Attack Profitability', () => {
  ...
  test('should calculate sandwich profit correctly', () => {
    ...
    console.log(
      `Frontrun: ${Number(frontrunAmountIn) / 1e18} ETH -> ${Number(frontrunOutput) / 1e6} USDC`,
    )
    ...
    const victimOutputAfterFrontrun = getAmountOut(
      victimAmountIn,
      reserve0AfterFrontrun,
      reserve1AfterFrontrun,
    )
    console.log(
      `Victim output after frontrun: ${Number(victimOutputAfterFrontrun) / 1e6} USDC`,
    )

    const victimLoss = victimOutputClean - victimOutputAfterFrontrun
    console.log(
      `Victim loss: ${Number(victimLoss) / 1e6} USDC (${Number((victimLoss * 10000n) / victimOutputClean)} bps)`,
    )

    // Step 5: Reserves after victim
    const reserve0AfterVictim = reserve0AfterFrontrun + victimAmountIn
    const reserve1AfterVictim =
      reserve1AfterFrontrun - victimOutputAfterFrontrun
    ...
    console.log(
      `Backrun: ${Number(frontrunOutput) / 1e6} USDC -> ${Number(backrunOutput) / 1e18} ETH`,
    )
  ...
  test('should show minimum victim size for profitable sandwich', () => {
    ...
    for (const victimEth of [1, 5, 10, 25, 50, 100]) {
      ...
      console.log(
        `  ${victimEth} ETH victim -> ${Number(netProfit) / 1e18} ETH net profit ${netProfit > 0n ? '✓' : '✗'}`,
      )
    }
  ...
  test('should show larger frontrun = more profit but more victim impact', () => {
    ...
    for (const frontrunPct of [5, 10, 20, 30, 50]) {
      ...
      const victimImpactBps = Number(
        (victimImpact * 10000n) / victimOutputClean,
      )

      console.log(
        `  ${frontrunPct}% frontrun: profit ${Number(profit) / 1e18} ETH, victim impact ${victimImpactBps} bps`,
      )
    }
...
describe('Profit Simulation Summary', () => {
  test('should print realistic profit scenarios', () => {
    ...
    for (const profitBps of [10, 50, 100, 200]) {
      ...
      console.log(
        `  ${profitBps} bps profit → min trade: ${Number(minTrade) / 1e18} ETH`,
      )
    }

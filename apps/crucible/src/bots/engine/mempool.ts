...
export class MempoolStreamer extends EventEmitter {
  ...
  private async startChainStream(
    chainId: ChainId,
  ...
  private async startAlchemyStream(
    chainId: ChainId,
    apiKey: string,
  ): Promise<void> {
  ...
  private async startWebSocketStream(
    chainId: ChainId,
    wsUrl: string,
  ): Promise<void> {
  ...
  private processPendingTx(
    ...
    const isKnownRouter = routers.some(
      (r) => r.toLowerCase() === tx.to.toLowerCase(),
    )

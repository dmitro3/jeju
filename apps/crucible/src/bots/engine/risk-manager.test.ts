...
describe('RiskManager', () => {
  ...
  describe('Position Sizing', () => {
    ...
    test('should cap position size at max', () => {
      ...
      expect(result.adjustedSize).toBeLessThanOrEqual(DEFAULT_RISK_CONFIG.maxPositionSizeWei);
    });
  ...
  describe('Chain Risk Profiles', () => {
    test('should apply chain risk multiplier', () => {
      ...
      if (resultMainnet.adjustedSize && resultArbitrum.adjustedSize) {
        expect(resultMainnet.adjustedSize).toBeLessThanOrEqual(resultArbitrum.adjustedSize);
      }
    });
  });

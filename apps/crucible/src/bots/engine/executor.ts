...
export class TransactionExecutor {
  ...
  async execute(opportunity: Opportunity): Promise<ExecutionResult> {
    ...
    if ('inputToken' in opportunity && 'inputAmount' in opportunity) {
      ...
      const balance = await this.getTokenBalance(publicClient, inputToken, this.account.address);
      if (balance < inputAmount) {
        return this.failResult(opportunity, `Insufficient balance: have ${balance}, need ${inputAmount}`, startTime);
      }
    }
    ...
    if ('expectedProfit' in opportunity) {
      ...
      if (estimatedGasCost >= expectedProfit) {
        return this.failResult(opportunity, `Estimated gas cost ${estimatedGasCost} exceeds expected profit ${expectedProfit}`, startTime);
      }
    }
  ...
  private async executeArbitrage(
    ...
    if (balanceBefore < BigInt(inputAmount)) {
      return this.failResult(opportunity, `Insufficient balance: have ${balanceBefore}, need ${inputAmount}`, context.startTime);
    }
    ...
    const balanceAfter = await this.getTokenBalance(publicClient, inputToken, this.account.address);
    const outputBalanceAfter = await this.getTokenBalance(publicClient, outputToken, this.account.address);
    ...
    const actualProfit = outputValueInInputTerms > inputSpent
  ...
  private async executeSandwichWithBundle(
    ...
    console.log(`📦 Sandwich bundle submitted: ${result.bundleHash} (block ${targetBlock}, gas ${simulation.totalGasUsed})`);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (stats.isIncluded && stats.blockNumber) {
        ...
        const actualProfit = outputReceived > inputSpent
  ...
  private async executeSandwichSequential(
    ...
    if (backrunReceipt.status !== 'success') {
      return this.failResult(opportunity, 'Backrun transaction failed', context.startTime);
    }
    ...
    const totalGasCost = (frontrunReceipt.gasUsed + backrunReceipt.gasUsed) * context.gasPrice;
    const actualProfit = outputReceived > inputSpent
  ...
  private async executeLiquidation(
    ...
    const balanceBefore = await publicClient.getBalance({ address: this.account.address });
    ...
    if (receipt.status !== 'success') {
      return this.failResult(opportunity, 'Liquidation transaction failed', context.startTime);
    }

    const balanceAfter = await publicClient.getBalance({ address: this.account.address });

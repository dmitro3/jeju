...
export class MevBundler {
  ...
  private pendingBundles: Map<
    string,
    { params: BundleParams; submittedAt: number; builder: string }
  > = new Map()
  ...
  private async prepareTransactions(
    txs: (BundleTransaction | SignedTransaction)[],
  ): Promise<string[]> {
    const signed: string[] = []
    for (const tx of txs) {
  ...
  private async sendToAllBuilders(params: BundleParams): Promise<BundleResult> {
    ...
    const results = await Promise.allSettled(
      Array.from(this.builders.entries()).map(async ([name, url]) => {
        ...
        if (stats) {
          stats.avgLatencyMs =
            (stats.avgLatencyMs * (stats.submissions - 1) + latency) /
            stats.submissions
        }
    ...
    const successes = results
      .filter(
        (r): r is PromiseFulfilledResult<BundleResult & { builder: string }> =>
    ...
    if (successes.length > 0) {
      ...
      console.log(
        `   ✓ Bundle accepted by ${successes.length}/${this.builders.size} builders (${best.builder})`,
      )
    ...
    try {
      ...
      if (!result?.result?.bundleHash) {
        return {
          bundleHash: '',
          success: false,
          error: 'No bundleHash in response',
        }
      }
  ...
  private async sendL2Bundle(params: BundleParams): Promise<BundleResult> {
    ...
    if (builderEntries.length === 0) {
      return {
        bundleHash: '',
        success: false,
        error: 'No L2 builder configured',
      }
    }
    ...
    return {
      bundleHash: '',
      success: false,
      error: 'No transactions in bundle',
    }
  }
  ...
  async simulateBundle(params: BundleParams): Promise<SimulationResult> {
    ...
    try {
      ...
      return {
        ...
        totalGasUsed: result.result.totalGasUsed
          ? BigInt(result.result.totalGasUsed)
          : undefined,

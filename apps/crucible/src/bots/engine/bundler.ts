...
export class MevBundler {
  ...
  private async signTransaction(tx: BundleTransaction): Promise<string> {
    ...
    if (!chain) {
      throw new Error(`Chain ${this.chainId} not supported`);
    }
  ...
  private async fetchRelay(
    ...
    signature: string,
    body: string
  ): Promise<{ result?: { bundleHash: string }; error?: { message: string } }> {
    try {
  ...
  async simulateBundle(params: BundleParams): Promise<SimulationResult> {
    ...
    if (result.error || !result.result) {
      return { success: false, error: result.error?.message || 'No result in response' };
    }

    const bundleResult = result.result as {
    ...
    return {
      success: true,
      results: bundleResult.results.map((r) => ({
        txHash: r.txHash,
        gasUsed: BigInt(r.gasUsed),
        revert: r.revert,
      })),
      totalGasUsed: bundleResult.totalGasUsed ? BigInt(bundleResult.totalGasUsed) : undefined,
  ...
  async getBundleStats(bundleHash: string): Promise<{ isIncluded: boolean; blockNumber?: bigint }> {
    ...
    return {
      isIncluded: stats.isIncluded === true || stats.isSimulated === true,
      blockNumber: stats.blockNumber ? BigInt(stats.blockNumber) : stats.simulatedAt ? BigInt(stats.simulatedAt) : undefined,
    };
  }
  ...
  async sendPrivateTransaction(tx: BundleTransaction, hints?: MevShareHint): Promise<{ txHash: string; success: boolean; error?: string }> {
    ...
    const txHash = result.result as string | { txHash?: string };
    const hash = typeof txHash === 'string' ? txHash : txHash.txHash || '';

    return { txHash: hash as `0x${string}`, success: !!hash };
  }

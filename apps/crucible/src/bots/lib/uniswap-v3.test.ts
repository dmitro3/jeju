...
describe('Amount Delta Calculations', () => {
  ...
  test('rounding up should give larger or equal result', () => {
    ...
    const amount0RoundUp = getAmount0Delta(sqrtPriceA, sqrtPriceB, liquidity, true);
    const amount0RoundDown = getAmount0Delta(sqrtPriceA, sqrtPriceB, liquidity, false);
    
    expect(amount0RoundUp).toBeGreaterThanOrEqual(amount0RoundDown);

    const amount1RoundUp = getAmount1Delta(sqrtPriceA, sqrtPriceB, liquidity, true);
    const amount1RoundDown = getAmount1Delta(sqrtPriceA, sqrtPriceB, liquidity, false);
...
describe('Next SqrtPrice From Input', () => {
  test('should decrease sqrtPrice for zeroForOne swap', () => {
    ...
    const nextSqrtPrice = getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, amountIn, true);
  ...
  test('should increase sqrtPrice for oneForZero swap', () => {
    ...
    const nextSqrtPrice = getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, amountIn, false);
  ...
  test('larger input should cause larger price movement', () => {
    ...
    const nextSmall = getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, smallInput, true);
    const nextLarge = getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, largeInput, true);
  ...
  test('higher liquidity should reduce price impact', () => {
    ...
    const nextLow = getNextSqrtPriceFromInput(sqrtPriceX96, lowLiquidity, amountIn, true);
    const nextHigh = getNextSqrtPriceFromInput(sqrtPriceX96, highLiquidity, amountIn, true);

...
export class XLPManager {
  ...
  async getXLPProfile(address?: Address): Promise<XLPProfile | null> {
    ...
    const result = await this.publicClient.readContract({
      ...
      functionName: 'getXLP',
      args: [xlpAddress],
    }) as { provider: Address; supportedChains: readonly bigint[]; totalProvided: bigint; totalEarned: bigint; registeredAt: bigint; isActive: boolean };
   ...
   */
  async getXLPsForRoute(sourceChain: ChainId, destChain: ChainId): Promise<Address[]> {
    return await this.publicClient.readContract({
      address: this.config.federatedLiquidityAddress,
      abi: FEDERATED_LIQUIDITY_ABI,
   ...
   */
  async createRequest(token: Address, amount: bigint, targetChainId: ChainId): Promise<`0x${string}`> {
    const value = token === '0x0000000000000000000000000000000000000000' ? amount : 0n;
   ...
   */
  async fulfillRequest(requestId: `0x${string}`, proof: `0x${string}`): Promise<`0x${string}`> {
    const hash = await this.walletClient.writeContract({
      address: this.config.federatedLiquidityAddress,
      abi: FEDERATED_LIQUIDITY_ABI,
  ...
  async getRequest(requestId: `0x${string}`): Promise<LiquidityRequest | null> {
    const result = await this.publicClient.readContract({
      ...
      args: [requestId],
    }) as { requestId: `0x${string}`; requester: Address; token: Address; amount: bigint; sourceChainId: bigint; targetChainId: bigint; createdAt: bigint; deadline: bigint; fulfilled: boolean; fulfiller: Address };
  ...
  async getNetworkLiquidity(chainId: ChainId): Promise<NetworkLiquidity> {
    const result = await this.publicClient.readContract({
      ...
      args: [BigInt(chainId)],
    }) as { chainId: bigint; vault: Address; ethLiquidity: bigint; tokenLiquidity: bigint; utilizationBps: bigint; lastUpdated: bigint };
   ...
   */
  async getBestNetworkForLiquidity(amount: bigint): Promise<{ chainId: ChainId; available: bigint } | null> {
   ...
   */
  async getBestQuote(tokenIn: Address, tokenOut: Address, amountIn: bigint): Promise<{
    poolType: number;
    pool: Address;
    amountOut: bigint;
    ...
    }) as { poolType: number; pool: Address; amountOut: bigint; priceImpactBps: bigint; fee: number };
  ...
  async calculateFulfillmentProfit(request: LiquidityRequest): Promise<{
    ...
    const feeBps = Number(await this.publicClient.readContract({
      address: this.config.federatedLiquidityAddress,
      abi: FEDERATED_LIQUIDITY_ABI,
  ...
  async getXLPStats(): Promise<{
    ...
    const [totalXLPs, activeXLPs, { totalEth, totalToken }, pendingRequests] = await Promise.all([

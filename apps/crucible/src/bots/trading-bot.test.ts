  ...
  describe('Configuration Validation', () => {
    ...
    test('should handle multiple chains', () => {
      const config = {
        ...baseConfig,
        chains: [
          { chainId: 1, name: 'Ethereum', rpcUrl: 'http://localhost:9545', blockTime: 12000, isL2: false, nativeSymbol: 'ETH' },
    ...
    test('should handle multiple strategies', () => {
      const config = {
        ...baseConfig,
        strategies: [
          { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10, maxGasGwei: 100, maxSlippageBps: 50 },
          { type: 'SANDWICH', enabled: true, minProfitBps: 50, maxGasGwei: 200, maxSlippageBps: 100 },
        ],
      };
    ...
    test('should handle disabled strategies', () => {
      const config = {
        ...baseConfig,
        strategies: [
          { type: 'DEX_ARBITRAGE', enabled: false, minProfitBps: 10, maxGasGwei: 100, maxSlippageBps: 50 },
        ],
      };
  ...
  describe('Strategy Configuration Validation', () => {
    test('should validate minProfitBps range', () => {
      const strategies: TradingBotStrategy[] = [
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 0, maxGasGwei: 100, maxSlippageBps: 50 },
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10000, maxGasGwei: 100, maxSlippageBps: 50 },
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: -10, maxGasGwei: 100, maxSlippageBps: 50 },
      ];
    ...
    test('should validate maxGasGwei', () => {
      const strategies: TradingBotStrategy[] = [
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10, maxGasGwei: 1, maxSlippageBps: 50 },
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10, maxGasGwei: 1000, maxSlippageBps: 50 },
      ];
    ...
    test('should validate maxSlippageBps', () => {
      const strategies: TradingBotStrategy[] = [
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10, maxGasGwei: 100, maxSlippageBps: 0 },
        { type: 'DEX_ARBITRAGE', enabled: true, minProfitBps: 10, maxGasGwei: 100, maxSlippageBps: 10000 },
      ];
  ...
  describe('Chain Configuration Validation', () => {
    test('should validate chain IDs', () => {
      const chains: TradingBotChain[] = [
        { chainId: 1, name: 'Ethereum', rpcUrl: 'http://localhost:9545', blockTime: 12000, isL2: false, nativeSymbol: 'ETH' },
        { chainId: 1337, name: 'Localnet', rpcUrl: 'http://localhost:9545', blockTime: 1000, isL2: false, nativeSymbol: 'ETH' },
        { chainId: 999999, name: 'Invalid', rpcUrl: 'http://localhost:9545', blockTime: 1000, isL2: false, nativeSymbol: 'ETH' },
      ];
    ...
    test('should validate block times', () => {
      const chains: TradingBotChain[] = [
        { chainId: 1, name: 'Ethereum', rpcUrl: 'http://localhost:9545', blockTime: 1, isL2: false, nativeSymbol: 'ETH' },
        { chainId: 1, name: 'Ethereum', rpcUrl: 'http://localhost:9545', blockTime: 60000, isL2: false, nativeSymbol: 'ETH' },
      ];
...
describe('Executor Bot Type Routing', () => {
  test('should route to correct handler based on bot type', () => {
    ...
    botTypes.forEach(botType => {
      expect(['ai_agent', 'trading_bot', 'org_tool'].includes(botType)).toBe(true);
    });
  });

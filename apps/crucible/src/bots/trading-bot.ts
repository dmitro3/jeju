...
export interface TradingBotOptions {
  ...
  contractAddresses?: Record<
    number,
    { xlpRouter?: string; perpetualMarket?: string; priceOracle?: string }
  >
}
...
export class TradingBot {
  ...
  constructor(config: TradingBotOptions) {
    ...
    const crossChainConfig = config.strategies.find(
      (s) => s.type === 'CROSS_CHAIN_ARBITRAGE',
    )
    if (crossChainConfig) {
  ...
  async initialize(): Promise<void> {
    this.log.info('Initializing trading bot', {
    ...
    for (const chainConfig of this.chainConfigs) {
      ...
      if (dexConfig?.enabled) {
        this.dexArbitrage.set(
          chainId,
          new DexArbitrageStrategy(chainId, dexConfig as StrategyConfig),
        )
      }
      ...
      if (sandwichConfig?.enabled) {
        this.sandwich.set(
          chainId,
          new SandwichStrategy(chainId, sandwichConfig as StrategyConfig),
        )
      }
      ...
      if (oracleConfig?.enabled && addresses?.priceOracle) {
        const strategy = new OracleKeeperStrategy(
          chainId,
          oracleConfig as StrategyConfig,
    ...
    if (solverConfig?.enabled) {
      this.solver = new SolverStrategy(
        this.chainConfigs,
        solverConfig as StrategyConfig,
        this.config.privateKey,
      )
  ...
  async start(): Promise<void> {
    ...
    this.processingLoop = setInterval(async () => {
      ...
      try {
        ...
        this.log.error('Error processing opportunities', {
          error: String(error),
        })
      } finally {
        processing = false

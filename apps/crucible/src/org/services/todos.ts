...
export class TodoService {
  ...
  ): Promise<{ todo: Todo; state: OrgState; cid: string }> {
    const todo: Todo = {
  ...
  async update(
    ...
    if (!state.todos.find((t) => t.id === todoId))
    ...
    if (params.dueDate !== undefined)
      updates.dueDate = params.dueDate ?? undefined
    if (params.assigneeAgentId !== undefined)
      updates.assigneeAgentId = params.assigneeAgentId
    if (params.assigneeName !== undefined)
      updates.assigneeName = params.assigneeName
  ...
  list(
    ...
    if (params.assigneeAgentId)
      todos = todos.filter((t) => t.assigneeAgentId === params.assigneeAgentId)

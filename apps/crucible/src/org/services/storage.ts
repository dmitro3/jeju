...
export class OrgStorage {
  ...
  async saveState(state: OrgState): Promise<string> {
    ...
    const r = await fetch(`${this.config.apiUrl}/api/v1/add`, {
      ...
      body: JSON.stringify({
        ...
        filename: `org-${state.orgId}-v${state.version}.json`,
        pin: true,
      }),
    })
  ...
  async updateState(
    current: OrgState,
    updates: Partial<OrgState>,
  ): Promise<{ state: OrgState; cid: string }> {
  ...
  async completeTodo(state: OrgState, todoId: string) {
    return this.updateTodo(state, todoId, {
      status: 'completed',
  ...
  async addCheckinSchedule(state: OrgState, schedule: CheckinSchedule) {
    return this.updateState(state, {
      checkinSchedules: [...state.checkinSchedules, schedule],
    })
  }

  async recordCheckinResponse(state: OrgState, response: CheckinResponse) {
    return this.updateState(state, {
      checkinResponses: [...state.checkinResponses, response],
    })
  }
      ...
      return this.updateState(state, {
        teamMembers: state.teamMembers.map((m) =>
          m.agentId === member.agentId
            ? { ...m, ...member, lastActiveAt: Date.now() }
            : m,
        ),
      })
    }

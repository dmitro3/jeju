 ...
 */
const ROUTES = [
  { path: '/', name: 'Home', expectedContent: 'Crucible', description: 'Crucible home page with fire emoji branding, hero section, feature cards, and navigation to Agents/Chat.' },
  { path: '/agents', name: 'Agents', expectedContent: 'Agent', description: 'Agents list page with Create Agent button, Active Only filter toggle, agent count, and agent cards grid.' },
  { path: '/agents/new', name: 'Create Agent', expectedContent: 'Create', description: 'Agent creation form with name, description, model selection, and configuration options.' },
  { path: '/chat', name: 'Chat', expectedContent: 'Chat', description: 'Chat page with New Room button, agent selector sidebar, and chat interface area.' },
]
...
test.describe('Crucible - Header Component', () => {
  test('header has all navigation elements', async ({ page }) => {
    ...
    await expect(page.locator('a:has-text("Agents"), [href*="agents"]').first()).toBeVisible()
    await expect(page.locator('a:has-text("Chat"), [href*="chat"]').first()).toBeVisible()
    
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Header-Components.png') })
  })
...
test.describe('Crucible - Agents Page Components', () => {
  ...
  test('has Active Only filter toggle', async ({ page }) => {
    ...
    const filterBtn = page.locator('button').filter({ hasText: /Show All|Active Only/ })
...
test.describe('Crucible - Create Agent Page', () => {
  test('has character selection grid', async ({ page }) => {
    ...
    const hasCards = await page.locator('button:has-text("🤖")').first().isVisible().catch(() => false)
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Create-Agent-Form.png'), fullPage: true })
  })
})
...
test.describe('Crucible - Chat Page Components', () => {
  test('has New Room button', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("New Room")').first()).toBeVisible()
  })
  ...
  test('New Room button opens create form', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Chat-Create-Room.png'), fullPage: true })
  })
  ...
  test('room creation form validation', async ({ page }) => {
    ...
    // Room name input
    const roomNameInput = page.locator('input#room-name, input[placeholder*="Security"]')
    await expect(roomNameInput).toBeVisible()
    
    // Description textarea
    const descriptionInput = page.locator('textarea#room-description, textarea[placeholder*="Describe"]')
...
test.describe('Crucible - Footer Component', () => {
  test('footer is visible', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Footer.png'), fullPage: true })
  })
})
...
test.describe('Crucible - Mobile Responsiveness', () => {
  test('Home renders on mobile', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Mobile-Home.png'), fullPage: true })
  })
  ...
  test('Agents renders on mobile', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Mobile-Agents.png'), fullPage: true })
  })
  ...
  test('Chat renders on mobile', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Mobile-Chat.png'), fullPage: true })
  })
})

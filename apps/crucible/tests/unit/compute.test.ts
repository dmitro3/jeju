...
import { type CrucibleCompute, createCompute } from '../../api/sdk/compute'
...
describe('CrucibleCompute', () => {
  ...
  describe('Model Discovery', () => {
    it('should fetch available models', async () => {
      ...
      // Note: SDK returns empty capabilities as models endpoint may not include them
    ...
    it('should find best model by capabilities', async () => {
      // Note: The SDK currently sets capabilities: [] for all models
      // because the models endpoint may not include them.
      // So capability filtering will return null until SDK is updated.
      const mockModels = {
        models: [
          {
            id: 'chat-only',
      ...
      // Capability filtering returns null since SDK strips capabilities
  ...
  describe('Inference', () => {
    it('should run basic inference', async () => {
      // Mock must match ChatCompletionResponseSchema format
      const mockResponse = {
        id: 'test-123',
        choices: [
          {
      ...
      // SDK returns 0n for cost since DWS response doesn't include it

...
import { cacheKeys } from '../../api/services/cache'
...
// Validated test addresses
const TEST_ADDRESS = expectAddress(
...
describe('Cache Keys', () => {
  ...
  test('should lowercase addresses', () => {
    // Use TEST_ADDRESS which is already valid and contains some uppercase in checksummed form
    // When generating cache keys, addresses should be lowercased for consistency
    const key = cacheKeys.todoList(TEST_ADDRESS)

    // The key should lowercase the address portion
...
import { z } from 'zod'
import { filterOverdue } from '../../lib/utils'

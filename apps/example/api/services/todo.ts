...
import { z } from 'zod'
import type { CreateTodoInput, Todo, UpdateTodoInput } from '../../lib/schemas'
import {
  addressSchema,
...
class TodoServiceImpl implements TodoService {
  ...
  async listTodos(
    ...
    if (cached) {
      const validatedList = expectValid(
        z.array(todoSchema),
        cached,
        'Cached todo list',
      )
  ...
  async getTodo(id: string, owner: Address): Promise<Todo | null> {
    ...
    const cached = await this.cache.get(cacheKey)
    if (cached) {
      const validatedCached = expectValid(
        todoSchema,
        cached,
        `Cached todo ${id}`,
      )
      if (validatedCached.owner.toLowerCase() === owner.toLowerCase()) {
  ...
  async createTodo(owner: Address, input: CreateTodoInput): Promise<Todo> {
    ...
    if (validatedInput.encrypt) {
      ...
      if (!updated) {
        throw new ValidationError(
          `Failed to retrieve todo ${todo.id} after encryption`,
        )
      }
    ...
    if (validatedInput.attachment) {
      const cid = await this.storage.upload(
        validatedInput.attachment,
        `${todo.id}-attachment`,
        owner,
      )
      ...
      if (!updated) {
        throw new ValidationError(
          `Failed to retrieve todo ${todo.id} after attachment`,
        )
      }

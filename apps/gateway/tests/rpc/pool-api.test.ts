...
const SKIP_INTEGRATION_TESTS =
  process.env.SKIP_INTEGRATION_TESTS === 'true' || !serverRunning
...
if (!serverRunning && process.env.SKIP_INTEGRATION_TESTS !== 'true') {
  ...
  console.log(
    '   Start with: bun run dev, or set SKIP_INTEGRATION_TESTS=true\n',
  )
}

;(SKIP_INTEGRATION_TESTS ? describe.skip : describe)(
  'Pool API - Edge Cases & Error Handling',
  () => {
    beforeAll(async () => {
    ...
    describe('GET /api/pools/stats', () => {
      ...
      test('totalPools calculation is correct', async () => {
        ...
        expect(status).toBe(200)
        const expectedTotal =
          data.v2Pools + data.v3Pools + (data.paymasterEnabled ? 1 : 0)
      ...
      test('quotes are sorted by amountOut descending', async () => {
        ...
        if (status === 200 && data.length > 1) {
          for (let i = 0; i < data.length - 1; i++) {
            expect(Number(data[i].amountOut)).toBeGreaterThanOrEqual(
              Number(data[i + 1].amountOut),
            )
          }
        }
      })

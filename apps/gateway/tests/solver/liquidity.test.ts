...
import {
  type AccountClient,
  type BalanceClient,
  type LiquidityClients,
  LiquidityManager,
} from '../../src/solver/liquidity'

/** Mock client that tracks calls - implements the minimal interfaces */
interface MockLiquidityClient extends LiquidityClients {
...
function createMockClient(
  ...
  const publicClient: BalanceClient = {
  ...
  const walletClient: AccountClient = {
    account: {
...
describe('Balance Tracking', () => {
  ...
  test('should fetch initial balances on initialize', async () => {
    ...
    await manager.initialize(clients)

    expect(mockClient.getCalls()).toContain(
  ...
  test('should track balance per chain', async () => {
    ...
    expect(
      manager.getBalance(84532, '0x0000000000000000000000000000000000000000'),
    ).toBe(parseEther('2.0'))
  })
  ...
  test('should return 0 for unknown chain', async () => {
    ...
    expect(
      manager.getBalance(999999, '0x0000000000000000000000000000000000000000'),
    ).toBe(0n)
  })
...
describe('hasLiquidity', () => {
  ...
  test('should return true when balance exceeds required', async () => {
    ...
    const result = await manager.hasLiquidity(
      11155111,
      '0x0000000000000000000000000000000000000000',
      '1000000000000000000',
    ) // 1 ETH
...
describe('recordFill', () => {
  ...
  test('should deduct fill amount from balance (before async refresh)', async () => {
    ...
    const mockClient: LiquidityClients = {
      public: publicClient,
      wallet: walletClient,
    }
    const clients = new Map([[11155111, mockClient]])
    ...
    const beforeBalance = manager.getBalance(
      11155111,
      '0x0000000000000000000000000000000000000000',
    )
    ...
    const afterBalance = manager.getBalance(
      11155111,
      '0x0000000000000000000000000000000000000000',
    )

...
describe('OIF Solver Metrics', () => {
  ...
  describe('recordIntentReceived', () => {
    it('should increment counter for chain', () => {
      ...
      const initialCount =
        before.counters.oif_intents_received_total?.find(
          (m) => m.labels.chain === '1',
        )?.value || 0
      ...
      const newCount =
        after.counters.oif_intents_received_total?.find(
          (m) => m.labels.chain === '1',
        )?.value || 0
  ...
  describe('recordIntentFilled', () => {
    it('should increment fill counter', () => {
      const before = getMetricsJson()
      const initialCount =
        before.counters.oif_intents_filled_total?.find(
          (m) =>
            m.labels.source_chain === '1' && m.labels.dest_chain === '42161',
        )?.value || 0
      ...
      const after = getMetricsJson()
      const newCount =
        after.counters.oif_intents_filled_total?.find(
          (m) =>
            m.labels.source_chain === '1' && m.labels.dest_chain === '42161',
        )?.value || 0
  ...
  describe('recordIntentSkipped', () => {
    it('should track skip reasons', () => {
      ...
      const liquiditySkips = metrics.counters.oif_intents_skipped_total?.find(
        (m) =>
          m.labels.chain === '1' &&
          m.labels.reason === 'insufficient_liquidity',
      )
  ...
  describe('recordSettlementClaimed', () => {
    ...
    it('should accumulate profit', () => {
      const before = getMetricsJson()
      const initialProfit =
        before.counters.oif_solver_profit_wei_total?.find(
          (m) => m.labels.chain === '1',
        )?.value || 0
      ...
      const after = getMetricsJson()
      const newProfit =
        after.counters.oif_solver_profit_wei_total?.find(
          (m) => m.labels.chain === '1',
        )?.value || 0

...
describe('Config Loading', () => {
  ...
  test('contracts.json has expected structure', () => {
    ...
    const hasConfig =
      contracts.testnet || contracts.mainnet || contracts.external
...
describe('Chain Validation', () => {
  ...
  function validateCrossChainRoute(
    source: number,
    dest: number,
  ): { valid: boolean; error?: string } {
    if (!isChainSupported(source))
      return { valid: false, error: `Unsupported source: ${source}` }
    if (!isChainSupported(dest))
      return { valid: false, error: `Unsupported dest: ${dest}` }
    if (source === dest)

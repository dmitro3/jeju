...
describe('Quote Service (Real)', () => {
  ...
  describe('getQuotes - Basic Behavior', () => {
    ...
    it('should preserve token addresses in quote', async () => {
      const quotes = await getQuotes(baseParams)
      for (const quote of quotes) {
        expect(quote.sourceToken.toLowerCase()).toBe(
          baseParams.sourceToken.toLowerCase(),
        )
        expect(quote.destinationToken.toLowerCase()).toBe(
          baseParams.destinationToken.toLowerCase(),
        )
      }
    })
  })
  ...
  describe('getQuotes - L2 Route Optimization', () => {
    ...
    it('should estimate faster fill time for L2-to-L2', async () => {
      ...
      expect(l2Quotes[0].estimatedFillTimeSeconds).toBeLessThanOrEqual(
        l1Quotes[0].estimatedFillTimeSeconds,
      )
    })
  })

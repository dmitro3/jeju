...
/** Testable subclass that exposes protected parseEvent method */
class TestableEventMonitor extends EventMonitor {
  public testParseEvent(chainId: number, log: EventLog): IntentEvent | null {
    return this.parseEvent(chainId, log)
  }
}

// Create testable monitor instance
const createMonitor = () => new TestableEventMonitor({ chains: [] })
...
function createMockLog(
  ...
  const defaultReceived = {
    ...
    recipient:
      '0x000000000000000000000000f39fd6e51aad88f6f4ce6ab8827279cfffb92266' as `0x${string}`,
    chainId: 84532n,
  }

  const args: EventArgs & Record<string, unknown> = {
...
describe('Event Parsing - Invalid Events', () => {
  test('should reject event without orderId', () => {
    const monitor = createMonitor()
    const log = createMockLog()
    delete (log.args as EventArgs).orderId
  ...
  test('should reject event without order struct', () => {
    const monitor = createMonitor()
    const log = createMockLog()
    delete (log.args as EventArgs).order
...
describe('Event Parsing - Edge Cases', () => {
  ...
  test('should handle missing fillDeadline', () => {
    ...
    const mutableOrder = (log.args as EventArgs).order as {
      fillDeadline?: number
    }

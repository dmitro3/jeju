...
const _USDT = '0xdAC17F958D2ee523a2206206994597C13D831ec7' as Address
...
describe('DEX Contract Addresses', () => {
  ...
  describe('Uniswap V2 Router', () => {
    it('should have router on Ethereum', () => {
      expect(UNISWAP_V2_ROUTER[1]).toBe(
...
describe('Intermediate Tokens', () => {
  it('should have intermediate tokens for Ethereum', () => {
    ...
    expect(ethTokens.some((t) => t.toLowerCase() === WETH.toLowerCase())).toBe(
      true,
    )
    expect(ethTokens.some((t) => t.toLowerCase() === USDC.toLowerCase())).toBe(
      true,
    )
  })
...
describe('Live DEX Quotes', () => {
  ...
  it('should get best quote across DEXes', async () => {
    ...
    if (quotes) {
      ...
      expect(Number(quotes.best.amountOut)).toBeGreaterThan(0)
      expect(['uniswap_v2', 'uniswap_v3', 'balancer']).toContain(
        quotes.best.dex,
      )

...
function createMockLiquidityPools(): Map<
  string,
...
describe('Solver Metrics Calculation', () => {
  ...
  it('should build solution for matching orders', () => {
    ...
    if (solution) {
      ...
      expect(solution.trades[0].executedBuyAmount).toBeGreaterThanOrEqual(
...
describe('Live Auction Validation', () => {
  ...
  it.skipIf(SKIP_NETWORK_TESTS)(
    'should validate against live Ethereum auction',
    async () => {
      ...
      if (metrics) {
        ...
        console.log(
          `   Would Be Competitive: ${metrics.competitive ? 'YES' : 'NO'}`,
        )
  ...
  it.skipIf(!RUN_EXTENDED_TESTS)(
    'should run continuous validation',
    async () => {
      await solver.start()
      ...
      if (results.length > 0) {
        const avgScore =
          results.reduce((sum, m) => sum + m.competitiveScore, 0) /
          results.length
        const avgFillRate =
          results.reduce((sum, m) => sum + m.fillRate, 0) / results.length
...
describe('Competition Comparison', () => {
  ...
  it('should identify when we would not win', () => {
    ...
    expect(ourMetrics.totalSurplusWei).toBeLessThan(
      winningSolution.totalSurplusWei,
    )
  })
  ...
  it('should identify when we would win', () => {
    ...
    expect(ourMetrics.totalSurplusWei).toBeGreaterThan(
      winningSolution.totalSurplusWei,
    )
  })
})
  ...
  it('should print comparison report without errors', () => {
    const result = {
      ...
      comparison: {
        ...
        reasons: [
          'Our surplus is 0.1 ETH higher',
          '✅ Our solution would win based on surplus',
        ],
      },
    }

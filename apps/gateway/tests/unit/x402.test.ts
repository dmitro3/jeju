...
describe('Gateway x402 - Payment Tiers', () => {
  ...
  test('tiers are in reasonable order', () => {
    ...
    expect(PAYMENT_TIERS.API_PREMIUM).toBeLessThan(
      PAYMENT_TIERS.PREMIUM_API_DAILY,
    )
    expect(PAYMENT_TIERS.PREMIUM_API_DAILY).toBeLessThan(
      PAYMENT_TIERS.PREMIUM_API_MONTHLY,
    )
    expect(PAYMENT_TIERS.LIQUIDITY_REMOVE).toBeLessThan(
      PAYMENT_TIERS.LIQUIDITY_ADD,
    )
    expect(PAYMENT_TIERS.NODE_REGISTRATION).toBeLessThan(
...
describe('Gateway x402 - Payment Requirement Structure', () => {
  ...
  test('creates payment requirement with correct structure', () => {
    ...
    const req = createPaymentRequirement(
      '/api/test',
      PAYMENT_TIERS.NODE_REGISTRATION,
      'Node fee',
      testAddress,
    )
  ...
  test('sets default token address to zero address (native)', () => {
    ...
    expect(req.accepts[0].asset).toBe(
  ...
  test('supports different networks', () => {
    ...
    const jejuReq = createPaymentRequirement(
      '/api/test',
      PAYMENT_TIERS.API_BASIC,
      'test',
      testAddress,
    ...
    const baseReq = createPaymentRequirement(
      '/api/test',
      PAYMENT_TIERS.API_BASIC,
      'test',
      testAddress,
    ...
    const testnetReq = createPaymentRequirement(
      '/api/test',
      PAYMENT_TIERS.API_BASIC,
      'test',
      testAddress,

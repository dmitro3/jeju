...
describe('x402 Settler - Retry Logic', () => {
  ...
  function calculateRetryDelayNoJitter(
    attempt: number,
    baseDelayMs: number,
    maxDelayMs: number,
  ): number {
...
describe('x402 Settler - Retryable Error Detection', () => {
  describe('non-retryable errors', () => {
    test('insufficient funds is not retryable', () => {
      expect(
        isRetryableError(new Error('Error: insufficient funds for gas')),
      ).toBe(false)
    })

    test('insufficient balance is not retryable', () => {
      expect(
        isRetryableError(new Error('Insufficient balance for transfer')),
      ).toBe(false)
    })

    test('insufficient allowance is not retryable', () => {
      expect(isRetryableError(new Error('ERC20: insufficient allowance'))).toBe(
        false,
      )
    })
    ...
    test('execution reverted is not retryable', () => {
      expect(
        isRetryableError(new Error('execution reverted: TRANSFER_FAILED')),
      ).toBe(false)
    })
    ...
    test('invalid signature is not retryable', () => {
      expect(
        isRetryableError(
          new Error('Invalid signature: ECDSA verification failed'),
        ),
      ).toBe(false)
    })

    test('user rejected is not retryable', () => {
      expect(isRetryableError(new Error('User rejected the request'))).toBe(
        false,
      )
    })
    ...
    test('user denied is not retryable', () => {
      expect(
        isRetryableError(new Error('User denied transaction signing')),
      ).toBe(false)
    })
  })
  ...
  describe('retryable errors', () => {
    test('timeout is retryable', () => {
      expect(isRetryableError(new Error('Request timeout after 30000ms'))).toBe(
        true,
      )
    })
    ...
    test('network error is retryable', () => {
      expect(
        isRetryableError(new Error('Network error: could not connect')),
      ).toBe(true)
    })
    ...
    test('replacement transaction underpriced is retryable', () => {
      expect(
        isRetryableError(new Error('replacement transaction underpriced')),
      ).toBe(true)
    })

    test('already known transaction is retryable', () => {
      expect(isRetryableError(new Error('Transaction already known'))).toBe(
        true,
      )
    })
  })
  ...
  describe('edge cases', () => {
    test('unknown error is not retryable', () => {
      expect(isRetryableError(new Error('Some unknown error occurred'))).toBe(
        false,
      )
    })

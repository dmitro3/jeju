...
import { clearNonceCache, markNoncePending, isNonceUsedLocally } from '../../src/x402/services/nonce-manager';
...
describe('Nonce Pending State', () => {
  ...
  test('pending nonce should be detected as used', async () => {
    const nonce = 'pending-test-nonce';
    
    expect(await isNonceUsedLocally(TEST_ACCOUNT.address, nonce)).toBe(false);
    
    await markNoncePending(TEST_ACCOUNT.address, nonce);
    
    expect(await isNonceUsedLocally(TEST_ACCOUNT.address, nonce)).toBe(true);
  });

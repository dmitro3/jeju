...
describe('Supported Schemes Endpoint', () => {
  ...
  test('GET /supported includes jeju network with both exact and upto schemes', async () => {
    ...
    const jejuSchemes = body.kinds.filter(
      (k: { scheme: string; network: string }) => k.network === 'jeju',
    )
    expect(jejuSchemes.length).toBeGreaterThanOrEqual(2)

    const exactScheme = jejuSchemes.find(
      (k: { scheme: string }) => k.scheme === 'exact',
    )
    const uptoScheme = jejuSchemes.find(
  ...
  test('GET /supported/networks returns network details', async () => {
    ...
    const jeju = body.networks.find(
      (n: { network: string }) => n.network === 'jeju',
    )

...
describe('Direct CHAIN_CONFIGS access', () => {
  ...
  test('usdc addresses are correct', () => {
    expect(CHAIN_CONFIGS['base-sepolia'].usdc.toLowerCase()).toBe(
      '0x036cbd53842c5426634e7929541ec2318f3dcf7e',
    )
    expect(CHAIN_CONFIGS.base.usdc.toLowerCase()).toBe(
      '0x833589fcd6edb6e08f4c7c32d4f71b54bda02913',
    )
    expect(CHAIN_CONFIGS.ethereum.usdc.toLowerCase()).toBe(
...
describe('Token Config', () => {
  ...
  test('case-insensitive USDC matching', () => {
    expect(
      getTokenConfig('jeju', usdcJeju.toLowerCase() as Address).symbol,
    ).toBe('USDC')
    expect(
      getTokenConfig('jeju', usdcJeju.toUpperCase() as Address).symbol,
    ).toBe('USDC')
  })
})

...
describe('Retry Configuration', () => {
  ...
  test('should respect environment variable overrides', () => {
    ...
    // Restore
    if (originalMaxRetries) process.env.SETTLEMENT_MAX_RETRIES = originalMaxRetries;
    else delete process.env.SETTLEMENT_MAX_RETRIES;
    if (originalGasMultiplier) process.env.SETTLEMENT_GAS_MULTIPLIER = originalGasMultiplier;
...
describe('Gas Multiplier', () => {
  ...
  test('should apply multiplier correctly', () => {
    ...
    const adjustedGas = BigInt(Math.ceil(Number(estimatedGas) * config.gasMultiplier));

...
describe('NodeStakingManager Contract', () => {
  ...
  beforeAll(async () => {
    ...
    hasNodeStakingManager =
      !!addresses.nodeStakingManager && addresses.nodeStakingManager !== '0x'
  })
  ...
  test('should read network stats', async () => {
    ...
    const [totalNodes, totalStaked, totalRewards] = stats as [
      bigint,
      bigint,
      bigint,
    ]
  ...
  test('should read node info if nodes exist', async () => {
    ...
    if (nodeIds.length > 0) {
      ...
      expect(node.operator.toLowerCase()).toBe(

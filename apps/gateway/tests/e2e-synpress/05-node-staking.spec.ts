...
test.describe('Node Staking Flow', () => {
  ...
  test('should display node staking interface', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-node-staking.png',
      fullPage: true,
    })
  })
  ...
  test('should display register node form', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-node-register.png',
      fullPage: true,
    })
  })
  ...
  test('should allow different reward token than staking token', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Register New Node/i }).click()
  ...
  test('should show geographic bonus for underserved regions', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Register New Node/i }).click()
    ...
    const regionSelect = page
      .locator('select')
      .filter({ hasText: /North America/i })
  ...
  test.skip('should register node successfully', async ({ page, metamask }) => {
    ...
    // Wait for success
    await expect(page.getByText(/Node registered successfully/i)).toBeVisible({
      timeout: 90000,
    })

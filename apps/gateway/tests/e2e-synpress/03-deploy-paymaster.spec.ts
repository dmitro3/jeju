...
test.describe('Deploy Paymaster Flow', () => {
  ...
  test('should display deployment interface', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-deploy-interface.png',
      fullPage: true,
    })
  })

  test('should include ALL tokens in selector (including elizaOS)', async ({
    page,
  }) => {
    // Open token selector
  ...
  test.skip('should deploy paymaster successfully', async ({
    page,
    metamask,
  }) => {
    ...
    await expect(page.getByText(/deployed successfully/i)).toBeVisible({
      timeout: 90000,
    })

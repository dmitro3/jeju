...
test.describe('OIF Cross-Chain Intent Flow', () => {
  ...
  test('should show available chains for cross-chain intents', async ({
    page,
  }) => {
    const transferTab = page.locator('text=Cross-Chain Transfer').first()
    ...
    const hasChainSelector =
      (await page.locator('text=Destination Chain').isVisible()) ||
  ...
  test('should show fee estimate for cross-chain transfer', async ({
    page,
  }) => {
    const transferTab = page.locator('text=Cross-Chain Transfer').first()
    if (await transferTab.isVisible()) {
...
test.describe('OIF Solver Integration', () => {
  ...
  test('should display contract addresses for transparency', async ({
    page,
  }) => {
    ...
    const hasEtherscan =
      (await page.locator('[href*="etherscan"]').isVisible()) ||
...
test.describe('Cross-Chain Route Validation', () => {
  test('Sepolia to Base Sepolia route should be valid', async ({
    page,
    metamask,
  }) => {
    await page.goto('/')

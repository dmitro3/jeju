...
import { approveTransaction, connectWallet } from '../helpers/wallet-helpers'
...
test.describe('App Registry Flow', () => {
  ...
  test('should display app registry interface', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-app-registry.png',
      fullPage: true,
    })
  })
  ...
  test('should show browse and register sections', async ({ page }) => {
    await expect(
      page.getByRole('button', { name: /Browse Apps/i }),
    ).toBeVisible()
    await expect(
      page.getByRole('button', { name: /Register App/i }),
    ).toBeVisible()
  ...
  test('should show A2A enabled badge for apps with endpoints', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Browse Apps/i }).click()
    await page.waitForTimeout(1000)
  ...
  test('should display registration form', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-app-register.png',
      fullPage: true,
    })
  })
  ...
  test('should have stake token selector with all protocol tokens', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Register App/i }).click()
  ...
  test('should show refundable stake info', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Your stake is fully refundable/i),
    ).toBeVisible()
  ...
  test.skip('should register app successfully', async ({ page, metamask }) => {
    ...
    // Wait for success
    await expect(page.getByText(/App registered successfully/i)).toBeVisible({
      timeout: 90000,
    })

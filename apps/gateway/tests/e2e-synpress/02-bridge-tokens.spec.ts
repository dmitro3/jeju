...
test.describe('Bridge from Ethereum Flow', () => {
  ...
  test('should display bridge interface', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/synpress-bridge-interface.png',
      fullPage: true,
    })
  })
  ...
  test.skip(
    !!process.env.CI,
    'should execute bridge transaction',
    async ({ page, metamask }) => {
      // Select token
      await page.locator('.input').first().click()
      ...
      await expect(page.getByText(/Bridge transaction submitted/i)).toBeVisible(
        { timeout: 60000 },
      )

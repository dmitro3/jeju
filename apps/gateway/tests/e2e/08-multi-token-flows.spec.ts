...
import { expect, testWithWallet as test } from '../fixtures/wallet'
...
test.describe('Multi-Token Equality Tests', () => {
  ...
  test('should display all 4 protocol tokens in balance view', async ({
    page,
  }) => {
  ...
  test('all tokens should have equal UI treatment in selectors', async ({
    page,
  }) => {
    // Go to liquidity tab
  ...
  test('all tokens should display USD values consistently', async ({
    page,
  }) => {
  ...
  test('all tokens should be available for paymaster deployment', async ({
    page,
  }) => {
  ...
  test('all tokens should be available for liquidity provision', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Add Liquidity/i }).click()
  ...
  test('all tokens should be available for app registry stakes', async ({
    page,
  }) => {
...
test.describe('Token-Specific Price Validation', () => {
  ...
  test('CLANKERMON should show $0.15 price', async ({ page }) => {
    const clankermonCard = page
      .locator('[style*="background: #f8fafc"]')
      .filter({ hasText: 'CLANKERMON' })

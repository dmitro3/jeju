 ...
 */
const TABS = [
  { id: 'registry', name: 'Registry', expectedContent: 'Registry', description: 'Token registry with registered tokens list, search, and registration form.' },
  { id: 'faucet', name: 'Faucet', expectedContent: 'Faucet', description: 'Testnet faucet with token selection and request button.' },
  { id: 'transfer', name: 'Transfer', expectedContent: 'Transfer', description: 'Cross-chain transfer interface with source/destination chain selectors, token amount input.' },
  { id: 'intents', name: 'Intents', expectedContent: 'Intent', description: 'Cross-chain intents dashboard showing pending intents, routes, solvers, and stats.' },
  { id: 'oracle', name: 'Oracle', expectedContent: 'Oracle', description: 'Price oracle with feeds list, operators view, and subscriptions.' },
  { id: 'xlp', name: 'Liquidity', expectedContent: 'Liquidity', description: 'XLP liquidity pools dashboard with pool stats and add/remove liquidity forms.' },
  { id: 'risk', name: 'Risk Pools', expectedContent: 'Risk', description: 'Risk allocation dashboard showing pool allocations and coverage stats.' },
  { id: 'tokens', name: 'Tokens', expectedContent: 'Token', description: 'Token list showing all registered tokens with balances and actions.' },
  { id: 'deploy', name: 'Deploy', expectedContent: 'Deploy', description: 'Paymaster deployment interface with deployment form and status.' },
  { id: 'nodes', name: 'Nodes', expectedContent: 'Node', description: 'Node staking dashboard with registered nodes, stake amounts, and registration form.' },
]
...
function setupErrorCapture(page: import('@playwright/test').Page): { errors: string[]; hasKnownBug: boolean } {
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      const text = msg.text()
      if (text.includes('favicon') || text.includes('Failed to load') || text.includes('net::ERR') || text.includes('status of 4') || text.includes('Failed to fetch')) return
      errors.push(text)
    }
  })
...
test.describe('Gateway - Main Page Load', () => {
  test('Dashboard loads with Gateway branding', async ({ page }) => {
    ...
    await runAIVerification(screenshotPath, 'Gateway dashboard with tab navigation bar showing Registry, Faucet, Transfer, Intents, Oracle, Liquidity, Risk Pools, Tokens, Deploy, Nodes tabs. Header with Gateway branding and wallet connect button.', '/')
...
test.describe('Gateway - Header Components', () => {
  ...
  test('header has wallet button', async ({ page }) => {
    ...
    const walletBtn = page.locator('button').filter({ hasText: /Connect|Wallet|0x/ })
...
test.describe('Gateway - Tab Navigation', () => {
  test('all tabs are visible', async ({ page }) => {
    ...
    for (const tab of TABS) {
      await expect(page.locator(`button:has-text("${tab.name}")`).first()).toBeVisible({ timeout: 5000 })
    }
    
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-All-Tabs.png'), fullPage: true })
  })
  ...
  for (const tab of TABS) {
    test(`Tab: ${tab.name} - loads and has content`, async ({ page }) => {
      ...
      const screenshotPath = join(SCREENSHOT_DIR, `Gateway-Tab-${tab.name.replace(/\s+/g, '-')}.png`)
      await page.screenshot({ path: screenshotPath, fullPage: true })
...
test.describe('Gateway - Registry Tab Components', () => {
  test('Registry tab has token list', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Registry-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Faucet Tab Components', () => {
  test('Faucet tab has request form', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Faucet-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Transfer Tab Components', () => {
  test('Transfer tab has transfer form', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Transfer-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Intents Tab Components', () => {
  test('Intents tab has stats and views', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Intents-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Oracle Tab Components', () => {
  test('Oracle tab has feeds view', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Oracle-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Liquidity Tab Components', () => {
  test('Liquidity tab has XLP dashboard', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Liquidity-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Risk Pools Tab Components', () => {
  test('Risk Pools tab has allocation dashboard', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Risk-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Tokens Tab Components', () => {
  test('Tokens tab has token list', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Tokens-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Deploy Tab Components', () => {
  test('Deploy tab has paymaster form', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Deploy-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Nodes Tab Components', () => {
  test('Nodes tab has staking dashboard', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Gateway-Nodes-Content.png'), fullPage: true })
  })
})
...
test.describe('Gateway - Mobile Responsiveness', () => {
  test('renders on mobile viewport', async ({ page }) => {
    ...
    await page.screenshot({ path: join(SCREENSHOT_DIR, 'Mobile-Gateway.png'), fullPage: true })
    
    if (isLLMConfigured?.() && verifyImage) {
      await runAIVerification(join(SCREENSHOT_DIR, 'Mobile-Gateway.png'), 'Mobile-responsive Gateway dashboard with horizontally scrollable tabs, readable text, no overflow issues.', '/mobile')
    }
  })

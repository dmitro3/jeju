import { connectWallet, testWithWallet as test } from '@jejunetwork/tests'
import { expect } from '@playwright/test'

const GATEWAY_URL = process.env.GATEWAY_URL || 'http://localhost:4001'

test.describe('Gateway Wallet Connection', () => {
  test('should display homepage and connect wallet with screenshots', async ({
    page,
    wallet,
  }) => {
    ...
    await page.screenshot({
      path: 'test-results/screenshots/02-before-connect.png',
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/03-wallet-connected.png',
      fullPage: true,
    })
  ...
  test('should display multi-token balances with screenshots', async ({
    page,
    wallet,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    await page.screenshot({
      path: 'test-results/screenshots/04-token-balances.png',
      fullPage: true,
    })
    ...
    await balanceCard.screenshot({
      path: 'test-results/screenshots/05-balance-card.png',
    })
    console.log('📸 Screenshot 5: Balance card detail')
  })

  test('should navigate all tabs with screenshots', async ({
    page,
    wallet,
  }) => {
    await page.goto(GATEWAY_URL)

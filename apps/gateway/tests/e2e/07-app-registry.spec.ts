...
test.describe('App Registry Flow', () => {
  ...
  test('should show A2A enabled badge for apps with endpoint', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Browse Apps/i }).click()
...
test.describe('Register New App Flow', () => {
  ...
  test('should have required app name field', async ({ page }) => {
    ...
    await expect(
      page.getByText(/App Name/i).filter({ hasText: '*' }),
    ).toBeVisible()
  })
  ...
  test('should have tag selection with multiple categories', async ({
    page,
  }) => {
  ...
  test('should calculate required stake in selected token', async ({
    page,
  }) => {
    // Select stake token

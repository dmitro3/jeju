...
const mockDb = {
  query: mock(async <T>(_sql: string, _params?: (string | number | boolean | null)[]): Promise<T[]> => {
    return [] as T[];
  }),
  exec: mock(async (_sql: string, _params?: (string | number | boolean | null)[]): Promise<{ rowsAffected: number }> => {
...
describe('Leaderboard API', () => {
  ...
  test('GET /api/wallet/verify without auth should return 401', async () => {
    const response = await leaderboardApp.request('/api/wallet/verify?username=test');
  ...
  test('POST /api/a2a should handle get-leaderboard skill', async () => {
    ...
    mockDb.query.mockImplementationOnce(async () => [
      { username: 'test1', avatar_url: 'https://github.com/test1.png', total_score: 100 },
    ]);
    ...
    const response = await leaderboardApp.request('/api/a2a', {
      ...
      body: JSON.stringify({
        ...
        params: {
          message: {
            messageId: 'test-123',
            parts: [{ kind: 'data', data: { skillId: 'get-leaderboard', limit: 5 } }],
          },
        },
      }),
    });

    const data = await response.json() as { result: { parts: Array<{ kind: string }> } };
  ...
  test('POST /api/a2a should handle get-contributor-profile skill', async () => {
    ...
    mockDb.query.mockImplementation(async (sql: string) => {
      if (sql.includes('FROM users')) {
        return [{ username: 'testuser', avatar_url: 'https://github.com/testuser.png' }];
      }
      if (sql.includes('user_daily_scores')) {
        return [{ total_score: 500, pr_score: 200, issue_score: 100, review_score: 100, comment_score: 100 }];
      }
    ...
    const response = await leaderboardApp.request('/api/a2a', {
      ...
      body: JSON.stringify({
        ...
        params: {
          message: {
            messageId: 'test-456',
            parts: [{ kind: 'data', data: { skillId: 'get-contributor-profile', username: 'testuser' } }],
          },
        },
      }),
    });

    const data = await response.json() as { result: { parts: Array<{ kind: string; data?: { profile?: { username: string } } }> } };
  ...
  test('POST /api/a2a should handle unknown skill', async () => {
    ...
    const data = await response.json() as { result: { parts: Array<{ kind: string; data?: { error?: string } }> } };

...
test.describe('SLIDERS - Complete Coverage', () => {
  ...
  test('should test fee margin slider on Deploy Paymaster', async ({
    page,
  }) => {
    ...
    const minLabel = await page
      .getByText(/0.*selected|min.*selected/i)
      .isVisible()
    ...
    const middle = Math.floor(
      (parseInt(max || '500', 10) + parseInt(min || '0', 10)) / 2,
    )
...
test.describe('MODALS - Complete Coverage', () => {
  ...
  test('should open and close app detail modal', async ({ page }) => {
    ...
    if (modalVisible) {
      ...
      // Check modal content
      await expect(
        page.getByText(/App Details|Description|Endpoint/i),
      ).toBeVisible()
  ...
  test('should test RainbowKit wallet modal', async ({ page }) => {
    ...
    const rainbowModal = page.locator(
      '[data-rk], .rk-modal, [class*="rainbow"]',
    )
...
test.describe('TOGGLE BUTTONS - Complete Coverage', () => {
  ...
  test('should test bridge mode toggle (Select Token / Custom Address)', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Bridge from Ethereum/i }).click()
    await page.waitForTimeout(1000)
    ...
    const customAddressBtn = page.getByRole('button', {
      name: /Custom Address/i,
    })
  ...
  test('should test category toggle buttons on App Registry', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /App Registry/i }).click()
    await page.waitForTimeout(500)
...
test.describe('INPUT FIELDS - Complete Coverage', () => {
  ...
  test('should test all text input fields', async ({ page }) => {
    ...
    for (const test of inputTests) {
      await page
        .getByRole('button', { name: new RegExp(test.tab, 'i') })
        .click()
      await page.waitForTimeout(500)
...
test.describe('FORM SUBMIT BUTTONS - Complete Coverage', () => {
  ...
  test('should test submit button disabled states', async ({ page }) => {
    ...
    for (const form of forms) {
      await page
        .getByRole('button', { name: new RegExp(form.tab, 'i') })
        .click()
      await page.waitForTimeout(500)

      if (form.tab === 'App Registry') {
        await page
          .getByRole('button', { name: /Register App/i })
          .first()
          .click()
        await page.waitForTimeout(500)
      }
      ...
      const submitBtn = page.getByRole('button', {
        name: new RegExp(form.button, 'i'),
      })
      if (await submitBtn.isVisible()) {
        const initialDisabled = await submitBtn.isDisabled()
        console.log(
          `📊 ${form.button} initial state: ${initialDisabled ? 'disabled' : 'enabled'}`,
        )
      }
    }
  })

  test('should test submit button loading states', async ({
    page: _page,
    metamask: _metamask,
  }) => {
...
test.describe('SCROLLABLE AREAS - Complete Coverage', () => {
  ...
  test('should test scrolling in dropdown with many items', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('HOVER STATES - Complete Coverage', () => {
  test('should test hover effects on all buttons', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)

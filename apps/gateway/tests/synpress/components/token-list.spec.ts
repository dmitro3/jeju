...
test.describe('Token List Display', () => {
  ...
  test('should display token list with all registered tokens', async ({
    page,
  }) => {
    ...
    if (hasHeading) {
      ...
      const count = parseInt(headingText?.match(/\d+/)?.[0] || '0', 10)
      ...
      if (count > 0) {
        const tokenCards = page
          .locator('.card')
          .filter({ hasText: /decimals|Address/i })
  ...
  test('should display token card with complete information', async ({
    page,
  }) => {
    ...
    const hasTransactions = await firstCard
  ...
  test('should show paymaster deployment status', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const deployed = await firstCard
        .getByText(/✅ Deployed|Deployed/i)
        .isVisible()
      const notDeployed = await firstCard
        .getByText(/❌ Not Deployed|Not Deployed/i)
        .isVisible()

      expect(deployed || notDeployed).toBe(true)

      console.log(
        `✅ Paymaster status: ${deployed ? 'Deployed' : 'Not Deployed'}`,
      )
    }
  })
...
test.describe('Token Card Interactions', () => {
  ...
  test('should display fee range in basis points and percentage', async ({
    page,
  }) => {
    ...
    if (count > 0) {
      const feeRangeText = await page
        .locator('p:has-text("Fee Range")')
        .locator('..')
        .locator('p')
        .nth(1)
        .textContent()
...
test.describe('Token List Ordering', () => {
  test('should display tokens in consistent order', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)

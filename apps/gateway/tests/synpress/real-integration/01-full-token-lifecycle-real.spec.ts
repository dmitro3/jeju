...
import { calculateStakeAmount, GATEWAY_URL } from '../fixtures/test-data'
import {
  executeTransaction,
  executeTwoStepTransaction,
...
test.describe('REAL INTEGRATION - Complete Gateway Feature Test', () => {
  test('FULL FLOW: Create Token → Register → Deploy Paymaster → Add Liquidity → Setup Node', async ({
    ...
    console.log(
      '🎯 REAL INTEGRATION TEST - Testing Actual On-Chain Functionality',
    )
    ...
    if (TEST_TOKEN.address === '0x0000000000000000000000000000000000000000') {
      ...
      console.log(
        '   Deploy test token first: bun run scripts/deploy-test-token.ts',
      )
      return
    }
    ...
    if (!alreadyDeployed) {
      ...
      await page
        .getByRole('button', {
          name: new RegExp(`Deploy Paymaster for ${TEST_TOKEN.symbol}`, 'i'),
        })
        .click()
    ...
    await page
      .getByRole('button', {
        name: new RegExp(`Add.*${TEST_TOKEN.symbol}`, 'i'),
      })
      .click()
    ...
    await expect(
      page.getByText(new RegExp(`Your ${TEST_TOKEN.symbol} LP Position`, 'i')),
    ...
    const maxNodesWarning = await page
      .getByText(/reached the maximum of 5 nodes/i)
      .isVisible()

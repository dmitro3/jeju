...
test.describe('Transaction Rejection Handling', () => {
  ...
  test('should handle user rejecting add liquidity transaction', async ({
    page,
    metamask,
  }) => {
  ...
  test('should handle rejected node registration', async ({
    page,
    metamask,
  }) => {
...
test.describe('Form Validation Errors', () => {
  ...
  test('should show validation error for invalid token address in registration', async ({
    page,
  }) => {
  ...
  test('should show validation for RPC URL in node registration', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Node Operators/i }).click()
    await page.waitForTimeout(500)
...
test.describe('Insufficient Balance Errors', () => {
  test('should show error when insufficient ETH for liquidity', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Network and Connection Errors', () => {
  ...
  test.skip('should detect wrong network and show warning', async ({
    page: _page,
    metamask: _metamask,
  }) => {
    // TODO: Switch to different network and verify warning
    console.log(
      '⚠️  Wrong network test - needs network switching implementation',
    )
  })

  test.skip('should handle RPC connection failures', async ({
    page: _page,
  }) => {
...
test.describe('Empty State Tests', () => {
  ...
  test('should show empty state for LP positions if none exist', async ({
    page,
  }) => {
  ...
  test('should show empty state for nodes if none registered', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Node Operators/i }).click()
    await page.waitForTimeout(500)
    ...
    if (hasEmpty) {
      await expect(
        page.getByText(/Stake tokens and register a node/i),
      ).toBeVisible()
      await page.screenshot({
  ...
  test('should show empty state for apps if none registered', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /App Registry/i }).click()
    await page.waitForTimeout(500)

...
import { REPORT_BONDS, SEVERITY } from '../fixtures/test-data'
...
test.describe('Moderation Dashboard - Navigation & Display', () => {
  ...
  test('should display all tabs: Active, Resolved, Submit', async ({
    page,
  }) => {
    await page.goto(MODERATION_URL)

    await expect(
      page.getByRole('button', { name: /Active Reports/i }),
    ).toBeVisible()
...
test.describe('Submit Report Flow', () => {
  ...
  test('should submit report with evidence upload', async ({
    page,
    metamask,
  }) => {
    ...
    // Fill details
    await page
      .getByPlaceholder(/Describe the violation/i)
      .fill('Test report for E2E testing - spam behavior detected')
  ...
  test('should display correct bond for each severity level', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Submit Report/i }).click()
    await page.waitForTimeout(1000)
...
test.describe('Moderation System Requirements', () => {
  test('should check if moderation contracts deployed', async ({ page }) => {
    ...
    const moderationWorks = await page
      .getByText('Moderation Dashboard')
      .isVisible()

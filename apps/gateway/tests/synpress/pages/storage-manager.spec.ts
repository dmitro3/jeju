...
test.describe('Storage Manager - Navigation & Display', () => {
  ...
  test('should navigate to storage manager page', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Decentralized file storage on the network IPFS/i),
    ).toBeVisible()
  ...
  test('should display all tabs: Upload, My Files, Funding', async ({
    page,
  }) => {
    await page.goto(STORAGE_URL)
...
test.describe('File Upload Flow', () => {
  ...
  test('should calculate storage price for different durations', async ({
    page,
  }) => {
    // Select a test file
    const fileInput = page.locator('input[type="file"]')
...
test.describe('Storage Funding Flow', () => {
  ...
  test('should display funding options', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: /Deposit elizaOS/i }),
    ).toBeVisible()
  ...
  test.skip('should fund storage balance with elizaOS', async ({
    page,
    metamask: _metamask,
  }) => {
...
test.describe('File Management', () => {
  test('should display file expiration warnings', async ({
    page,
    metamask,
  }) => {
    await page.goto('http://localhost:4001')
...
test.describe('Storage System Requirements', () => {
  test('should check if IPFS service available', async ({ page }) => {
    ...
    const ipfsAvailable = await page.request
      .get(`${ipfsUrl}/health`)
      .then((res) => res.ok)
      .catch(() => false)

...
test.describe('Multi-User LP Scenarios', () => {
  test('should show different LP positions for different users', async ({
    page,
    metamask,
  }) => {
    ...
    console.log(
      `ℹ️  User 1 (${TEST_WALLET.address.slice(0, 10)}...): ${user1Count} positions`,
    )
  ...
  test.skip('should not show other users LP positions', async ({
    page: _page,
  }) => {
    ...
    console.log(
      '⚠️  Multi-account testing - needs Synpress account switching support',
    )
  })
})

test.describe('Multi-User Node Scenarios', () => {
  test('should show only operator-owned nodes in My Nodes', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const nodeCountText = await page
      .getByText(/My Nodes \((\d+)\)/i)
      .textContent()
    const nodeCount = nodeCountText
      ? parseInt(nodeCountText.match(/\d+/)?.[0] || '0', 10)
      : 0
    ...
    if (nodeCount > 0) {
      const nodeCard = page
        .locator('.card')
        .filter({ hasText: /Node ID:/i })
        .first()

      // Claim and Deregister buttons should be visible (operator-only actions)
      await expect(
        nodeCard.getByRole('button', { name: /Claim|Deregister/i }),
      ).toBeVisible()
  ...
  test('should show all network nodes in Network Overview', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const totalNodesElement = page
      .locator('p:has-text("Total Nodes")')
      .locator('../..')
      .locator('p')
      .nth(1)
...
test.describe('Multi-User App Registry Scenarios', () => {
  test('should show all registered apps in browse (not just owned)', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
  ...
  test('should show owner actions only for owned apps', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Ownership Validation', () => {
  test('should prevent non-owners from managing apps', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Network-Wide vs User-Specific Data', () => {
  test('should distinguish network stats from user stats', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const networkTotalElement = page
      .locator('p:has-text("Total Nodes")')
      .locator('../..')
      .locator('p')
      .nth(1)
    ...
    const myNodesText = await page
      .getByText(/My Nodes \((\d+)\)/i)
      .textContent()
    const myNodesCount = myNodesText
      ? parseInt(myNodesText.match(/\d+/)?.[0] || '0', 10)
      : 0
    ...
    expect(myNodesCount <= parseInt(networkTotal || '999', 10)).toBe(true)

...
test.describe('Bridge - elizaOS Exclusion (Native Token)', () => {
  ...
  test('should display warning that elizaOS cannot be bridged', async ({
    page,
  }) => {
    // Warning should be prominently displayed
    const warning = page
      .locator('[style*="background: #fef3c7"]')
...
test.describe('Bridge - USD Price Calculations', () => {
  ...
  test('should calculate USD value for CLANKERMON correctly', async ({
    page,
  }) => {
    await page.locator('.input').first().click()
    await page.waitForTimeout(500)
...
test.describe('Bridge - Custom Token Comprehensive Testing', () => {
  ...
  test('should show helper text for custom tokens', async ({ page }) => {
    await expect(
      page.getByText(/Enter any ERC20 token address from Ethereum/i),
    ).toBeVisible()
    await expect(
  ...
  test('should handle extremely long input in custom address', async ({
    page,
  }) => {
...
test.describe('Bridge - Button State Management', () => {
  ...
  test('should disable bridge button without token selection', async ({
    page,
  }) => {
    // No token selected
    const bridgeButton = page.getByRole('button', {
  ...
  test('should enable bridge button with valid token and amount', async ({
    page,
  }) => {
    await page.locator('.input').first().click()
    await page.waitForTimeout(500)
...
test.describe('Bridge - Decimal and Formatting', () => {
  ...
  test('should format large numbers with commas in USD display', async ({
    page,
  }) => {
    await page.locator('.input').first().click()
    await page.waitForTimeout(500)
...
test.describe('Bridge - Information Display Completeness', () => {
  ...
  test('should display destination information', async ({ page, metamask }) => {
    ...
    await expect(
      page.getByText(/Tokens will appear on the network after confirmation/i),
    ).toBeVisible()

    console.log('✅ Destination info displayed')
  })

  test('should show all bridge information together', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const infoPanel = page
      ...
      .filter({ hasText: /Estimated Time/i })
...
test.describe('Bridge - Ethereum Network Tokens Only', () => {
  test('should only accept tokens from Ethereum network', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Bridge - Visual Feedback', () => {
  test('should highlight selected token in dropdown', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)

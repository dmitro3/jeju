...
test.describe('Bridge - Complete Approval Flow', () => {
  ...
  test('COMPLETE: Select CLANKER → Approve → Simulate Bridge → Verify', async ({
    page,
    metamask,
  }) => {
    // ===================
    ...
    let bridgeButton = page.getByRole('button', {
...
test.describe('Bridge - All Bridgeable Tokens', () => {
  ...
  for (const token of bridgeableTokens) {
    test(`should approve ${token.symbol} for bridge`, async ({
      page,
      metamask,
    }) => {
      await page.goto(GATEWAY_URL)
      ...
      const hasSecond = await metamask
        .rejectTransaction()
        .then(() => true)
        .catch(() => false)
...
test.describe('Bridge - Custom Token Address Mode', () => {
  test('COMPLETE: Custom token flow with approval', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
  ...
  test('should validate custom token address format', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Bridge - Recipient Address Handling', () => {
  ...
  test('should validate recipient address format', async ({ page }) => {
    ...
    console.log(
      `ℹ️  Invalid recipient: Button ${enabled ? 'enabled' : 'disabled'}`,
    )
...
test.describe('Bridge - Error Handling', () => {
  ...
  test('should show insufficient balance error (via MetaMask)', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Bridge - Transaction Tracking', () => {
  test('should display bridge information panel', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    await expect(
      page.getByText(/Bridge.*OP Stack Standard Bridge/i),
    ).toBeVisible()
    await expect(
      page.getByText(/Tokens will appear on the network/i),
    ).toBeVisible()
  ...
  test('should display bridge history section', async ({ page, metamask }) => {
    ...
    if (hasHistory) {
      ...
      if (isEmpty) {
        await expect(
          page.getByText(/Your bridged tokens will appear here/i),
        ).toBeVisible()
        console.log('✅ Empty bridge history displayed')
      } else {
        // Has transfers
        const transferCards = page
          .locator('[style*="background: #f8fafc"]')
          .filter({ hasText: /→/i })
  ...
  test.skip('should add completed bridge to history', async ({
    page: _page,
    metamask: _metamask,
  }) => {
    ...
    console.log(
      '⚠️  Bridge history population - requires completed bridge transaction',
    )
    console.log(
      '   Needs: Subsquid indexer integration or local event tracking',
    )
  })
})
...
test.describe('Bridge - UI State Management', () => {
  ...
  test('should maintain selection when switching between modes', async ({
    page,
  }) => {
  ...
  test('should clear form after successful bridge', async ({
    page,
    metamask,
  }) => {
    await page.locator('.input').first().click()
...
test.describe('Bridge - Transaction Success Indicators', () => {
  test.skip('should display success message with transaction hash', async ({
    page: _page,
  }) => {
  ...
  test.skip('should update balance after bridge confirms', async ({
    page: _page,
  }) => {
  ...
  test.skip('should show pending status during bridge relay', async ({
    page: _page,
  }) => {

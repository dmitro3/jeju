...
test.describe('Complete Token Lifecycle', () => {
  test('FULL FLOW: Register elizaOS → Deploy Paymaster → Add Liquidity → Claim Fees → Remove', async ({
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/01-connected.png',
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/02-token-registry.png',
      fullPage: true,
    })
    ...
    if (!alreadyRegistered && elizaOSAddress) {
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/03-token-registered.png',
        fullPage: true,
      })
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/03-token-exists.png',
        fullPage: true,
      })
    }
    ...
    if (!alreadyDeployed) {
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/04-before-deploy.png',
        fullPage: true,
      })

      // Deploy paymaster
      await page
        .getByRole('button', { name: /Deploy Paymaster for elizaOS/i })
        .click()
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/05-paymaster-deployed.png',
        fullPage: true,
      })
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/04-paymaster-exists.png',
        fullPage: true,
      })
    }
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/06-before-add-liquidity.png',
      fullPage: true,
    })

    // Add liquidity
    await page
      .getByRole('button', { name: /Add.*ETH to elizaOS Vault/i })
      .click()
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/07-liquidity-added.png',
      fullPage: true,
    })
    ...
    await expect(page.getByText(/Your elizaOS LP Position/i)).toBeVisible({
      timeout: 10000,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/08-lp-position.png',
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/09-lp-dashboard.png',
      fullPage: true,
    })
    ...
    if (claimVisible && (await claimButton.isEnabled())) {
      ...
      await page.screenshot({
        path: 'test-results/screenshots/flow1/10-fees-claimed.png',
        fullPage: true,
      })
    ...
    const removeButton = page.getByRole('button', {
      name: /Remove All Liquidity/i,
    })
    await expect(removeButton).toBeVisible()

    await page.screenshot({
      path: 'test-results/screenshots/flow1/11-before-remove.png',
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/12-liquidity-removed.png',
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: 'test-results/screenshots/flow1/13-final-state.png',
      fullPage: true,
    })
  ...
  test('FULL FLOW: Register → Deploy → Add Liquidity for CLANKER token', async ({
    ...
    if (!registered) {
      console.log(
        `ℹ️  ${token.symbol} not registered - would need Base bridge setup`,
      )
    ...
    if (clankerAvailable) {
      ...
      if (!deployed) {
        ...
        await page
          .getByRole('button', {
            name: new RegExp(`Deploy Paymaster for ${token.symbol}`, 'i'),
          })
          .click()
      ...
      if (await liquidityInput.isVisible()) {
        ...
        await page
          .getByRole('button', {
            name: new RegExp(`Add.*${token.symbol}`, 'i'),
          })
          .click()

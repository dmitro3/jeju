...
test.describe('GitHub Reputation Panel', () => {
  ...
  test('should display GitHub reputation section in navigation', async ({
    page,
  }) => {
  ...
  test('should show leaderboard preview', async ({ page }) => {
    ...
    const leaderboardElements = page
      .locator('[data-testid="leaderboard"]')
      .or(page.getByText(/Top Contributors|Leaderboard/i))

    // May or may not be present depending on UI layout
    const isVisible = await leaderboardElements
      .first()
      .isVisible()
      .catch(() => false)
    if (isVisible) {
...
test.describe('GitHub Reputation with Connected Wallet', () => {
  ...
  test('should show wallet verification option', async ({ page }) => {
    ...
    const verifyOptions = page.getByRole('button', {
      name: /Verify|Link|Connect.*GitHub/i,
    })

    const hasVerifyOption = await verifyOptions
      .first()
      .isVisible()
      .catch(() => false)
    if (hasVerifyOption) {
  ...
  test('should display on-chain reputation status', async ({ page }) => {
    ...
    const hasStatus = await onChainStatus
      .first()
      .isVisible()
      .catch(() => false)
    if (hasStatus) {
  ...
  test('should fetch leaderboard data successfully', async ({ page }) => {
    ...
    page.on('response', (response) => {
      if (
        ...
        response.url().includes('/api/attestation')
      ) {
        _apiCalled = true
      }
    })
...
test.describe('Attestation Flow', () => {
  ...
  test('should display attestation requirements', async ({ page }) => {
    ...
    const hasInfo = await attestationInfo
      .first()
      .isVisible()
      .catch(() => false)
    if (hasInfo) {
  ...
  test('should show oracle status when available', async ({ page }) => {
    ...
    const hasStatus = await oracleStatus
      .first()
      .isVisible()
      .catch(() => false)
    if (hasStatus) {
...
test.describe('Agent Linking', () => {
  ...
  test('should display agent linking option if applicable', async ({
    page,
  }) => {
    // Look for agent-related UI
    const agentUI = page.getByText(/Agent|Link.*Agent|AI.*Agent/i)

    const hasAgentUI = await agentUI
      .first()
      .isVisible()
      .catch(() => false)
    if (hasAgentUI) {

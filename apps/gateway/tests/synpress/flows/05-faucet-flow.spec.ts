...
test.describe('Faucet with Connected Wallet', () => {
  ...
  test('should show registration status', async ({ page }) => {
    ...
    const registryStatus = await page
      .locator('.flex.items-center.justify-between')
      .filter({ hasText: /ERC-8004 Registry/i })
      .textContent()
  ...
  test('should show registration warning if not registered', async ({
    page,
  }) => {
    // If user is not registered, should show warning
    const isNotRegistered = await page
      .getByText(/Not Registered/)
      .isVisible()
      .catch(() => false)
...
test.describe('Faucet Claim Flow', () => {
  ...
  test('should show faucet balance', async ({ page }) => {
    ...
    const balanceSection = page
      .locator('.stat-card')
      .filter({ hasText: /Faucet Balance/i })

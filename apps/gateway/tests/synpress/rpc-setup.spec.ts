...
test.describe('RPC Setup Tab', () => {
  ...
  test('should display RPC setup tab when wallet connected', async ({
    ...
    const rpcTab = page
      .locator('text=RPC Access')
      .or(page.locator('[data-testid="rpc-tab"]'))
  ...
  test('should show RPC endpoints', async ({
    ...
    await expect(page.locator('text=RPC Endpoints')).toBeVisible({
      timeout: 10000,
    })
...
test.describe('RPC API Key Management', () => {
  ...
  test('should copy API key', async ({
    ...
    const copyButton = page
      .locator('[data-testid="copy-key"]')
      .or(
        page
          .locator('button:has(svg)')
          .filter({ has: page.locator('title:has-text("Copy")') }),
      )

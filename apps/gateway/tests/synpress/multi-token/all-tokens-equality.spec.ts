...
test.describe('Token Balance Display Equality', () => {
  test('should display ALL 4 tokens in balance view with equal treatment', async ({
    page,
    metamask,
  }) => {
    ...
    const tokenCards = page
      .locator('[style*="background: #f8fafc"]')
      .filter({ hasText: /elizaOS|CLANKER|VIRTUAL|CLANKERMON/ })
  ...
  test('should calculate total portfolio value including all tokens', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    await expect(
      page.locator('text=/Total:/i').locator('../..').getByText(/\$/),
    ).toBeVisible()
...
test.describe('Token Selector Equality', () => {
  ...
  test('ALL tokens should appear in paymaster deployment selector', async ({
    page,
  }) => {
  ...
  test('ALL tokens should appear in liquidity provision selector', async ({
    page,
  }) => {
  ...
  test('ALL tokens should appear in node staking selector', async ({
    page,
  }) => {
  ...
  test('ALL tokens should appear in app registry stake selector', async ({
    page,
  }) => {
  ...
  test('ONLY bridgeable tokens should appear in bridge selector', async ({
    page,
  }) => {
...
test.describe('Price Display Consistency', () => {
  ...
  test('should display correct price for each token in balance view', async ({
    page,
  }) => {
    await page.waitForTimeout(3000)
  ...
  test('should allow paymaster deployment for ALL tokens', async ({ page }) => {
    ...
    for (const token of ALL_TOKENS) {
      ...
      const formVisible = await page
        .getByText(/Deploy Paymaster|already deployed|not registered/i)
        .isVisible()
...
test.describe('Bridge Token Filtering', () => {
  test('should EXCLUDE elizaOS from bridge (native token)', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
  ...
  test('should allow custom ANY Base ERC20 token', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
...
test.describe('Cross-Token Operations', () => {
  test('should allow staking one token and earning rewards in different token', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    if (await clankerOption.isVisible()) {
      ...
      console.log(
        '✅ Cross-token staking/rewards works (stake elizaOS, earn CLANKER)',
      )
...
test.describe('Token Information Display', () => {
  test('should show complete token info in dropdown', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)

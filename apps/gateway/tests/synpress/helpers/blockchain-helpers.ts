...
export async function setNextBlockTimestamp(
  ...
  console.log(
    `⏰ Set next block timestamp to ${new Date(timestamp * 1000).toISOString()}`,
  )
}
...
export async function getBlockTimestamp(page: Page): Promise<number> {
  const block = await rpcCall<EthBlock>(page, 'eth_getBlockByNumber', [
    'latest',
    false,
  ])
...
export async function getBalance(page: Page, address: string): Promise<bigint> {
  const balanceHex = await rpcCall<string>(page, 'eth_getBalance', [
    address,
    'latest',
  ])
 ...
 */
export async function fastForwardToDate(
  page: Page,
  targetDate: Date,
): Promise<void> {
...
export async function getTransactionReceipt(
  ...
  return await rpcCall<TransactionReceipt | null>(
    page,

...
interface RpcPageRequest {
  request: {
    post: (
      url: string,
      options: { data: JsonRpcRequestData },
...
test.describe('ON-CHAIN VALIDATION: Balance Changes', () => {
  test('should verify ETH balance decreases after adding liquidity', async ({
    context,
    page,
    metamaskPage,
    extensionId,
  ...
  test('should verify token balance decreases after staking for node', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    await page
      .getByPlaceholder(/https:\/\/your-node/i)
      .fill('https://test-node.example.com:8545')

    const submitButton = page.getByRole('button', {
      name: /Stake & Register Node/i,
    })
    ...
    console.log(
      `✅ Token balance decreased by ${Number(actualDecrease) / 1e18}`,
    )
  })
})

test.describe('ON-CHAIN VALIDATION: Contract State', () => {
  test('should verify paymaster is deployed on-chain after deployment', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    if (alreadyDeployed) {
      ...
      if (addressMatch) {
        ...
        console.log(
          `✅ Paymaster contract verified on-chain: ${paymasterAddress}`,
        )
      }
    } else {
  ...
  test('should verify app registration creates on-chain record', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    await page
      ...
      .fill('Testing on-chain validation')
    await page
      .getByPlaceholder('https://myapp.com/a2a')
      .fill('https://test.example.com/a2a')
...
test.describe('ON-CHAIN VALIDATION: Transaction Receipts', () => {
  test('should verify transaction receipt after token registration', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    if (hasTxHash) {
      ...
      if (match) {
        ...
        console.log(
          `✅ Transaction ${match[1].slice(0, 10)}... verified on-chain`,
        )
...
test.describe('ON-CHAIN VALIDATION: Event Verification', () => {
  test('should verify TokenRegistered event emitted after registration', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    const logs = await rpcCall(page, 'eth_getLogs', [
      {
        ...
        topics: [
          '0x' +
            'a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2', // mock topic
        ],
      },
    ]).catch(() => '[]')
    ...
    if (logs !== '[]') {
      console.log(
        `✅ TokenRegistered event found in blocks ${initialBlock}-${finalBlock}`,
      )
    } else {
...
test.describe('ON-CHAIN VALIDATION: Multi-Token Verification', () => {
  test('should verify balances for all protocol tokens', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    for (const tokenData of Object.values(PROTOCOL_TOKENS)) {
      ...
      const uiBalanceElement = page
        .locator(`.card:has-text("${tokenData.symbol}")`)
        .locator('p')
        .filter({ hasText: /\d+\.\d+/ })
        .first()
      const uiBalanceText = await uiBalanceElement
        .textContent()
        .catch(() => '0')
...
test.describe('ON-CHAIN VALIDATION: Snapshot and Revert', () => {
  test('should take snapshot before destructive operation', async ({
    context,
    page,
    metamaskPage,
    extensionId,

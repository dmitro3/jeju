...
test.describe('Wallet Connection Flow', () => {
  ...
  test('should maintain connection across page reload', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const stillConnected = await page
...
test.describe('Wallet Account Switching', () => {
  test('should handle switching accounts in MetaMask', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
  ...
  test('should update balances when switching accounts', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    if (await balanceSection.isVisible()) {
      ...
      const balancesContainer = page
        .locator('[data-testid="balances"], .balances, div:has-text("elizaOS")')
        .first()
      ...
      console.log(
        'ℹ️  Balance section not visible - test structure may need update',
      )
    }
  })
})
...
test.describe('Network Management', () => {
  test('should display correct network (Network Localnet)', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const hasNetworkConfig = logs.some(
      (log) => log.includes('1337') || log.includes('9545'),
    )
  ...
  test('should warn if wrong network connected', async ({ page, metamask }) => {
    ...
    const wrongNetworkText = page.getByText(
      /Wrong Network|Switch Network|Unsupported Chain/i,
    )
    const switchButton = page.getByRole('button', {
      name: /Switch|Change Network/i,
    })
  ...
  test('should prompt network switch if on wrong chain', async ({
    page,
    metamask,
  }) => {
    await page.goto(GATEWAY_URL)
    ...
    const switchPrompt = page
      .locator('[role="dialog"], [role="alertdialog"]')
      .filter({ hasText: /Network|Chain/i })

    // On correct network, no prompt should appear
    await page.waitForTimeout(1000)
    const hasUnexpectedPrompt = await switchPrompt
      .isVisible()
      .catch(() => false)
  ...
  test('should disconnect wallet via UI', async ({ page, metamask }) => {
    ...
    if (await disconnectButton.isVisible({ timeout: 3000 })) {
      ...
      if (await alternateDisconnect.isVisible()) {
        ...
        console.log(
          'ℹ️  Disconnect button not found - modal structure may differ',
        )
      }
    }
  })
...
test.describe('Connection Error Handling', () => {
  ...
  test('should display wallet options when connecting', async ({ page }) => {
    ...
    const walletOptions = page
      .locator('[role="dialog"], [role="menu"]')
      .filter({ hasText: /MetaMask|Wallet/i })

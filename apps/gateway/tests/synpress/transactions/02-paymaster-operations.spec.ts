...
test.describe('Paymaster Deployment Transactions', () => {
  ...
  for (const tokenData of Object.values(PROTOCOL_TOKENS)) {
    test(`should deploy paymaster for ${tokenData.symbol}`, async ({
      page,
      metamask,
    }) => {
      // Select token
      ...
      if (notRegistered) {
        console.log(
          `ℹ️  ${tokenData.symbol} not registered - cannot deploy paymaster`,
        )
        return
      }
      ...
      await page
        .getByRole('button', {
          name: new RegExp(`Deploy Paymaster for ${tokenData.symbol}`, 'i'),
        })
        .click()
  ...
  test('should show deployment information before deploying', async ({
    page,
  }) => {
    // Select any token
    await page.locator('.input').first().click()
  ...
  test('should validate token must be registered first', async ({
    page: _page,
  }) => {
    ...
    console.log(
      '⚠️  Token registration validation test - needs unregistered token setup',
    )
  })
})

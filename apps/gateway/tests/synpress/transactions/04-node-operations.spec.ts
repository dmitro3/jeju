...
test.describe('Node Registration Transactions', () => {
  ...
  test('should register node with elizaOS stake, earn CLANKER rewards', async ({
    page,
    metamask,
  }) => {
    // Check node limit
    ...
    const nodeCountMatch = await page
      .locator('h2:has-text("My Nodes")')
      .textContent()
    const currentNodes = nodeCountMatch
      ? parseInt(nodeCountMatch.match(/\d+/)?.[0] || '0', 10)
      : 0
    ...
    const stakeAmount = calculateStakeAmount(
      PROTOCOL_TOKENS.ELIZAOS.priceUSD,
      1000,
    )
    ...
    await page
      .locator('select')
      .first()
      .selectOption({ value: TEST_NODE.region.toString() })
...
test.describe('Claim Node Rewards Transactions', () => {
  ...
  test('should claim rewards for node with pending rewards', async ({
    page,
    metamask,
  }) => {
...
test.describe('Deregister Node Transactions', () => {
  test('should deregister node after 7 day period', async ({
    context,
    page,
    metamaskPage,
    extensionId,
    ...
    if (!canDeregister) {
      const waitWarning = await page
        .getByText(/Can deregister in \d+ days/i)
        .isVisible()

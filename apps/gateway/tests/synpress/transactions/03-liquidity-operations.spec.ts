...
test.describe('Add Liquidity Transactions', () => {
  ...
  test('should add 0.1 ETH liquidity to elizaOS vault', async ({
    page,
    metamask,
  }) => {
    // Select elizaOS
  ...
  test('should add liquidity to multiple token vaults', async ({
    page,
    metamask,
  }) => {
    ...
    for (const tokenSymbol of tokensToTest) {
      ...
      if (await ethInput.isVisible()) {
        ...
        await page
          .getByRole('button', { name: new RegExp(`Add.*${tokenSymbol}`, 'i') })
          .click()
  ...
  test('should reject liquidity below minimum (if enforced)', async ({
    page,
  }) => {
    // Select token
    await page.locator('.input').first().click()
...
test.describe('Remove Liquidity Transactions', () => {
  ...
  test('should remove all liquidity from position', async ({
    page,
    metamask,
  }) => {
    // Select elizaOS
    ...
    const positionExists = await page
      .getByText(/Your elizaOS LP Position/i)
      .isVisible()
    ...
    // Get current shares
    const sharesText = await page
      .locator('p:has-text("ETH Shares")')
      .locator('..')
      .locator('p')
      .nth(1)
      .textContent()
...
test.describe('Claim Fees Transactions', () => {
  ...
  test('should claim LP fees for token with pending fees', async ({
    page,
    metamask,
  }) => {

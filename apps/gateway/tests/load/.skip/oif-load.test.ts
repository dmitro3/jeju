...
const CONCURRENT_REQUESTS = parseInt(
  process.env.LOAD_TEST_CONCURRENT || '20',
  10,
)
...
describe('OIF Load Tests', () => {
  ...
  test('quote service sequential throughput', async () => {
    ...
    console.log(
      `[Quote Sequential] Total: ${totalTime}ms, Throughput: ${(ITERATIONS / (totalTime / 1000)).toFixed(2)} quotes/s`,
    )
  ...
  test('quote service concurrent throughput', async () => {
    ...
    console.log(
      `[Quote Concurrent] Total: ${totalTime}ms, Throughput: ${(CONCURRENT_REQUESTS / (totalTime / 1000)).toFixed(2)} quotes/s`,
    )
  ...
  test('metrics recording throughput', () => {
    ...
    console.log(
      `[Metrics Record] Throughput: ${((iterations * 2) / (totalTime / 1000)).toFixed(0)} ops/s`,
    )
  ...
  test('prometheus export throughput', () => {
    ...
    console.log(
      `[Prometheus Export] Throughput: ${(iterations / (totalTime / 1000)).toFixed(2)} exports/s`,
    )
  ...
  test('burst load (batches of concurrent requests)', async () => {
    ...
    console.log(
      `[Burst Load] Total: ${totalTime}ms, Throughput: ${(stats.count / (totalTime / 1000)).toFixed(2)} req/s`,
    )
  ...
  test('memory stability under sustained load', async () => {
    ...
    console.log(
      `[Memory] Growth: ${memGrowthMB.toFixed(2)}MB over ${iterations} iterations`,
    )
...
describe('OIF Stress Tests', () => {
  test('handles rapid quote requests without degradation', async () => {
    ...
    phases.forEach((concurrent, i) => {
      ...
      console.log(
        `  ${concurrent} concurrent: avg=${avg.toFixed(2)}ms p95=${p95.toFixed(2)}ms`,
      )
    })

    const avgFirst =
      latencies[0].reduce((a, b) => a + b, 0) / latencies[0].length || 1
    const avgLast =
      latencies[3].reduce((a, b) => a + b, 0) / latencies[3].length

...
beforeAll(async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(
        `⚠️  Gateway server returned ${response.status}, tests will be skipped`,
      )
      serverAvailable = false
    }
  } catch {
    console.warn(
      `⚠️  Gateway server not running at ${BASE_URL}. Tests will be skipped.`,
    )
...
test('Load Test: Concurrent Mixed Endpoints', async () => {
  ...
  expect(successful.length / results.length).toBeGreaterThan(0.9)
  expect(avgDuration).toBeLessThan(2000)
})

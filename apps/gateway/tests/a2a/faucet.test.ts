...
describe('Faucet Agent Card Skills', () => {
  ...
  test('should have correct faucet skill metadata', async () => {
    ...
    const faucetStatusSkill = agentCard.skills.find(
      (s: { id: string }) => s.id === 'faucet-status',
    )
    ...
    const faucetClaimSkill = agentCard.skills.find(
      (s: { id: string }) => s.id === 'faucet-claim',
    )
    ...
    const request = {
      ...
      params: {
        message: {
          ...
          parts: [{ kind: 'data', data: { skillId: 'faucet-info' } }],
        },
      },
      id: 1,
    }
      ...
      params: {
        message: {
          ...
          parts: [
            {
              kind: 'data',
              data: { skillId: 'faucet-status', address: TEST_ADDRESS },
            },
          ],
        },
      },
    ...
    const request = {
      ...
      params: {
        message: {
          ...
          parts: [{ kind: 'data', data: { skillId: 'faucet-status' } }],
        },
      },
      id: 3,
    }
    ...
    const request = {
      ...
      params: {
        message: {
          ...
          parts: [{ kind: 'data', data: { skillId: 'faucet-claim' } }],
        },
      },
      id: 4,
    }
...
describe('Faucet REST API', () => {
  ...
  test('should return faucet status for address', async () => {
    const response = await fetch(
      `${A2A_BASE_URL}/api/faucet/status/${TEST_ADDRESS}`,
    )
...
describe('Faucet Anti-Bot Protection', () => {
  ...
  test('should require ERC-8004 registration', async () => {
    ...
    const registryRequirement = data.requirements.find(
      (r: string) => r.includes('ERC-8004') || r.includes('Identity Registry'),
    )

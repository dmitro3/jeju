...
function validateAgainstRequirements(
  ...
  if (payload.network !== requirements.network) {
    return {
      valid: false,
      error: `Network mismatch: ${payload.network} !== ${requirements.network}`,
    }
  }
  if (payload.payTo.toLowerCase() !== requirements.payTo.toLowerCase()) {
    return {
      valid: false,
      error: `Recipient mismatch: ${payload.payTo} !== ${requirements.payTo}`,
    }
  }
  if (payload.asset.toLowerCase() !== requirements.asset.toLowerCase()) {
    return {
      valid: false,
      error: `Asset mismatch: ${payload.asset} !== ${requirements.asset}`,
    }
  }
  if (payload.resource !== requirements.resource) {
    return {
      valid: false,
      error: `Resource mismatch: ${payload.resource} !== ${requirements.resource}`,
    }
  }
  ...
  if (requirements.scheme === 'exact') {
    if (paymentAmount !== maxAmount) {
      return {
        valid: false,
        error: `Exact scheme requires amount ${maxAmount}, got ${paymentAmount}`,
      }
    }
    ...
    if (paymentAmount > maxAmount) {
      return {
        valid: false,
        error: `Upto scheme: amount ${paymentAmount} exceeds max ${maxAmount}`,
      }
    }
    if (paymentAmount === 0n) {
      return {
        valid: false,
        error: 'Upto scheme: amount must be greater than 0',
      }
    }
  } else {
...
function validateTimestamp(timestamp: number): {
  ...
  if (age > config().maxPaymentAge)
    return {
      valid: false,
      error: `Payment expired (${age}s > ${config().maxPaymentAge}s)`,
    }
  return { valid: true }
}
...
export function encodePaymentHeader(
  payload: Omit<PaymentPayload, 'signature'> & { signature: Hex },
): string {

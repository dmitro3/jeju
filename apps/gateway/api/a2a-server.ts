...
const MCP_SERVER_INFO = {
  name: 'jeju-gateway',
  version: '1.0.0',
  description: 'Gateway - Protocol infrastructure and cross-chain intents',
...
async function executeSkill(
  ...
  switch (skillId) {
    ...
    case 'get-best-route': {
      ...
      if (!validated.sourceChain || !validated.destinationChain) {
        throw new Error('sourceChain and destinationChain are required')
      }
    ...
    case 'list-solvers': {
      ...
      return {
        ...
        data: toResponseData({ solvers, activeSolvers: solvers.length }),
      }
    }
    ...
    case 'list-v2-pools': {
      ...
      return {
        ...
        data: toResponseData({ pools, count: pools.length }),
      }
    }
    case 'list-v3-pools': {
      const validated = expect(
    ...
    case 'get-all-swap-quotes': {
      ...
      return {
        ...
        data: toResponseData({ quotes, bestQuote: quotes[0] }),
      }
    }
    ...
    case 'get-moderation-cases': {
      ...
      return {
        ...
        data: toResponseData({ cases, count: cases.length }),
      }
    }
    ...
    case 'get-reports': {
      ...
      return {
        ...
        data: toResponseData({ reports, count: reports.length }),
      }
    }
    ...
    case 'prepare-vote': {
      ...
      return {
        ...
        data: toResponseData({ action: 'sign-and-send', transaction: tx }),
      }
    }
...
async function executeMcpTool(
...
): Promise<{ result: JsonObject; isError: boolean }> {
  ...
  switch (name) {
    ...
    case 'track_intent': {
      ...
      result = toResponseData(
        (await intentService.getIntent(intentId)) ?? {
    ...
    case 'list_routes': {
      const validatedArgs =
        args && Object.keys(args).length > 0
          ...
          : undefined
      result = toResponseData(await routeService.listRoutes(validatedArgs))
      break
    }
    case 'list_solvers': {
      const validatedArgs =
        args && Object.keys(args).length > 0
          ...
          : undefined
      result = toResponseData(await solverService.listSolvers(validatedArgs))
      break
    }
    case 'list_v2_pools':
      result = toResponseData(await poolService.listV2Pools())
      break
    case 'get_pool_reserves': {
      const validatedArgs = expect(args, TokenPairSchema, 'get_pool_reserves')
      result = toResponseData(
        await poolService.listPoolsForPair(
    ...
    case 'get_swap_quote': {
      ...
      result = toResponseData(
        (await poolService.getSwapQuote(
    ...
    case 'get_all_swap_quotes': {
      ...
      result = toResponseData(
        await poolService.getAllSwapQuotes(
          validatedArgs.tokenIn as Address,
          validatedArgs.tokenOut as Address,
          validatedArgs.amountIn,
        ),
    ...
    case 'get_pool_stats':
      result = toResponseData(await poolService.getPoolStats())
      break
    ...
    case 'check_ban_status': {
      ...
      result = toResponseData(
        await checkBanStatus(validatedArgs.address as Address),
      )
      break
    }
    case 'get_moderator_profile': {
      ...
      result = toResponseData(
        (await getModeratorProfile(validatedArgs.address as Address)) ?? {
    ...
    case 'get_moderation_cases': {
      const validatedArgs =
        args && Object.keys(args).length > 0
          ...
          : {}
      result = toResponseData(await getModerationCases(validatedArgs))
      break
    }
    case 'get_moderation_case': {
      ...
      result = toResponseData(
        (await getModerationCase(caseId)) ?? { error: 'Case not found' },
      )
      break
    }
    case 'get_reports': {
      const validatedArgs =
        args && Object.keys(args).length > 0
          ...
          : {}
      result = toResponseData(await getReports(validatedArgs))
      break
    }
    case 'get_agent_labels': {
      ...
      result = toResponseData(await getAgentLabels(agentId))
      break
    }
    case 'get_moderation_stats':
      result = toResponseData(await getModerationStats())
      break
    case 'prepare_stake': {
      const validatedArgs = expect(
        args,
      ...
      result = {
        action: 'sign-and-send',
        transaction: serializeTransactionRequest(
          prepareStakeTransaction(validatedArgs.amount),
        ),
      }
      break
    }
    ...
    case 'prepare_vote': {
      ...
      result = {
        action: 'sign-and-send',
        transaction: serializeTransactionRequest(
          prepareVoteTransaction(validatedArgs.caseId, validatedArgs.voteYes),
        ),
      }
      break
    }
    ...
    case 'faucet_status': {
      ...
      result = toResponseData(
        await faucetService.getFaucetStatus(validatedArgs.address as Address),
      )
      break
    }
    case 'faucet_claim': {
      ...
      result = toResponseData(
        await faucetService.claimFromFaucet(validatedArgs.address as Address),
      )
      break
    }
    case 'faucet_info': {
      ...
      result = toResponseData(faucetService.getFaucetInfo())
      break
    }
    case 'get_risk_tiers':
      result = {
    ...
    case 'get_sleeve_stats': {
      const tierArg = args.tier
      if (typeof tierArg !== 'number' || tierArg < 0 || tierArg > 2) {
        result = { error: 'Invalid tier' }
        isError = true
        break
      }
      // tierArg is now narrowed to 0 | 1 | 2 which is compatible with RiskTier
      result = toResponseData(await getSleeveStats(tierArg as RiskTier))
      break
    }
    case 'get_sleeve_position': {
      const tier = typeof args.tier === 'number' ? args.tier : -1
      ...
      result = toResponseData(
        await getSleevePosition(address, tier as RiskTier),
      )
      break
    }
    case 'prepare_sleeve_deposit': {
      ...
      const amount = typeof args.amount === 'string' ? args.amount : '0'
    ...
    case 'get_allocation_strategy':
      result = toResponseData(await getDefaultStrategy())
      break
    ...
    case 'get_router_position': {
      ...
      result = toResponseData(await getRouterPosition(address))
      break
    }
    case 'estimate_yield': {
      const address = typeof args.address === 'string' ? args.address : ''
      ...
      result = await estimateYield(address)
      break
    }
    default:
...
app
  ...
  .post('/mcp/resources/read', async ({ body, set }) => {
    const parseResult = McpResourceReadRequestSchema.safeParse(body)
    if (!parseResult.success) {
      set.status = 400
      return { error: parseResult.error.message }
    }
    const { uri } = parseResult.data
    const sendResource = (data: JsonObject) => ({
      contents: [
        {
          uri,
          mimeType: 'application/json',
    ...
    switch (uri) {
      case 'oif://routes':
        return sendResource(toResponseData(await routeService.listRoutes()))
      case 'oif://solvers':
        return sendResource(toResponseData(await solverService.listSolvers()))
      case 'oif://intents/recent':
        return sendResource(
          toResponseData(await intentService.listIntents({ limit: 100 })),
        )
      case 'oif://stats':
        return sendResource(await intentService.getStats())
      case 'xlp://pools/v2':
        return sendResource(toResponseData(await poolService.listV2Pools()))
      case 'xlp://pools/v3':
        return sendResource({
          note: 'V3 pools require specific token pair query',
          stats: toResponseData(await poolService.getPoolStats()),
        })
      case 'xlp://pools/stats':
        return sendResource(toResponseData(await poolService.getPoolStats()))
      case 'xlp://tokens':
      ...
      case 'moderation://cases':
        return sendResource(
          toResponseData(await getModerationCases({ limit: 100 })),
        )
      ...
      case 'moderation://reports':
        return sendResource(toResponseData(await getReports({ limit: 100 })))
      case 'moderation://stats':
        return sendResource(toResponseData(await getModerationStats()))
      case 'risk://tiers':
      ...
      case 'faucet://info':
        ...
        return sendResource(toResponseData(faucetService.getFaucetInfo()))
      default:
        set.status = 404
...
app
  ...
  .post('/mcp/tools/call', async ({ body, set }) => {
    const parseResult = McpToolCallRequestSchema.safeParse(body)
    if (!parseResult.success) {
      set.status = 400
      return { error: parseResult.error.message }
    }
...
app.use(strictRateLimitPlugin()).post('/api/intents', async ({ body, set }) => {
  const parseResult = CreateIntentRequestSchema.safeParse(body)
  if (!parseResult.success) {
    set.status = 400
    return { error: parseResult.error.message }
  }
...
app
  ...
  .post('/api/intents/:intentId/cancel', async ({ params, body, set }) => {
    const intentId = expect(params.intentId, IntentIdSchema, 'intentId')
    const parseResult = CancelIntentRequestSchema.safeParse(body)
    if (!parseResult.success) {
      set.status = 400
      return { error: parseResult.error.message }
    }
...
app.post('/api/intents/quote', async ({ body, set }) => {
  const parseResult = GetQuoteRequestSchema.safeParse(body)
  if (!parseResult.success) {
    set.status = 400
    return { error: parseResult.error.message }
  }
...
app.post('/api/routes/best', async ({ body, set }) => {
  const parseResult = GetBestRouteRequestSchema.safeParse(body)
  if (!parseResult.success) {
    set.status = 400
    return { error: parseResult.error.message }
  }
  ...
  if (!validated.sourceChain || !validated.destinationChain) {
    set.status = 400
    return { error: 'sourceChain and destinationChain are required' }
  }
  return await routeService.getBestRoute({
    sourceChain: validated.sourceChain,
    destinationChain: validated.destinationChain,
    prioritize: validated.prioritize,
  })
...
app.post('/api/pools/quote', async ({ body, set }) => {
  const parseResult = SwapQuoteRequestSchema.safeParse(body)
  if (!parseResult.success) {
    set.status = 400
    return { error: parseResult.error.message }
  }
...
app
  ...
  .post('/api/faucet/claim', async ({ body, set }) => {
    ...
    const parseResult = FaucetClaimRequestSchema.safeParse(body)
    if (!parseResult.success) {
      set.status = 400
      return { error: parseResult.error.message }
    }

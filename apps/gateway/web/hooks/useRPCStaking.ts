...
import { CONTRACTS, RPC_GATEWAY_URL } from '../../lib/config'
...
export function useRPCStaking() {
  ...
  const { data: positionData, refetch: refetchPosition } = useReadContract({
    address: enabled ? STAKING : undefined,
    abi: STAKING_ABI,
  ...
  const { data: rateLimitData } = useReadContract({
    address: enabled ? STAKING : undefined,
    abi: STAKING_ABI,
  ...
  const { data: discountData } = useReadContract({
    address: enabled ? STAKING : undefined,
    abi: STAKING_ABI,
    functionName: 'getReputationDiscount',
    args: positionArgs,
    query: { enabled },
  })
  const { data: stakeUsdData } = useReadContract({
    address: enabled ? STAKING : undefined,
    abi: STAKING_ABI,
    functionName: 'getStakeUsdValue',
    args: positionArgs,
    query: { enabled },
  })
  const { data: priceData } = useReadContract({
    address: isConfigured ? STAKING : undefined,
    abi: STAKING_ABI,
  ...
  const { data: balanceData } = useReadContract({
    address: tokenEnabled ? TOKEN : undefined,
    abi: ERC20_ABI,
  ...
  const allowanceArgs =
    allowanceEnabled && address ? ([address, STAKING] as const) : undefined
  const { data: allowanceData, refetch: refetchAllowance } = useReadContract({
    address: allowanceEnabled ? TOKEN : undefined,
    abi: ERC20_ABI,
  ...
  const position = useMemo(
    (): StakingPosition | null => (positionData ? { ...positionData } : null),
    [positionData],
  )
  const tier = useMemo(
    (): RateTier =>
      (['FREE', 'BASIC', 'PRO', 'UNLIMITED'] as const)[Number(tierData ?? 0)] ??
      'FREE',
    [tierData],
  )
  const jejuPrice = priceData ? Number(formatUnits(priceData, 8)) : 0.1
  const tierRequirements = useMemo(
    (): Record<RateTier, TierInfo> =>
      jejuPrice <= 0
        ? TIER_CONFIG
        : {
            ...
            BASIC: {
              ...TIER_CONFIG.BASIC,
              jejuNeeded: Math.ceil(10 / jejuPrice),
            },
            PRO: { ...TIER_CONFIG.PRO, jejuNeeded: Math.ceil(100 / jejuPrice) },
            UNLIMITED: {
              ...TIER_CONFIG.UNLIMITED,
              jejuNeeded: Math.ceil(1000 / jejuPrice),
            },
          },
    [jejuPrice],
  )
  ...
  const approve = useCallback(
    async (amount: string) => {
      ...
      const hash = await writeContractAsync({
        ...
        args: [STAKING, parseEther(amount)],
      })
      setLoading(false)
  ...
  const stake = useCallback(
    async (amount: string) => {
      if (!address || !isConfigured) return null
      if ((allowanceData ?? 0n) < parseEther(amount)) {
  ...
  const fetchApiKeys = useCallback(async () => {
    if (!address) return
    const res = await fetch(`${RPC_GATEWAY_URL}/v1/keys`, {
  ...
  const revokeApiKey = useCallback(
    async (keyId: string) => {
      ...
      const ok = (
        await fetch(`${RPC_GATEWAY_URL}/v1/keys/${keyId}`, {
          method: 'DELETE',
          headers: { 'X-Wallet-Address': address },
        })
      ).ok
  ...
  return {
    ...
    position,
    tier,
    rateLimit: Number(rateLimitData ?? 10),
    reputationDiscount: Number(discountData ?? 0),
    jejuBalance: balanceData ? formatEther(balanceData) : '0',
    allowance: allowanceData ?? 0n,
    jejuPrice,
    stakeUsdValue: stakeUsdData ? Number(formatUnits(stakeUsdData, 8)) : 0,
    tierRequirements,
    apiKeys,
    approve,
    stake,

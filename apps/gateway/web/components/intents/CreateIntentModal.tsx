...
export function CreateIntentModal({ onClose }: CreateIntentModalProps) {
  ...
  const inputSettlerAddress =
    oifConfig.inputSettlers[sourceChain as keyof typeof oifConfig.inputSettlers]
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            >
              {chains
                ?.filter((c) => c.chainId !== sourceChain)
                .map((c) => (
                  <option key={c.chainId} value={c.chainId}>
                    {c.name}
                  </option>
          ...
          <button
            ...
            disabled={
              !canSubmit || txStatus === 'pending' || txStatus === 'confirming'
            }

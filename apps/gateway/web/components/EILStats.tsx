...
async function fetchEILStats(): Promise<{
  stats: EILStatsData
  chainStats: EILChainStats[]
}> {
  const query = `
    query EILStats {
  ...
  const failed = requests.filter(
    (r: { status: string }) =>
      r.status === 'EXPIRED' || r.status === 'REFUNDED',
  ).length
  const totalCompleted = fulfilled + failed
  const successRate =
    totalCompleted > 0
      ? Math.round((fulfilled / totalCompleted) * 1000) / 10
      : 0
  ...
  const chainVolumes = new Map<
    number,
    { volume: bigint; transfers: number; xlps: Set<string> }
  >()
  ...
  const chainStats: EILChainStats[] = Array.from(chainVolumes.entries())
    .map(([chainId, data]) => ({
      chainId,
  ...
  return (
    <div style={{ marginBottom: '1.5rem' }}>
      ...
      >
        <StatCard
          label="Total Volume"
          value={`${stats.totalVolumeEth} ETH`}
          subtext={`${stats.totalTransactions} transfers`}
          variant="info"
          icon="📊"
        />
        <StatCard
        ...
        <StatCard
          label="Success Rate"
          value={stats.totalTransactions > 0 ? `${stats.successRate}%` : '-'}
          subtext={`${stats.totalTransactions.toLocaleString()} txns`}
          variant="accent"
          icon="✓"
        />
      </div>
        ...
        >
          {chainStats.map((chain) => (
            ...
            >
              ...
              >
                {chain.totalTransfers.toLocaleString()} txns
              </div>
            </div>
          ))}
        </div>
...
function StatCard({
  ...
  return (
    <div
      className="stat-card"
      style={{ borderTop: `3px solid ${colors[variant]}` }}
    >
      <div
        style={{
          display: 'flex',

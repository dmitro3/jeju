...
function calculateEffectivePrice(amountIn: string, amountOut: bigint): string {
  ...
  return inNum > 0 && !Number.isNaN(inNum) && !Number.isNaN(outNum)
    ? (outNum / inNum).toFixed(8)
    : '0'
}
...
function logContractError(
  ...
  console.error(
    `[PoolService] Failed to ${operation}${context ? ` for ${context}` : ''}:`,
    formatError(error),
  )
}
...
> = {
  ...
  WETH: {
    address: (process.env.WETH_ADDRESS || ZERO_ADDRESS) as Address,
    symbol: 'WETH',
    decimals: 18,
  },
  USDC: {
...
class PoolService {
  ...
  private validateConfiguration(): void {
    ...
    if (missing.length > 0) {
      ...
      if (process.env.NODE_ENV === 'production') {
        console.error(
          '[PoolService] CRITICAL: Required configuration missing in production!',
        )
      }
    }
  }
  ...
  private async getETHPriceUSD(): Promise<bigint> {
    if (
      this.ethPriceCache &&
      Date.now() - this.ethPriceCache.timestamp < this.ETH_PRICE_CACHE_TTL
    ) {
    ...
    if (ethMainnetFeed) {
      try {
        ...
        console.error(
          '[PoolService] Failed to fetch ETH price from Chainlink:',
          error,
        )
    ...
    if (envPrice) {
      ...
      if (Number.isNaN(priceNum) || priceNum <= 0) {
        console.error(
          `[PoolService] Invalid ETH_PRICE_USD env var: "${envPrice}". Must be a positive number.`,
        )
      } else {
        const price = BigInt(Math.floor(priceNum * 1e18))
        this.ethPriceCache = { price, timestamp: Date.now() }
        console.warn(
          '[PoolService] Using ETH_PRICE_USD from environment variable',
        )
        return price
      }
    }
    ...
    console.error(
      '[PoolService] CRITICAL: Using hardcoded fallback ETH price: $3000. Chainlink fetch failed and ETH_PRICE_USD not set. USD calculations will be incorrect!',
    )
    if (process.env.NODE_ENV === 'production') {
      console.error(
        '[PoolService] PRODUCTION ALERT: Price oracle failure - manual intervention required!',
      )
    }
  ...
  async listV2Pools(): Promise<V2Pool[]> {
    ...
    const pairCount = await this.client
      .readContract({
    ...
    for (let i = 0; i < Math.min(count, MAX_POOLS_TO_FETCH); i++) {
      const pairAddress = await this.client
        .readContract({
          address: this.contracts.v2Factory,
          abi: V2_FACTORY_ABI,
  ...
  async getV3Pool(
    ...
    const poolAddress = await this.client
      .readContract({
      ...
      .catch((error) => {
        logContractError(
          'get V3 pool',
          `${sortedToken0}/${sortedToken1} fee ${fee}`,
          error,
        )
        return null
      })
  ...
  async listPoolsForPair(token0: Address, token1: Address): Promise<Pool[]> {
    const validated = expect(
      { token0, token1 },
      TokenPairSchema,
      'listPoolsForPair params',
    )
    const pools: Pool[] = []

    if (this.contracts.v2Factory !== ZERO_ADDRESS) {
      const v2Pair = await this.client
        .readContract({
          address: this.contracts.v2Factory,
          abi: V2_FACTORY_ABI,
    ...
    for (const fee of V3_FEE_TIERS) {
      const v3Pool = await this.getV3Pool(
  ...
  async getSwapQuote(
    ...
    const pools = await this.listPoolsForPair(
      validated.tokenIn as Address,
      validated.tokenOut as Address,
    )
    ...
    if (v2Pool) {
      const isToken0In =
        validated.tokenIn.toLowerCase() === v2Pool.token0.toLowerCase()
      const reserveIn = parseEther(
        isToken0In ? v2Pool.reserve0 : v2Pool.reserve1,
      )
      const reserveOut = parseEther(
        isToken0In ? v2Pool.reserve1 : v2Pool.reserve0,
      )
  ...
  async getAllSwapQuotes(
    ...
    const validated = expect(
      { tokenIn, tokenOut, amountIn },
      SwapQuoteRequestSchema,
      'getAllSwapQuotes params',
    )
    const amountInWei = parseEther(validated.amountIn)
    const quotes: SwapQuote[] = []

    if (this.contracts.aggregator !== ZERO_ADDRESS) {
      const allQuotes = await this.client
        .readContract({
          address: this.contracts.aggregator,
          abi: AGGREGATOR_ABI,
    ...
    if (quotes.length === 0) {
      const bestQuote = await this.getSwapQuote(
        validated.tokenIn as Address,
        validated.tokenOut as Address,
        validated.amountIn,
      )
  ...
  async getPoolStats(): Promise<PoolStats> {
    ...
    const totalLiquidityUsd = formatEther(
      (totalLiquidity * ethPriceUSD) / 10n ** 18n,
    )
  ...
  getHealthStatus() {
    ...
    return {
      ...
      usingFallbackPrice:
        !this.ethPriceCache ||
        this.ethPriceCache.price === ETH_PRICE_FALLBACK_USD,
    }
  }
}

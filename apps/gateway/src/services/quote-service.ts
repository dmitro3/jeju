...
export async function getQuotes(params: QuoteParams): Promise<IntentQuote[]> {
  ...
  const solverResults = await Promise.all(knownSolvers.map(addr => fetchSolverInfo(addr)));
  ...
  const destGasPrice = await client.getGasPrice().catch((err: Error) => {
    console.warn(`[quote-service] Gas price fetch failed (chain ${destinationChain}): ${err.message}`);
  ...
  for (const solver of activeSolvers) {
    ...
    if (isL2Chain(sourceChain) && isL2Chain(destinationChain)) marginBps -= L2_DISCOUNT_BPS;
    ...
    if (isL2Chain(sourceChain) && isL2Chain(destinationChain)) estimatedFillTimeSeconds = 15;
    else if (sourceChain === 1 || destinationChain === 1) estimatedFillTimeSeconds = 60;
  ...
  if (quotes.length === 0) {
    ...
    const quoteId = keccak256(encodePacked(

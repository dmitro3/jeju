...
async function discoverSolvers(): Promise<`0x${string}`[]> {
  ...
  if (discoveredSolvers.size > 0) {
    console.log(
      `[quote-service] Discovered ${discoveredSolvers.size} solvers from registry`,
    )
  }
...
const GAS_COSTS: Record<
  number,
  { fillGas: bigint; settlementGas: bigint; baseFee: bigint }
> = {
...
async function fetchSolverInfo(
  address: `0x${string}`,
  ...
  const reputation =
    totalFills > 0 ? Math.round((successfulFills / totalFills) * 100) : 50
...
function calculateGasCost(
  ...
  return (
    destCosts.fillGas * destGasPrice + srcCosts.settlementGas * srcCosts.baseFee
  )
}

function isL2Chain(chainId: number): boolean {
  return [10, 42161, 8453, 11155420, 421614, 84532, 420690, 420691].includes(
    chainId,
  )
}
...
export async function getQuotes(params: QuoteParams): Promise<IntentQuote[]> {
  ...
  const knownSolvers = await discoverSolvers()
  const solverResults = await Promise.all(
    knownSolvers.map((addr) => fetchSolverInfo(addr)),
  )
  ...
  const destGasPrice = await client.getGasPrice().catch((err: Error) => {
    console.warn(
      `[quote-service] Gas price fetch failed (chain ${destinationChain}): ${err.message}`,
    )
  ...
  for (const solver of activeSolvers) {
    const supportsSource = solver.supportedChains.some(
      (c) => Number(c) === sourceChain,
    )
    const supportsDest = solver.supportedChains.some(
      (c) => Number(c) === destinationChain,
    )
    ...
    let estimatedFillTimeSeconds = 30
    if (isL2Chain(sourceChain) && isL2Chain(destinationChain))

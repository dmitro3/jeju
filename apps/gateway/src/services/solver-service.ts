...
import { initializeState, solverState } from './state.js'

const KNOWN_SOLVER_ADDRESSES: string[] = (
  process.env.OIF_DEV_SOLVER_ADDRESSES || ''
)
  .split(',')
...
export class SolverService {
  ...
  private async refreshSolverFromChain(
    address: `0x${string}`,
  ): Promise<Solver | null> {
    ...
    if (chainInfo?.isActive) {
      ...
      const successRate =
        totalFills > 0 ? (successfulFills / totalFills) * 100 : 0
      ...
      const solver: Solver = {
        ...
        supportedChains: chainInfo.supportedChains.map(
          (c) => Number(c) as SupportedChainId,
        ),
        supportedTokens: {},
        liquidity: [],
        reputation,
  ...
  async listSolvers(params?: {
    chainId?: number
    minReputation?: number
    active?: boolean
  }): Promise<Solver[]> {
    const validated = params
      ? expect(params, ListSolversQuerySchema, 'listSolvers params')
      : undefined
    if (!this.initialized) {
  ...
  async getLeaderboard(params?: {
    ...
    const validated = params
      ? expect(params, SolverLeaderboardQuerySchema, 'getLeaderboard params')
      : undefined
  ...
  async findSolversForRoute(
    ...
    const validatedSourceChain = expectChainId(
      sourceChain,
      'findSolversForRoute sourceChain',
    )
    const validatedDestChain = expectChainId(
      destinationChain,
      'findSolversForRoute destinationChain',
    )
    ...
    return allSolvers.filter((solver) => {
      const supportsSource = (solver.supportedChains as number[]).includes(
        srcChain,
      )
      const supportsDest = (solver.supportedChains as number[]).includes(
        destChain,
      )

      const sourceTokens =
        solver.supportedTokens[validatedSourceChain.toString()] || []

      const supportsToken =
        sourceTokens.includes(validatedToken) || validatedToken === ZERO_ADDRESS

      return supportsSource && supportsDest && supportsToken
    })
  }

  async updateSolverStats(
    address: string,
    stats: {
      totalFills?: number
    ...
    if (stats.successfulFills !== undefined)
      solver.successfulFills = stats.successfulFills

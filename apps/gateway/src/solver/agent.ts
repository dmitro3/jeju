...
export class SolverAgent {
  ...
  private async fillExternalOpportunity(
    ...
    console.log(
      `   🔄 Filling ${opp.type} opportunity: ${opp.id.slice(0, 20)}...`,
    )

    if (!this.externalAggregator)
      throw new Error('External aggregator not initialized')
  ...
  private async checkPendingSettlements(): Promise<void> {
    ...
    for (const [orderId, settlement] of entries) {
      ...
      if (now - settlement.filledAt > SETTLEMENT_STALE_MS) {
        ...
        console.log(
          `   🧹 Removed stale settlement: ${orderId.slice(0, 10)}...`,
        )
        continue
      }
      ...
      if (result.settled) {
        ...
        if (settlement.retryCount >= MAX_SETTLEMENT_RETRIES) {
          ...
          console.log(
            `   ❌ Settlement failed after ${MAX_SETTLEMENT_RETRIES} retries: ${orderId.slice(0, 10)}...`,
          )
        } else {
          ...
          console.log(
            `   ⏳ Will retry in ${backoffMs / 1000}s: ${orderId.slice(0, 10)}...`,
          )
        }
      } else {
        this.pendingSettlements.delete(orderId)
  ...
  private async trySettle(
    ...
    const canSettle = await client.public
      .readContract({

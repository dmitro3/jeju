/**
 * Local Nonce Manager - In-memory for single-replica deployments
 * Set CACHE_SERVICE_URL to enable distributed mode
 */
...
export async function isNonceUsed(publicClient: PublicClient, payer: Address, nonce: string): Promise<boolean> {
  ...
  if (usedOnChain) await markNonceUsed(payer, nonce);
  return usedOnChain;
}
...
export async function getNonceCacheStats(): Promise<{ 
  distributed: boolean;
  total: number; 
  used: number; 
  pending: number; 
...
export const clearLocalNonceCache = clearNonceCache;
...
export function startNonceCleanup(): void {
  ...
  cleanupInterval = setInterval(() => {
    ...
    if (cleaned > 0) console.log(`[NonceManager] Cleaned ${cleaned} nonces`);
  }, 60 * 60 * 1000);
}
...
export async function initDistributedNonceManager(): Promise<void> {
  console.log('[NonceManager] Local mode (set CACHE_SERVICE_URL for distributed)');
}

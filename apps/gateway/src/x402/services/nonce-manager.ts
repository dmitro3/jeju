...
export async function reserveNonce(
  ...
  if (pendingNonces.has(key))
    return { reserved: false, error: 'Nonce is being processed' }
  if (usedNonces.has(key))
    return { reserved: false, error: 'Nonce already used' }
...
export async function initDistributedNonceManager(): Promise<void> {
  console.log(
    '[NonceManager] Local mode (set CACHE_SERVICE_URL for distributed)',
  )
}

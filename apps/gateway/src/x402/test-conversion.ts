...
async function test() {
  ...
  const supportedRes = await request('/supported')
  const supportedBody = await supportedRes.json()
  console.log(
    '\nGET /supported status:',
    supportedRes.status,
    supportedRes.status === 200 ? '✓' : '✗',
  )
  ...
  const networksRes = await request('/supported/networks')
  const networksBody = await networksRes.json()
  console.log(
    '\nGET /supported/networks status:',
    networksRes.status,
    networksRes.status === 200 ? '✓' : '✗',
  )
  console.log('  networks count:', networksBody.networks?.length)

  // Test 4: Tokens endpoint
  const tokensRes = await request('/supported/tokens/jeju')
  const tokensBody = await tokensRes.json()
  console.log(
    '\nGET /supported/tokens/jeju status:',
    tokensRes.status,
    tokensRes.status === 200 ? '✓' : '✗',
  )
  ...
  const badTokensRes = await request('/supported/tokens/invalid-network')
  console.log(
    '\nGET /supported/tokens/invalid status:',
    badTokensRes.status,
    badTokensRes.status === 400 ? '✓' : '✗',
  )

  // Test 6: Not found (should 404)
  const notFoundRes = await request('/unknown/route')
  const notFoundBody = await notFoundRes.json()
  console.log(
    '\nGET /unknown/route status:',
    notFoundRes.status,
  ...
  const verifyBadRes = await request('/verify', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: 'not json',
  })
  console.log(
    '\nPOST /verify bad JSON status:',
    verifyBadRes.status,
    verifyBadRes.status === 400 ? '✓' : '✗',
  )

  // Test 8: Settle with invalid JSON (should 400)
  const settleBadRes = await request('/settle', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: 'not json',
  })
  console.log(
    'POST /settle bad JSON status:',
    settleBadRes.status,
    settleBadRes.status === 400 ? '✓' : '✗',
  )

  // Test 9: CORS headers
  const corsRes = await request('/', { method: 'OPTIONS' })
  const allowOrigin = corsRes.headers.get('access-control-allow-origin')
  console.log('\nOPTIONS / CORS:', allowOrigin === '*' ? '✓' : '✗')

  console.log('\n✓ All tests passed (visual verification above)!')
}

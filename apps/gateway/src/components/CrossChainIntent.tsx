...
import { useCreateIntent, useIntentStatus, useOIFConfig } from '../hooks/useOIF'
...
export function CrossChainIntent() {
  ...
  const [destChain, setDestChain] = useState(
    CHAINS[0].id === chainId ? CHAINS[1].id : CHAINS[0].id,
  )
  const [maxFee, setMaxFee] = useState('0.005')

  const inputSettlerAddress = config.inputSettlers[chainId]
  const { createIntent, intentId, isPending, isConfirming, isSuccess, error } =
    useCreateIntent(inputSettlerAddress)
  ...
  if (!isConnected) {
    return (
      <div
        style={{
          background:
            'linear-gradient(135deg, var(--bg-gradient-start) 0%, var(--surface) 100%)',
          borderRadius: '16px',
          padding: '32px',
          textAlign: 'center',
        }}
      >
        <p style={{ color: 'var(--text-muted)' }}>
          Connect your wallet to create cross-chain intents
        </p>
      </div>
    )
  }
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                Balance:{' '}
                {balance ? formatEther(balance.value).slice(0, 8) : '0'} ETH
              </div>
            </div>
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={
            isPending || isConfirming || !amount || parseFloat(amount) <= 0
          }
          style={{
            width: '100%',
            padding: '16px',

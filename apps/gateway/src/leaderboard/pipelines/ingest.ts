...
interface GitHubCommit {
  ...
  author: { name: string; email: string; date: string; user?: { login: string } };
  additions: number;
  deletions: number;
...
export async function runIngest(options: IngestOptions = {}): Promise<{
  ...
  console.log(`[Ingest] Starting ingestion for ${repositories.length} repositories`);
  console.log(`[Ingest] Date range: ${dateRange.after || 'all'} to ${dateRange.before || 'now'}`);
  ...
  for (const repo of repositories) {
    ...
    const prs = await fetchAndStorePullRequests(token, owner, name, dateRange, options.verbose);
    const issues = await fetchAndStoreIssues(token, owner, name, dateRange, options.verbose);
    const commits = await fetchAndStoreCommits(token, owner, name, dateRange, options.verbose);
  ...
  console.log(`[Ingest] Completed: ${totalPrs} PRs, ${totalIssues} issues, ${totalCommits} commits`);
...
function calculateDateRange(options: IngestOptions): { after?: string; before?: string } {
  ...
  while (hasNextPage) {
    ...
    for (const pr of prs.nodes) {
      ...
      await exec(
        ...
        [
          pr.id, pr.number, pr.title, pr.body || '', pr.state, pr.merged ? 1 : 0,
          pr.author.login, pr.createdAt, pr.updatedAt, pr.closedAt, pr.mergedAt,
          repoId, pr.headRefOid, pr.baseRefOid, pr.additions, pr.deletions,
          pr.changedFiles, now,
        ]
      );
      count++;
    ...
    for (const issue of issues.nodes) {
      ...
      await exec(
        ...
        [
          ...
          issue.locked ? 1 : 0, issue.author.login, issue.createdAt, issue.updatedAt,
          issue.closedAt, repoId, now,
        ]
      );
      count++;
    }
  ...
  while (hasNextPage) {
    ...
    for (const commit of history.nodes) {
      ...
      await exec(
        ...
        [
          commit.oid, commit.message, commit.messageHeadline, commit.committedDate,
          commit.author.name, commit.author.email, commit.author.date,
          username || null, repoId, commit.additions, commit.deletions,
          commit.changedFilesIfAvailable || 0, now,
        ]
      );
      count++;
    }

...
export async function runSummarize(options: SummarizeOptions): Promise<{
  ...
  console.log(`[Summarize] Generating ${options.type} summaries (${interval}) from ${dateRange.after} to ${dateRange.before}`);
...
function calculateDateRange(options: SummarizeOptions): { after: string; before: string } {
...
async function generateContributorSummaries(
  ...
  for (const contrib of contributors) {
    const activity = await getContributorActivity(contrib.username, dateRange, interval);
    ...
    // Generate summary
    const summary = await callLLM(apiKey, buildContributorPrompt(contrib.username, activity));
    ...
    if (existing.length > 0) {
      ...
      await exec(
        `INSERT INTO user_summaries (id, username, interval_type, date, summary, last_updated)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [summaryId, contrib.username, interval, dateRange.before, summary.text, now]
      );
    }
...
async function generateRepositorySummaries(
  ...
  for (const repo of repositories) {
    ...
    const summary = await callLLM(apiKey, buildRepoPrompt(repo.repo_id, activity));
    tokens += summary.tokens;
    ...
    if (existing.length > 0) {
      ...
      await exec(
        `INSERT INTO repo_summaries (id, repo_id, interval_type, date, summary, last_updated)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [summaryId, repo.repo_id, interval, dateRange.before, summary.text, now]
      );
    }
...
async function generateOverallSummaries(
  ...
  const summary = await callLLM(apiKey, buildOverallPrompt(stats, topContribs, dateRange, interval));
...
async function getOverallStats(dateRange: { after: string; before: string }): Promise<{
  prs: number;
  mergedPrs: number;
  issues: number;
  commits: number;
...
async function getTopContributorsForSummary(
  ...
  return result.map(r => ({ username: r.username, score: Math.round(r.total_score) }));
}

function buildContributorPrompt(username: string, activity: ActivitySummary): string {
...
function buildOverallPrompt(
  stats: { prs: number; mergedPrs: number; issues: number; commits: number; contributors: number },
  topContribs: Array<{ username: string; score: number }>,
  dateRange: { after: string; before: string },
  _interval: string
): string {
  const topList = topContribs.slice(0, 5).map((c, i) => `${i + 1}. ${c.username} (${c.score} pts)`).join('\n');

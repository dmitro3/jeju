...
export async function runProcess(options: ProcessOptions = {}): Promise<{
  ...
  console.log(`[Process] Processing contributions from ${dateRange.after || 'all'} to ${dateRange.before || 'now'}`);
  ...
  for (const date of dates) {
    ...
    for (const activity of activities) {
      ...
      const metrics = JSON.stringify({
        ...
        reviews: activity.prReviewsApproved + activity.prReviewsChanges + activity.prReviewsComment,
      ...
      if (existing.length > 0) {
        await exec(
          `UPDATE user_daily_scores SET
           ...
           WHERE id = ?`,
          [totalScore, prScore, issueScore, reviewScore, commitScore, metrics, now, scoreId]
        );
      } else {
        await exec(
          ...
          [scoreId, activity.username, date, totalScore, prScore, issueScore, reviewScore,
           commitScore, metrics, now, now]
        );
      }
      scoresUpdated++;
    }
  ...
  console.log(`[Process] Completed: ${dates.length} days, ${usersProcessed.size} users, ${scoresUpdated} scores`);
...
function calculateDateRange(options: ProcessOptions): { after?: string; before?: string } {
...
async function getDailyActivities(date: string, repository?: string): Promise<DailyActivity[]> {
  const repoFilter = repository ? 'AND repository = ?' : '';

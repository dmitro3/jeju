...
import type { OracleNodeConfig, PriceSourceConfig, NetworkType } from './types';
...
function validatePrivateKey(key: string, name: string): Hex {
  if (!key || !key.startsWith('0x') || key.length !== 66) {
    throw new ConfigurationError(`${name} must be a valid 32-byte hex string (0x + 64 chars)`);
  }
  return key as Hex;
}
...
export async function loadNetworkConfig(network: NetworkType): Promise<NetworkConfig> {
  const configPath = new URL('../../../../packages/config/oracle/networks.json', import.meta.url);
...
export function loadContractAddresses(networkConfig: NetworkConfig): Record<string, Address> {
...
export function buildPriceSources(
  ...
  for (const [symbol, sourceConfig] of Object.entries(networkConfig.priceSources)) {
    const feedId = feedIdMap.get(symbol);
    if (!feedId) continue;
    
    sources.push({
      type: sourceConfig.type,
      address: (sourceConfig.address || '0x0000000000000000000000000000000000000000') as Address,
      feedId,
...
export function createConfig(network: NetworkType = 'localnet'): Promise<OracleNodeConfig> {
  return createConfigAsync(network);
}

async function createConfigAsync(network: NetworkType): Promise<OracleNodeConfig> {
  ...
  if (!process.env.OPERATOR_PRIVATE_KEY) throw new Error('OPERATOR_PRIVATE_KEY environment variable required');
  if (!process.env.WORKER_PRIVATE_KEY) throw new Error('WORKER_PRIVATE_KEY environment variable required');

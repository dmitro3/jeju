...
const PRIVATE_KEY = (process.env.DWS_PRIVATE_KEY || '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80') as Hex;
...
describe.skipIf(SKIP)('Package Registry On-Chain Integration', () => {
  ...
  beforeAll(async () => {
    ...
    if (packageRegistryAddress === '0x0000000000000000000000000000000000000000') {
      console.warn('[Pkg OnChain Test] PACKAGE_REGISTRY_ADDRESS not set, skipping on-chain tests');
      return;
    }
  ...
  test('should verify contract address is valid', async () => {
    ...
    const code = await publicClient.getBytecode({ address: packageRegistryAddress });
  ...
  test('should handle non-existent package gracefully', async () => {
    ...
    const pkg = await registryManager.getPackageByName('nonexistent-package-xyz-12345');
    expect(pkg).toBeNull();
  });

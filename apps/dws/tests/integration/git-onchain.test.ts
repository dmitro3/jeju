...
describe.skipIf(SKIP)('Git Registry On-Chain Integration', () => {
  ...
  beforeAll(async () => {
    ...
    repoRegistryAddress = (process.env.REPO_REGISTRY_ADDRESS || '0x0000000000000000000000000000000000000000') as Address;

    if (repoRegistryAddress === '0x0000000000000000000000000000000000000000') {
      console.warn('[Git OnChain Test] REPO_REGISTRY_ADDRESS not set, skipping on-chain tests');
      return;
    }
  ...
  test('should verify contract address is valid', async () => {
    ...
    const code = await publicClient.getBytecode({ address: repoRegistryAddress });

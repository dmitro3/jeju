...
describe('calculateRequiredSamples', () => {
  ...
  it('should require more samples for higher confidence', () => {
    const samples90 = calculateRequiredSamples(0.90, 0.5);
  ...
  it('should satisfy the probability formula', () => {
    ...
    const actualConfidence = 1 - Math.pow(1 - threshold, k);
...
describe('generateSampleIndices', () => {
  ...
  it('should produce different results with different seeds', () => {
    const seed1 = '0x1111111111111111111111111111111111111111111111111111111111111111' as Hex;
    const seed2 = '0x2222222222222222222222222222222222222222222222222222222222222222' as Hex;
...
describe('generatePeerDASSamples', () => {
  it('should generate samples for each custody group', () => {
    ...
    const samples = generatePeerDASSamples(totalChunks, custodyGroups, samplesPerGroup, seed);
...
describe('verifyPeerDASSampling', () => {
  it('should verify successful sampling when all groups pass', () => {
    const samples = [[0, 1, 2, 3], [8, 9, 10, 11], [16, 17, 18, 19]];
  ...
  it('should respect minGroupSuccess parameter', () => {
    ...
    expect(verifyPeerDASSampling(samples, verifiedSamples, 0.5).success).toBe(true);
    
    // 75% threshold - should fail
    expect(verifyPeerDASSampling(samples, verifiedSamples, 0.75).success).toBe(false);
  });
});
...
describe('DASampler', () => {
  ...
  it('should update operators', () => {
    ...
    const operators = [
      { address: TEST_ADDRESS, status: 'active' as const, endpoint: 'http://localhost:8080', reputation: 100 },
    ];
...
describe('SampleVerifier', () => {
  ...
  const blobId = '0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef' as Hex;

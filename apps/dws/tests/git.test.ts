...
describe.skipIf(SKIP)('GitObjectStore', () => {
  ...
  describe('Tree Operations', () => {
    ...
    test('should handle tree with subdirectory', async () => {
      ...
      const subTree = await store.storeTree([{ mode: '100644', name: 'nested.txt', oid: file.oid, type: 'blob' }]);
      const rootTree = await store.storeTree([{ mode: '040000', name: 'subdir', oid: subTree.oid, type: 'tree' }]);
    ...
    test('should handle tree with many entries', async () => {
      const entries = await Promise.all(
        Array.from({ length: 100 }, async (_, i) => {
          ...
          return { mode: '100644', name: `file${i.toString().padStart(3, '0')}.txt`, oid: blob.oid, type: 'blob' as const };
        })
      );
    ...
    test('should handle special characters in filenames', async () => {
      ...
      const tree = await store.storeTree([
        { mode: '100644', name: 'file with spaces.txt', oid: blob.oid, type: 'blob' },
  ...
  describe('Commit Operations', () => {
    ...
    test('should handle commit with parent', async () => {
      ...
      const tree = await store.storeTree([{ mode: '100644', name: 'file.txt', oid: blob.oid, type: 'blob' }]);
    ...
    test('should handle merge commit with multiple parents', async () => {
      ...
      const merge = await store.storeCommit({
        ...
        author: { name: 'Test', email: 'test@example.com', timestamp: 1700000002, timezoneOffset: 0 },
        committer: { name: 'Test', email: 'test@example.com', timestamp: 1700000002, timezoneOffset: 0 },
    ...
    test('should handle multi-line commit message', async () => {
      const blob = await store.storeBlob(Buffer.from('x'));
      const tree = await store.storeTree([{ mode: '100644', name: 'x', oid: blob.oid, type: 'blob' }]);
...
Reviewed-by: Alice <alice@example.com>`;
    ...
    test('should handle different timezones', async () => {
      ...
      const tree = await store.storeTree([{ mode: '100644', name: 'tz', oid: blob.oid, type: 'blob' }]);

      const commit = await store.storeCommit({
        tree: tree.oid,
        parents: [],
        author: { name: 'Test', email: 'test@example.com', timestamp: 1700000000, timezoneOffset: -480 }, // -8:00
        committer: { name: 'Test', email: 'test@example.com', timestamp: 1700000000, timezoneOffset: 330 }, // +5:30
    ...
    test('should respect walk limit', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        lastCommit = await store.storeCommit({
          ...
          author: { name: 'Test', email: 'test@example.com', timestamp: 1700000000 + i, timezoneOffset: 0 },
          committer: { name: 'Test', email: 'test@example.com', timestamp: 1700000000 + i, timezoneOffset: 0 },
          message: `Commit ${i}`,
        });
...
describe.skipIf(SKIP)('Concurrent Git Operations', () => {
  test('should handle concurrent blob stores', async () => {
    ...
    const stores = Array.from({ length: 50 }, (_, i) => store.storeBlob(Buffer.from(`concurrent content ${i}`)));
  ...
  test('should handle concurrent commit creation', async () => {
    ...
    const trees = await Promise.all(
      Array.from({ length: 10 }, async (_, i) => {
        ...
        return store.storeTree([{ mode: '100644', name: 'file.txt', oid: blob.oid, type: 'blob' }]);
      })
    );

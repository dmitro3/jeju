...
describe('CISecretsStore', () => {
  ...
  test('creates and retrieves secret', async () => {
    const secret = await store.createSecret(TEST_REPO_ID, 'API_KEY', 'secret123', TEST_ADDRESS);
  ...
  test('creates environment-scoped secret', async () => {
    const secret = await store.createSecret(TEST_REPO_ID, 'PROD_KEY', 'prodvalue', TEST_ADDRESS, 'production');
  ...
  test('updates secret value', async () => {
    const secret = await store.createSecret(TEST_REPO_ID, 'TOKEN', 'old', TEST_ADDRESS);
  ...
  test('deletes secret', async () => {
    const secret = await store.createSecret(TEST_REPO_ID, 'TEMP', 'temp', TEST_ADDRESS);
  ...
  test('checks environment protection', async () => {
    ...
    const result1 = await store.checkEnvironmentProtection(TEST_REPO_ID, 'prod', TEST_ADDRESS, 'main');
    expect(result1.allowed).toBe(true);

    const result2 = await store.checkEnvironmentProtection(TEST_REPO_ID, 'prod', TEST_ADDRESS, 'feature');
...
describe('RunnerManager', () => {
  ...
  test('filters runners by labels', () => {
    manager.registerRunner({
      ...
      nodeId: 'node-1',
      nodeAddress: TEST_ADDRESS,
      capabilities: { architecture: 'amd64', os: 'linux', docker: true, gpu: false, cpuCores: 4, memoryMb: 8192, storageMb: 50000 },
      lastHeartbeat: Date.now(),
      owner: TEST_ADDRESS,
      selfHosted: false,
    });
    ...
    manager.registerRunner({
      ...
      nodeId: 'node-2',
      nodeAddress: TEST_ADDRESS,
      capabilities: { architecture: 'amd64', os: 'linux', docker: true, gpu: true, gpuType: 'A100', cpuCores: 8, memoryMb: 32768, storageMb: 100000 },
      lastHeartbeat: Date.now(),
      owner: TEST_ADDRESS,
      selfHosted: false,
    });
  ...
  test('tracks runner heartbeat', () => {
    manager.registerRunner({
      ...
      nodeId: 'node-1',
      nodeAddress: TEST_ADDRESS,
      capabilities: { architecture: 'amd64', os: 'linux', docker: false, gpu: false, cpuCores: 2, memoryMb: 4096, storageMb: 20000 },
  ...
  test('unregisters runner', () => {
    manager.registerRunner({
      ...
      nodeId: 'node-1',
      nodeAddress: TEST_ADDRESS,
      capabilities: { architecture: 'amd64', os: 'linux', docker: false, gpu: false, cpuCores: 1, memoryMb: 2048, storageMb: 10000 },
      lastHeartbeat: Date.now(),
      owner: TEST_ADDRESS,
      selfHosted: true,
    });
...
`;
    ...
    expect(config.jobs.build.steps[0].if).toBe("github.ref == 'refs/heads/main'");

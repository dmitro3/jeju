...
describe('K3s/K3d Infrastructure', () => {
  ...
  describe.skipIf(!hasDocker || !hasK3d)('K3d Cluster Operations', () => {
    ...
    test('create k3d cluster', async () => {
      ...
      const body = await res.json() as { name: string; provider: string; status: string };
...
describe('Helm Provider', () => {
  ...
  describe('Full Helm Chart Deployment', () => {
    test('deploy multi-resource chart', async () => {
      ...
      expect([200, 201]).toContain(res.status);
      const body = await res.json() as { workers: number; services: number; namespace: string };
...
describe('Terraform Provider', () => {
  describe('Schema and Health', () => {
    ...
    test('get provider schema', async () => {
      ...
      expect(res.status).toBe(200);
      const body = await res.json() as { provider: Record<string, unknown>; resources: Record<string, unknown> };
  ...
  describe('Worker Resource CRUD', () => {
    ...
    test('read dws_worker resource', async () => {
      ...
      const res = await app.request(`/terraform/v1/resources/dws_worker/${workerId}`);
      expect([200, 404]).toContain(res.status);
    ...
    test('update dws_worker resource', async () => {
      ...
      if (res.status === 200) {
        const body = await res.json() as { memory_mb: number; max_instances: number };
  ...
  describe('Container Resource CRUD', () => {
    ...
    test('read dws_container resource', async () => {
      ...
      const res = await app.request(`/terraform/v1/resources/dws_container/${containerId}`);
      expect([200, 404]).toContain(res.status);
    });
  ...
  describe('Workspace State Management', () => {
    test('create resource in specific workspace', async () => {
      const res = await app.request('/terraform/v1/resources/dws_worker?workspace=staging', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-jeju-address': TEST_ADDRESS,
    ...
    test('workspaces are isolated', async () => {
      ...
      const prodRes = await app.request('/terraform/v1/resources/dws_worker?workspace=production', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-jeju-address': TEST_ADDRESS,
      ...
      const stagingRes = await app.request(`/terraform/v1/resources/dws_worker/${prodBody.id}?workspace=staging`);

...
import { Lock, Plus, RefreshCw, Eye, EyeOff, Trash2, Copy, Check, Clock } from 'lucide-react';
import { useSecrets, useCreateSecret } from '../../hooks';
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p className="page-subtitle">Encrypted secrets storage with TEE protection</p>
        </div>
      ...
      <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
        <div className="stat-card">
          <div className="stat-icon compute"><Lock size={24} /></div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            ...
            <div className="stat-value">
              {secrets.filter(s => s.expiresAt && s.expiresAt < Date.now() + 7 * 24 * 60 * 60 * 1000).length}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title"><Lock size={18} /> Secrets</h3>
        </div>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    {secret.expiresAt && (
                      <span className={`badge ${secret.expiresAt < Date.now() ? 'badge-error' : secret.expiresAt < Date.now() + 7 * 24 * 60 * 60 * 1000 ? 'badge-warning' : 'badge-neutral'}`}>
                        {secret.expiresAt < Date.now() ? 'Expired' : `Expires ${new Date(secret.expiresAt).toLocaleDateString()}`}
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
                    Created {new Date(secret.createdAt).toLocaleDateString()}
                    {secret.updatedAt !== secret.createdAt && ` · Updated ${new Date(secret.updatedAt).toLocaleDateString()}`}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.25rem' }}>
                  ...
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => handleCopy(`\${secrets.${secret.name}}`, secret.id)}
                    title="Copy reference"
                  >
                    {copied === secret.id ? <Check size={14} /> : <Copy size={14} />}
                  </button>
      ...
      <div className="card" style={{ marginTop: '1.5rem' }}>
        <div className="card-header">
          <h3 className="card-title"><Lock size={18} /> Usage</h3>
        </div>
        <div style={{ display: 'grid', gap: '1rem' }}>
          <div style={{ padding: '1rem', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
            <div style={{ fontWeight: 500, marginBottom: '0.5rem' }}>In Workers</div>
            <code style={{ display: 'block', padding: '0.75rem', background: 'var(--bg-primary)', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem' }}>
              {`const apiKey = env.SECRET_API_KEY;`}
            </code>
          </div>
          <div style={{ padding: '1rem', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
            <div style={{ fontWeight: 500, marginBottom: '0.5rem' }}>In Containers</div>
            <code style={{ display: 'block', padding: '0.75rem', background: 'var(--bg-primary)', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem' }}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={createSecret.isPending}>
                  {createSecret.isPending ? 'Creating...' : <><Lock size={16} /> Add Secret</>}
                </button>
              </div>
            </form>
          </div>

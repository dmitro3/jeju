...
import { Store, Search, Star, Zap, Shield } from 'lucide-react';
import { useAPIProviders, useAPIListings } from '../../hooks';
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Discover and subscribe to decentralized API services</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card">
          <div className="empty-state">
            <Store size={48} />
            <h3>{searchQuery || selectedCategory ? 'No APIs found' : 'No APIs available'}</h3>
            <p>{searchQuery || selectedCategory ? 'Try different search terms or filters' : 'Check back later for available APIs'}</p>
          </div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1rem' }}>
          {filteredProviders.map((provider) => {
            ...
            const avgRating = providerListings.length > 0 
              ? providerListings.reduce((sum, l) => sum + l.rating, 0) / providerListings.length 
              : 0;
            const totalRequests = providerListings.reduce((sum, l) => sum + parseInt(l.totalRequests), 0);
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1rem' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{ fontWeight: 600, fontSize: '1.1rem' }}>{provider.name}</span>
                      {provider.configured && (
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem', flexWrap: 'wrap' }}>
                      {provider.categories.slice(0, 2).map(cat => (
                        <span key={cat} className="badge badge-neutral" style={{ fontSize: '0.7rem' }}>{cat}</span>
                      ))}
                    </div>
                  </div>
                </div>

...
import { Gauge, Play, RefreshCw, Server, Users, Activity } from 'lucide-react';
...
export default function TrainingPage() {
  ...
  const gpuNodes = nodesData?.nodes?.filter(n => n.resources.totalCpu > 4).length ?? 0;
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p className="page-subtitle">Train machine learning models across the decentralized compute network</p>
        </div>
      ...
      <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="stat-card">
          <div className="stat-icon storage"><Gauge size={24} /></div>
          <div className="stat-content">
        ...
        <div className="stat-card">
          <div className="stat-icon network"><Users size={24} /></div>
          <div className="stat-content">
            <div className="stat-label">Total Clients</div>
            <div className="stat-value">{runs.reduce((sum, r) => sum + r.clients, 0)}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedRun ? '1fr 1fr' : '1fr', gap: '1.5rem' }}>
        <div className="card">
          <div className="card-header">
            <h3 className="card-title"><Gauge size={18} /> Training Runs</h3>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="empty-state">
              ...
              <p>Start a distributed training job to train models across the network</p>
            </div>
          ) : (
                  ...
                  {runs.map((run) => {
                    ...
                    return (
                      <tr 
                        ...
                        style={{ cursor: 'pointer', background: selectedRun === run.runId ? 'var(--accent-soft)' : undefined }}
                      >
        ...
        {selectedRun && (
          <div className="card">
            <div className="card-header">
              <h3 className="card-title"><Activity size={18} /> Run Details</h3>
            ...
            {(() => {
              ...
              return (
                <div style={{ display: 'grid', gap: '1rem' }}>
                  <div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Run ID</div>
                    <code style={{ fontSize: '0.85rem' }}>{run.runId}</code>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Model</div>
                    <span>{run.model}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)' }}>{run.step} / {run.totalSteps} steps</span>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Connected Clients</div>
                    <span style={{ fontFamily: 'var(--font-mono)' }}>{run.clients}</span>
                  </div>
                  <div>

...
import { Box, Play, Square, RefreshCw, Plus, HardDrive, AlertCircle } from 'lucide-react';
import { useContainers, useRunContainer, useHealth } from '../../hooks';
...
export default function ContainersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowModal(false);
    setFormData({ image: '', command: '', mode: 'serverless', cpuCores: '1', memoryMb: '512' });
  };
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p className="page-subtitle">Run containerized workloads on the decentralized compute network</p>
        </div>
      ...
      <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="stat-card">
          <div className="stat-icon storage"><Box size={24} /></div>
          <div className="stat-content">
        ...
        <div className="stat-card">
          <div className="stat-icon" style={{ background: 'var(--error-soft)', color: 'var(--error)' }}><AlertCircle size={24} /></div>
          <div className="stat-content">
        ...
        <div className="stat-card">
          <div className="stat-icon network"><HardDrive size={24} /></div>
          <div className="stat-content">
            <div className="stat-label">Available Nodes</div>
            <div className="stat-value">{health?.decentralized.registeredNodes ?? 0}</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title"><Box size={18} /> Executions</h3>
        </div>
                  ...
                  <tr key={container.executionId}>
                    ...
                    <td>
                      <span className={`badge ${
                        container.status === 'running' ? 'badge-info' :
                        container.status === 'completed' ? 'badge-success' :
                        container.status === 'failed' ? 'badge-error' :
                        container.status === 'pending' ? 'badge-warning' :
                        'badge-neutral'
                      }`}>
                        {container.status}
                      </span>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={runContainer.isPending}>
                  {runContainer.isPending ? <><div className="spinner" style={{ width: 16, height: 16 }} /> Running...</> : <><Play size={16} /> Run</>}
                </button>
              </div>
            </form>
          </div>

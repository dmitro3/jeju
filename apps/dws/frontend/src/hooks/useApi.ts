...
import type {
  DWSHealth, Container, WorkerFunction, ComputeJob,
  Repository, Package, CIPipeline, KMSKey, Secret, VPNSession,
  RPCChain, APIListing, APIProvider, TrainingRun, ComputeNode,
  UserAccount
} from '../types';

async function fetchApi<T>(endpoint: string, options?: RequestInit & { address?: string }): Promise<T> {
...
export function useContainers() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi<{ executions: Container[] }>('/containers/executions', { address }),
    enabled: !!address,
...
export function useRunContainer() {
  ...
  return useMutation({
    mutationFn: (params: { image: string; command?: string[]; env?: Record<string, string>; mode?: string }) =>
...
export function useWorkers() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi<{ functions: WorkerFunction[] }>('/workers', { address }),
    enabled: !!address,
  });
}
...
export function useDeployWorker() {
  ...
  return useMutation({
    mutationFn: (params: { name: string; code: string; runtime?: string; handler?: string; memory?: number; timeout?: number }) =>
...
export function useJobs() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi<{ jobs: ComputeJob[] }>('/compute/jobs', { address }),
    enabled: !!address,
    refetchInterval: 5000,
  });
}
...
export function useSubmitJob() {
  ...
  return useMutation({
    mutationFn: (params: { command: string; shell?: string; timeout?: number }) =>
...
export function useStorageHealth() {
  return useQuery({
    ...
    queryFn: () => fetchApi<{ service: string; status: string; backends: string[] }>('/storage/health'),
  });
}
...
export function useRepositories(limit = 20) {
  return useQuery({
    ...
    queryFn: () => fetchApi<{ repositories: Repository[]; total: number }>(`/git/repos?limit=${limit}`),
  });
}
...
export function useCreateRepository() {
  ...
  return useMutation({
    mutationFn: (params: { name: string; description?: string; visibility?: string }) =>
...
export function usePackages(limit = 20) {
  return useQuery({
    ...
    queryFn: () => fetchApi<{ packages: Package[]; total: number }>(`/pkg/packages?limit=${limit}`),
  });
}
...
export function usePipelines() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi<{ pipelines: CIPipeline[] }>('/ci/pipelines', { address }),
    enabled: !!address,
...
export function useInference() {
  return useMutation({
    mutationFn: (params: { model?: string; messages: Array<{ role: string; content: string }> }) =>
      fetchApi<{
        id: string;
        model: string;
...
export function useSecrets() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi<{ secrets: Secret[] }>('/kms/vault/secrets', { address }),
    enabled: !!address,
  });
}
...
export function useCreateRPCKey() {
  ...
  return useMutation({
    mutationFn: (params: { tier?: string }) =>
      fetchApi<{ apiKey: string; tier: string; limits: { rps: number; daily: number } }>('/rpc/keys', {
        method: 'POST',
        body: JSON.stringify(params),
        address,
      }),
...
export function useVPNRegions() {
  return useQuery({
    ...
    queryFn: () => fetchApi<{ regions: Array<{ code: string; name: string; country: string; nodeCount: number }> }>('/vpn/regions'),
  });
}
...
export function useCreateVPNSession() {
  ...
  return useMutation({
    mutationFn: (params: { region?: string; country?: string; type?: string; duration?: number }) =>
...
export function useAPIListings(providerId?: string) {
  return useQuery({
    ...
    queryFn: () => fetchApi<{ listings: APIListing[] }>(`/api/listings${providerId ? `?provider=${providerId}` : ''}`),
  });
}
...
export function useCreateListing() {
  ...
  return useMutation({
    mutationFn: (params: { providerId: string; apiKey: string; pricePerRequest?: string }) =>
...
export function useDeposit() {
  ...
  return useMutation({
    mutationFn: (amount: string) =>
      fetchApi<{ success: boolean; newBalance: string }>('/api/account/deposit', {
        method: 'POST',

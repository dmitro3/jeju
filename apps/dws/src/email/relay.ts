import { createPublicClient, http, type Address, type Hex, keccak256, toBytes, parseAbiItem } from 'viem';
import type {
  EmailEnvelope,
  EmailContent,
  JejuEmailAddress,
...
import { rateLimitHitsTotal, deliveryQueueLength, deliveryDuration, emailsReceivedTotal } from './metrics';
...
export class EmailRelayService {
  ...
  private async deliverEmail(envelope: EmailEnvelope): Promise<void> {
    ...
    for (const recipient of envelope.to) {
      try {
        ...
        emailsReceivedTotal.inc({ source: this.isJejuEmail(recipient) ? 'internal' : 'external', status: 'failed' });
      }
    }
  ...
  private async resolveEmailIdentity(
    ...
    const owner = await this.publicClient.readContract({
      ...
      console.warn(`[EmailRelay] JNS owner lookup failed for ${address.full}: ${e.message}`);
      return null;
    });
    ...
    const getAccountAbi = parseAbiItem('function getAccount(address owner) view returns (address owner_, bytes32 publicKeyHash, bytes32 jnsNode, uint8 status, uint8 tier, uint256 stakedAmount, uint256 quotaUsedBytes, uint256 quotaLimitBytes, uint256 emailsSentToday, uint256 lastResetTimestamp, uint256 createdAt, uint256 lastActivityAt)');
    ...
    const accountResult = await this.publicClient.readContract({
      ...
      console.debug(`[EmailRelay] getAccount failed for ${owner}: ${e.message}`);
      return null;
    });
    ...
    if (accountResult) {
      ...
      if (status === 2 || status === 3) {
        console.warn(`[EmailRelay] Account ${owner} is suspended/banned (status: ${status})`);
        return null;
      }
    }

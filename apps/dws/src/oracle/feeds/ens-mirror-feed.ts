...
export class ENSMirrorFeed {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `[ENS Feed] Starting with ${this.config.watchedNames.length} names`,
    )
  ...
  async fetchAndSign(name: string): Promise<SignedENSReport | null> {
    ...
    console.log(
      `[ENS Feed] New state for ${name}: contenthash=${state.contenthash !== '0x'}`,
    )
  ...
  async fetchENSState(name: string): Promise<ENSState | null> {
    ...
    for (const key of textKeys) {
      const value = await this.ethClient
        .readContract({
          address: resolverAddr,
          abi: ENS_RESOLVER_ABI,
          functionName: 'text',
 ...
 */
export function createENSMirrorFeed(
  config?: Partial<ENSMirrorFeedConfig>,
): ENSMirrorFeed {
  const watchedNamesEnv = process.env.ENS_WATCH_NAMES
  const watchedNames = watchedNamesEnv
    ? watchedNamesEnv.split(',').map((s) => s.trim())
    : []

...
export function createDARouter(ctx: DARouterContext = {}): Hono {
  ...
  router.post('/blob', async (c) => {
    ...
    if (data.length > maxSize) {
      return c.json({ error: `Blob too large: ${data.length} bytes (max: ${maxSize})` }, 400);
    }
  ...
  router.post('/sample', async (c) => {
    ...
    const result = await disperser.getSampler().sample(
      body.blobId,
      metadata.commitment,
      body.requester
    );
  ...
  router.post('/chunk', async (c) => {
    ...
    if (!stored) {
      return c.json({ error: 'Failed to store chunk (proof verification failed)' }, 400);
    }
  ...
  router.get('/blobs', (c) => {
    ...
    let blobs = status 
      ? disperser.getBlobManager().listByStatus(status as 'pending' | 'dispersing' | 'available' | 'expired' | 'unavailable')
      : submitter

...
import {
  ...
  jobParamsSchema,
  nodeParamsSchema,
  trainingNodeRegistrationSchema,
  trainingRunParamsSchema,
  trainingRunSchema,
  trainingRunsQuerySchema,
  validateBody,
  validateHeaders,
  validateParams,
  validateQuery,
...
export function createComputeRouter(): Hono {
  ...
  app.post('/jobs', async (c) => {
    const { 'x-jeju-address': submitter } = validateHeaders(
      jejuAddressHeaderSchema,
      c,
    )
...
export function addTrainingRoutes(app: Hono): void {
  ...
  app.get('/training/runs', async (c) => {
    ...
    const runs = await trainingState.listRuns(
      status as 'active' | 'completed' | 'paused' | undefined,
    )

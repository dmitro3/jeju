...
async function fetchNpmPackage(packageName: string): Promise<PackageMetadata> {
  ...
  return {
    ...
    maintainers: Array.isArray(data.maintainers)
      ? (data.maintainers as Array<{ name?: string }>).map(m => m.name || 'unknown')
      : [],
...
async function fetchPyPIPackage(packageName: string): Promise<PackageMetadata> {
  ...
  const data = await response.json() as { info: Record<string, unknown>; releases: Record<string, unknown> };
  ...
  return {
    ...
    repository: info.project_urls && (info.project_urls as Record<string, string>).Source,
    maintainers,
...
export function createPkgRegistryProxyRouter(): Hono {
  ...
  router.post('/batch', async (c) => {
    const body = await c.req.json() as {
      packages: Array<{ name: string; registry: 'npm' | 'pypi' | 'cargo' | 'go' }>;
    };

...
export function createCIRouter(ctx: CIContext) {
  ...
  return new Elysia({ prefix: '/ci' })
    ...
    .get('/workflows/:repoId/:workflowId', async ({ params }) => {
      const { repoId, workflowId } = expectValid(workflowDetailParamsSchema, params)
    ...
    .post('/trigger/:repoId/:workflowId', async ({ params, body, request }) => {
      const { repoId, workflowId } = expectValid(workflowDetailParamsSchema, params)
    ...
    .get('/runs/:runId/logs/stream', async ({ params, request }) => {
      const { runId } = expectValid(z.object({ runId: z.string().min(1) }), params)
      const run = workflowEngine.getRun(runId)
      if (!run) throw new Error('Run not found')
    ...
    .get('/repos/:repoId/runs', async ({ params, query }) => {
      const { repoId } = expectValid(z.object({ repoId: strictHexSchema }), params)
      const {
        limit,
        status: statusFilter,
        branch,
    ...
    .get('/environments/:repoId/:name', async ({ params }) => {
      const { repoId, name } = expectValid(environmentNameParamsSchema, params)
    ...
    .put('/environments/:repoId/:name', async ({ params, body, request }) => {
      ...
      const env = await secretsStore.updateEnvironment(repoId, name, validBody)
      return {
        environmentId: env.environmentId,
        updatedAt: env.updatedAt,
      }
    ...
    .post('/environments/:repoId/:name/secrets', async ({ params, body, request }) => {
      ...
      switch (event) {
        ...
        case 'pull_request':
          ciEvent = {
            ...
            headSha: (validBody.pull_request as { head: { sha: string } }).head.sha,
            baseBranch: (validBody.pull_request as { base: { ref: string } }).base.ref,
            author: (validBody.pull_request as { user: { login: string } }).user
              .login as Address,
          }
          break
        case 'release':
    ...
    .put('/triggers/:id', async ({ params, body, request }) => {
      ...
      const validBody = expectValid(createTriggerRequestSchema.partial(), body)

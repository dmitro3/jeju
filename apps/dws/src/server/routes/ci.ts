...
export function createCIRouter(ctx: CIContext): Hono {
  ...
  router.post('/runs/:repoId/:workflowId', async (c) => {
    const { repoId, workflowId } = validateParams(workflowDetailParamsSchema, c)
    const { 'x-jeju-address': triggeredBy } = validateHeaders(
      jejuAddressHeaderSchema,
      c,
    )
  ...
  router.put('/environments/:repoId/:name', async (c) => {
    const { repoId, name } = validateParams(
      z.object({
  ...
  router.post('/environments/:repoId/:name/secrets', async (c) => {
    const { repoId, name: envName } = validateParams(
      environmentNameParamsSchema,
      c,
    )
    const { 'x-jeju-address': triggeredBy } = validateHeaders(
  ...
  router.get('/runners', async (c) => {
    const { labels: labelsStr } = validateQuery(
    ...
    switch (event) {
      ...
      case 'pull_request':
        ciEvent = {
          ...
          author: (body.pull_request as { user: { login: string } }).user
            .login as Address,
        }
        break
      case 'release':

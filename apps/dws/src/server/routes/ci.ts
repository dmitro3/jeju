/**
 * CI/CD Routes - workflow management and execution
 */
...
export function createCIRouter(ctx: CIContext): Hono {
  ...
  router.get('/health', (c) => c.json({ service: 'dws-ci', status: 'healthy' }));
  ...
  router.get('/runs/:runId/logs', async (c) => {
    ...
    const logs = [
      ...
      '',
      ...run.jobs.flatMap((job) => [`--- Job: ${job.name} ---`, job.logs || '', '']),
    ];
  ...
  router.get('/badge/:repoId/:workflowId', async (c) => {
    ...
    if (latestRun) {
      switch (latestRun.conclusion) {
        case 'success': color = '#10b981'; status = 'passing'; break;
        case 'failure': color = '#ef4444'; status = 'failing'; break;
        case 'cancelled': color = '#f59e0b'; status = 'cancelled'; break;
        default:
          if (latestRun.status === 'in_progress') { color = '#3b82f6'; status = 'running'; }
          else if (latestRun.status === 'queued') { color = '#6366f1'; status = 'queued'; }
      }
    }

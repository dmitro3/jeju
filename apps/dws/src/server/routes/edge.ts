...
// ============================================================================
// Types for WebSocket Compatibility
// ============================================================================

/**
 * Minimal WebSocket interface for edge node communication.
 * Compatible with both Bun's ServerWebSocket and browser WebSocket.
 */
...
/** WebSocket.OPEN constant value */
const WS_OPEN = 1

/**
 * Callbacks returned by handleEdgeWebSocket for the caller to wire up
 * to their WebSocket implementation's event system.
 */
export interface EdgeWebSocketCallbacks {
...
const websockets = new Map<string, EdgeWebSocket>()
...
/**
 * Sets up edge WebSocket handling and returns callbacks for the caller to wire up.
 * This design works with both Bun's ServerWebSocket (callback-based) and browser WebSocket.
 */
export function handleEdgeWebSocket(
  ws: EdgeWebSocket,
  nodeId?: string,
): EdgeWebSocketCallbacks {
  ...
  return {
    onMessage(data: string) {
      ...
      switch (message.type) {
        ...
        case 'cache_complete': {
          ...
          console.log(
            `[EdgeCoordinator] Node ${currentNodeId} cached ${message.cid}`,
          )
          break
        }
      }
    ...
    onError() {
      console.error(`[EdgeCoordinator] WS error for ${currentNodeId}`)
    },
  }
}

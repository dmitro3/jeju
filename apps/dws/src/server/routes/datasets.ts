...
export function createDatasetsRouter(ctx: DatasetsContext) {
  ...
  return new Elysia({ prefix: '/datasets' })
    // Health check
    ...
    .get('/api/datasets/:org/:name', async ({ params }) => {
      ...
      return {
        ...
        cardData: {
          license: (Object.keys(DatasetLicense) as (keyof typeof DatasetLicense)[])
    ...
    .get('/api/datasets/:org/:name/resolve/:revision/*', async ({ params, request }) => {
      const { org, name } = params
    ...
    .post('/', async ({ body, request, set }) => {
      ...
      const format: DatasetFormat =
        typeof validBody.format === 'string'
          ? (DatasetFormat[validBody.format.toUpperCase() as keyof typeof DatasetFormat] ?? DatasetFormat.PARQUET)

...
export function createAPIMarketplaceRouter() {
  return new Elysia({ prefix: '/api-marketplace' })
    ...
    .get('/providers', ({ query }) => {
      const { category, configured } = expectValid(providerListQuerySchema, query)
    ...
    .post('/listings', async ({ body, request, set }) => {
      ...
      // Store key in vault
      const vaultKey = storeKey(validBody.providerId, userAddress, validBody.apiKey)
    ...
    .post('/proxy', async ({ body, request }) => {
      ...
      if (!listingId) {
        ...
        if (!listing) {
          throw new Error(`No active listings for provider: ${validBody.providerId}`)
        }
    ...
    .all('/proxy/:providerId/*', async ({ params, body, request }) => {
      ...
      const originDomain = request.headers.get('origin') || request.headers.get('referer')

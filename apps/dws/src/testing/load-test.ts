...
export async function runLoadTest(config: LoadTestConfig): Promise<LoadTestResult> {
  ...
  console.log(`[LoadTest] Duration: ${config.duration}s, RPS: ${config.rps}, Concurrency: ${config.concurrency}`);
  ...
  await runRequests(config, normalizedScenarios, config.duration * 1000, results);
...
async function executeRequest(
  ...
  try {
    ...
    const success = response.ok && (!scenario.validate || scenario.validate(response));
...
function calculateResults(results: RequestResult[], durationSeconds: number): LoadTestResult {
  ...
  for (const result of results) {
    if (!result.success) {
      errorsByCode[result.statusCode] = (errorsByCode[result.statusCode] ?? 0) + 1;
    }
  }
  ...
  for (const [name, group] of scenarioGroups) {
    ...
    scenarioResults[name] = {
      ...
      avgLatencyMs: group.reduce((sum, r) => sum + r.latencyMs, 0) / group.length,
...
export function printResults(result: LoadTestResult): void {
  ...
  if (Object.keys(result.scenarioResults).length > 1) {
    ...
    for (const [name, sr] of Object.entries(result.scenarioResults)) {
      ...
      console.log(`    Success Rate:     ${((1 - sr.errorRate) * 100).toFixed(2)}%`);

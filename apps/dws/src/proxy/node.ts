...
async function initializeWallet(): Promise<void> {
  ...
  if (!privateKey) {
    console.log('[DWS Proxy Node] No NODE_PRIVATE_KEY set, running without wallet');
    return;
  }
...
async function registerWithCoordinator(): Promise<void> {
  const coordinatorUrl = process.env.PROXY_COORDINATOR_URL?.replace('ws://', 'http://').replace(':4021', ':4020');
  if (!coordinatorUrl) {
    console.log('[DWS Proxy Node] No PROXY_COORDINATOR_URL set, running standalone');
    return;
  }
...
app.post('/proxy', async (c) => {
  ...
  const body = await c.req.json<{ url: string; method?: string; headers?: Record<string, string> }>();
  currentConnections++;

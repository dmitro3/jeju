...
class GRPOTrainer:
    ...
    def setup(self, reference_model_cid: Optional[str] = None):
    ...
    def _trajectory_to_messages(self, trajectory: dict) -> List[dict]:
        ...
        for step in trajectory.get("steps", []):
            ...
            messages.append({
                "role": "user",
                "content": f"Observation: {json.dumps(observation)[:500]}"
            })
    ...
    def train_step(self, batch_data: List[dict]) -> dict:
        ...
        grpo_loss = (
            ((-grpo_term * mask).sum(-1) / mask_sum) * advantages.squeeze(-1)
        ).mean()
    ...
    def train(
        ...
        final_metrics = {
            ...
            "average_grad_norm": np.mean([m.get("grad_norm", 0) for m in all_metrics]) if all_metrics else 0,
        }

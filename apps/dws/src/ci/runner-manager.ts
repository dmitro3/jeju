...
export class RunnerManager {
  ...
  private selectCheapestNode(labels: string[], container?: { image: string }): ComputeNode | null {
  ...
  private nodeMatchesLabels(node: ComputeNode, labels: string[]): boolean {
    for (const label of labels) {
      ...
      if (label === 'windows' && node.capabilities.os !== 'windows') return false;
      if (label === 'x64' && node.capabilities.architecture !== 'amd64') return false;
      if (label === 'arm64' && node.capabilities.architecture !== 'arm64') return false;
  ...
  private async executeJob(
    ...
    dispatch.logs.push(`[${new Date().toISOString()}] Pulling runner image: ${containerImage}`);
    ...
    dispatch.logs.push(`[${new Date().toISOString()}] Job completed with exit code: ${exitCode}`);
  ...
  private startHealthCheck(): void {
    setInterval(() => {
      ...
      for (const runner of this.runners.values()) {
        if (now - runner.lastHeartbeat > staleThreshold && runner.status !== 'offline') {
          runner.status = 'offline';
        }
      }

      for (const node of this.computeNodes.values()) {
        if (now - node.lastHeartbeat > staleThreshold && node.status !== 'offline') {
          node.status = 'offline';
        }
      }
    }, 30000);

...
async function detectProvider(): Promise<{ provider: ClusterProvider; binary: string } | null> {
...
export async function createCluster(config: K3sClusterConfig): Promise<K3sCluster> {
...
async function createK3dCluster(
  ...
  const args = [
    ...
    '--agents', String(Math.max(0, config.nodes - 1)),
...
async function getK3dNodes(binary: string, clusterName: string): Promise<K3sNode[]> {
  ...
  return allNodes
    ...
    .map(n => ({
      name: n.name,
      role: n.role.includes('server') ? 'server' as const : 'agent' as const,
      ip: n.IP?.IP || 'unknown',
...
async function createMinikubeCluster(
  ...
  // Get kubeconfig
  const kubeconfigProc = Bun.spawn([binary, 'kubectl', '-p', config.name, '--', 'config', 'view', '--raw'], {
    stdout: 'pipe',
  });
  await kubeconfigProc.exited;
  const kubeconfig = await new Response(kubeconfigProc.stdout).text();
...
export async function installHelmChart(
  ...
  const args = [
    'install', params.release, params.chart,
...
export async function applyManifest(
  ...
  const proc = Bun.spawn([kubectl, 'apply', '-f', '-', '--kubeconfig', cluster.kubeconfig], {
        ...
        spec: {
          containers: [{
            ...
            env: [
              ...
              { name: 'DWS_CAPABILITIES', value: (params.capabilities || ['compute']).join(',') },
              ...(params.privateKey ? [{ name: 'DWS_PRIVATE_KEY', value: params.privateKey }] : []),
              ...(params.pricing ? [
                { name: 'DWS_PRICE_PER_HOUR', value: params.pricing.pricePerHour },
                { name: 'DWS_PRICE_PER_GB', value: params.pricing.pricePerGb },
                { name: 'DWS_PRICE_PER_REQUEST', value: params.pricing.pricePerRequest },
              ] : []),
            ],
...
async function waitForKubeApi(kubeconfigPath: string, timeoutMs = 60000): Promise<void> {
  const kubectl = await findBinary('kubectl');
  if (!kubectl) {
  ...
  while (Date.now() < deadline) {
    const proc = Bun.spawn([kubectl, 'get', 'nodes', '--kubeconfig', kubeconfigPath], {
      stdout: 'pipe',
      stderr: 'pipe',
    });
...
export function createK3sRouter(): Hono {
  ...
  router.get('/providers', async (c) => {
    const providers: Array<{ name: ClusterProvider; available: boolean; path?: string }> = [];

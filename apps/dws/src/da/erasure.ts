...
export class ReedSolomonCodec {
  ...
  constructor(config: ReedSolomonCodecConfig) {
    ...
    if (this.totalShards > GF_SIZE - 1) {
      throw new Error(`Total shards (${this.totalShards}) exceeds maximum (${GF_SIZE - 1})`);
    }
    
    // Create Vandermonde parity matrix
    this.parityMatrix = createVandermondeMatrix(this.parityShards, this.dataShards);
  }
  ...
  encode(data: Uint8Array, chunkSize?: number): Uint8Array[] {
    ...
    const parityShards = matrixMul(this.parityMatrix, dataShards, this.dataShards);
  ...
  verify(shards: Uint8Array[]): boolean {
    ...
    const expectedParity = matrixMul(this.parityMatrix, dataShards, this.dataShards);
   ...
   * Verify Merkle proof
   */
  verifyMerkleProof(leaf: Hex, proof: Hex[], root: Hex, index: number): boolean {
    let hash = leaf;
    let idx = index;
...
export function createReedSolomonCodec(config?: Partial<ErasureConfig>): ReedSolomonCodec {
...
export const defaultCodec = createReedSolomonCodec({ dataShards: 16, parityShards: 16 });

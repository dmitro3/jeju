...
import { createDAClient, type DAClient } from '../client'
import type { AvailabilityAttestation, BlobCommitment } from '../types'
...
export class RollupDAAdapter {
  ...
  constructor(config: RollupConfig) {
    this.config = {
      ...
      contracts: {
        operatorRegistry:
          config.contracts?.operatorRegistry ?? ('0x' as Address),
        blobRegistry: config.contracts?.blobRegistry ?? ('0x' as Address),
        attestationManager:
          config.contracts?.attestationManager ?? ('0x' as Address),
        daCommitmentVerifier:
          config.contracts?.daCommitmentVerifier ?? ('0x' as Address),
        calldataFallback:
          config.contracts?.calldataFallback ?? ('0x' as Address),
        l2OutputOracleAdapter:
          config.contracts?.l2OutputOracleAdapter ?? ('0x' as Address),
      },
      ...
      namespace:
        config.namespace ?? (keccak256(toBytes('default-rollup')) as Hex),
  ...
  async getDAStatus(): Promise<{
    ...
    const health = await this.daClient
      .healthCheck()
      .catch(() => ({ status: 'error', operators: 0 }))
  ...
  generateDAProof(
    ...
    const proofParts: Hex[] = [stateRoot, messagePasserRoot, blockHash]
  ...
  async submitToL1WithDA(
    ...
    while (retryCount < this.config.maxDARetries) {
      const result = await this.submitBatch(batch).catch((error: Error) => {
        console.error(
          `DA submission attempt ${retryCount + 1} failed:`,
          error.message,
        )
        this.daFailureCount++
    ...
    const proofBytes = this.generateDAProof(
      {
        blobId: daCommitment,
        commitment: {} as BlobCommitment,
        attestation: {} as AvailabilityAttestation,
        submittedAt: Date.now(),
      },
      daProof.stateRoot,
  ...
  private async postToCalldataFallback(data: Uint8Array): Promise<Hex> {
    ...
    const receipt = await this.l1Client.waitForTransactionReceipt({
      hash: txHash,
    })

    // Extract blobId from CalldataPosted event
    const calldataPostedTopic = keccak256(
      toBytes('CalldataPosted(bytes32,address,uint256,bytes32)'),
    )
  ...
  private encodeBatch(batch: BatchData): Uint8Array {
    ...
    view.setBigUint64(offset, batch.batchNumber, false)
    offset += 8
    view.setBigUint64(offset, batch.l2BlockRange.start, false)
    offset += 8
    view.setBigUint64(offset, batch.l2BlockRange.end, false)
    offset += 8

    // State root
    const stateRootBytes = toBytes(batch.stateRoot)
    header.set(stateRootBytes.slice(0, 32), offset)
    offset += 32

    // Timestamp
    view.setBigUint64(offset, BigInt(batch.timestamp), false)
    offset += 8
  ...
  private decodeBatch(data: Uint8Array): BatchData {
    ...
    const batchNumber = view.getBigUint64(offset, false)
    offset += 8
    const blockStart = view.getBigUint64(offset, false)
    offset += 8
    const blockEnd = view.getBigUint64(offset, false)
    offset += 8

    const stateRootBytes = data.slice(offset, offset + 32)
    offset += 32
    const stateRoot = toHex(stateRootBytes)

    const timestamp = Number(view.getBigUint64(offset, false))
    offset += 8
    const txLength = view.getUint32(offset, false)
    offset += 4
  ...
  private aggregateTransactions(batches: BatchData[]): Uint8Array {
    const totalSize = batches.reduce(
      (sum, b) => sum + b.transactions.length + 4,
      0,
    )
...
export function createOPStackDAAdapter(
...
export class ArbitrumOrbitDAAdapter extends RollupDAAdapter {
  ...
  createBatchDataHash(daRef: DAReference): Hex {
    return keccak256(
      toBytes(
        `${daRef.blobId}${daRef.commitment.commitment}${daRef.commitment.merkleRoot}`,
      ),
    )
  }
}

export function createArbitrumOrbitDAAdapter(
  config: ArbitrumOrbitConfig,

 ...
 */
export function hashToG1(message: Uint8Array, dst: DST = DST_DA_ATTEST): G1Point {
 ...
 */
export function encodeToG1(message: Uint8Array, dst: DST = DST_DA_ATTEST): G1Point {
  if (dst.length > MAX_DST_LEN) {
    throw new Error(`DST too long: ${dst.length} > ${MAX_DST_LEN}`);
  }
 ...
 * Uses SSWU method per RFC 9380
 */
export function hashToG2(message: Uint8Array, dst: DST = DST_DA_ATTEST): G2Point {
  if (dst.length > MAX_DST_LEN) {
    throw new Error(`DST too long: ${dst.length} > ${MAX_DST_LEN}`);
  }
 ...
 */
export function encodeToG2(message: Uint8Array, dst: DST = DST_DA_ATTEST): G2Point {
  if (dst.length > MAX_DST_LEN) {
    throw new Error(`DST too long: ${dst.length} > ${MAX_DST_LEN}`);
  }
...
export function expandMessageXMD(
  ...
  const b0Input = concatBytes(zPad, message, libStr, new Uint8Array([0]), dstPrime);

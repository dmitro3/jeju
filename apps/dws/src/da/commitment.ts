...
function lagrangeInterpolate(
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      if (i !== j) {
        ...
        for (let k = 0; k < numerator.length; k++) {
          newNumerator[k] = (newNumerator[k] + numerator[k] * (-points[j].x)) % FIELD_PRIME;
          newNumerator[k + 1] = (newNumerator[k + 1] + numerator[k]) % FIELD_PRIME;
        }
    ...
    const scale = (points[i].y * modInverse(denominator, FIELD_PRIME)) % FIELD_PRIME;
...
export function createCommitment(
  ...
  const coeffHashInput = _coefficients.map(c => c.toString(16).padStart(64, '0')).join('');
...
export function createPolynomialCommitment(
  ...
  const commitment = createCommitment(chunks, chunkSize, dataChunkCount, parityChunkCount);

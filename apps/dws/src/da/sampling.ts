...
export function generateSampleIndices(
  ...
  const seedBytes = seed ? toBytes(seed) : crypto.getRandomValues(new Uint8Array(32));
...
export class DASampler {
  ...
  async sample(
    ...
    const responses = await this.fetchSamples(blobId, operatorIndices, requester);
  ...
  private verifySamples(
    ...
    const confidence = 1 - Math.pow(1 - availabilityThreshold, samplesVerified);
...
export function generatePeerDASSamples(
  ...
  for (let group = 0; group < custodyGroups; group++) {
    ...
    const indices = generateSampleIndices(groupChunks, samplesPerGroup, groupSeed);

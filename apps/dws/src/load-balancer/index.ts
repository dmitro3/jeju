...
import { AutoScaler, LocalInstanceFactory, type InstanceFactory } from './auto-scaler';
...
export class LoadBalancer {
  ...
  constructor(
    ...
    this.scaler = new AutoScaler(this.config, factory ?? new LocalInstanceFactory());
  ...
  private async evaluateAll(): Promise<void> {
    for (const [serviceId, queue] of this.requestQueues) {
      ...
      const activeConnections = instances.reduce((sum, i) => sum + i.currentConnections, 0);
  ...
  private selectInstance(instances: Instance[]): Instance | null {
    ...
    return available.sort((a, b) => a.currentConnections - b.currentConnections)[0];
  }

  private async processRequest(instance: Instance, queued: QueuedRequest): Promise<void> {
  ...
  async route(serviceId: string, request: Request): Promise<Response> {
    ...
    if (queue.length >= this.config.maxQueueSize) {
      return new Response(JSON.stringify({ error: 'Service overloaded' }), { status: 503 });
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      queue.push(queued);
      this.requestCounts.set(serviceId, (this.requestCounts.get(serviceId) ?? 0) + 1);
    });
  }
...
// Exports
export * from './types';
export { AutoScaler, LocalInstanceFactory, type InstanceFactory } from './auto-scaler';

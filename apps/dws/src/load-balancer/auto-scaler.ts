...
export class AutoScaler {
  ...
  private shouldScaleUp(
    ...
    if (currentCount > 0 && activeConnections >= totalCapacity * 0.8)
      return true

    // Scale up from zero if there's any demand
    if (currentCount === 0 && (queueDepth > 0 || activeConnections > 0))
      return true
  ...
  async scaleUp(serviceId: string): Promise<Instance | null> {
    ...
    console.log(
      `[AutoScaler] Scaling up ${serviceId}: ${instances.length} -> ${instances.length + 1}`,
    )
  ...
  async scaleDown(serviceId: string): Promise<boolean> {
    ...
    console.log(
      `[AutoScaler] Scaling down ${serviceId}: ${instances.length} -> ${instances.length - 1}`,
    )

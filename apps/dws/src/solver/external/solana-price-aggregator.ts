...
const KNOWN_TOKENS: Record<string, { symbol: string; decimals: number; isStablecoin: boolean }> = {
  // SOL
  'So11111111111111111111111111111111111111112': { symbol: 'SOL', decimals: 9, isStablecoin: false },
  // USDC
  'EPjFWdd5AufqSSqeM2qN1xzybapT8G4wEGGkZwyTDt1v': { symbol: 'USDC', decimals: 6, isStablecoin: true },
  // USDT
  'Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB': { symbol: 'USDT', decimals: 6, isStablecoin: true },
  // BONK
  'DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263': { symbol: 'BONK', decimals: 5, isStablecoin: false },
  // JUP
  'JUPyiwrYJFskUPiHa7hkeepFNjGXvMPGM2TQ5sUtjHA': { symbol: 'JUP', decimals: 6, isStablecoin: false },
  // WIF
  'EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm': { symbol: 'WIF', decimals: 6, isStablecoin: false },
  // RAY
  '4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R': { symbol: 'RAY', decimals: 6, isStablecoin: false },
  // PYTH
  'HZ1JovNiVvGrGNiiYvEozEVgZ58xaU3RKwX8eACQBCt3': { symbol: 'PYTH', decimals: 6, isStablecoin: false },
  // ORCA
  'orcaEKTdK7LKz57vaAYr9QeNsVEPfiu6QeMU1kektZE': { symbol: 'ORCA', decimals: 6, isStablecoin: false },
};

// Major pools for price discovery
const MAJOR_POOLS = {
  raydium: [
    { pool: '58oQChx4yWmvKdwLLZzBi4ChoCc2fqCUWBkwMihLYQo2', base: 'SOL', quote: 'USDC' },
    { pool: '2wT8Yq49kHgDzXuPxZSaeLiH3YzdVjyKBY12mMB4uvZk', base: 'SOL', quote: 'USDT' },
  ],
  orca: [
    { pool: 'HJPjoWUrhoZzkNfRpHuieeFk9WcZWjwy6PBjZ81ngndJ', base: 'SOL', quote: 'USDC' },
  ],
};
...
// ============ Solana Price Aggregator Class ============

export class SolanaPriceAggregator {
  private connection: Connection;
  private priceCache: Map<string, { price: SolanaTokenPrice; expires: number }> = new Map();
  ...
  async getPrice(mint: string): Promise<SolanaTokenPrice | null> {
    ...
    const tokenInfo = KNOWN_TOKENS[mint] ?? { symbol: 'UNKNOWN', decimals: 9, isStablecoin: false };
    ...
    if (tokenInfo.isStablecoin) {
      const price: SolanaTokenPrice = {
        ...
        confidence: 100,
        sources: [{ dex: 'raydium', pool: mint, price: 1.0, liquidity: 0, lastUpdate: Date.now() }],
        timestamp: Date.now(),
        liquidityUSD: 0,
      };
      this.priceCache.set(cacheKey, { price, expires: Date.now() + this.CACHE_TTL });
      return price;
    }
    ...
    if (mint === 'So11111111111111111111111111111111111111112') {
      ...
      const price: SolanaTokenPrice = {
        ...
        priceSOL: 1.0,
        confidence: 100,
        sources: [{ dex: 'raydium', pool: MAJOR_POOLS.raydium[0].pool, price: solPrice, liquidity: 0, lastUpdate: Date.now() }],
        timestamp: Date.now(),
        liquidityUSD: 0,
      };
      this.priceCache.set(cacheKey, { price, expires: Date.now() + this.CACHE_TTL });
    ...
    const weightedPrice = totalLiquidity > 0 
      ? sources.reduce((sum, s) => sum + (s.price * s.liquidity), 0) / totalLiquidity
      : sources[0].price;
  ...
  private async findRaydiumPools(mint: string): Promise<SolanaPriceSource[]> {
    ...
    const accounts = await this.connection.getProgramAccounts(RAYDIUM_AMM_PROGRAM, { filters }).catch(() => []);
   ...
   */
  private async getRaydiumPoolState(poolAddress: string): Promise<RaydiumPoolState | null> {
   ...
   */
  private decodeRaydiumPool(account: AccountInfo<Buffer>): RaydiumPoolState | null {
   ...
   */
  private calculateConfidence(sources: SolanaPriceSource[], totalLiquidity: number): number {
...
export function getSolanaPriceAggregator(rpcUrl?: string): SolanaPriceAggregator {

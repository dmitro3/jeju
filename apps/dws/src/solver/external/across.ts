...
export class AcrossAdapter extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    for (const chainId of this.supportedChains) {
      ...
      const unwatch = client.public.watchContractEvent({
        ...
        onError: (err) => console.error(`Across error on ${chainId}:`, err.message),
      });
  ...
  private handleDeposit(
    ...
    const deposit: AcrossDeposit = {
      ...
      relayerFeePct: ((args.inputAmount - args.outputAmount) * 10000n) / args.inputAmount,
    ...
    console.log(`🌉 Across deposit: ${deposit.depositId} | ${originChainId} → ${deposit.destinationChainId}`);
   ...
   */
  async fill(deposit: AcrossDeposit): Promise<{ success: boolean; txHash?: string; error?: string }> {
    ...
    if (
      deposit.exclusiveRelayer !== '0x0000000000000000000000000000000000000000' &&
    ...
    const value = deposit.outputToken === '0x0000000000000000000000000000000000000000' 

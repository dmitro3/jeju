...
const MAJOR_POOLS: Record<number, Array<{ pool: Address; dex: string; version: 'v2' | 'v3' }>> = {
  1: [
    { pool: '0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640', dex: 'Uniswap V3', version: 'v3' }, // USDC/ETH 0.05%
    { pool: '0x4e68Ccd3E89f51C3074ca5072bbAC773960dFa36', dex: 'Uniswap V3', version: 'v3' }, // ETH/USDT 0.3%
    { pool: '0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc', dex: 'Uniswap V2', version: 'v2' }, // USDC/ETH V2
  ],
  42161: [
    { pool: '0xC31E54c7a869B9FcBEcc14363CF510d1c41fa443', dex: 'Uniswap V3', version: 'v3' }, // ETH/USDC
    { pool: '0x80A9ae39310abf666A87C743d6ebBD0E8C42158E', dex: 'Camelot', version: 'v2' }, // ETH/USDC Camelot
  ],
  8453: [
    { pool: '0xd0b53D9277642d899DF5C87A3966A349A798F224', dex: 'Uniswap V3', version: 'v3' }, // ETH/USDC
  ],
  10: [
    { pool: '0x85149247691df622eaF1a8Bd0CaFd40BC45154a9', dex: 'Uniswap V3', version: 'v3' }, // ETH/USDC
  ],
};
...
export class MultiChainPriceAggregator {
  ...
  async getPrice(
    ...
    if (this.isStablecoin(chainId, tokenAddress)) {
      const price: TokenPrice = {
        ...
        confidence: 100,
        sources: [{ dex: 'stablecoin', pool: tokenAddress, price: 1.0, liquidity: 0, lastUpdate: Date.now() }],
        timestamp: Date.now(),
        liquidityUSD: 0,
      };
    ...
    if (this.isWETH(chainId, tokenAddress)) {
      ...
      const price: TokenPrice = {
        ...
        priceETH: 1.0,
        confidence: 100,
        sources: [{ dex: 'chainlink', pool: '0x0' as Address, price: ethPrice, liquidity: 0, lastUpdate: Date.now() }],
        timestamp: Date.now(),
        liquidityUSD: 0,
      };
      this.priceCache.set(cacheKey, { price, expires: Date.now() + this.CACHE_TTL });
    ...
    const weightedPrice = sources.reduce((sum, s) => sum + (s.price * s.liquidity), 0) / totalLiquidity;
  ...
  async getETHPrice(chainId: number): Promise<number> {
    ...
    if (!feedAddress) {
      ...
      return this.fetchChainlinkPrice(mainnetClient, CHAINLINK_FEEDS[1]['ETH/USD']);
    }
   ...
   */
  private async fetchChainlinkPrice(client: PublicClient, feedAddress: Address): Promise<number> {
  ...
  private async aggregateFromPools(
    ...
    for (const { pool, dex, version } of pools) {
      ...
      const isToken0 = poolState.token0.toLowerCase() === tokenAddress.toLowerCase();
      const isToken1 = poolState.token1.toLowerCase() === tokenAddress.toLowerCase();
  ...
  private async getTokenInfo(
    ...
    const [symbol, decimals, name] = await Promise.all([
      client.readContract({ address: tokenAddress, abi: ERC20_ABI, functionName: 'symbol' }).catch(() => 'UNKNOWN'),
      client.readContract({ address: tokenAddress, abi: ERC20_ABI, functionName: 'decimals' }).catch(() => 18),
      client.readContract({ address: tokenAddress, abi: ERC20_ABI, functionName: 'name' }).catch(() => 'Unknown Token'),
    ]);
  ...
  private isStablecoin(chainId: number, address: Address): boolean {
    ...
    return Object.values(stables).some(s => s.toLowerCase() === address.toLowerCase());
  }
   ...
   */
  private calculateConfidence(sources: PriceSource[], totalLiquidity: number): number {
    ...
    if (sources.length > 1) {
      ...
      if (maxDeviation < 0.01) confidence += 10; // <1% deviation

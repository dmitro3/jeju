...
export function createAgentRouter() {
  return new Elysia({ name: 'agents', prefix: '/agents' })
    // ============================================================================
    // Health & Info
    // ============================================================================
    .get('/health', () => {
    ...
    // List agents
    .get('/', ({ request, query }) => {
      const owner = request.headers.get('x-jeju-address') as Address | undefined
    ...
    .put(
      ...
      async ({ params, body, request, set }) => {
        ...
        try {
          const agent = await registry.updateAgent(params.id, owner, typedBody)
          if (!agent) {
            set.status = 404
            return { error: 'Agent not found' }
          }

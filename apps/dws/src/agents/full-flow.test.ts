...
class MockWorkerdWithInference {
  ...
  async deployWorker(worker: { id: string; bindings?: Array<{ name: string; value: string }> }) {
    ...
    const characterBinding = worker.bindings?.find(b => b.name === 'AGENT_CHARACTER');
    const character = characterBinding 
      ? JSON.parse(characterBinding.value) as AgentCharacter
  ...
  async invoke(workerId: string, request: { body: string }) {
    ...
    if (!worker) {
      return { status: 404, body: JSON.stringify({ error: 'Worker not found' }) };
    }
  ...
  private generateResponse(character: AgentCharacter, userMessage: string): string {
...
describe('Full Agent Flow', () => {
  ...
  test('7. Verify invocation stats increased', async () => {
    ...
    const data = await res.json() as { totalInvocations: number; avgLatencyMs: number };
    expect(data.totalInvocations).toBe(3); // We sent 3 messages
    
    console.log(`[Test] Stats: ${data.totalInvocations} invocations, ${data.avgLatencyMs}ms avg latency`);
  });
  ...
  test('8. Pause and resume agent', async () => {
    ...
    let statusData = await statusRes.json() as { status: string };
...
describe('Multiple Agents', () => {
  ...
  test('can register multiple agents with different characters', async () => {
    const characters = [
      { name: 'SecurityBot', system: 'You are a security expert.', bio: ['Security specialist'] },
      { name: 'TradeBot', system: 'You are a trading assistant.', bio: ['Trading expert'] },
      { name: 'SupportBot', system: 'You provide customer support.', bio: ['Support agent'] },
    ];
...
describe('Error Handling', () => {
  ...
  test('returns 403 for wrong owner update', async () => {
    ...
    const req = new Request(`http://localhost/agents/${id}`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',

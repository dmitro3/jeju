...
export class WorkerRuntime {
  ...
  async invokeHTTP(functionId: string, event: HTTPEvent): Promise<HTTPResponse> {
    ...
    return {
      ...
      body: response?.body ?? JSON.stringify(result.result),
    };
  }

  private async acquireInstance(fn: WorkerFunction): Promise<WorkerInstance | null> {
    const instances = this.instances.get(fn.id) ?? [];

    // Find a ready instance
    const ready = instances.find(i => 
      i.status === 'ready' && 
    ...
    const available = instances.find(i =>
      i.status === 'busy' &&
      i.activeInvocations < this.config.maxConcurrentInvocations
    );
  ...
  private async createInstance(fn: WorkerFunction): Promise<WorkerInstance | null> {
  ...
  private async waitForReady(instance: WorkerInstance, timeoutMs = 30000): Promise<boolean> {
  ...
  private async executeInInstance(
    ...
    try {
      ...
      if (result.memoryUsedMb) {
        ...
        instance.memoryUsedMb = Math.max(instance.memoryUsedMb, result.memoryUsedMb);
      }
  ...
  private buildResult(invocation: WorkerInvocation): InvokeResult {
    return {
      invocationId: invocation.id,
      status: invocation.status === 'success' ? 'success' : 
              invocation.status === 'timeout' ? 'timeout' : 'error',
      result: invocation.result,
      error: invocation.error,
      durationMs: invocation.durationMs ?? 0,
      billedDurationMs: invocation.billedDurationMs ?? Math.ceil((invocation.durationMs ?? 0) / 100) * 100,
    ...
    for (const [functionId, instances] of this.instances) {
      ...
      for (const instance of instances) {
        if (
          ...
          const warmCount = instances.filter(i => 
            i.status === 'ready' || i.status === 'busy'
          ).length;

          if (warmCount > 1 || now - (fn.lastInvokedAt ?? 0) > this.config.idleTimeout) {

...
export class S3Backend {
  ...
  async putObject(params: PutObjectParams): Promise<PutObjectResponse> {
    ...
    if (!bucketObjects) {
      throw new S3Error('NoSuchBucket', `Bucket ${params.bucket} does not exist`);
    }

    const content = typeof params.body === 'string' 
    ...
    const versionId = this.buckets.get(params.bucket)?.versioning === 'Enabled'
      ? crypto.randomUUID()
      : undefined;
  ...
  async deleteObjects(params: DeleteObjectsParams): Promise<DeleteObjectsResponse> {
    const bucketObjects = this.objects.get(params.bucket);
    if (!bucketObjects) {
      throw new S3Error('NoSuchBucket', `Bucket ${params.bucket} does not exist`);
    }
  ...
  async copyObject(params: CopyObjectParams): Promise<PutObjectResponse> {
    ...
    const metadata = params.metadataDirective === 'REPLACE'
      ? params.metadata ?? {}
      : sourceObj.metadata;
  ...
  async completeMultipartUpload(
    ...
    for (let i = 0; i < sortedParts.length; i++) {
      if (sortedParts[i].partNumber !== i + 1) {
        throw new S3Error('InvalidPartOrder', 'Parts are not in ascending order');
      }
    }
  ...
  generatePresignedUrl(params: PresignedUrlParams): PresignedUrlResponse {
    ...
    return {
      url,
      expiresAt,
      headers: params.operation === 'putObject' && params.contentType

...
export class WebTorrentBackend extends EventEmitter {
  ...
  constructor(config: Partial<WebTorrentConfig> = {}) {
    ...
    if (process.env.WEBTORRENT_MAX_CACHE_GB) {
      this.config.maxCacheSizeGB = parseInt(process.env.WEBTORRENT_MAX_CACHE_GB, 10);
    }
    if (process.env.WEBTORRENT_SYSTEM_BW_MBPS) {
      this.config.systemContentBandwidthMbps = parseInt(process.env.WEBTORRENT_SYSTEM_BW_MBPS, 10);
    }
  }
  ...
  async stopSeeding(infoHash: string): Promise<void> {
    ...
    if (torrent.tier === 'system' && this.config.autoSeedSystemContent) {
      console.warn(`[WebTorrent] Cannot stop seeding system content: ${torrent.name}`);
      return;
    }
  ...
  async replicatePopular(
    ...
    const eligible = content.filter(c => c.score >= this.config.minPopularityScore);
...
export function getWebTorrentBackend(config?: Partial<WebTorrentConfig>): WebTorrentBackend {

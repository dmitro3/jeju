...
export class SystemManifestBuilder {
  ...
  async build(): Promise<SystemContentManifest> {
    ...
    console.log(
      `[SystemManifest] Built manifest: ${manifest.totalItems} items, ${(manifest.totalSize / 1024 / 1024).toFixed(2)} MB`,
    )
    return manifest
  }
  ...
  private async collectApps(): Promise<SystemAppEntry[]> {
    ...
    for (const appDir of appDirs) {
      ...
      console.log(
        `[SystemManifest] Added app: ${manifest.name} (${(size / 1024).toFixed(1)} KB)`,
      )
    }
  ...
  private async collectJNSRecords(): Promise<SystemJNSEntry[]> {
    ...
    for (const appDir of appDirs) {
      ...
      const buildHash = buildDir
        ? await this.hashDirectory(buildDir)
        : 'pending'
      const contentCid = this.generateCid(
        manifest.name,
        manifest.version,
        buildHash,
      )
...
export class SystemContentSeeder {
  ...
  async packageSystemContent(
    ...
    for (const abi of manifest.abis) {
      const abiPath = join(
        this.rootDir,
        'packages/sdk/src/contracts/abis',
...
export async function buildSystemManifest(
  rootDir?: string,
): Promise<SystemContentManifest> {

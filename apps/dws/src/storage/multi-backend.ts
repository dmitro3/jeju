...
import { ArweaveBackend, getArweaveBackend } from './arweave-backend';
import { WebTorrentBackend, getWebTorrentBackend } from './webtorrent-backend';
...
export class MultiBackendManager {
  ...
  private initializeBackends(): void {
    ...
    if (ipfsApiUrl) {
      this.backends.set('ipfs', {
        ...
        async exists(cid: string): Promise<boolean> {
          const response = await fetch(`${ipfsGatewayUrl}/ipfs/${cid}`, { method: 'HEAD' });
          return response.ok;
        },
        async healthCheck(): Promise<boolean> {
          const response = await fetch(`${ipfsApiUrl}/api/v0/id`, { method: 'POST' }).catch(() => null);
    ...
    this.backends.set('arweave', {
      ...
      upload: async (content: Buffer, options?: { filename?: string }) => {
        const result = await this.arweaveBackend.upload(content, { filename: options?.filename });
    ...
    this.backends.set('webtorrent', {
      ...
      exists: (cid: string) => Promise.resolve(this.webtorrentBackend.hasTorrent(cid)),
  ...
  async upload(content: Buffer, options: UploadOptions = {}): Promise<UploadResult> {
    ...
    const replicationFactor = options.replicationFactor ?? this.config.replicationFactor;
    ...
    for (const backendType of backends) {
      ...
      const result = await backend.upload(uploadContent, { filename: options.filename }).catch((e: Error) => {
        console.warn(`[MultiBackend] Upload to ${backendType} failed: ${e.message}`);
        return null;
      });
    ...
    if ((tier === 'system' || tier === 'popular') && !addresses.magnetUri) {
      const torrent = await this.webtorrentBackend.createTorrent(uploadContent, {
  ...
  async download(cid: string, options: DownloadOptions = {}): Promise<DownloadResult> {
    ...
    const backends = options.preferredBackends ?? 
      (metadata ? Array.from(this.cidToBackends.get(cid) ?? []) : ['webtorrent', 'ipfs', 'local']);
    ...
    for (const backendType of backends) {
      ...
      const content = await backend.download(cid).catch((e: Error) => {
        console.debug(`[MultiBackend] Download from ${backendType} failed: ${e.message}`);
        return null;
      });
      ...
      if (content) {
        ...
        if (metadata?.encrypted && options.decryptionKeyId) {
          finalContent = await this.decryptContent(content, options.decryptionKeyId);
        }
  ...
  listByTier(tier: ContentTier): ContentMetadata[] {
    return Array.from(this.contentRegistry.values()).filter(m => m.tier === tier);
  }
  ...
  listByCategory(category: ContentCategory): ContentMetadata[] {
    return Array.from(this.contentRegistry.values()).filter(m => m.category === category);
  }
  ...
  private updatePopularityScore(cid: string): void {
    ...
    const seederCount = torrent ? this.webtorrentBackend.getTorrentStats(torrent.infoHash)?.seeds ?? 0 : 0;
  ...
  private async encryptContent(
    ...
    if (!this.kmsEndpoint) {
      ...
      const keyId = createHash('sha256').update(crypto.randomUUID()).digest('hex').slice(0, 32);
  ...
  private async decryptContent(content: Buffer, keyId: string): Promise<Buffer> {
    if (!this.kmsEndpoint) {
...
export function getMultiBackendManager(config?: Partial<MultiBackendConfig>): MultiBackendManager {

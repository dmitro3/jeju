...
export class GitObjectStore {
   ...
   */
  async storeObject(type: GitObjectType, content: Buffer): Promise<StoredGitObject> {
   ...
   * Store a commit
   */
  async storeCommit(commit: Omit<GitCommit, 'type' | 'oid'>): Promise<GitCommit> {
    const lines: string[] = [];
  ...
  parseCommit(content: Buffer): Omit<GitCommit, 'type' | 'oid'> {
    ...
    let author: GitCommitAuthor = { name: '', email: '', timestamp: 0, timezoneOffset: 0 };
    let committer: GitCommitAuthor = { name: '', email: '', timestamp: 0, timezoneOffset: 0 };
   ...
   * Walk commit history
   */
  async walkCommits(startOid: string, maxCount: number = 100): Promise<GitCommit[]> {
  ...
  private parseAuthor(line: string): GitCommitAuthor {
    ...
    if (!match) {
      return { name: 'Unknown', email: 'unknown@example.com', timestamp: 0, timezoneOffset: 0 };
    }

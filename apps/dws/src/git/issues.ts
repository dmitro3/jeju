...
export class IssuesManager {
  ...
  async getIssueIndex(repoId: Hex, metadataCid?: string): Promise<IssueIndex> {
    ...
    if (metadataCid) {
      ...
      if (result) {
        const metadata = JSON.parse(result.content.toString()) as { issueIndexCid?: string };
        if (metadata.issueIndexCid) {
          const indexResult = await this.backend.download(metadata.issueIndexCid).catch(() => null);
          if (indexResult) {
            const index = JSON.parse(indexResult.content.toString()) as IssueIndex;
  ...
  async listIssues(
    ...
    // Filter by author
    if (options.author) {
      issueRefs = issueRefs.filter(i => i.author.toLowerCase() === options.author?.toLowerCase());
    }
    ...
    for (const ref of paginatedRefs) {
      ...
      if (issue) {
        ...
        if (options.labels && options.labels.length > 0) {
          const hasAllLabels = options.labels.every(l => issue.labels.includes(l));
  ...
  ): Promise<{ issue: Issue; indexCid: string; contributionEvent: ContributionEvent }> {

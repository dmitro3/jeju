...
export class TrainingSDK {
  ...
  private validateConfig(config: CoordinatorConfig): void {
    ...
    if (config.witnessNodes <= 0)
      throw new Error('witnessNodes must be positive')
    if (config.witnessNodes > config.minClients) {
      throw new Error('witnessNodes cannot exceed minClients')
    }
    if (config.globalBatchSizeStart <= 0)
      throw new Error('globalBatchSizeStart must be positive')
    ...
    if (config.warmupTime <= BigInt(0))
      throw new Error('warmupTime must be positive')
    if (config.cooldownTime <= BigInt(0))
      throw new Error('cooldownTime must be positive')
    if (config.maxRoundTrainTime <= BigInt(0))
      throw new Error('maxRoundTrainTime must be positive')
    if (config.roundWitnessTime <= BigInt(0))
      throw new Error('roundWitnessTime must be positive')
    if (config.epochTime <= BigInt(0))
      throw new Error('epochTime must be positive')
  }
  ...
  private validateModel(model: ModelConfig): void {
    ...
    if (model.coldStartWarmupSteps < 0)
      throw new Error('coldStartWarmupSteps cannot be negative')
  }
  ...
  async createRun(options: CreateRunOptions): Promise<Hash> {
    ...
    if (
      ...
      options.rewardAmount > BigInt(0)
    ) {
      const account = this.getAccount()
      const currentAllowance = await this.read<bigint>(
        options.rewardToken,
        erc20Abi,
        'allowance',
        [account.address, this.addresses.rewards],
      )

      if (currentAllowance < options.rewardAmount) {
        await this.write(options.rewardToken, erc20Abi, 'approve', [
  ...
  async joinRun(runId: Hex, p2pEndpointId: Hex): Promise<Hash> {
    return this.write(this.addresses.coordinator, coordinatorAbi, 'joinRun', [
      runId,
      p2pEndpointId,
    ])
  }
  ...
  async tick(runId: Hex): Promise<Hash> {
    return this.write(this.addresses.coordinator, coordinatorAbi, 'tick', [
      runId,
    ])
  }
  ...
  async pauseRun(runId: Hex): Promise<Hash> {
    return this.write(this.addresses.coordinator, coordinatorAbi, 'pauseRun', [
      runId,
    ])
  }

  async resumeRun(runId: Hex): Promise<Hash> {
    return this.write(this.addresses.coordinator, coordinatorAbi, 'resumeRun', [
      runId,
    ])
  }

  async getRunInfo(runId: Hex): Promise<TrainingRunInfo> {
    const result = await this.read<RunResult>(
      this.addresses.coordinator,
      coordinatorAbi,
      'getRun',
      [runId],
  ...
  async getRunConfig(runId: Hex): Promise<CoordinatorConfig> {
    return this.read<CoordinatorConfig>(
      this.addresses.coordinator,
      coordinatorAbi,
      'getRunConfig',
      [runId],
  ...
  async getClients(runId: Hex): Promise<Client[]> {
    ...
    const rawClients = await this.read<ClientResult[]>(
      this.addresses.coordinator,
      coordinatorAbi,
      'getClients',
      [runId],
  ...
  async claimMultiple(runIds: Hex[]): Promise<Hash> {
    return this.write(this.addresses.rewards, rewardsAbi, 'claimMultiple', [
      runIds,
    ])
  }
  ...
  async getOptimalNodes(
    ...
    return this.read<Address[]>(
      ...
      performanceAbi,
      'getOptimalNodes',
      [count, minGpuTier, minBandwidth, minScore],
    )
  }
  ...
  async getPrivateRunConfig(runId: Hex): Promise<PrivateRunConfig> {
    return this.read<PrivateRunConfig>(
      this.addresses.registry,
      registryAbi,
      'getPrivateRunConfig',
      [runId],
    ...
    return this.publicClient.watchContractEvent({
      ...
      onLogs: (logs) => {
        for (const log of logs) {
          ...
          callback(
            args.runId,
            args.oldState as RunState,
            args.newState as RunState,
            args.timestamp,
          )
        }
      },
  ...
  watchEpochCompleted(
    runId: Hex | null,
    callback: (runId: Hex, epoch: number, stepsCompleted: number) => void,
  ) {
    return this.publicClient.watchContractEvent({
      address: this.addresses.coordinator,
      abi: coordinatorAbi,

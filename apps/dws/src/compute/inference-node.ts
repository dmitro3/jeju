...
const MODEL_PROVIDERS: Record<string, string[]> = {
  ...
  'gpt-4-turbo': ['openai'], 'gpt-3.5': ['openai'], 'gpt-3.5-turbo': ['openai'],
  'o1': ['openai'], 'o3': ['openai'],
  ...
  'llama': ['groq', 'together', 'local'], 'llama-3': ['groq', 'together', 'local'],
  'llama-3.1': ['groq', 'together', 'local'], 'llama-3.2': ['groq', 'together', 'local'],
  'llama-3.3': ['groq', 'together', 'local'],
  'mixtral': ['groq', 'together', 'local'], 'gemma': ['groq', 'local'],
  'qwen': ['together', 'local'], 'deepseek': ['together', 'local'],
};

/**
 * Register an inference node with DWS
 */
export function registerNode(node: Omit<InferenceNode, 'registeredAt' | 'lastHeartbeat' | 'currentLoad'>): InferenceNode {
  ...
  console.log(`[Inference] Node registered: ${node.address} (${node.provider}, ${node.models.length} models)`);
...
export function getActiveNodes(): InferenceNode[] {
  ...
  return Array.from(inferenceNodes.values())
    .filter(node => node.isActive && (now - node.lastHeartbeat) < staleThreshold);
}
...
export function findNodeForModel(model: string): InferenceNode | null {
  ...
  const compatibleProviders = MODEL_PROVIDERS[modelPrefix] || MODEL_PROVIDERS[model.toLowerCase()] || [];
  
  const compatibleNodes = activeNodes.filter(node => {
    // Check if node's provider is compatible
    if (compatibleProviders.length > 0 && !compatibleProviders.includes(node.provider)) {
      return false;
    }
    // Check if node explicitly lists this model
    if (node.models.length > 0 && !node.models.some(m => 
      model.toLowerCase().includes(m.toLowerCase()) || m.toLowerCase().includes(modelPrefix)
    )) {
      return false;
    }
    // Check load
...
export async function routeInference(request: InferenceRequest): Promise<InferenceResponse> {
  ...
  if (!node) {
    throw new Error('No inference nodes available. Register a node with DWS or start a local inference node.');
  }

...
function getProviderForModel(model: string): (ProviderConfig & { apiKey: string }) | null {
  ...
  for (const provider of configured) {
    ...
    if (provider.models.some(m => model.includes(m) || m.includes(model.split('-')[0]))) {
      return provider;
    }
  }
  ...
  for (const provider of configured) {
    if (modelLower.startsWith('gpt') && provider.id === 'openai') return provider;
    if (modelLower.startsWith('claude') && provider.id === 'anthropic') return provider;
    if (modelLower.startsWith('llama') && (provider.id === 'groq' || provider.id === 'together')) return provider;
    if (modelLower.startsWith('mixtral') && provider.id === 'groq') return provider;
  }
...
app.post('/v1/chat/completions', async (c) => {
  ...
  if (provider.isAnthropic) {
    ...
    if (!response.ok) {
      ...
      return c.json({ error: `Anthropic error: ${error}` }, response.status as 400 | 500);
    }
  ...
  if (!response.ok) {
    const error = await response.text();
    return c.json({ error: `${provider.id} error: ${error}` }, response.status as 400 | 500);
  }
...
app.post('/v1/embeddings', async (c) => {
  ...
  const configured = getConfiguredProviders();
  const provider = configured.find(p => p.id === 'openai') || 
  ...
  if (!response.ok) {
    const error = await response.text();
    return c.json({ error: `${provider.id} embeddings error: ${error}` }, response.status as 400 | 500);
  }
...
const PORT = parseInt(process.env.INFERENCE_PORT || '4031');
const DWS_URL = process.env.DWS_URL || 'http://localhost:4030';

async function registerWithDWS(): Promise<void> {
  const configured = getConfiguredProviders();
  if (configured.length === 0) {
    console.log('[Inference Node] No providers configured - skipping DWS registration');
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn('[Inference Node] Failed to register with DWS:', await response.text());
    }
  } catch (error) {
    console.warn('[Inference Node] Could not reach DWS for registration:', (error as Error).message);
  }
}
...
if (import.meta.main) {
  ...
  const configured = getConfiguredProviders();
  console.log(`[Inference Node] Configured providers: ${configured.map(p => p.id).join(', ') || 'none'}`);

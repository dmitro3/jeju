...
import { createSanitizationConfig, sanitizeResponse, checkForLeaks } from './sanitizer';
 ...
 */
function buildUpstreamUrl(provider: APIProvider, endpoint: string, queryParams?: Record<string, string>): URL {
...
export async function proxyRequest(
  ...
  if (!provider) {
    return createErrorResponse(500, 'Provider configuration error', requestId, startTime);
  }
  ...
  try {
    ...
    if (leakCheck.leaked) {
      console.warn(`[Proxy] Potential credential leak detected in response: ${leakCheck.details.join(', ')}`);
    }
    ...
    return createErrorResponse(502, `Upstream error: ${message}`, requestId, startTime);
  }
}
...
export async function* proxyStreamingRequest(
  ...
  if (!provider.supportsStreaming) {
    yield { chunk: JSON.stringify({ error: 'Provider does not support streaming' }), done: true };
    return;
  }

  const accessCheck = checkAccess(options.userAddress, listing, request.endpoint, request.method);
  if (!accessCheck.allowed) {
  ...
  const apiKey = decryptKeyForRequest({
    keyId: listing.keyVaultId,
    requester: options.userAddress,
    requestContext: { listingId: listing.id, endpoint: request.endpoint, requestId },
  });
  ...
  try {
    ...
    if (!response.ok || !response.body) {
      yield { chunk: JSON.stringify({ error: `Upstream error: ${response.status}` }), done: true };
      return;
    }

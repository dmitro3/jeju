...
async function main() {
  ...
  console.log(
    `  Solana (${SOLANA_RPC_URL}): ${solanaAvailable ? '✅ Running' : '❌ Not available'}`,
  )
  console.log(
    `  EVM (${EVM_RPC_URL}): ${evmAvailable ? '✅ Running' : '❌ Not available'}`,
  )
  console.log()
  ...
  console.log(
    `  Solana pubkey: ${psycheClient.getPublicKey()?.toBase58() ?? 'Not configured'}`,
  )
  console.log(
    `  EVM address: ${psycheClient.getEvmAddress() ?? 'Not configured'}`,
  )
  console.log()
  ...
  for (let i = 0; i < testRewards.length; i++) {
    ...
    console.log(
      `  Proof ${i} (${reward.client.slice(0, 10)}...): ${isValid ? '✅ Valid' : '❌ Invalid'}`,
    )
  ...
  if (solanaAvailable) {
    ...
    try {
      ...
      console.log(
        '     This is expected if Psyche coordinator program is not deployed.',
      )
      console.log('     To test with real Psyche:')
      console.log(
        '     1. Run: docker-compose -f vendor_examples/psyche/docker-compose.yml up',
      )
    ...
    console.log(
      '  1. Install solana-test-validator: sh -c "$(curl -sSfL https://release.solana.com/v1.18.0/install)"',
    )
  ...
  console.log(
    `  Solana (Psyche):      ${solanaAvailable ? '✅ Available' : '❌ Not running'}`,
  )
  console.log(
    `  EVM (Jeju):           ${evmAvailable ? '✅ Available' : '❌ Not running'}`,
  )
  console.log(
    `  Merkle proofs:        ${allProofsValid ? '✅ Working' : '❌ Failed'}`,
  )
  ...
  console.log(
    '  ⚠️ Psyche coordinator program on Solana (or use Psyche testnet)',
  )

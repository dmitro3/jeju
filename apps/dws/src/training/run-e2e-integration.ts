...
async function checkService(name: string, url: string, healthPath = '/health'): Promise<ServiceStatus> {
  try {
...
async function checkAllServices(): Promise<{ services: ServiceStatus[]; allHealthy: boolean }> {
  ...
  const solanaStatus = await checkService('Solana (Psyche)', CONFIG.solanaRpcUrl, '/health')
    .catch(() => ({ 
      name: 'Solana (Psyche)', 
...
async function startJejuDev(): Promise<boolean> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`[Jeju] Localnet already running at block ${parseInt(data.result, 16)}`);
      return true;
    }
  } catch {
  ...
  console.log('[Jeju] Run in another terminal: cd /home/shaw/Documents/jeju && bun packages/cli/src/index.ts dev');
...
async function runTrainingPipeline(atroposUrl: string) {
  ...
  const trajectories = env.generateTrajectoryBatch(50, ['agent-1', 'agent-2', 'agent-3']);
  ...
  // Submit scored data
  const scoredData = trainingData.map((t, i) => ({
  ...
  const trainingDataB64 = Buffer.from(JSON.stringify(trainingTexts.slice(0, 20))).toString('base64');
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║           JEJU E2E TRAINING INTEGRATION                       ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  console.log('║  Components: Jeju CLI, DWS, CovenantSQL, Psyche               ║');
  console.log('╚══════════════════════════════════════════════════════════════╝');
  ...
  console.log(`[Atropos] Server running on http://localhost:${atroposServer.port}`);
  ...
  console.log(`[DWS Training] API running on http://localhost:${dwsServer.port}`);

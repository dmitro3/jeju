...
async function main() {
  ...
  const submitRes = await fetch(`http://localhost:${CONFIG.atroposPort}/scored_data_list`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  for (let step = 1; step <= CONFIG.trainingSteps; step++) {
    ...
    const batchRes = await fetch(`http://localhost:${CONFIG.atroposPort}/batch?trainer_id=demo-trainer`);
    let batchSize = 0;
    
    if (batchRes.ok) {
      const batchData = await batchRes.json() as { batch?: Array<{ tokens: number[][] }> };
    ...
    console.log(`      Step ${step}/${CONFIG.trainingSteps}: loss=${loss.toFixed(4)}, acc=${accuracy.toFixed(3)}, samples=${batchSize}`);
  ...
  // Submit a job
  const jobRes = await fetch(`http://localhost:${CONFIG.dwsApiPort}/training/jobs`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  if (jobRes.ok) {
    ...
    // Check job status
    const statusRes = await fetch(`http://localhost:${CONFIG.dwsApiPort}/training/jobs/${job.jobId}`);
    if (statusRes.ok) {
  ...
  // List jobs
  const listRes = await fetch(`http://localhost:${CONFIG.dwsApiPort}/training/jobs`);
  if (listRes.ok) {

/**
 * Fundamental Prediction Environment for Jeju DWS
 * 
...
async function loadDataset(): Promise<{ train: DatasetItem[]; test: DatasetItem[] }> {
...
function extractPrediction(
  ...
  const [thinkingSection, answerSection] = parts;
  ...
  return {
    prediction: matches[1].toLowerCase(),
    magnitude: matches[2],
  };
}
...
export class FundamentalPredictionEnv {
  ...
  static configInit(): { envConfig: EnvConfig; serverConfigs: APIServerConfig[] } {
  ...
  async collectTrajectories(item: TrainingItem): Promise<{
    ...
    const messages: Message[] = [
      ...
      { role: 'user', content: createUserMessage(item.context, item.fundamentalMetric) },
    ];
  ...
  async score(
    ...
    // Shuffle for unbiased selection
    const shuffled = [...rolloutData].sort(() => Math.random() - 0.5);

    for (const data of shuffled) {
      const { prediction, magnitude } = extractPrediction(data.text, item.fundamentalMetric);

      let finalScore: number;
      if (prediction === null) {
        finalScore = 0.0; // Invalid format
        ...
        finalScore = 0.0; // Incorrect direction
      }
      ...
      // Create mask (all tokens are valid for training)
      const mask = data.tokens.map((_, i) => (i < data.tokens.length - 1 ? i : -100));
      ...
      // Track metrics
      const directionalCorrect = prediction === item.answer && prediction !== null ? 1.0 : 0.0;
    ...
    // Return null if all scores are the same (no learning signal)
  ...
  async evaluate(): Promise<{
    ...
    for (const testItem of this.test) {
      const messages: Message[] = [
        { role: 'system', content: SYSTEM_PROMPT },
        { role: 'user', content: createUserMessage(testItem.context, testItem.fundamental_metric) },
      ];
      ...
      const completions = await this.generateCompletions(prompt, 1, 1024 * 16, 0.2);
      const modelResponse = completions.choices[0].text;
      ...
      const directionScore = prediction === testItem.answer && prediction !== null ? 1 : 0;

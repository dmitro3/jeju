...
async function main() {
  ...
  console.log(`    Generated 10 games: X=${wins.X}, O=${wins.O}, Draw=${wins.draw}`);
  ...
  // Fetch a batch
  const batchRes = await fetch(`http://localhost:${CONFIG.atroposPort}/batch?trainer_id=test-trainer`);
  ...
  // List jobs
  const listRes = await fetch(`http://localhost:${CONFIG.dwsPort}/training/jobs`);
  if (listRes.ok) {

...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < 120; i++) {
      try {
        const response = await fetch(`http://localhost:${config.vllmPort}/health`);
        if (response.ok) {
          vllmReady = true;
          break;
        }
    ...
    if (config.usePsyche && trainer instanceof (await import('../grpo-trainer')).DistributedGRPOTrainer) {
    ...
    const envResponse = await fetch(`http://localhost:${config.atroposPort}/register-env`, {
      method: 'POST',
    ...
    const envData = await envResponse.json() as { status: string; env_id: number };
    ...
    while (step < config.trainingSteps) {
      ...
      console.log(`   Direction Accuracy: ${(metrics.directionAccuracy * 100).toFixed(1)}%`);
      console.log(`   Magnitude Accuracy: ${(metrics.magnitudeAccuracy * 100).toFixed(1)}%`);
    ...
    console.log(`Final Direction Accuracy: ${(evalResults.directionAccuracy * 100).toFixed(1)}%`);
    console.log(`Final Magnitude Accuracy: ${(evalResults.magnitudeAccuracy * 100).toFixed(1)}%`);

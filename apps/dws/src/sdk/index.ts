...
export class DWSSDK {
  ...
  async uploadFile(file: File | Blob | ArrayBuffer, options?: { filename?: string; permanent?: boolean }): Promise<UploadResult> {
    const formData = new FormData();
    const blob = file instanceof ArrayBuffer ? new Blob([new Uint8Array(file)]) : file;

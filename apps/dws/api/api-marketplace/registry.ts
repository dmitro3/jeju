...
import { isRiskLevel } from '../shared/utils/type-guards'
import {
  apiListingState,
  apiUserAccountState,
  initializeDWSState,
...
function rowToListing(row: {
  ...
  if (!isAddress(row.seller)) {
    throw new Error(`Invalid seller address for listing ${row.listing_id}`)
  }
...
export async function getAccount(
  ...
  // Validate address from DB
  if (!isAddress(row.address)) {
    throw new Error(`Invalid address in user account: ${row.address}`)
  }
  return {
    address: row.address,

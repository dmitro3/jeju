...
class LeaderboardQueue {
  ...
  private async processQueue(): Promise<void> {
    ...
    for (const event of batch) {
      const success = await postToLeaderboardWithRetry(
        event.endpoint,
        event.body,
        event.retries,
      )
      if (!success && event.retries < MAX_RETRIES) {
        ...
        console.error(
          `[Pkg Leaderboard] Failed to post to ${event.endpoint} after ${MAX_RETRIES} retries`,
        )
      }
    }
...
async function postToLeaderboardWithRetry(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[Pkg Leaderboard] POST to ${endpoint} returned ${response.status}: ${errorText}`,
      )
      return false
    }
    ...
    if (retryCount < MAX_RETRIES) {
      console.debug(
        `[Pkg Leaderboard] Retry ${retryCount + 1}/${MAX_RETRIES} for ${endpoint}: ${errorMessage}`,
      )
    } else {
      console.error(
        `[Pkg Leaderboard] Failed to post to ${endpoint} after retries: ${errorMessage}`,
      )
    }
    return false
  }

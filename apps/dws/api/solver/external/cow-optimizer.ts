...
import type { CowAuction, CowOrder, CowSolution } from './cow'
...
export class CowSolverOptimizer {
    ...
    for (const r of routing) {
      ...
      if (order) {
        ...
        if (trade) {
          ...
          if (price) {
            totalSurplusUsd +=
              (Number(surplus) / 10 ** price.decimals) * price.priceUsd
          }
        }
      }
    }
  ...
  private findCowMatches(orders: CowOrder[]): {
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      for (let j = i + 1; j < orders.length; j++) {
        ...
        if (
          buyer.sellToken.toLowerCase() === seller.buyToken.toLowerCase() &&
  ...
  private getMultiHopRoutes(order: CowOrder): Array<{
    ...
    for (const intermediate of this.intermediateTokens) {
      if (
        intermediate === order.sellToken.toLowerCase() ||
      ...
      const intermediateAmount = this.getAmountOut(
        order.sellAmount,
        order.sellToken,
        pool1,
      )
      const finalAmount = this.getAmountOut(
        intermediateAmount,
  ...
  getStats(solution: OptimizedSolution): {
    ...
    const directRoutes = solution.routing.filter(
      (r) => r.type === 'direct',
    ).length
    const multiHopRoutes = solution.routing.filter(
      (r) => r.type === 'multi-hop',
    ).length
    const cowMatches = solution.routing.filter(
      (r) => r.type === 'cow-match',
    ).length

    const avgSurplusBps =
      solution.routing.length > 0
        ? Math.round(
            solution.routing.reduce((sum, r) => sum + r.surplusBps, 0) /
              solution.routing.length,
          )
        : 0

    const gasPerTrade =
      solution.trades.length > 0

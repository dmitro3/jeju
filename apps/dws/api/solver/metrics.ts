...
export function recordIntentReceived(chainId: number): void {
  registry.incrementCounter(METRICS.INTENTS_RECEIVED, {
...
export function recordSettlementClaimed(
  ...
  registry.incrementCounter(METRICS.SETTLEMENTS_CLAIMED, {
    chain: chainId.toString(),
  })
  registry.incrementCounter(
    METRICS.SOLVER_PROFIT_WEI,
    { chain: chainId.toString() },
    Number(amountWei),
  )
}
...
export function recordSettlementFailed(chainId: number, reason: string): void {
  registry.incrementCounter(METRICS.SETTLEMENTS_FAILED, {
    chain: chainId.toString(),
    reason,
  })
}
...
export function updateLiquidity(
  ...
  registry.setGauge(
    `${METRICS.LIQUIDITY_AVAILABLE}_${chainId}`,
    Number(amountWei),
  )
}

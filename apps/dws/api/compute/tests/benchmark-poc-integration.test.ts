...
describe('BenchmarkOrchestrator → PoCNodeVerifier integration', () => {
  ...
  beforeEach(() => {
    ...
    parseQuoteSpy = spyOn(quoteParser, 'parseQuote').mockImplementation(
      (hex: Hex) => ({
        success: true,
        quote: {
          raw: hex,
    ...
    pocVerifier = new PoCNodeVerifier({
  ...
  test('PoC verification is triggered for TEE-enabled machines', async () => {
    ...
    const hardwareIdHash = quoteParser.hashHardwareId(
      `0x${'11'.repeat(16)}` as Hex,
      TEST_SALT,
    )
    ...
    const updatedResults = await orchestrator.runPoCVerification(
      TEST_AGENT_ID,
      results,
    )

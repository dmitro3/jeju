...
export class CIEventBus {
  ...
  async emit(event: CIEvent): Promise<void> {
    ...
    if (handlers) {
      await Promise.all(
        Array.from(handlers).map((h) => h(event).catch(console.error)),
      )
    }
  ...
  private async triggerMatchingWorkflows(event: CIEvent): Promise<void> {
    ...
    for (const workflow of workflows) {
      ...
      const { branch, commitSha, inputs, prNumber } =
        this.extractRunParams(event)
  ...
  private findMatchingTrigger(
    ...
    for (const trigger of triggers) {
      ...
      if (
        ...
        if (event.workflowId !== this.getWorkflowIdFromTrigger(trigger))
          continue
        return trigger
      }
  ...
  private eventTypeMatchesTrigger(
    eventType: CIEvent['type'],
  ...
  getEventHistory(
    repoId?: Hex,
    limit = 100,
  ): Array<{ event: CIEvent; timestamp: number }> {

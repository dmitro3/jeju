...
/** Primitive values allowed in matrix configurations */
export type MatrixPrimitive = string | number | boolean

/** A single matrix combination record */
export type MatrixCombination = Record<string, MatrixPrimitive>

/** Array of values for a matrix dimension */
export type MatrixDimensionValues = string[] | number[] | boolean[]

export interface MatrixConfig {
  include?: MatrixCombination[]
  exclude?: MatrixCombination[]
  /** Dynamic matrix dimensions - each key maps to an array of values */
  [key: string]: MatrixDimensionValues | MatrixCombination[] | undefined
}
...
// Import RunStatus from @jejunetwork/types
...
export type ActionRuntime = (typeof ActionRuntime)[keyof typeof ActionRuntime]

export interface CompositeActionRuns {
  using: typeof ActionRuntime.COMPOSITE
  steps: WorkflowStep[]
}

export interface NodeActionRuns {
  using: typeof ActionRuntime.NODE16 | typeof ActionRuntime.NODE20
  main: string
  pre?: string
  post?: string
}
...
export interface DockerActionRuns {
  using: typeof ActionRuntime.DOCKER
  image: string
  args?: string[]
  env?: Record<string, string>
  entrypoint?: string
...
export const BUILTIN_ACTIONS: Record<string, Action> = {
  ...
  'actions/setup-node@v4': {
    ...
    inputs: {
      ...
      'node-version-file': {
        description: 'File containing version spec',
        required: false,
      },
  ...
  'actions/cache@v4': {
    ...
    inputs: {
      ...
      key: {
        description: 'Key for restoring and saving the cache',
        required: true,
      },
      'restore-keys': {
        description: 'Ordered list of keys for restoring stale cache',
        required: false,
      },
    },
    outputs: {
      'cache-hit': {
        description: 'Whether an exact match was found for the key',
      },
    },
    runs: {
      using: 'node20',

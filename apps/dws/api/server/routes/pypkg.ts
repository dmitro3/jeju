...
export function createPyPkgRouter(ctx: PyPkgContext) {
  ...
  return new Elysia({ name: 'pypkg', prefix: '/pypkg' })
...
${links}
    ...
    .get(
      ...
      ({ params }) => {
        ...
        const releases: Record<string, Array<{
          filename: string
          url: string
    ...
    .post(
      ...
      async ({ request, headers, set }) => {
        ...
        const pkg: PythonPackage = {
          ...
          requiresPython: (formData.get('requires_python') as string) ?? '>=3.8',
          keywords: ((formData.get('keywords') as string) ?? '').split(',').filter(Boolean),
          classifiers: formData.getAll('classifiers') as string[],
          requiresDist: formData.getAll('requires_dist') as string[],
          projectUrls: {},
          wheelData,
          wheelFilename: content.name || `${normalizedName}-${version}-py3-none-any.whl`,

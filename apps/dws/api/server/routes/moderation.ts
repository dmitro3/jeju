...
export function createModerationRouter() {
  ...
  return new Elysia({ name: 'moderation', prefix: '/moderation' })
    ...
    .get('/transparency', async ({ query }) => {
      ...
      switch (period) {
        case '24h': sinceTimestamp = now - day; break
        case '7d': sinceTimestamp = now - 7 * day; break
        case '30d': sinceTimestamp = now - 30 * day; break
        case '90d': sinceTimestamp = now - 90 * day; break
        default: sinceTimestamp = now - 30 * day
      }
    ...
    .post('/user-report', async ({ body, set, headers }) => {
      ...
      body: t.Object({
        reporterAddress: t.Optional(t.String({ pattern: '^0x[a-fA-F0-9]{40}$' })),
        targetType: t.Union([
          t.Literal('content'),
          t.Literal('user'),
          t.Literal('domain'),
    ...
    .get('/user-reports', async ({ query, headers, set }) => {
    ...
    .post('/user-reports/:reportId/review', async ({ params, body, headers, set }) => {
      const authHeader = headers.authorization
      if (authHeader !== `Bearer ${process.env.INTERNAL_API_KEY}`) {
        set.status = 401
        return { error: 'Unauthorized' }
      ...
      body: t.Object({
        ...
        reviewedBy: t.Optional(t.String({ pattern: '^0x[a-fA-F0-9]{40}$' })),
      ...
      detail: { tags: ['moderation'], summary: 'Review a user report (admin)' },
    })

    .get('/user-reports/stats', async ({ headers, set }) => {
      const authHeader = headers.authorization
      if (authHeader !== `Bearer ${process.env.INTERNAL_API_KEY}`) {
        set.status = 401
        return { error: 'Unauthorized' }
      ...
      detail: { tags: ['moderation'], summary: 'Get user report stats (admin)' },
    })

    // ========== TRUSTED FLAGGERS ==========

    .post('/trusted-flagger', async ({ body, headers, set }) => {
      const authHeader = headers.authorization
      if (authHeader !== `Bearer ${process.env.INTERNAL_API_KEY}`) {
        set.status = 401
        return { error: 'Unauthorized' }
      ...
      detail: { tags: ['moderation'], summary: 'Register a trusted flagger (admin)' },
    })

    .get('/trusted-flaggers', async ({ headers, set }) => {
      const authHeader = headers.authorization
      if (authHeader !== `Bearer ${process.env.INTERNAL_API_KEY}`) {
        set.status = 401
        return { error: 'Unauthorized' }
      ...
      detail: { tags: ['moderation'], summary: 'List trusted flaggers (admin)' },
    })

    .post('/trusted-flagger/report', async ({ body, headers, set }) => {
      ...
      return {
        ...
        message: flagger.priority === 'urgent'
      ...
      detail: { tags: ['moderation'], summary: 'Submit report as trusted flagger' },
    })
}

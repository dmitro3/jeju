  ...
  return new Elysia({ prefix: '/cache' })
    ...
    .post(
      ...
      async ({ body }) => {
        ...
        // TEE provider
        const success = await engine.set(namespace, body.key, body.value, options)
        return { success }
      },
    ...
    .post(
      ...
      async ({ body }) => {
        ...
        if (engine instanceof CacheEngine) {
          ...
          for (const entry of body.entries) {
            await engine.set(namespace, entry.key, entry.value, { ttl: entry.ttl })
          }
        }
    ...
    .post(
      ...
      async ({ body }) => {
        ...
        if (engine instanceof CacheEngine) {
          const values = engine.lrange(namespace, body.key, body.start, body.stop)
          return { values }
        }

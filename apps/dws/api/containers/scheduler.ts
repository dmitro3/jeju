 ...
 *
 * Includes Proof-of-Cloud verification for high-risk TEE workloads
 */
...
async function getNodePoCStatus(
  nodeId: string,
  agentId?: bigint,
): Promise<PoCCache> {
...
export async function scheduleExecution(
  ...
  if (eligibleNodes.length === 0) {
    ...
    if (context.riskLevel === 'high') {
      console.warn(
        '[Scheduler] No PoC-verified nodes available for high-risk task',
      )
    }
    return null
  }
...
async function getEligibleNodes(
  context: SchedulingContext,
): Promise<ComputeNode[]> {
  ...
  const resourceEligible = allNodes.filter((node) => {
    ...
    if (
      resources.gpuType &&
      !node.resources.gpuTypes.includes(resources.gpuType)
    )
      return false
...
async function scheduleBestFit(
  nodes: ComputeNode[],
  context: SchedulingContext,
  ...
  const scored = await Promise.all(
    nodes.map(async (node) => {
      ...
      const memUtilization =
        resources.memoryMb / node.resources.availableMemoryMb
...
async function scheduleWorstFit(
  nodes: ComputeNode[],
  context: SchedulingContext,
): Promise<ScheduleResult> {
  const riskLevel = context.riskLevel ?? 'low'
...
function scheduleWorstFitSync(
  nodes: ComputeNode[],
  context: SchedulingContext,
): ScheduleResult {
  const scored = nodes.map((node) => {
    const availableScore =
      node.resources.availableCpu * 10 + node.resources.availableMemoryMb / 100
    let bonus = 0
    if (node.cachedImages.has(context.image.digest)) bonus += 50
    if (context.preferredRegion && node.region === context.preferredRegion)
      bonus += 20
...
async function scheduleFirstFit(
  nodes: ComputeNode[],
  context: SchedulingContext,
): Promise<ScheduleResult> {
    ...
    for (const node of nodes) {
      ...
      if (pocStatus.verified) {
        return {
          nodeId: node.nodeId,
          score: 100,
          reason: 'First PoC-verified node',
          pocVerified: true,
        }
      }
    }
  ...
  if (withCache) {
    return {
      nodeId: withCache.nodeId,
      score: 100,
      reason: 'First fit with cache hit',
    }
  }
...
function scheduleRoundRobinSync(
  nodes: ComputeNode[],
...
export function getRegionsOrderedByDistance(
...
export function getSchedulerStats(): SchedulerStats {
  ...
  const stats: SchedulerStats = {
    ...
    availableCpu: allNodes.reduce(
      (sum, n) => sum + n.resources.availableCpu,
      0,
    ),
    totalMemoryMb: allNodes.reduce(
      (sum, n) => sum + n.resources.totalMemoryMb,
      0,
    ),
    availableMemoryMb: allNodes.reduce(
 ...
 */
export async function getPoCVerifiedNodes(
  agentId: bigint,
): Promise<ComputeNode[]> {

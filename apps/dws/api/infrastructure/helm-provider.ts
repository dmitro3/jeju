...
interface VolumeSpec {
  ...
  emptyDir?: Record<string, JSONValue>
}
...
interface Affinity {
  nodeAffinity?: Record<string, JSONValue>
  podAffinity?: Record<string, JSONValue>
  podAntiAffinity?: Record<string, JSONValue>
}
...
class ManifestParser {
  ...
  private isDeploymentSpec(
    spec: Record<string, unknown>,
  ): spec is Record<string, unknown> & DeploymentSpec {
    const obj = spec
    if (
      !('template' in obj) ||
    ...
    if (
      ...
      template.spec === null ||
      Array.isArray(template.spec)
    ) {
      return false
    }
    const podSpec = template.spec as Record<string, unknown>
    return 'containers' in podSpec && Array.isArray(podSpec.containers)
  }

  private parseDeployment(manifest: KubeManifest): DWSWorkerConfig[] {
    // KubeManifest.spec is Record<string, JSONValue> but for Deployment it matches DeploymentSpec
    if (!manifest.spec) {
      throw new Error('Deployment manifest missing spec')
    }
    if (!this.isDeploymentSpec(manifest.spec)) {
      throw new Error('Invalid Deployment spec structure')
    }

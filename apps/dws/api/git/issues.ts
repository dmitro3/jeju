...
export class IssuesManager {
  ...
  async getIssueIndex(repoId: Hex, metadataCid?: string): Promise<IssueIndex> {
    ...
    if (metadataCid) {
      const result = await this.backend.download(metadataCid)
      if (result) {
        ...
        if (metadata.issueIndexCid) {
          const indexResult = await this.backend.download(
            metadata.issueIndexCid,
          )
          if (indexResult) {
  ...
  async getIssue(repoId: Hex, issueNumber: number): Promise<Issue | null> {
    ...
    const result = await this.backend.download(issueRef.cid)
  ...
  async preloadIndex(repoId: Hex, issueIndexCid: string): Promise<void> {
    const result = await this.backend.download(issueIndexCid)
    if (result) {
      const index = expectJson(
        result.content.toString(),
        IssueIndexSchema,

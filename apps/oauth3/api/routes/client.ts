  ...
  return new Elysia({ name: 'client', prefix: '/client' })
    .post(
      ...
      async ({ body, set }) => {
        ...
        if (reputationCheck.isBanned) {
          ...
          return {
            error: 'owner_banned',
            error_description: 'This address is banned from registering clients',
          }
        }
        ...
        const requestedTier = (body.requestedTier ?? ClientTier.FREE) as ClientTier
    ...
    .post(
      ...
      async ({ params, body, headers, set }) => {
        ...
        // Validate category
        if (!VALID_REPORT_CATEGORIES.includes(body.category as ReportCategory)) {
          set.status = 400
          return {
        ...
        if (moderation.activeReports >= 5 && moderation.status === 'flagged') {
          moderation.status = 'suspended'
          moderation.suspensionReason = 'Auto-suspended due to multiple reports'

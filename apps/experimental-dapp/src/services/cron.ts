...
class ComputeCronService implements CronService {
  ...
  async scheduleReminder(todoId: string, owner: Address, reminderTime: number): Promise<Reminder> {
    ...
    // Register with compute cron if available
    if (this.cronAvailable) {
      await this.registerCronTrigger(id, reminderTime).catch(() => {
  ...
  async cancelReminder(reminderId: string, owner: Address): Promise<boolean> {
    ...
    if (this.cronAvailable) {
      await this.cancelCronTrigger(reminderId).catch(() => {});
    }
  ...
  async scheduleCleanup(owner: Address): Promise<CronJob> {
    ...
    if (this.cronAvailable) {
      await this.registerCleanupJob(jobId, owner).catch(() => {
        this.cronAvailable = false;
      });
    }
  ...
  private async cancelCronTrigger(triggerId: string): Promise<void> {
    await fetch(`${CRON_ENDPOINT}/cancel`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  private async checkCronHealth(): Promise<boolean> {

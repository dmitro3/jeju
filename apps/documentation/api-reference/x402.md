...
```typescript
const response = await fetch('https://compute.jejunetwork.org/api/inference', {
  method: 'POST',
...
const client = new X402Client({
  wallet,
  facilitatorUrl: 'https://facilitator.jejunetwork.org',
});

// Automatically handles 402 and payment
const response = await client.fetch('https://compute.jejunetwork.org/api/inference', {
  method: 'POST',
  body: JSON.stringify({ prompt: 'Hello' }),
});
```

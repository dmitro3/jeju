...
**URLs:** A2A at http://127.0.0.1:3003, Prometheus at http://127.0.0.1:9090, Grafana at http://127.0.0.1:4010. Production uses https://monitoring.jejunetwork.org.
...
Install with `cd packages/monitoring && bun install`.
...
This deploys the monitoring worker to DWS with the A2A/MCP API and configures CDN for the frontend.

## Grafana Access

Local Grafana runs at http://localhost:4010 with default credentials admin/admin.
...
"Grafana dashboard missing" — reload dashboards with `curl -X POST http://admin:admin@localhost:4010/api/admin/provisioning/dashboards/reload`.

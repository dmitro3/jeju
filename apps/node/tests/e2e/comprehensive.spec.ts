...
const _VIEWS = [
  {
    ...
    description: 'Node operator dashboard with status.',
  },
  {
    name: 'Services',
    expectedContent: 'Service',
    description: 'Running services list.',
  },
  { name: 'Bots', expectedContent: 'Bot', description: 'AI bots management.' },
  {
    name: 'Earnings',
    expectedContent: 'Earning',
    description: 'Earnings and rewards.',
  },
  {
    name: 'Staking',
    expectedContent: 'Stak',
    description: 'Staking interface.',
  },
  {
    name: 'Settings',
  ...
  test('Dashboard loads', async ({ page }) => {
    ...
    if (errors.length > 0 && !hasKnownBug) {
      await page.screenshot({
        path: join(SCREENSHOT_DIR, 'Dashboard-ERROR.png'),
        fullPage: true,
      })
    ...
    if (isLLMConfigured?.() && verifyImage) {
      ...
      const verification = cached
        ? cached.result
        : await verifyImage(screenshotPath, 'Node operator dashboard')
      if (!cached) {
      ...
      console.log(
        `${cached ? '📦' : '🔍'} Dashboard: ${verification.quality} (${Math.round(verification.confidence * 100)}%)`,
      )
    }
  })
})
...
test.afterAll(() => {
  saveCache()
  console.log(`📊 Node tests completed`)
})

 ...
 */
function calculateNetProfitBps(
  priceDiffBps: number,
  bridgeCostBps = 15,
): number {
...
describe('Position Sizing Calculations', () => {
  ...
  test('throws for zero price difference', () => {
    expect(() => calculatePositionSize(30, 0, 10000)).toThrow(
      'priceDiffBps must be positive',
    )
  })

  test('throws for negative price difference', () => {
    expect(() => calculatePositionSize(30, -10, 10000)).toThrow(
      'priceDiffBps must be positive',
    )
  })
})
...
describe('Minimum Output Calculation (Slippage)', () => {
  ...
  test('throws for invalid slippage values', () => {
    expect(() => calculateMinOutput(1000n, -1)).toThrow(
      'slippageBps must be between 0 and 10000',
    )
    expect(() => calculateMinOutput(1000n, 10001)).toThrow(
      'slippageBps must be between 0 and 10000',
    )
  })
})
...
describe('Arbitrage Opportunity Type Detection', () => {
  function determineArbType(
    buyChain: string,
    sellChain: string,
  ): 'solana_evm' | 'hyperliquid' | 'cross_dex' {

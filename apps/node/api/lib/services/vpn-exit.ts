...
/** Protocol type for NAT */
type NATProtocol = 'tcp' | 'udp' | 'icmp'

function isVPNNodeData(node: unknown): node is VPNNodeData {
  if (!isPlainObject(node)) return false
  return (
    typeof node.operator === 'string' &&
    typeof node.countryCode === 'string' &&
    typeof node.registeredAt === 'bigint'
  )
}

function toNATProtocol(protocol: number): NATProtocol {
...
export class VPNExitService {
  ...
  async getState(address: Address): Promise<VPNExitState | null> {
    ...
    if (!isVPNNodeData(node) || node.registeredAt === BigInt(0)) {
      return null
    }
    ...
    const validAddress = expectAddress(address, 'VPN node address')

    return {
      isRegistered: true,
      nodeId: validAddress,
      countryCode: Buffer.from(node.countryCode.slice(2), 'hex').toString(),
      status,
      activeClients: this.clients.size,
      totalSessions: Number(node.totalSessions),
      totalBytesServed: node.totalBytesServed,
      earnings: node.stake,
    }
  }

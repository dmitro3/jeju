...
describe('Price Difference Calculations', () => {
  ...
  test('throws for zero prices', () => {
    expect(() => calculatePriceDiffBps(0, 100)).toThrow(
      'Prices must be positive',
    )
    expect(() => calculatePriceDiffBps(100, 0)).toThrow(
      'Prices must be positive',
    )
  })

  test('throws for negative prices', () => {
    expect(() => calculatePriceDiffBps(-100, 100)).toThrow(
      'Prices must be positive',
    )
    expect(() => calculatePriceDiffBps(100, -100)).toThrow(
      'Prices must be positive',
...
describe('Transfer Event Types', () => {
  ...
  test('transfer types are correctly enumerated', () => {
    const types: Array<TransferEvent['type']> = [
      'initiated',
      'completed',
      'failed',
    ]

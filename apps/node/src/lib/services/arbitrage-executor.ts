...
export class ArbitrageExecutor {
  ...
  async executeSolanaEvmArb(opportunity: ArbOpportunity): Promise<{
    ...
    console.log(`   Buy: ${opportunity.buyChain}, Sell: ${opportunity.sellChain}`);

    const isSolanaToBuy = opportunity.buyChain === 'solana';
    const evmChainId = this.parseEvmChainId(isSolanaToBuy ? opportunity.sellChain : opportunity.buyChain);

    if (!evmChainId) {
      return { success: false, error: `Invalid chain: ${opportunity.buyChain} / ${opportunity.sellChain}` };
    }
    ...
    if (isSolanaToBuy) {
      return this.executeBuyOnSolanaSellOnEvm(opportunity.token, positionSizeUsd, evmChainId);
    } else {
      return this.executeBuyOnEvmSellOnSolana(opportunity.token, positionSizeUsd, evmChainId);
    }
  }
  ...
  async executeCrossDexArb(opportunity: ArbOpportunity): Promise<{
    ...
    return this.executeCrossChainEvmArb(opportunity.token, positionSizeUsd, buyChainId, sellChainId);
  }
  ...
  private async executeBuyOnSolanaSellOnEvm(
    ...
    if (!solanaToken || !evmToken) {
      return { success: false, error: `Token ${token} not supported on both chains` };
    }
    ...
    console.log(`   Route: ${jupiterQuote.routePlan.map(r => r.swapInfo.label).join(' → ')}`);
    ...
    const estimatedProfit = positionSizeUsd * (await this.getPriceDiffBps(token, 'solana', evmChainId) / 10000);
  ...
  private async executeBuyOnEvmSellOnSolana(
    ...
    console.log(`   1inch quote: ${formatUnits(swapQuote.outputAmount, 18)} ${token}`);
    ...
    const estimatedProfit = positionSizeUsd * (await this.getPriceDiffBps(token, evmChainId, 'solana') / 10000);
  ...
  private async executeHyperliquidSpotArb(
    ...
    if (hlCheaper) {
      ...
      const bridgeTx = await this.bridgeFromHyperliquid(evmChainId, positionSizeUsd);
      ...
      const swapQuote = await this.get1inchSwapQuote(evmChainId, usdcAddress, evmToken, inputAmount);
      ...
      // Sell on Hyperliquid
      const sellResult = await this.placeHyperliquidOrder(hlSymbol, 'sell', positionSizeUsd, true);
  ...
  private async executeCrossChainEvmArb(
    ...
    const inputAmount = BigInt(Math.floor(positionSizeUsd * 1e6));
    const buyQuote = await this.get1inchSwapQuote(buyChainId, buyUsdc, buyToken, inputAmount);
    ...
    const estimatedProfit = positionSizeUsd * (await this.getPriceDiffBps(token, buyChainId, sellChainId) / 10000);
  ...
  private async submitViaJito(tx: VersionedTransaction): Promise<string> {
    ...
    // Submit bundle
    const bundle = [tx, tipTx].map(t => Buffer.from(t.serialize()).toString('base64'));
    ...
    if (result.error) {
      ...
      console.warn(`Jito bundle failed, using regular submission: ${result.error.message}`);
  ...
  private async execute1inchSwap(chainId: number, quote: SwapQuote): Promise<string> {
    const clients = this.evmClients.get(chainId);
  ...
  private async executeUniswapSwap(chainId: number, quote: SwapQuote): Promise<string> {
    const clients = this.evmClients.get(chainId);
    if (!clients) throw new Error(`Chain ${chainId} not configured`);
    ...
    // Then swap
    const minOut = (quote.outputAmount * BigInt(10000 - this.config.maxSlippageBps)) / 10000n;
  ...
  private async bridgeSolanaToEvm(
    ...
    const result = await response.json() as { transferId: string; status: string };
  ...
  private async bridgeEvmToSolana(
    ...
    const tokenBytes = tokenAddress.startsWith('0x') ? tokenAddress.slice(2) : tokenAddress;

    // Solana pubkey is 32 bytes when decoded from base58
    // For now, pass as string and let the API handle conversion
    const recipientBytes = Buffer.from(recipient).toString('hex').padStart(64, '0');
  ...
  private async bridgeFromHyperliquid(destChainId: number, amountUsd: number): Promise<string> {

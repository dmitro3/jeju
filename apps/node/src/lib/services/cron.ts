...
function validateEndpointUrl(endpoint: string): void {
  ...
  if (ipv4Match) {
    ...
    const [a, b] = octets;
    // 10.0.0.0/8
    if (a === 10) throw new Error('SSRF protection: private network 10.0.0.0/8 not allowed');
    // 172.16.0.0/12
    if (a === 172 && b >= 16 && b <= 31) throw new Error('SSRF protection: private network 172.16.0.0/12 not allowed');
    // 192.168.0.0/16
    if (a === 192 && b === 168) throw new Error('SSRF protection: private network 192.168.0.0/16 not allowed');
    // 169.254.0.0/16 (link-local)
    if (a === 169 && b === 254) throw new Error('SSRF protection: link-local 169.254.0.0/16 not allowed');
    // 0.0.0.0
    if (a === 0) throw new Error('SSRF protection: 0.0.0.0/8 not allowed');
  }
  
  // Block common internal hostnames
  const blockedPatterns = ['internal', 'intranet', 'corp', 'private', 'metadata', 'instance-data'];
  for (const pattern of blockedPatterns) {
    if (hostname.includes(pattern)) {
      throw new Error(`SSRF protection: hostname containing '${pattern}' not allowed`);
    }
  }
  
  // Block AWS/GCP/Azure metadata endpoints
  const metadataHosts = ['169.254.169.254', 'metadata.google.internal', 'metadata.azure.internal'];

...
function loadContractAbi(contractName: string): readonly object[] {
  ...
  throw new Error(`Contract ABI not found for ${contractName}. Run 'forge build' first.`);
}
...
async function deployBlocking() {
  console.log('╔══════════════════════════════════════════════════════════════════╗');
  console.log('║     USER BLOCKING SYSTEM - LOCAL DEVNET DEPLOYMENT               ║');
  console.log('╚══════════════════════════════════════════════════════════════════╝');
  ...
  const deployment: BlockingDeployment = {
    ...
    contracts: {
      ...
      userBlockRegistry: '0x0000000000000000000000000000000000000000' as Address,
    },
  };
  ...
  deployment.contracts.identityRegistry = identityReceipt.contractAddress as Address;
  ...
  const blockRegistryReceipt = await publicClient.waitForTransactionReceipt({ hash: blockRegistryHash });
  deployment.contracts.userBlockRegistry = blockRegistryReceipt.contractAddress as Address;
  ...
  const tokenHash = await walletClient.deployContract({
    ...
    args: ['TestToken', 'TST', tokenConfig.maxSupply, account.address, tokenConfig],
  });
  ...
  const tradeEscrowReceipt = await publicClient.waitForTransactionReceipt({ hash: tradeEscrowHash });
  deployment.contracts.playerTradeEscrow = tradeEscrowReceipt.contractAddress as Address;

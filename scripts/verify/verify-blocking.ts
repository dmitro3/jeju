...
async function verifyBlockRegistryIntegration(
  client: ReturnType<typeof createPublicClient>,
  contract: { address: Address; name: string; abi: ReturnType<typeof parseAbi> },
  expectedRegistry: Address
): Promise<{ integrated: boolean; currentRegistry: Address | null; error?: string }> {
  const result = await verifyContractExists(client, contract.address, contract.name);
  if (!result.exists) {
  ...
  const integrated = currentRegistry.toLowerCase() === expectedRegistry.toLowerCase();
  
  return {
    integrated,
    currentRegistry,
    error: integrated ? undefined : `Registry mismatch: expected ${expectedRegistry}, got ${currentRegistry}`
  };
}
...
async function runIntegrationTests(
  ...
  if (isBlocked) {
    ...
    results.push({ name: 'Block address', passed: false, error: 'Block not registered' });
    failed++;
  ...
  if (isInteractionBlocked) {
    ...
    results.push({ name: 'Interaction blocked check', passed: false, error: 'Interaction should be blocked' });
    failed++;
  ...
  if (!stillBlocked) {
    ...
    results.push({ name: 'Unblock address', passed: false, error: 'Unblock failed' });
    failed++;
  ...
  if (count === 0n) {
    ...
    results.push({ name: 'Blocked count after unblock', passed: false, error: `Count should be 0, got ${count}` });
    failed++;
  ...
  if (version === '1.0.0') {
    ...
    results.push({ name: 'Version check', passed: false, error: `Expected 1.0.0, got ${version}` });
    failed++;
...
function loadDeployment(network: string): DeployedContracts | null {
  ...
  if (process.env.USER_BLOCK_REGISTRY_ADDRESS) {
    fromEnv.userBlockRegistry = process.env.USER_BLOCK_REGISTRY_ADDRESS as Address;
  }
  ...
  if (process.env.MESSAGING_KEY_REGISTRY_ADDRESS) {
    fromEnv.messagingKeyRegistry = process.env.MESSAGING_KEY_REGISTRY_ADDRESS as Address;
  }
  ...
  if (process.env.PLAYER_TRADE_ESCROW_ADDRESS) {
    fromEnv.playerTradeEscrow = process.env.PLAYER_TRADE_ESCROW_ADDRESS as Address;
  }
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════════╗');
  console.log('║           USER BLOCKING SYSTEM VERIFICATION                      ║');
  console.log('╚══════════════════════════════════════════════════════════════════╝');
  ...
  if (!contracts) {
    console.log('⚠️  No deployment info found. Using environment variables or run deployment first.');
  ...
  for (const contract of contractsToVerify) {
    ...
    const result = await verifyContractExists(publicClient, contract.address, contract.name);
    if (result.exists) {
  ...
  if (contracts?.userBlockRegistry) {
    ...
    const integrations = [
      ...
      { name: 'MessagingKeyRegistry', address: contracts.messagingKeyRegistry, abi: MESSAGING_KEY_REGISTRY_ABI },
      { name: 'Marketplace', address: contracts.marketplace, abi: MARKETPLACE_ABI },
      { name: 'X402Facilitator', address: contracts.x402Facilitator, abi: X402_ABI },
      { name: 'PlayerTradeEscrow', address: contracts.playerTradeEscrow, abi: PLAYER_TRADE_ESCROW_ABI },
    ];
    ...
    for (const integration of integrations) {
      ...
      const result = await verifyBlockRegistryIntegration(
        publicClient,
        { address: integration.address, name: integration.name, abi: integration.abi },
      ...
      } else if (result.currentRegistry === '0x0000000000000000000000000000000000000000') {
        console.log(`  ${integration.name}: ⚠️  Registry not set (run setBlockRegistry)`);
      } else {
  ...
  console.log('  4. Run integration tests: bun scripts/verify/verify-blocking.ts <network> --test');

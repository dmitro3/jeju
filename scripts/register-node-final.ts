import { createWalletClient, http, publicActions, parseEther, hexToBytes } from 'viem'
...
const REGISTRY_ABI = [
  ...
  { name: 'getIdentity', type: 'function', inputs: [{ name: 'nodeId', type: 'bytes32' }], outputs: [{ type: 'tuple', components: [
    { name: 'nodeId', type: 'bytes32' },
    { name: 'publicKey', type: 'bytes' },
    { name: 'nonce', type: 'tuple', components: [
      { name: 'a', type: 'uint64' },
...
async function main() {
  ...
  console.log('JEJU Balance:', (balance / BigInt(1e18)).toString(), 'JEJU')

  // Stake amount (10k for miner)
  ...
  console.log('')
  console.log('Total miners registered:', minerCount.toString())

  // Get the identity
  const registeredIdentity = await client.readContract({
    address: IDENTITY_REGISTRY,
    abi: REGISTRY_ABI,
    functionName: 'getIdentity',
    args: [identity.nodeId as `0x${string}`]
  })
  console.log('Registered identity:', registeredIdentity.operator)
}

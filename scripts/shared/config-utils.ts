 ...
 * @module scripts/shared/config-utils
 * 
...
export function getNetworkFromArgs(): NetworkType {
  ...
  const networkArg = process.argv.find(arg => 
    arg === 'localnet' || arg === 'testnet' || arg === 'mainnet'
  );
 ...
 */
export async function getDeployerAccount(network?: NetworkType): Promise<Account> {
  const net = network ?? getNetworkFromArgs();
 ...
 */
export async function getPrivateKeyForRole(role: KeyRole, network?: NetworkType): Promise<Hex> {
  const net = network ?? getNetworkFromArgs();
  ...
  return await requireSecret(envName as Parameters<typeof requireSecret>[0]) as Hex;
}
 ...
 */
export async function getVerificationKey(chainId: number): Promise<string | undefined> {
 ...
 */
export async function validateDeploymentPrereqs(network?: NetworkType): Promise<{
  valid: boolean;
  errors: string[];
}> {
  const net = network ?? getNetworkFromArgs();
  ...
  try {
    ...
    errors.push('Deployer key not found. Run: bun run scripts/keys/manager.ts generate');
  }

...
export function hasJejuHostsBlock(): boolean {
  ...
  return content.includes(HOSTS_BLOCK_START) && content.includes(HOSTS_BLOCK_END);
}

export function getHostsBlockStatus(): { exists: boolean; current: string; expected: string } {
  const content = readHostsFile();
...
if (import.meta.main) {
  ...
  switch (command) {
    ...
    case 'hosts:check': {
      const status = getHostsBlockStatus();
      if (status.exists) {
        console.log('✅ Jeju hosts block found:\n');
        console.log(status.current);
      } else {
        console.log('❌ Jeju hosts block not found');
        console.log('\nExpected block:\n');
        console.log(status.expected);
        console.log('\nRun: bun run scripts/shared/local-proxy.ts hosts:add');
      }
      break;
    }
    
    case 'hosts:add':
      console.log('📝 Adding Jeju block to hosts file...\n');
      await ensureHostsFile();
      break;
      
    case 'hosts:remove':
      console.log('🗑️  Removing Jeju block from hosts file...\n');
...
Usage: bun run scripts/shared/local-proxy.ts <command>

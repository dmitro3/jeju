...
import { keccak256, encodeAbiParameters, toHex, concat, pad, hexToBytes, bytesToHex, type Hex } from 'viem';
...
import type { L2StateSnapshot, PreimageData, AccountProof } from './state-fetcher';

// MIPS Register indices
const MIPS_REGISTERS = {
  ZERO: 0,  // Always zero
  AT: 1,    // Assembler temporary
  V0: 2, V1: 3,  // Function return values
  A0: 4, A1: 5, A2: 6, A3: 7,  // Function arguments
  T0: 8, T1: 9, T2: 10, T3: 11, T4: 12, T5: 13, T6: 14, T7: 15,  // Temporaries
  S0: 16, S1: 17, S2: 18, S3: 19, S4: 20, S5: 21, S6: 22, S7: 23,  // Saved
...
export class CannonInterface {
   ...
   */
  encodeRType(rs: number, rt: number, rd: number, shamt: number, funct: number): number {
    return ((rs & 0x1f) << 21) |
           ((rt & 0x1f) << 16) |
           ((rd & 0x1f) << 11) |
           ((shamt & 0x1f) << 6) |
  ...
  executeStep(
    ...
    } else if (decoded.opcode === MIPS_OPCODES.ADDI || decoded.opcode === MIPS_OPCODES.ADDIU) {
      newState.registers[decoded.rt] = state.registers[decoded.rs] + signExtend16(decoded.imm);
  ...
  ): Promise<{ step: bigint; preStateAtStep: MIPSState; instruction: MIPSInstruction }> {
    let low = 0n;
    let high = maxSteps;
  ...
  async runCannonBinary(
    ...
    return new Promise((resolve) => {
      const proc = spawn(this.cannonPath!, [
        'run',
        '--preimage-dir', preimageDir,
        '--output', outputPath,
      ]);
  ...
  generateStatePreimages(snapshot: L2StateSnapshot): PreimageData[] {
    ...
    const headerKey = this.computePreimageKey(PREIMAGE_KEY_KECCAK256, headerData);
    preimages.push({
      key: headerKey,
    ...
    const outputKey = this.computePreimageKey(PREIMAGE_KEY_KECCAK256, outputData);
    preimages.push({
      key: outputKey,
    ...
    if (state.preimageKey !== pad('0x00', { size: 32 })) {
      ...
      if (preimageData) {
        preimageProof = encodeAbiParameters(
          ...
          [state.preimageKey, bytesToHex(preimageData), BigInt(state.preimageOffset)]
        );
      }
    }
  ...
  async runFullTrace(
    ...
    const args = [
      ...
      '--l1', l1RpcUrl,
      '--l2', l2RpcUrl,
      '--l2-block', l2BlockNumber.toString(),
    ];

    return new Promise((resolve) => {
      const proc = spawn(this.cannonPath!, args, { 
        cwd: this.workDir,
        env: process.env,
